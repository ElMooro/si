...
- **justhodl.ai 526 incident (ops 4906, 2026-08-19)**: GitHub Pages' LE cert for the custom domain EXPIRED (13:57:23 UTC) after ACME renewals rotted in bad_authz — GH-Pages cert provisioning is unreliable behind Cloudflare orange-cloud (which /data/* Workers REQUIRE). Steady state = CF SSL mode **Full** (non-strict; cf-emergency.yml dispatch mode=full|strict). Recovery playbook: Pages API GET /repos/ElMooro/si/pages shows https_certificate.state; reset = PUT cname null -> wait -> PUT cname back. https_enforced re-enable only after state=approved. Queue: sentinel canary on https_certificate.expires_at.\n- **BEFORE writing ANY ECB or FRED puller, read docs/AI_DATA_RETRIEVAL_PLAYBOOK.md** (ops 4900) — every burned lesson, exact headers, pacing, storage layout, copy-paste examples. Also embedded at the bottom of data.html and mirrored to data/warm/playbooks/ (deny-Delete).
...
- **Chain-busy engines can't be event-kicked** (ops 4912): a healthy self-chain holds its lease back-to-back; the lease-free window is ~seconds, so kick_when_free at 20s granularity misses forever. Pass control via a STATE FLAG the engine reads on its next natural run, not via racing the lease.
- **Board 'retries N' is cumulative retried_ok, NOT a failures count** (ops 4911: eurostat '4032' phantom — real ledger was 6). Always histogram the state file before declaring a gap.
- **Slow-drip streams livelock 900s Lambdas** (ops 4908 autopsy): urllib timeout is per-socket-op, so a dripping mega-download runs past the function wall, the invoke dies Status:timeout, self-chain never fires, and the SAME window eats every run. Cure = per-window wall-clock deadline inside the read loop -> abort as 'slow' -> auto-split down (year->months), terminal flag after retries. Applies to any streaming engine.
- **Giant SDMX flows beat every RAM cap** (ops 4895: 31 ECB flows >450MB raw; bare pulls truncate the RECENT tail, not the old data). House answer = the ecb-deep pattern: startPeriod/endPeriod windows streamed to /tmp (10GB ephemeral), gzip file-to-file, permanent parts + manifest with inception parsed from the real TIME_PERIOD column (regex year-scans false-positive on integer OBS_VALUEs). Reuse for any agency's giants.
