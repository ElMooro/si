(function(root){
  ...
  const CURRENT='data/massive-research.json',PREFIX='data/massive-research/',CONTRACT='massive-composite-research.v1',MAX=16*1024*1024;
  const SOURCES={options:['option-flow-original-research.v1','data/option-flow-research/','option-flow-replay.v1','Captured contracts'],
    ...
    holdings:['etf-holdings-original-research.v1','data/etf-holdings-research/','etf-holdings-replay.v1','Fund constituents']};
  ...
  const ticker=v=>typeof v==='string'&&/^[A-Z][A-Z0-9.-]{0,14}$/.test(v),authority=v=>object(v)&&FLAGS.every(k=>v[k]===false);
  ...
  function typed(p){return p?.contract===CONTRACT&&authority(p)&&p.call===null&&p.score===null&&p.portfolio_action==='WAIT'&&p.independent_investment_votes===0&&A.clock(p.generated_at)&&
    object(p.sources)&&A.same(Object.keys(p.sources).sort(),Object.keys(SOURCES).sort())&&object(p.instruments)&&Object.keys(p.instruments).length<=3000&&
    object(p.dependency_graph?.nodes)&&Object.keys(p.dependency_graph.nodes).length<=12&&Array.isArray(p.dependency_graph.edges)&&
  ...
  async function verifyPacket(p,fetcher,signal){
    ...
    const result=await runOutput(p.replay,PREFIX,'massive-composite-replay.v1',fetcher,signal),{replay,...body}=p;
    ...
    const entries=await Promise.all(Object.entries(p.dependency_graph.nodes).map(async([id,node])=>{
      const spec=SOURCES[node.kind];if(!spec)throw Error('Unreviewed parent kind');
    ...
    for(const[kind,row]of Object.entries(p.sources)){
      ...
      if(!out||!row.node.startsWith(kind+':')||row.source_generated_at!==out.generated_at||!object(out[collection])||row.instrument_count!==Object.keys(out[collection]).length)throw Error('Source row inventory differs');
      for(const symbol of Object.keys(out[collection])){if(!ticker(symbol))throw Error('Invalid instrument identity');expected.add([symbol,kind,row.node,'/'+collection+'/'+symbol].join('|'));}
    }
    ...
    freeze(p);owners.set(p,parents);return p;
  }
  async function recordedRun(id,fetcher,signal){
    ...
    if(await A.sha(item.raw)!==id||item.doc.contract!=='massive-composite-replay.v1')throw Error('Recorded composite run differs');
  ...
  function instrumentView(p,symbol,at=Date.now()){
    return instrument(p,symbol).map(item=>{
      ...
      return '<section class="mr-evidence"><h3>'+A.esc(SOURCES[r.source][3])+'</h3><p>'+A.esc(review(source,at))+' · Parent publication '+A.esc(r.source_generated_at)+'</p>'+detail+
        '<p><a href="'+A.esc(parentUrl(item,symbol))+'">'+(r.source==='etf_desk'?'Open this recorded fund and hypothetical outcome':r.source==='options'?'Open this recorded chain and contract payoff':r.source==='populations'?'Inspect this recorded population':'Open the exact retained parent output')+'</a></p>'+
  ...
  function exportEvidence(p,symbol){requirePacket(p);return {contract:'composite-instrument-evidence.v1',symbol,composition:p.replay,
    composition_generated_at:p.generated_at,measurements:instrument(p,symbol),verification:p.verification,
    forecast_qualified:false,calls_eligible:false,sizing_eligible:false,execution_eligible:false};}
  const api={CURRENT,PREFIX,CONTRACT,SOURCES,load,retained,typed,verifyPacket,recordedRun,instrument,recordedUrl,parentUrl,review,sourcesView,familiesView,instrumentView,exportEvidence};
