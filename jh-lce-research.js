(function(root){
 'use strict';
 const contract='liquidity-credit-research.v1',limits={D:10,W:21,BW:35,M:100,Q:200,SA:370,A:550};
 const esc=v=>String(v==null?'—':v).replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));
 const path=k=>typeof k==='string'&&/^data\/(evidence|lce-research|report-research)\/[A-Za-z0-9_./-]+$/.test(k)&&!k.includes('..')?'/'+k:null;
 ...
 function rowHTML(row,packet,horizon,now){
  ...
  return `<article class="lce-observation"><div class="lce-row"><div><h3>${esc(row.series_id)} · ${esc(row._label)}</h3><p class="muted">${esc(state.replaceAll('_',' '))} · ${esc(row.frequency)} · ${esc(row.seasonal_adjustment)}</p></div><div><strong>${fresh?esc(row.latest_value_decimal):'Unavailable'}</strong><p>${esc(row._units)}</p></div><div><strong>${changeText}</strong><p>${esc(horizon)} change · baseline ${esc(change?.baseline_date)}</p></div></div>
 ...
 function render(packet,selection={},now=Date.now()){
  if(packet?.contract!==contract||!packet.series||!packet.replay?.manifest_key)throw Error('Canonical LCE research unavailable');
  ...
  return `<section class="lce-summary"><h2>${fresh} / ${all.length} measurements current</h2><p>${all.length-fresh} unavailable or outside the stated age ceilings. Missing inputs remain visible.</p><p>Source packet ${esc(packet.source_generated_at)} · desk ${esc(packet.generated_at)}</p><p>${link(packet.replay.manifest_key,'Replay this complete desk')} · <a href="/data/ops/releases/justhodl-liquidity-credit-engine.json">Deployed runtime</a></p></section>`+
   rows.map(r=>rowHTML(r,packet,horizon,now)).join('')+(!rows.length?'<p>No matching series.</p>':'');
 }
 ...
 const api={status,chart,render,verify,decimal,path};if(typeof module==='object'&&module.exports)module.exports=api;
 if(!root.document)return;root.JHLceResearch=api;
 const host=root.document.getElementById('lce-research'),search=root.document.getElementById('lce-search'),category=root.document.getElementById('lce-category'),horizon=root.document.getElementById('lce-horizon');
 ...
 async function refresh(){
  try{const response=await root.fetch('/data/liquidity-credit-engine.json?exact=1',{cache:'no-store'});if(!response.ok)throw Error('HTTP '+response.status);const next=await response.json();render(next);packet=next;draw();}
  catch(_){packet=null;host.innerHTML='<p role="status">Source-backed LCE measurements are unavailable. Earlier readings have been cleared. WAIT means abstention.</p>';root.JHCissReadthrough?.render(root.document.getElementById('ciss-readthrough'),{});}
 }
 ...
 root.document.getElementById('lce-export')?.addEventListener('click',()=>{if(!packet)return;const url=URL.createObjectURL(new Blob([JSON.stringify(packet,null,2)],{type:'application/json'})),a=root.document.createElement('a');a.href=url;a.download='liquidity-credit-research.json';a.click();URL.revokeObjectURL(url);});
