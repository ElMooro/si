(function(root){
  ...
  function comparison(row){
    if(finite(row?.value)&&finite(row?.prev)){
      ...
      return {value,unit:row.unit==='% YoY'?'percentage points':typeof row.unit==='string'&&row.unit?row.unit:'source units unverified'};
    }
  ...
  const api={CONTRACT,describe,comparison,paginate};
