(function (root) {
  ...
  const unit = x => ({Index:'index points', Percent:'%', percentage_points:'percentage points', index_points:'index points', JPY_per_AUD:'JPY per AUD'}[x] || x || '');
  ...
  function history(rows, unit) {
    ...
    const first = Date.parse(data[0].date), last = Date.parse(data[data.length-1].date);
    if (!Number.isFinite(first) || !Number.isFinite(last) || last <= first) return '<p>History dates unavailable.</p>';
    let drawing = '', connected = false;
    data.forEach((r, i) => {
      if (number(r.value) === null) { connected = false; return; }
      const at = Date.parse(r.date); if (!Number.isFinite(at)) { connected = false; return; }
      const x = 8 + 584*(at-first)/(last-first), y = 78 - (r.value-low)/span*64;
  ...
  function measurement(row) {
    ...
    return '<article class="measure"><header><h3>'+esc(row.series_id)+'</h3><span class="badge">'+esc(q.status || 'unavailable')+'</span></header>'+
      '<p class="muted">'+esc(row.title || 'Provider measurement unavailable')+'</p><div class="value">'+fmt(row.value)+' <small>'+esc(unit(row.unit))+'</small></div>'+
      '<p>Observation '+date(row.observation_date)+' · source age '+fmt(q.age_days,0)+' days</p>'+
      history(row.history, unit(row.unit))+'<dl><dt>Change across 5 provider rows</dt><dd>'+fmt(c.value)+' '+esc(unit(c.unit))+'</dd>'+
      ...
      '<div class="table-wrap"><table><caption>Most recent 12 provider rows · '+esc(unit(row.unit))+'</caption><thead><tr><th>Date</th><th>Value</th><th>Source row</th></tr></thead><tbody>'+
  ...
  function options(row) {
    ...
    return '<article class="options"><header><h3>'+esc(row.symbol)+' options</h3><span class="badge">'+(row.pagination_complete ? 'Complete bounded universe' : 'Incomplete / unavailable')+'</span></header>'+
      '<p>'+fmt(row.contracts,0)+' contracts · '+fmt(row.pages,0)+(row.pages === 1 ? ' page' : ' pages')+' · expiry '+date(u.expiration_min)+'–'+date(u.expiration_max)+'. Strike bounds '+esc(u.strike_min || '—')+'–'+esc(u.strike_max || '—')+'.</p>'+
      '<p class="muted">'+esc(u.note || 'No verified universe available.')+' Volume and open interest are separate. OI and IV observation timestamps are not supplied. Undated IV snapshots cannot establish synchronized market skew.</p>'+
  ...
  function render(packet, now = Date.now()) {
    ...
    return '<section class="notice"><div class="eyebrow">DECISION AUTHORITY</div><h2>WAIT <span>— research abstains</span></h2><p>No validated trade or sizing score. These observations authorize no position or hedge-budget change.</p>'+
      ...
      '<section aria-labelledby="native-heading"><h2 id="native-heading">Volatility and credit</h2><p class="muted">Independent source measurements are visible together; correlated readings do not become additional votes.</p><div class="measure-grid">'+['VIXCLS','VXVCLS','BAMLH0A0HYM2'].map(k => (packet.measurements || {})[k]).filter(Boolean).map(measurement).join('')+'</div>'+
      '<article><h3>Matched-date volatility spread</h3><p><b>'+fmt(term.value)+' '+esc(unit(term.unit))+'</b> · '+date(term.observation_date)+'</p><p>'+esc(term.formula || '')+'. '+esc(term.interpretation || '')+'.</p></article></section>'+
      '<section aria-labelledby="options-heading"><h2 id="options-heading">Options by expiry</h2>'+['SPY','HYG'].map(k => (packet.option_cohorts || {})[k]).filter(Boolean).map(options).join('')+'</section>'+
      '<section class="two-columns"><article><h2>AUD/JPY</h2><div class="value">'+fmt(fx.value)+' <small>'+esc(unit(fx.unit))+'</small></div><p>Observation '+date(fx.observation_date)+' · '+esc((fx.quality || {}).status || 'unavailable')+'</p>'+history(fx.history,unit(fx.unit))+
