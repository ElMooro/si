...
| Risk Sizer and Khalid Risk portfolio overlay | Date-aligned adjusted returns and same-book factor loadings, net market beta, ES/stress scenarios, direct/proxy coverage and exposure timestamp | Correlation still aligns numeric arrays by position; actual date joins, missing-day policy and factor/covariance validation remain required. Market permission alone does not establish portfolio suitability or hedge effectiveness. |
