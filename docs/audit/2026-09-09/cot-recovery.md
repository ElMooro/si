...
`cot-extremes.html` reads its dedicated producer through the existing public data proxy. Only `/data/*` has a same-origin zone route; `/cot/*` on the Pages domain returned 404. The shared inspector now sends other approved public namespaces to that proxy without browser credentials. Owner artifacts retain their authenticated client and never fall back to public reads. The page displays unavailable contracts, current rank counts, report dates and a selector for every complete calculation history embedded in the producer's output. The shared inspector exposes every field. Expiry removes stale ranks; a failed reload clears previous results. Text is escaped and numeric zero remains visible.
