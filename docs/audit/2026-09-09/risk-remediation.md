...
The workflow executes `scripts/deploy_lambdas.sh` outside its YAML body, avoiding GitHub's expression-size ceiling. Preflight tests and `scripts/prime_governed_aliases.py` finish before any code replacement. The primer also protects unchanged governed dependencies when a generic caller changes, and protects the snapshot dependency before portfolio-admin changes. Automated scheduler, event coordinator, liveness repair and backend restart calls use `governed_targets.py`; the original ten core targets include Khalid, and `7cc19c3` adds the public archive index as the eleventh governed target. Explicit numbered releases and named aliases remain pinned; governed bare names and `$LATEST` route to `live`. The current source scope and operational evidence limits are recorded in `risk-release-evidence.md`.
