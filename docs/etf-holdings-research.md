...
Nonzero source numeric fields have explicit representation bounds: no more than 128 significant digits, no more than 128 fractional places and absolute magnitude no greater than 1e30. Out-of-contract fields remain in the protected original and are named in field_errors; they do not become zero. Exact zero survives unusual exponent notation without expanding a string. JSON decimals outside the supported Decimal range reject source content rather than corrupting the retained store. Raw tables group the exact decimal string without converting through a JavaScript floating-point value.

Compatibility is limited to individually pinned, reviewed compiler versions. The older source parser lacked expansion bounds; valid original data is recalculated with the current parser and the full output must match the retained output. Retained compiler files are never executed. A same-clock publication with a different run reference remains a conflict; validation does not bypass that guard or recollect data merely to update a receipt.
