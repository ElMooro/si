"""
JUSTHODL V9 BLOOMBERG TERMINAL - DASHBOARD BUILDER
Generates a single HTML file with 25+ tabs
Auto-refreshing, real-time data from S3
"""
...
<div class="tab-bar" id="tab-bar">
  ...
  <div class="tab" data-tab="khalid-index">Khalid Index™</div>
...
<div class="footer">
  JustHodl Bloomberg Terminal V9 | 200+ FRED Series | 40+ Stocks/ETFs | 26 Tabs | Auto-Refresh 5min | Khalid Index™
</div>
...
'overview': () => {
  ...
  // KI Card
  html += '<div class="card" style="grid-column:span 1;text-align:center;border-color:'+colors[ki.regime]+'"><div class="label">Khalid Index™</div><div class="value" style="font-size:36px;color:'+colors[ki.regime]+'">'+ki.score+'</div><div style="color:'+colors[ki.regime]+';font-size:12px;font-weight:700">'+ki.regime+'</div></div>';
  // Risk Composite
  ...
  // Key Metrics Row
  html += '<div class="section-header"><span class="icon">📊</span> KEY METRICS</div>';
  ...
  // Market Indices
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📈</span> MARKET INDICES</div>';
  html += '<div class="grid grid-6">';
  for (const t of ['SPY','QQQ','DIA','IWM','TLT','GLD']) html += stockCard(t);
  html += '</div>';
  
  // Sectors Heatmap
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">🏭</span> SECTOR HEATMAP</div>';
  html += '<div class="grid grid-6">';
  if (DATA.sectors) {
  ...
  // Signals
  if (DATA.signals) {
    html += '<div class="section-header" style="margin-top:16px"><span class="icon">⚡</span> SIGNALS</div>';
    ...
    for (const t of (DATA.signals.warnings||[])) html += '<span class="signal-badge signal-warn">⚠ '+t+'</span>';
    html += '</div>';
  }
...
'khalid-index': () => {
  ...
  html += '<div class="section-header"><span class="icon">📡</span> SIGNAL DECOMPOSITION</div>';
...
'risk': () => {
  const r = DATA.risk_dashboard;
  let html = '<div class="section-header"><span class="icon">🛡️</span> RISK SCOREBOARD (0=DANGER → 100=SAFE)</div>';
  ...
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📉</span> FINANCIAL STRESS INDICES</div>';
...
// ── LIQUIDITY ──
'liquidity': () => {
  let html = '<div class="section-header"><span class="icon">💧</span> FED LIQUIDITY COMPONENTS</div>';
  html += '<div class="grid grid-4">';
  ...
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📊</span> FULL LIQUIDITY TABLE</div>';
...
'net-liquidity': () => {
  const nl = DATA.net_liquidity;
  let html = '<div class="section-header"><span class="icon">🌊</span> FED NET LIQUIDITY = Fed Assets − TGA − RRP</div>';
...
// ── DXY DOLLAR ──
'dxy': () => {
  let html = '<div class="section-header"><span class="icon">💵</span> US DOLLAR INDEX & EXCHANGE RATES</div>';
  html += '<div class="grid grid-4">';
  for (const sid of Object.keys(DATA.fred.dxy||{})) html += fredCard('dxy',sid);
  html += '</div>';
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📊</span> DXY TABLE</div>';
...
'treasury': () => {
  ...
  let html = '<div class="section-header"><span class="icon">📐</span> YIELD CURVE</div>';
  ...
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📊</span> SPREADS & BREAKEVENS</div>';
  html += '<div class="grid grid-4">';
  for (const sid of ['T10Y2Y','T10Y3M','T10YFF','T5YFF','T10YIE','T5YIE','T5YIFR','DFII10','DFII5','DFII30']) html += fredCard('treasury',sid,{suffix:'%'});
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📊</span> FULL TABLE</div>';
...
// ── ICE BofA CREDIT ──
'ice-bofa': () => {
  let html = '<div class="section-header"><span class="icon">🏦</span> ICE BofA CREDIT SPREADS (OAS)</div>';
  html += '<div class="grid grid-4">';
  ...
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">💰</span> EFFECTIVE YIELDS</div>';
  html += '<div class="grid grid-4">';
...
// ── CREDIT & LENDING ──
'credit': () => {
  let html = '<div class="section-header"><span class="icon">💳</span> CREDIT OUTSTANDING</div>';
  html += '<div class="grid grid-4">';
  ...
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">⚠️</span> DELINQUENCY RATES</div>';
  html += '<div class="grid grid-4">';
  for (const sid of ['DRCCLACBS','DRSFRMACBS','DRALACBS','DRCLACBS']) html += fredCard('credit',sid,{suffix:'%'});
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">🔥</span> CHARGE-OFF RATES</div>';
  html += '<div class="grid grid-4">';
  for (const sid of ['CHARGE','CORCBS','CRELACBS','CORCCACBS']) html += fredCard('credit',sid,{suffix:'%'});
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📊</span> LENDING STANDARDS</div>';
...
// ── MACRO ECONOMY ──
'macro': () => {
  let html = '<div class="section-header"><span class="icon">🏛️</span> GDP & OUTPUT</div>';
  html += '<div class="grid grid-4">';
  for (const sid of ['GDP','GDPC1','A191RL1Q225SBEA','INDPRO','TCU','DGORDER','NEWORDER']) html += fredCard('macro',sid);
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">👷</span> LABOR MARKET</div>';
  html += '<div class="grid grid-4">';
  ...
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">🛒</span> CONSUMER & SPENDING</div>';
  html += '<div class="grid grid-4">';
...
// ── INFLATION ──
'inflation': () => {
  let html = '<div class="section-header"><span class="icon">🔥</span> INFLATION MEASURES</div>';
  html += '<div class="grid grid-4">';
...
// ── PMI & MANUFACTURING ──
'pmi': () => {
  let html = '<div class="section-header"><span class="icon">🏭</span> ISM PURCHASING MANAGERS</div>';
  html += '<div class="grid grid-4">';
  for (const sid of ['NAPM','NAPMNOI','NAPMPI','NAPMPRI','NAPMSDI','NAPMII','NAPMEI','NMFBAI']) html += fredCard('global_cycle',sid);
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">🌍</span> WORLD PMI / CLI (OECD)</div>';
  html += '<div class="grid grid-4">';
...
'global-cycle': () => {
  let html = '<div class="section-header"><span class="icon">🌐</span> GLOBAL BUSINESS CYCLE</div>';
  html += '<div class="grid grid-4">';
...
// ── ECB & EUROPE ──
'ecb': () => {
  let html = '<div class="section-header"><span class="icon">🇪🇺</span> ECB & EURO AREA</div>';
  html += '<div class="grid grid-4">';
...
// ── GLOBAL LIQUIDITY ──
'global-liquidity': () => {
  let html = '<div class="section-header"><span class="icon">🌊</span> GLOBAL CENTRAL BANK LIQUIDITY</div>';
  html += '<div class="grid grid-4">';
...
// ── SYSTEMIC RISK ──
'systemic': () => {
  let html = '<div class="section-header"><span class="icon">☢️</span> SYSTEMIC RISK INDICATORS</div>';
  html += '<div class="grid grid-4">';
  for (const sid of Object.keys(DATA.fred.systemic_risk||{})) html += fredCard('systemic_risk',sid);
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📊</span> MOODY\'S SPREADS</div>';
  html += '<div class="grid grid-4">';
...
// ── SECTORS ──
'sectors': () => {
  let html = '<div class="section-header"><span class="icon">🏭</span> S&P 500 SECTOR ETFs</div>';
  html += '<div class="grid grid-4">';
...
// ── STOCKS & ETFs ──
'stocks': () => {
  let html = '<div class="section-header"><span class="icon">📈</span> MAJOR INDICES</div>';
  html += '<div class="grid grid-5">';
  for (const t of ['SPY','QQQ','DIA','IWM','VTI']) html += stockCard(t);
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">🏢</span> MEGA CAPS</div>';
  html += '<div class="grid grid-4">';
  for (const t of ['AAPL','MSFT','GOOGL','AMZN','NVDA','META','TSLA']) html += stockCard(t);
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">🌍</span> INTERNATIONAL</div>';
  html += '<div class="grid grid-4">';
...
// ── BOND ETFs ──
'bonds-etf': () => {
  let html = '<div class="section-header"><span class="icon">🏦</span> BOND ETFs</div>';
  html += '<div class="grid grid-4">';
...
// ── COMMODITIES ──
'commodities': () => {
  let html = '<div class="section-header"><span class="icon">🛢️</span> COMMODITY PRICES (FRED)</div>';
  html += '<div class="grid grid-4">';
  for (const sid of Object.keys(DATA.fred.commodities||{})) html += fredCard('commodities',sid);
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📦</span> COMMODITY ETFs</div>';
  html += '<div class="grid grid-5">';
...
// ── GLOBAL FX ──
'fx': () => {
  let html = '<div class="section-header"><span class="icon">💱</span> CURRENCY EXCHANGE RATES</div>';
  html += '<div class="grid grid-4">';
  for (const sid of Object.keys(DATA.fred.dxy||{})) html += fredCard('dxy',sid);
  html += '</div>';
  
  html += '<div class="section-header" style="margin-top:16px"><span class="icon">📊</span> CURRENCY ETFs</div>';
  html += '<div class="grid grid-4">';
...
'signals': () => {
  ...
  let html = '<div class="section-header"><span class="icon">⚡</span> TRADING SIGNALS (SMA50/SMA200 Based)</div>';
  ...
  html += '<div class="card" style="border-left:3px solid var(--yellow)"><div class="label">⚠️ WARNINGS</div><div class="signal-list">';
  for (const t of (sig.warnings||[])) html += '<span class="signal-badge signal-warn">'+t+'</span>';
  html += '</div></div>';
  html += '<div class="card" style="border-left:3px solid var(--purple)"><div class="label">AT RISK (SMA50 ≈ SMA200 Crossover)</div><div class="signal-list">';
...
// ── EMPLOYMENT ──
'employment': () => {
  let html = '<div class="section-header"><span class="icon">👷</span> EMPLOYMENT DATA</div>';
  html += '<div class="grid grid-4">';
  for (const sid of ['PAYEMS','UNRATE','U6RATE','CIVPART','ICSA','CCSA','JTSJOL','JTSHIR','JTSQUR','CES0500000003','AWHMAN']) html += fredCard('macro',sid);
  html += '</div>';
  return html;
...
// ── HOUSING ──
'housing': () => {
  let html = '<div class="section-header"><span class="icon">🏠</span> HOUSING MARKET</div>';
  html += '<div class="grid grid-4">';
...
// ── CONSUMER ──
'consumer': () => {
  let html = '<div class="section-header"><span class="icon">🛒</span> CONSUMER DATA</div>';
  html += '<div class="grid grid-4">';
...
// ── ALL FRED DATA ──
'all-fred': () => {
  let html = '<div class="section-header"><span class="icon">📚</span> ALL FRED SERIES (' + DATA.stats.fred_series + ' Total)</div>';
  for (const [cat, series] of Object.entries(DATA.fred||{})) {
    const keys = Object.keys(series);
    if (!keys.length) continue;
    html += '<div class="section-header" style="margin-top:12px"><span class="icon">📊</span> ' + cat.toUpperCase().replace('_',' ') + ' (' + keys.length + ')</div>';
...
if __name__ == '__main__':
    ...
    with open('dashboard.html', 'w') as f:
        f.write(html)
    print(f"Dashboard built: {len(html):,} bytes, 26 tabs")
