...
(function(root){
 ...
 function detail(p,label){
  ...
  return '<article><h3>'+esc(m.definition||label)+'</h3><p>'+publisher(m)+' Â· '+esc(m.provider)+' Â· source unit '+esc(m.source_unit||'Unavailable')+' Â· '+esc(m.frequency||'Unavailable')+'</p><p>Exact retained value: <code>'+esc(m.exact?.value??'Unavailable')+'</code> index points. Observed '+esc(m.observation_date||'Unavailable')+'.</p><p>'+fmt(h.provider_rows)+' provider rows, '+fmt(h.numeric_rows)+' numeric and '+fmt(h.null_rows)+' missing; '+esc(h.first_date||'Unavailable')+' â†’ '+esc(h.last_date||'Unavailable')+'.</p><div class="table-wrap" tabindex="0" role="region" aria-label="Descriptive history windows"><table><thead><tr><th>Numeric observations</th><th>Actual dates</th><th>Mean (points)</th><th>Sample SD (points)</th><th>Descriptive z-score</th><th>Missing rows inside span</th></tr></thead><tbody>'+Object.values(stats.observation_windows||{}).map(s=>'<tr><td>'+fmt(s.numeric_observations)+' / '+fmt(s.requested_observations)+'</td><td>'+esc(s.first_date||'Unavailable')+' â†’ '+esc(s.last_date||'Unavailable')+'</td><td>'+fmt(s.mean_index_points)+'</td><td>'+fmt(s.sample_stddev_points)+'</td><td>'+fmt(s.z_score)+'</td><td>'+fmt(s.missing_rows_inside_span)+'</td></tr>').join('')+'</tbody></table></div><p>The 60/252 windows count numeric provider observations, not assumed trading days. Missing rows stay in the date span. A constant window has no z-score.</p><p>365-day empirical percentile: '+fmt(rank.percentile_pct)+'%; '+fmt(rank.numeric_observations)+' numeric observations, '+esc(rank.first_returned_date||'Unavailable')+' â†’ '+esc(rank.last_returned_date||'Unavailable')+'. Full requested span: '+(rank.full_requested_span_available?'yes':'no')+'. Ties receive half weight. This is a sample rank, not a forecast probability.</p><details><summary>Inspect retained source evidence</summary>'+p.source_evidence.filter(e=>e.label===label).map(e=>'<p>'+esc(e.kind)+' Â· acquired '+esc(e.acquired_at)+' Â· '+fmt(e.bytes)+' bytes<br><code>'+esc(e.sha256)+'</code></p>').join('')+'<p>Complete source histories stay in the protected archive. Public compiler and run records identify the retained originals.</p></details></article>';
 }
 function render(p,now=Date.now()){
  ...
  let html='<section class="notice"><p class="eyebrow">MEASUREMENT STATUS</p><h2>'+count+' / 16 within the age ceiling</h2><p>Collected '+esc(p.generated_at)+'. Observation dates are shown separately for each index.</p><p>Research only Â· WAIT. Trade, forecast, sizing and execution authority are withheld.</p><button type="button" data-volatility-refresh>Refresh and verify</button><p>'+link(p.replay?.manifest_key,'Inspect this run')+' Â· <a href="/data/volatility-research-verification.json">Deployment acceptance</a></p></section>';
  html+='<section class="grid">'+['VIX_30D','VIX_3M','VVIX','SKEW'].map(k=>{const m=p.measurements[k];return '<article><h3>'+publisher(m)+'</h3><p class="value">'+(rowCurrent(p,m,now)?fmt(m.value):'Withheld')+'</p><p>index points Â· '+esc(m.observation_date||'Unavailable')+'</p></article>';}).join('')+'</section>';
  ...
  html+='<section><h2>Connect an explicit shock to a position</h2><p>A hypothetical first-order vega scenario. An index move does not supply the optionâ€™s IV shock or its vega. Enter both explicitly; no account is read.</p><div class="scenario-grid"><label>Signed position vega (USD per volatility point)<input id="volatility-vega" type="number" value="1000" min="-1000000000" max="1000000000" step="100"></label><label>Option IV shock (volatility points)<input id="volatility-shock" type="number" value="5" min="-100" max="100" step="1"></label></div><div id="volatility-scenario" aria-live="polite"></div></section>';
 ...
 function scenario(vega,shock){
  if(![vega,shock].every(finite)||Math.abs(vega)>1e9||Math.abs(shock)>100)return '<p role="alert">Enter finite signed vega within Â±1 billion USD per volatility point and a shock within Â±100 volatility points.</p>';
  return '<article><h3>Illustrative vega-only effect</h3><p class="value">'+fmt(vega*shock,2)+' <small>USD</small></p><p>Signed position vega Ã— explicitly entered option-IV shock. One volatility point is one percentage point of annualized option IV; +5 points is distinct from +5% relative.</p><p>Delta, gamma, time decay, volatility smile, rates, FX and changing vega are excluded. Large shocks require full repricing.</p><p>No forecast or order is produced. Inputs stay in this page.</p></article>';
 }
 ...
 async function mount(){
  ...
  const timer=setTimeout(()=>c.abort(),15000);host.innerHTML='<p role="status">Loading and verifying volatility measurementsâ€¦</p>';
  try{
