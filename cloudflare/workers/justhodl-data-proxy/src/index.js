...
function mutablePublication(path) {
  ...
  return ['report.json', 'report-measurements.json', 'khalid-adaptive.json', 'risk-regime.json', 'jsi.json', 'jsi-history.json', 'jsi-calibration.json',
    ...
    /^(?:provider-flow|capital-radar)-research\/requests\/[a-f0-9]{64}\.json$/.test(key) ||
    /^etf-flows\/(?:daily|measurements|composite|event-study|rotation|per-ticker-context|ai-analysis)\.json$/.test(key) ||
