...
function mutablePublication(path) {
  ...
  return ['report.json', 'report-measurements.json', 'khalid-adaptive.json', 'risk-regime.json', 'jsi.json', 'jsi-history.json', 'jsi-calibration.json',
    ...
    /^(?:provider-flow|capital-radar|etf-holdings|holdings-lookthrough)-research\/requests\/[a-f0-9]{64}\.json$/.test(key) ||
    /^etf-flows\/(?:daily|measurements|composite|event-study|rotation|per-ticker-context|ai-analysis|constituent-pressure|stock-exposure-lookup)\.json$/.test(key) ||
    /^breadth-research\/requests\/[a-f0-9]{64}\.json$/.test(key) ||
    ...
    /^(?:insider-research|eurodollar-research|extremes-research|vrp-research|retail-research|valuation-research|tail-research|implied-research|fedwatch-research|fomc-research|cycle-research|nowcast-research|activity-research|sector-research|sector-tilt-research|money-volume-research|sector-fusion-research|sector-capital-research)\/requests\/[a-f0-9]{64}\.json$/.test(key) ||
