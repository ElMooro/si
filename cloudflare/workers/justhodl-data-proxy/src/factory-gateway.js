...
export async function factoryGateway(request, env, url, deps) {
  ...
  if (!['sandbox', 'predictions', 'traces', 'control', 'invites'].includes(action)) return jsonResp({ error: 'unknown factory action' }, 404);
  if ((action === 'sandbox' && request.method !== 'GET') || (action !== 'sandbox' && request.method !== 'POST')) return jsonResp({ error: 'method not allowed' }, 405);
  ...
  try {
    ...
    const upstream = await fetch(base.replace(/\/$/, '') + '/factory/' + action, init);
