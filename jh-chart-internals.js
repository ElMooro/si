/* Internals from warehouse data/jh-internals.json (ops 5501). */
(function () {
  ...
  function paint(j) {
    ...
    box.innerHTML = "<div style=\"margin-top:10px;padding-top:8px;border-top:1px solid #1d2636\">" +
      ...
      cell("NFCI", f.nfci) +
      cell("A-D", f.ad_breadth) +
      "<div style=\"font:10px Inter,sans-serif;color:#6b7480;margin-top:6px\">" + (j.generated_at || "").slice(0, 19) + "Z · warehouse</div></div>";
    host.appendChild(box);
  }
  fetch("/data/jh-internals.json?t=" + Date.now(), { cache: "no-store" })
    .then(function (r) { return r.ok ? r.json() : Promise.reject(); })
    .catch(function () {
      return fetch(PROXY + "/data/jh-internals.json").then(function (r) { return r.ok ? r.json() : null; });
    })
    .then(paint);
})();
