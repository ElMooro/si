...
## KATLIN buy desk (ops 5203, 2026-09-04)
- `justhodl-katlin` (8GB/900s, python3.12) → `data/katlin.json` daily 04:10 UTC Tue-Sat; `{"mode":"backtest"}` Sundays 09:30 UTC → `data/katlin-backtest.json`; snapshots `data/katlin/history/`; banked lanes `data/warm/katlin/crypto-bars/` (Polygon X:{SYM}USD daily) and `data/warm/katlin/intraday-4h/`.
- Page `katlin.html` (marker KATLIN_DESK_V1). Command desk: katlin#2 + engine panels prime/ready/war_room. Alert-router `check_katlin` (Telegram).
- Doctrine: war-room posture (bond desk, auctions, risk-gate, black swan, crisis, tail, regime, vol, VIX curve, credit, recession, cycle, dollar, liquidity, crypto cycle) gates sizing; picks = under SMA200/250 + W/M/Q bottom structure + oversold RSI + Wyckoff accumulation + inflows + named catalyst + 4h sniper, dilution/valuation vetoes.


## Compound Factory — bounded Gear A (2026-09-13)

Existing `/ai.html` pane; preserves SageMaker Brain and `data/ai.json`. Student `justhodl-student-rsi` and separate `justhodl-factory-grader`; Scheduler `justhodl-student-rsi-1m`. External OFR/NY Fed and delayed warehouse evidence, one finite coding repair family, private grader, append-only salon and conditional dual state. Existing AI service/data proxy admit authenticated guests. No generative model verified, no official price adapter verified, no training or automatic promotion. See [Gear A contract](docs/factory/GEAR_A.md) and [handoff](PASTE_ALL.md) for exact capability limits, keys, tests and release procedure.
