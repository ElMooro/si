"""Replay native fedwatch sources using the authorized AWS runner IAM.
Reads reviewed research and protected originals only; no invocation/publication.
"""
from pathlib import Path
import argparse,json,sys
ROOT=Path(__file__).resolve().parents[1]
sys.path[:0]=[str(ROOT/'aws/lambdas/justhodl-fedwatch-rate-probability/source'),str(ROOT/'aws/shared')]
import fedwatch_research_store as store

def verify(packet,read):
    output=store.replay(packet['replay'],read)
    if {k:v for k,v in packet.items() if k!='replay'}!=output:raise ValueError('Published fedwatch differs from original-source replay')
    return {'contract':output['contract'],'generated_at':output['generated_at'],'replayed':True,
        'policy_measurements':len(output['policy_measurements']),'contracts':len(output['contracts']),
        'dated_contracts':output['quality']['dated_contracts_within_age_ceiling'],'no_qualified_meeting_probability':True}
