"""Replay native volatility originals with the authorized AWS runner's IAM.

Reads only reviewed volatility artifacts and their protected source originals.
No producer invocation, account data, credential acquisition or publication.
"""
from pathlib import Path
import argparse,json,sys
ROOT=Path(__file__).resolve().parents[1]
sys.path[:0]=[str(ROOT/'aws/lambdas/justhodl-vol-surface/source'),str(ROOT/'aws/shared')]
...
def verify(packet,read):
    ...
    if {k:v for k,v in packet.items() if k!='replay'}!=output:
        raise ValueError('Published volatility differs from retained original-source replay')
    return {'contract':output['contract'],'generated_at':output['generated_at'],'as_of':output['as_of'],
        ...
        'source_originals':len(output['source_evidence']),'forecast_qualified':output['forecast_qualified']}
