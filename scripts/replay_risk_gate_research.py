...
if __name__=='__main__':
    packet=json.loads(read_public('data/risk-gate.json'))
    key=packet['replay']['manifest_key'];manifest=json.loads(read_public(key))
    if key!='data/risk-gate-research/runs/'+digest(manifest)+'.json':raise ValueError('Risk Gate run identity differs')
    out=replay(manifest)
    if out!={k:v for k,v in packet.items() if k!='replay'}:raise ValueError('current packet differs')
    print('REPRODUCED',len(out['series']),'Risk Gate series',digest(out),'FRED and ECB original responses; fleet remains unqualified context')
