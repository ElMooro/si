"""Replay native credit originals with the authorized AWS runner's IAM.

Reads only reviewed credit artifacts and their protected source originals.
No producer invocation, account data, credential acquisition or publication.
"""
from pathlib import Path
import argparse,json,sys
ROOT=Path(__file__).resolve().parents[1]
sys.path[:0]=[str(ROOT/'aws/lambdas/justhodl-credit-stress/source'),str(ROOT/'aws/shared')]
...
def verify(packet,read):
    ...
    if {k:v for k,v in packet.items() if k!='replay'}!=output:
        raise ValueError('Published credit differs from retained original-source replay')
