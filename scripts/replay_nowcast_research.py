...
sys.path[:0]=[str(ROOT/'aws/lambdas/justhodl-macro-nowcast/source'),str(ROOT/'aws/shared')]
...
def verify(packet,read):
    ...
    return {'contract':output['contract'],'generated_at':output['generated_at'],'replayed':True,
        'measurements':len(output['measurements']),'reference_months':len(output['research_index']['trail']),'available_histories':output['quality']['available_histories'],
        'no_qualified_nowcast_forecast_or_portfolio_authority':True}
