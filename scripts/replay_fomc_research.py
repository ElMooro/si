"""Replay native fomc sources using the authorized AWS runner IAM.
Reads reviewed research and protected originals only; no invocation/publication.
"""
from pathlib import Path
import argparse,json,sys
ROOT=Path(__file__).resolve().parents[1]
sys.path[:0]=[str(ROOT/'aws/lambdas/justhodl-fomc-reaction/source'),str(ROOT/'aws/shared')]
import fomc_research_store as store

def verify(packet,read):
    output=store.replay(packet['replay'],read)
    if {k:v for k,v in packet.items() if k!='replay'}!=output:raise ValueError('Published fomc differs from original-source replay')
    return {'contract':output['contract'],'generated_at':output['generated_at'],'replayed':True,
        'measurements':len(output['measurements']),'historical_scheduled_events':len(output['events']),'available_histories':output['quality']['available_histories'],
        'no_identified_shock_or_qualified_forecast':True}
