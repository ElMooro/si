"""Replay native vrp originals with the authorized AWS runner's IAM.

Reads only reviewed vrp artifacts and their protected source originals.
...
def verify(packet,read):
    ...
    if {k:v for k,v in packet.items() if k!='replay'}!=output:
        raise ValueError('Published vrp differs from retained original-source replay')
