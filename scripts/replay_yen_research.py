"""Reproduce yen measurements and every history shard from retained originals."""
import argparse,json,re,sys
from pathlib import Path
ROOT=Path(__file__).resolve().parents[1]
sys.path[:0]=[str(ROOT/'aws/shared'),str(ROOT/'aws/lambdas/justhodl-yen-carry/source')]
import yen_research as model
from yen_store import replay
...
if __name__=='__main__':
    ...
    print('REPRODUCED',len(output['measurements']),'original yen measurements;',model.digest(output),'; no Calls/sizing authority')
