"""Replay native nowcast sources using the authorized AWS runner IAM.
Reads reviewed research and protected originals only; no invocation/publication.
"""
from pathlib import Path
import argparse,json,sys
ROOT=Path(__file__).resolve().parents[1]
sys.path[:0]=[str(ROOT/'aws/lambdas/justhodl-activity-nowcast/source'),str(ROOT/'aws/shared')]
import activity_research_store as store

def verify(packet,read):
    output=store.replay(packet['replay'],read)
    if {k:v for k,v in packet.items() if k!='replay'}!=output:raise ValueError('Published nowcast differs from original-source replay')
    return {'contract':output['contract'],'generated_at':output['generated_at'],'replayed':True,
        'measurements':len(output['measurements']),'reference_weeks':len(output['weekly_context']['trail']),'available_histories':output['quality']['available_histories'],
        'no_qualified_activity_forecast_or_portfolio_authority':True}
