"""Replay the native liquidity packet from retained original FRED responses."""
import argparse, json, sys
from pathlib import Path
ROOT = Path(__file__).resolve().parents[1]
sys.path[:0] = [str(ROOT/'aws/lambdas/justhodl-liquidity-flow/source'), str(ROOT/'aws/shared')]
...
def verify(packet, reader=read):
    ...
    return {'contract': output['contract'], 'generated_at': output['generated_at'], 'replayed': True,
        ...
        'calendar_dates': len(output['calendar_history_180d']), 'comparison_windows': len(output['comparisons']),
