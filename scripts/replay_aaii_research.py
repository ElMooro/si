"""Replay native aaii originals using the authorized AWS runner's IAM.
...
sys.path.insert(0, str(ROOT/'aws/lambdas/justhodl-aaii-sentiment/source'))
...
def verify(packet, read):
    ...
    if {k: v for k, v in packet.items() if k != 'replay'} != output:
        raise ValueError('published aaii differs from retained original-source replay')
    return {'contract': output['contract'], 'generated_at': output['generated_at'], 'as_of': output['as_of'],
        'replayed': True, 'quality': output['quality'], 'survey_weeks': len(output['history']),
        'cross_checked_weeks': output['quality']['cross_checked_weeks'], 'forecast_qualified': output['forecast_qualified']}
