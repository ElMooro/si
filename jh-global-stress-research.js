(function(root){
 ...
 function render(p,options={}){
  ...
  html+='<section><h2>Credit spreads, VIX and Treasury yields</h2><p>Each level and change retains its native unit. VIX uses index points; yields and OAS use percent levels with basis-point changes. DGS10 is a yield, not MOVE or implied rate volatility.</p><div class="table-wrap"><table><thead><tr><th>Series / original sources</th><th>Level</th><th>Observed</th><th>13-week change</th><th>Requested baseline</th><th>Quality</th></tr></thead><tbody>'+Object.values(p.measurements||{}).map(r=>'<tr><td><details><summary>'+esc(r.series_id)+'</summary><p>'+esc(r.title)+'</p><p>'+link(((r.originals||{}).definition||{}).key,'Definition')+' · '+link(((r.originals||{}).observations||{}).key,'Observations')+'</p><p>'+esc(r.frequency)+' · '+esc(r.seasonal_adjustment)+'</p><p>Provider updated '+esc(r.provider_updated_at)+'; acquired '+esc(r.acquired_at)+'. First publication: not supplied.</p><p>'+esc((r.change||{}).formula)+((r.change||{}).reason?' — An observation is missing on the exact comparison date. No nearby date was substituted.':'')+'</p></details></td><td>'+fmt(r.value)+' '+(r.unit==='percent'?'%':'index points')+'</td><td>'+day(r.observation_date)+'</td><td>'+(numeric((r.change||{}).value)?fmt(r.change.value)+' '+(r.change.unit==='basis_points'?'bp':'index points'):'Unavailable')+'</td><td>'+day((r.change||{}).start_date)+'</td><td>'+esc((r.quality||{}).status)+'</td></tr>').join('')+'</tbody></table></div><p>CCC-and-lower minus BB OAS: '+fmt((p.credit_dispersion||{}).value)+' basis points on '+day((p.credit_dispersion||{}).observation_date)+'. Both source rows must share that date.</p></section>';
