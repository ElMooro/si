...
test('publication proofs and exact reads bypass cached versions; missing public objects are never negative-cached',async()=>{
  ...
  for(const key of ['data/daily-research-verification.json','data/ops/releases/justhodl-daily-report-v3.json',
      ...
      'data/bond-vol.json','bond-vol.json','data/signal-board.json','signal-board.json',
      ...
      'data/report.json','data/report-measurements.json','data/khalid-adaptive.json','data/risk-regime.json','risk-regime.json','data/jsi.json','jsi.json','data/jsi-history.json','data/jsi-calibration.json',
...
test('native insider publication and request status bypass cached samples',async()=>{
 const {env}=fresh(),w=await worker(),calls=[];
 globalThis.caches={default:{async match(){throw Error('mutable insider packet must bypass cache')},async put(){throw Error('mutable insider packet cannot be cached')}}};
 globalThis.fetch=async(url,opts)=>{calls.push({url:String(url),opts});return Response.json({ok:true});};
 for(const key of ['insider-aggregate.json','insider-research/requests/'+'a'.repeat(64)+'.json']){
  const r=await w.fetch(req('/data/'+key),env,{waitUntil(){}});assert.equal(r.status,200);
  assert.equal(r.headers.get('Cache-Control'),'no-store');assert.equal(calls.at(-1).opts.cache,'no-store');
 }
});
...
test('native extremes publications and requests bypass cached legacy cycle labels',async()=>{
 const {env}=fresh(),w=await worker(),calls=[];
 globalThis.caches={default:{async match(){throw Error('mutable synthesis must bypass cache')},async put(){throw Error('mutable synthesis cannot be cached')}}};
 globalThis.fetch=async(url,opts)=>{calls.push({url:String(url),opts});return Response.json({ok:true});};
 for(const key of ['capitulation.json','market-extremes.json','extremes-research/requests/'+'a'.repeat(64)+'.json']){
  const r=await w.fetch(req('/data/'+key),env,{waitUntil(){}});assert.equal(r.status,200);
  assert.equal(r.headers.get('Cache-Control'),'no-store');assert.equal(calls.at(-1).opts.cache,'no-store');
 }
});
...
test('FX original publication and retired regime alias bypass edge caches',async()=>{
 ...
 globalThis.fetch=async(url,opts)=>{calls.push({url:String(url),opts});return Response.json({ok:true});};
 for(const key of ['fx-quote-research.json','polygon-fx-regime.json','futures-research.json','polygon-futures-curves.json']){
  const r=await w.fetch(req('/data/'+key),env,{waitUntil(){}});assert.equal(r.status,200);
  assert.equal(r.headers.get('Cache-Control'),'no-store');assert.equal(calls.at(-1).opts.cache,'no-store');
 }
});
