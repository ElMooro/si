...
import statement_research_arithmetic as independent
...
class Tests(unittest.TestCase):
    def test_whole_original_replay_and_independent_fraction_check(self):
        ...
        ref=store.retain(s3,'test',fixture.ref,compiled)
        self.assertEqual(store.replay(ref,store.reader(s3,'test')),compiled)
        self.assertEqual(independent.verify(fixture.ref,compiled,s3.files.__getitem__)['metric_comparisons'],68)
        self.assertTrue(all(key.startswith(model.PREFIX) and 'current' not in key for key in s3.writes))
        self.assertEqual(len(s3.writes),9)
        before=len(s3.writes);self.assertEqual(store.retain(s3,'test',fixture.ref,compiled),ref)
    ...
    def test_independent_check_finds_mutated_results_sources_and_missing_rows(self):
        ...
        for mutation in mutations:
            ...
            with self.assertRaises((AssertionError,KeyError)):independent.verify(fixture.ref,candidate,fixture.files.__getitem__)
    ...
    def test_fraction_check_covers_missing_fields_duplicate_endpoints_and_invalid_rows(self):
        ...
        result=independent.verify(fixture.ref,compiled,fixture.files.__getitem__)
        self.assertEqual(result['original_rows_checked'],6)
        self.assertEqual(result['invalid_identity_rows_checked'],1)
    ...
    def test_compiler_identity_is_part_of_the_recorded_run(self):
        fixture=Fixture();s3=S3(fixture.files);ref=store.retain(s3,'test',fixture.ref,fixture.compile())
    ...
    def test_wrong_issuer_index_fails_before_any_write(self):
        ...
        with self.assertRaises(ValueError):store.retain(s3,'test',fixture.ref,compiled)
