...
class Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.legacy=model.encoded({'engine':'polygon-fx-regime','version':'2.0.0','pair_data':{'preserve':'whole'},'fx_roro':{'fx_roro_score':99}})
    ...
    def test_original_replay_every_row_and_clock(self):
        ...
        self.assertEqual(out['returned_rows'],437);self.assertEqual(out['generated_at'],GENERATED)
    ...
    def test_publication_and_compatibility_preserve_whole_predecessor(self):
        ...
        self.assertIsNone(alias['fx_roro']['fx_roro_score']);self.assertEqual(alias['regime_signals'],[])
    ...
    def test_changed_compiler_is_not_executed_or_accepted(self):
        ...
        doc['compilers']['fx_research_model']['sha256']='0'*64;raw=model.encoded(doc);ref=model.ref(raw,'runs');self.memory.data[ref['key']]=raw
    ...
    def test_tampered_original_or_bar_block_is_rejected(self):
        for kind in ('original','block'):
            ...
            key=self.sources['EUR_USD']['pages'][0]['original']['key'] if kind=='original' else out['pairs']['EUR_USD']['bar_blocks'][0]['key']
    ...
    def test_older_capture_cannot_replace_newer_head(self):
        ...
        newer['generated_at']='2026-09-21T18:02:00+00:00';self.memory.data[model.CURRENT]=model.encoded(newer)
    ...
    def test_one_pair_clock_rollback_is_rejected_even_when_max_clock_is_same(self):
        status=self.run_();current=json.loads(self.memory.data[model.CURRENT]);new=deepcopy(current)
        new['generated_at']='2026-09-21T18:02:00+00:00';new['pairs']['EUR_USD']['source_capture_completed_at']='2026-09-21T17:00:00+00:00'
    ...
    def test_same_clock_conflict_fails(self):
        self.run_();packet=json.loads(self.memory.data[model.CURRENT]);packet['returned_rows']+=1
    ...
    def test_no_usable_comparison_does_not_replace_heads(self):
        originals,self.sources=fixture([]);self.memory.data.update(originals)
        status=self.run_();self.assertFalse(status['published']);self.assertEqual(self.memory.data[model.LEGACY],self.legacy)
        self.assertNotIn(model.CURRENT,self.memory.data);self.assertEqual(self.output(status)['returned_rows'],0)
    ...
    def test_collection_leaves_replay_reserve(self):
        with patch.object(store,'time') as clock,patch.object(store,'collect',side_effect=ValueError('stop')) as collect:
            ...
            with self.assertRaises(RuntimeError):store.run(self.memory,'bucket','time','time',credential='test',remaining_seconds=60)
        self.assertEqual(collect.call_args.args[4],130)
    ...
    def test_publication_rejects_promoted_or_wrong_target_contract(self):
        ...
        for key,value in (('calls_eligible',True),('contract','fx-original-compatibility.v1')):
