...
import ops_6019_cross_asset_native_acceptance as op
...
class Tests(unittest.TestCase):
    ...
    def test_uncertain_dispatch_is_never_reinvoked(self):
        commit = 'a'*40; request = 'chatgpt-cross-asset-native-'+commit[:12]+'-1'
        claim = {'contract': 'massive-composite-native-dispatch.v2', 'request_id': request, 'recovery': op.RECOVERY}
    ...
    def test_ambiguous_transport_failure_leaves_claim_for_inspection(self):
        ...
        key = op.store.request_key('chatgpt-cross-asset-native-'+commit[:12]+'-1-dispatch')
