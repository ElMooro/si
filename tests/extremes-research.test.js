...
test('both pages load the verified renderer and Intelligence links to evidence',()=>{
 for(const engine of ['capitulation','market-extremes']){const html=fs.readFileSync(path.join(__dirname,'..',engine+'.html'),'utf8');assert.match(html,/jh-extremes-research.js\?v=20260920-native1/);assert.ok(html.includes('data-extremes-engine="'+engine+'"'));assert.match(html,/id="extremes-scenario"/);}
