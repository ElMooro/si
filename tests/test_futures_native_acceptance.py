...
import ops_6016_futures_native_acceptance as op
...
class Tests(unittest.TestCase):
    ...
    def test_source_commit_includes_configuration_and_transitive_helpers(self):
        ...
        helper = ROOT/'aws/shared/futures_research_context.py'
        with patch.object(op.subprocess, 'check_output', side_effect=[source+'/lambda_function.py\n', 'a'*40+'\n']) as git, patch.object(op, 'shared_imports', return_value=[helper]):
            ...
            self.assertEqual(git.call_args.args[0], ['git', 'log', '-1', '--format=%H', '--', source, 'aws/lambdas/'+fn+'/config.json', 'aws/shared/futures_research_context.py'])
    ...
    def test_one_explicit_recovery_has_durable_dispatch_and_alias_completion(self):
        ...
        def accept(lam, args):
            ...
            status = {'status': 'complete', 'provider_requests_this_execution': 0, 'recovered_from': op.RECOVERY,
                      'replay': op.RECOVERY, 'published': True, 'compatibility_published': True}
    ...
    def test_uncertain_dispatch_is_never_reinvoked(self):
        commit = 'a'*40; request = 'chatgpt-futures-native-'+commit[:12]+'-1'
        claim = {'contract': 'futures-original-native-dispatch.v1', 'request_id': request, 'recovery': op.RECOVERY}
        status = {'status': 'complete', 'provider_requests_this_execution': 0, 'recovered_from': op.RECOVERY, 'replay': op.RECOVERY}
    ...
    def test_ambiguous_transport_failure_leaves_claim_for_inspection(self):
        ...
        key = op.store.request_key('chatgpt-futures-native-'+commit[:12]+'-1-dispatch')
