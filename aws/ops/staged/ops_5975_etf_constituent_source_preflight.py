...
def snapshot_probe(s3,credential,ticker,cutoff,budget):
    ...
    query={'composite_ticker':ticker,'processed_date':processed,'sort':'constituent_rank.asc','limit':5000}
...
def main():
    ...
    with report('ops_5975_etf_constituent_source_preflight') as r:
        subprocess.run([sys.executable,str(ROOT/'tests/test_etf_constituent_preflight.py')],cwd=ROOT,check=True)
        ...
        probes={ticker:snapshot_probe(s3,credential,ticker,today.isoformat(),budget) for ticker in PROBE}
        prior={ticker:snapshot_probe(s3,credential,ticker,(today-timedelta(days=30)).isoformat(),budget) for ticker in ('SPY','VOO')};del credential
        ...
        manifest_ref=retain(s3,encoded(manifest));protected_keys={manifest_ref['key'],*(ref['key'] for ref in refs.values())}
