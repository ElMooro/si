...
6108 diagnosed that the failed final journal omitted its temporary proof field.
...
REQUEST='chatgpt-accounting-completed-stage-acceptance-6110'
...
def main():
    subprocess.run([sys.executable,str(ROOT/'tests/test_statement_completed_stage_acceptance.py')],cwd=ROOT,check=True)
    client=boto3.client('s3',region_name='us-east-1');bucket=capture.BUCKET
    with report('ops_6110_accounting_completed_stage_acceptance') as r:
