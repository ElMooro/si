...
STATUS=model.PRIVATE+'requests/'+model.sha(b'chatgpt-futures-calendar-candidate-6011')+'.json'
...
def independent(output,sources,read):
    ...
    for product,out in output['products'].items():
        ...
        for session,group in groups.items():
            ...
            eligible=bool(len(closes)==1 and opens and all(datetime.fromisoformat(t)<datetime.fromisoformat(closes[0]) for t in opens))
...
def main():
    ...
    with report('ops_6011_futures_calendar_candidate') as r:
