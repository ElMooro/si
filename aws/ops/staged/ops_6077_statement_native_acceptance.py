...
def main():
    ...
    with report('ops_6077_statement_native_acceptance') as r:
        ...
        assert all(value['receipt']=={'status':'matched','commit':commit} for value in actual.values())
        for fn in baseline_ops.FUNCTIONS:
            for field in ('timeout','memory_mb','runtime','handler','architectures','role','ephemeral_storage_mb'):
                assert actual[fn][field]==old['runtime'][fn][field],(fn,field)
        ...
        proof={'contract':'statement-native-acceptance.v1','generated_at':producer.now(),'commit':commit,
            ...
            'notifications_sent':0,'portfolio_writes':0,'existing_lambda_schedules_changed':0,
