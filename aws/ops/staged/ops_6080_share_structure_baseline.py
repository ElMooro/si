...
REQUEST='chatgpt-share-structure-baseline-6080'
...
def read(s3,key):
    if key not in inventory.INPUTS and key!=STATUS and not re.fullmatch(re.escape(PRIVATE)+r'[a-f0-9]{64}\.bin',key):
...
def main():
    ...
    with report('ops_6080_share_structure_baseline') as r:
        subprocess.run([sys.executable,str(ROOT/'tests/test_share_structure_inventory.py')],cwd=ROOT,check=True)
        commit=subprocess.check_output(['git','log','-1','--format=%H','--','aws/ops/staged/ops_6080_share_structure_baseline.py'],cwd=ROOT,text=True).strip()
        ...
        try:
            ...
            for key in inventory.INPUTS:
                try:response=s3.get_object(Bucket=BUCKET,Key=key)
                    ...
                    progress['captures'][key]={'source_key':key,'status':'missing','captured_at':now(),'error_code':code}
                    packets[key]=None;journal(s3,progress);continue
                body=bounded(response['Body'],32*1024*1024)
                captured={'source_key':key,'status':'retained','original':protect(s3,body),'captured_at':now(),
                ...
                progress['captures'][key]=captured;journal(s3,progress)
            ...
            protected={STATUS,manifest['key'],result_ref['key'],*(v['original']['key'] for v in progress['captures'].values() if v.get('original')),
