"""Qualify native FX retention/replay without replacing the live producer."""
...
import ops_6006_fx_calculation_candidate as calculation
import fx_research_model as model
import fx_research_store as store
BUCKET=baseline.BUCKET
QUALIFIED={'key':model.PRIVATE+'63dfbf8d50d4d2993f90123b3864028232ba24932c86bc9a41f65968d0d62ee3.bin',
    'sha256':'63dfbf8d50d4d2993f90123b3864028232ba24932c86bc9a41f65968d0d62ee3','bytes':6317}
REQUEST='chatgpt-ops-6007-fx-native-candidate'


def candidate_inputs(source,expected):
    return {'contract':'fx-original-inputs.v1','generated_at':expected['generated_at'],'sources':source['sources'],
...
def main():
    ...
    with report('ops_6007_fx_native_candidate') as r:
        for name in ('fx_research_model','fx_research_store','fx_calculation_candidate'):
        ...
        else:raise RuntimeError('Native FX head exists; inspect current publication before migration')
        ...
        candidate={'contract':'fx-native-candidate.v1','source_qualification':QUALIFIED,'source_audit':accepted['source_audit'],
