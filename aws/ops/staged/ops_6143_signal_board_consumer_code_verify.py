...
FUNCTIONS = ('justhodl-desk-allocator', 'justhodl-master-allocator', 'justhodl-cross-asset-confirm')


def main():
    subprocess.run([sys.executable, str(ROOT/'tests/test_signal_board_consumers.py')], cwd=ROOT, check=True)
    expected = subprocess.check_output(['git', 'log', '-1', '--format=%H', '--', 'aws/shared/signal_board_authority.py'], cwd=ROOT, text=True).strip()
    if len(expected) != 40:
        raise ValueError('Full deployment commit required')
    lam, s3 = (boto3.client(n, region_name='us-east-1') for n in ('lambda', 's3'))
    with report('ops_6143_signal_board_consumer_code_verify') as r:
