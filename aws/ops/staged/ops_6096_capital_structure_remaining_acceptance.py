"""Verify four additional native consumer guard packages without invoking consumers.
...
import ops_6095_capital_structure_remaining_predecessors as baseline

PACKETS={'justhodl-industry-boom':'data/industry-boom.json','justhodl-impact-graph':'data/impact/exposure-graph.json',
    'justhodl-deal-scanner':'data/deal-scanner.json','justhodl-spx-beaters':'data/spx-beaters.json'}


def main():
    s3,lam,events,scheduler=(boto3.client(name,region_name='us-east-1') for name in ('s3','lambda','events','scheduler'))
    with report('ops_6096_capital_structure_remaining_acceptance') as r:
        expected=subprocess.check_output(['git','log','-1','--format=%H','--',
            'aws/ops/staged/ops_6096_capital_structure_remaining_acceptance.py'],cwd=ROOT,text=True).strip()
        ...
        for function,key in PACKETS.items():
            ...
            if function=='justhodl-spx-beaters':
                nodes=[n for n in ast.parse(handler).body if isinstance(n,ast.FunctionDef) and n.name in ('ai_call','ai_verdict')]
                assert len(nodes)==2
                pure={};exec(compile(ast.Module(body=nodes,type_ignores=[]),'verified-spx-ai-boundary','exec'),pure)
                assert pure['ai_call']({}, {}, 'momentum') is None
                forecast=pure['ai_verdict']({'t':'ABC','score':100,'odds_base_26w_pct':99},'momentum',{'ABC':{'stance':'BUY'}})
                assert forecast['stance']=='WAIT' and forecast['odds_beat_spx_26w_pct'] is None
                assert forecast['sizing_eligible'] is False
