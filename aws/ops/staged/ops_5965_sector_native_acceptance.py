"""Verify exact Sector packages, original replay and public page; invoke one reviewed public research producer.
...
def runtime(lam,fn):
    ...
    if fn=='justhodl-sector-tilt':assert cfg['MemorySize']==256 and cfg['Timeout']==180
...
def main():
    ...
    with report('ops_5965_sector_native_acceptance') as r:
        subprocess.run([sys.executable,str(ROOT/'tests/test_sector_acceptance.py')],cwd=ROOT,check=True)
