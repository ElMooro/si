"""Verify two additional native consumer guard packages without invoking consumers.
...
import ast,base64,hashlib,io,json,subprocess,sys,urllib.request,zipfile
import boto3
...
import ops_6100_capital_structure_final_predecessors as baseline

PACKETS={'justhodl-buyback-engine':'data/buyback-engine.json','justhodl-equity-research':None}


def main():
    s3,lam,events,scheduler=(boto3.client(name,region_name='us-east-1') for name in ('s3','lambda','events','scheduler'))
    with report('ops_6101_capital_structure_final_acceptance') as r:
        expected=subprocess.check_output(['git','log','-1','--format=%H','--',
            'aws/ops/staged/ops_6101_capital_structure_final_acceptance.py'],cwd=ROOT,text=True).strip()
        ...
        for function,key in PACKETS.items():
            ...
            for field in ('timeout','memory_mb','function_name','runtime','handler','architectures','role','ephemeral_storage_mb'):
                assert current[field]==prior['runtime'][field], (function,field,'changed unexpectedly')
            def canonical_schedules(rows):
                return sorted([{**row,**({'group':row.get('group','default')} if row['kind']=='EventBridge Scheduler' else {})} for row in rows],key=lambda v:json.dumps(v,sort_keys=True))
            assert canonical_schedules(current['schedules'])==canonical_schedules(prior['runtime']['schedules'])
            ...
            with zipfile.ZipFile(io.BytesIO(raw)) as archive:
                ...
                handler=archive.read('lambda_function.py')
            assert helper==(ROOT/'aws/shared/capital_structure_context.py').read_bytes()
            if function=='justhodl-equity-research':
                names={'claude_call','_anthropic_call','model_research_view','_http_ok'}
                nodes=[n for n in ast.parse(handler).body if isinstance(n,ast.FunctionDef) and n.name in names]
                assert len(nodes)==len(names)
                pure={'json':json};exec(compile(ast.Module(body=nodes,type_ignores=[]),'verified-equity-policy','exec'),pure)
                deterministic=json.loads(pure['claude_call']('public synthetic system','public synthetic input'))
                assert deterministic['verdict']['rating']=='WAIT' and deterministic['sizing_eligible'] is False
                try:pure['_anthropic_call']('public synthetic system','public synthetic input')
                except RuntimeError as exc:assert str(exc)=='paid_ai_disabled'
                else:raise AssertionError('Paid provider boundary unexpectedly enabled')
                served=json.loads(pure['_http_ok']({'verdict':{'rating':'BUY','price_target_12m':999},'scenarios':{'expected_value_12m':999}})['body'])
                assert served['verdict']['rating']=='WAIT' and served['verdict']['price_target_12m'] is None
                assert served['scenarios'] is None and served['sizing_eligible'] is False
                assert served['unqualified_model_output']['original_fields']['verdict']['price_target_12m']==999
            ...
            if key is None:
                public[function]={'guard_output_observed':False,'packet_refreshed_by_acceptance':False,
                    'read_policy':'On-demand company research not requested or invoked during acceptance'}
        ...
        r.kv(source_commit=expected,native_packages=accepted,public_packets=public,
            actual_zip_bytes_verified=True,predecessor_zips_reverified=2,consumer_invocations=0,
