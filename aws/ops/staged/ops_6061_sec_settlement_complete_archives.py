...
REQUEST = 'chatgpt-sec-settlement-complete-archives-6061'
...
def plan(s3, baseline):
    ...
    assert base.checked(s3, baseline['inventory_parser']) == (ROOT / 'aws/ops/checks/sec_ftd_inventory.py').read_bytes()
...
def main():
    ...
    with report('ops_6061_sec_settlement_complete_archives') as r:
        try:
        ...
        if prior:
            ...
            try:
                ...
                manifest = {'contract': 'sec-settlement-complete-sources.v1', 'request_id': REQUEST, 'generated_at': raw.now(),
                            'baseline': baseline_ref, 'inventory_parser': baseline['inventory_parser'],
