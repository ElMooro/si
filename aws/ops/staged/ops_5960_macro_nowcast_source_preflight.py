"""Preserve Macro Nowcast predecessor and reconstruct canonical source definitions without invocation.
...
FN='justhodl-macro-nowcast';BUCKET='justhodl-dashboard-live'
PREFIX='audit-private/20260909-originals/nowcast-research/'
PACKETS=('data/macro-nowcast.json','data/report-measurements.json')
SERIES=('INDPRO','PAYEMS','RSAFS','HOUST','UMCSENT','T10Y2Y','UNRATE','SP500','RRSFS')
...
def main():
    ...
    with report('ops_5960_macro_nowcast_source_preflight') as r:
        ...
        for key in PACKETS:
            ...
            if key=='data/macro-nowcast.json' and isinstance(p,dict):
                metadata[key]['claims']={k:p.get(k) for k in ('v','methodology_version','normalized_score','raw_score','regime','coverage_pct','components','historical_summary','regime_spy_performance','return_study','global_confidence','call','calls_eligible','forecast_qualified')}
        ...
        manifest={'contract':'nowcast-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtime':runtime,
