...
REQUEST = 'chatgpt-bond-vol-original-baseline-6133'
...
def package_inventory(raw, expected):
    """Baseline conservation records drift; it never certifies it as a release."""
    with zipfile.ZipFile(io.BytesIO(raw)) as archive:
        names=archive.namelist()
        if len(set(names))!=len(names):raise ValueError('Ambiguous duplicate ZIP member')
        if sum(x.file_size for x in archive.infolist())>128*1024*1024:raise ValueError('Expanded package exceeds bound')
        inventory={name:{'sha256':sha(archive.read(name)),'bytes':archive.getinfo(name).file_size} for name in names}
    differences={}
    for name,body in expected.items():
        actual=inventory.get(name);tracked={'sha256':sha(body),'bytes':len(body)}
        if actual!=tracked:differences[name]={'status':'missing_from_package' if actual is None else 'different_bytes','tracked':tracked,'deployed':actual}
    return {'source_files_checked':len(expected),'code_matches_repository':not differences,
        'source_differences':differences,'complete_zip_inventory':inventory,
        'additional_packaged_files':sorted(set(names)-set(expected)),
        'release_verification':False,'baseline_only':True}
...
def runtime(lam,s3,events,scheduler,function):
    ...
    result={**inventory,'code_sha256':cfg['CodeSha256'],'whole_zip':retain(s3,raw),
        'handler_bytes':inventory['complete_zip_inventory'].get('lambda_function.py',{}).get('bytes'),
...
def main():
    ...
    with report('ops_6133_bond_vol_original_baseline') as r:
