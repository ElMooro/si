"""Preserve valuation predecessor and replay existing canonical sources without invocation.
...
FN='justhodl-valuations-agent';BUCKET='justhodl-dashboard-live'
PREFIX='audit-private/20260909-originals/valuation-research/'
PACKETS=('valuations-data.json','data/valuations-data.json','data/report-measurements.json')
SERIES=('SP500','CAPE','GDP','WILL5000INDFC','DGS10','VIXCLS','BAA','AAA','BAMLH0A0HYM2','BAMLC0A0CM','T10YIE',
        'GOLDAMGBD228NLBM','M2SL','CPIAUCSL','DTWEXBGS','WALCL','DCOILWTICO','DCOILBRENTEU','DHHNGSP')
...
def main():
    ...
    with report('ops_5944_valuation_source_preflight') as r:
        ...
        for key in PACKETS:
            ...
            if key=='valuations-data.json' and isinstance(p,dict):
                metadata[key]['sections']={k:{'keys':sorted(v),'metrics':v.get('metrics')} for k,v in p.items() if isinstance(v,dict)}
        def read(key):
        ...
        manifest={'contract':'valuation-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtime':runtime,
            'packets':refs,'metadata':metadata,'canonical_originals':originals,'canonical_inventory':available}
        ref=retain(s3,encoded(manifest))
        for item in (ref,refs['valuations-data.json']):
        ...
        r.kv(runtime=runtime,retained_manifest=ref,packet_inventory=metadata,canonical_inventory=available,
            ...
            next_work='Native dated valuation context; index divided by GDP is not market-cap/GDP. No invented prices, fair values or neutral missing defaults.')
