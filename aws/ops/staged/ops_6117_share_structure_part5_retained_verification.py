"""Inspect completed part-5 originals and record every anonymous-access outcome.

6087 failed after collection. Never acquire again or overwrite its control.
...
REQUEST = 'chatgpt-share-structure-part5-retained-6117'
...
def main():
    ...
    with report('ops_6117_share_structure_part5_retained_verification') as r:
        failed_report = (ROOT / 'aws/ops/reports/latest/ops_6087_share_structure_sources_part_5.md').read_text(encoding='utf-8')
        ...
        control = source.request_key(batches.PARENT, 'batch:5')
        ...
        r.kv(whole_original_journal=whole, observed_status=state.get('status'),
             ...
             batch_manifest=state.get('manifest'), original_failed_run=36179729131, provider_requests=0)
        assert state['request_id'] == batches.PARENT and state['part'] == 5 and state['status'] == 'complete'
        ...
        try:
            ...
            final = {'part': 5, 'batch_manifest': state['manifest'], 'whole_original_journal': whole,
