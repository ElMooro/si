...
STATUS=model.PRIVATE+'requests/'+model.sha(b'chatgpt-futures-holiday-calendar-candidate-6013')+'.json'
...
def independent(output,sources,read):
    ...
    for name,source in sources.items():
        ...
        if source['scope']['kind']=='schedules':
            ...
            schedule_diagnostics[name]={'product_codes':dict(Counter(str(r.get('product_code')) for r in original)),
                ...
                'product_names':dict(Counter(str(r.get('product_name')) for r in original)),
                'first_events':[{k:r.get(k) for k in ('product_code','trading_venue','name','event','session_end_date','timestamp')} for r in original[:6]]}
    def session_status(name,index):
        row=raw_rows[name][index];product=name.split(':')[0];out=output['products'][product]['session_calendar']
        session=out['sessions'].get(row['session_end_date'],{})
        close=session.get('scheduled_close_utc') if session.get('scheduled_close_qualified') else None
        acquired=locations[name][index]['acquired_at']
        ended=datetime.fromisoformat(close)<=datetime.fromisoformat(acquired) if close is not None else None
        return {'calendar_dataset':product+':schedules','session_end_date':row['session_end_date'],'scheduled_close_utc':close,
            ...
            'bar_finality_independently_verified':False,'source_capture_at':acquired}
    def endpoint(name,index,field):
        ...
        return {'ordinal':index,'ticker':row['ticker'],'session_end_date':row['session_end_date'],
            'window_start_ns':str(row['window_start']),'field':field,'reported_decimal':format(Decimal(value),'f') if value is not None else None,'source':locations[name][index],'session_status':session_status(name,index)}
    ...
    for product,out in output['products'].items():
        ...
        cal=out['session_calendar'];groups=defaultdict(lambda:defaultdict(list));event_rows=raw_rows[product+':schedules']
        assert cal['returned_rows']==len(event_rows) and cal['invalid_source_row_ordinals']==[]
        for i,event in enumerate(event_rows):
            assert event['product_code']==product and event['trading_venue']==capture.PRODUCTS[product]
            assert event['event'] in ('open','pre_open','close')
            dt=datetime.fromisoformat(event['timestamp'].replace('Z','+00:00'));assert dt.tzinfo is not None
            stamp=dt.astimezone(timezone.utc).isoformat();groups[event['session_end_date']][(event['event'],stamp)].append(i)
        assert cal['exchange_calendar_timezone']=='America/Chicago'
        assert set(cal['sessions'])==set(groups);repeated=0;qualified=0
        for session,group in groups.items():
            entry=cal['sessions'][session];opens=sorted(t for event,t in group if event=='open');closes=sorted(t for event,t in group if event=='close')
            eligible=bool(len(closes)==1 and opens and datetime.fromisoformat(closes[0]).astimezone(ZoneInfo('America/Chicago')).date().isoformat()==session and all(datetime.fromisoformat(t)<datetime.fromisoformat(closes[0]) for t in opens))
            assert entry['event_identities']==[{'event':ev,'timestamp_utc':ts,'source_row_ordinals':indices} for (ev,ts),indices in sorted(group.items())]
            assert entry['unique_open_times']==opens and entry['unique_close_times']==closes
            assert entry['scheduled_close_qualified'] is eligible and entry['scheduled_close_utc']==(closes[0] if eligible else None)
            assert entry['bar_finality_independently_verified'] is False
            qualified+=eligible;repeated+=sum(len(v)-1 for v in group.values())
        assert cal['qualified_session_closes']==qualified and cal['repeated_event_identity_rows']==repeated
        ...
        for contract_index,record in zip(order,out['contracts']):
            ...
            assert latest['session_status']==session_status(name,indices[-1])
            for field in ('open','high','low','close','settlement_price','volume','dollar_volume','transactions','window_start'):
                original=rows[indices[-1]]
                if field not in original:assert latest['values'][field]=={'state':'missing','decimal':None}
                elif original[field] is None:assert latest['values'][field]=={'state':'null','decimal':None}
                else:same_typed({'number':latest['values'][field]},original[field])
            ended=[i for i in indices if session_status(name,i)['scheduled_session_ended_by_capture'] is True]
            assert record['scheduled_ended_source_ordinals']==ended
            assert record['coverage']['scheduled_ended_rows']==len(ended)
            assert record['coverage']['scheduled_open_rows']==sum(session_status(name,i)['scheduled_session_ended_by_capture'] is False for i in indices)
            assert record['coverage']['unqualified_calendar_rows']==sum(session_status(name,i)['scheduled_session_ended_by_capture'] is None for i in indices)
            for group_name,comparison_order in (('comparisons',indices),('scheduled_ended_comparisons',ended)):
              for field,windows in record[group_name].items():
                  ...
                  for n in (1,5,20):
                      ...
                      if len(comparison_order)<=n:
                          assert value['available'] is False and value['reason']=='insufficient_returned_rows';continue
                      ai,bi=comparison_order[-1-n],comparison_order[-1];a,b=rows[ai].get(field),rows[bi].get(field)
                      assert value['from']==endpoint(name,ai,field) and value['to']==endpoint(name,bi,field)
                      assert value['source_row_ordinals']==comparison_order[-1-n:]
    ...
    return {'products':len(output['products']),'datasets':len(sources),'original_rows':total_rows,'numeric_field_values_checked':numeric_fields,
        ...
        'calendar_coverage':{p:{'qualified_session_closes':v['session_calendar']['qualified_session_closes'],
            'ambiguous_sessions':[d for d,s in v['session_calendar']['sessions'].items() if not s['scheduled_close_qualified']],
            'contracts':{c['ticker']:{k:c['coverage'][k] for k in ('returned_rows','scheduled_ended_rows','scheduled_open_rows','unqualified_calendar_rows')} for c in v['contracts']}} for p,v in output['products'].items()}}
...
def main():
    ...
    def emit(kind,doc):
        ...
        if ref['key'] in artifacts:
            assert read_ref(ref,read)==raw
            return ref
    ...
    with report('ops_6013_futures_holiday_calendar_candidate') as r:
        subprocess.run([sys.executable,str(ROOT/'tests/test_futures_session_calendar.py')],cwd=ROOT,check=True)
        ...
        compilers={m.__name__:baseline.retain(s3,Path(m.__file__).read_bytes()) for m in (capture,primitives,calendar,model)}
        ...
        candidate={'contract':'futures-calendar-calculation-candidate.v1','generated_at':capture.now(),'source_audit':AUDIT,
