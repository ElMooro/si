...
import ops_6118_liquidity_pulse_original_baseline as baseline
import canonical_fred_replay as canonical
import liquidity_pulse_candidate as candidate
import verify_liquidity_pulse_arithmetic as independent
import retained_access_evidence as access

REQUEST = 'chatgpt-liquidity-pulse-retained-arithmetic-6119'
STATUS = baseline.PRIVATE+'requests/'+baseline.sha(REQUEST.encode())+'.json'
BASELINE = {'key': 'audit-private/20260909-originals/liquidity-pulse-research/c086ed9e2e48bb86171364c84ec33e7d7403fe0f5858f862cdf0b2a96d9fde35.bin', 'sha256': 'c086ed9e2e48bb86171364c84ec33e7d7403fe0f5858f862cdf0b2a96d9fde35', 'bytes': 32123}
...
def main():
    ...
    for test in ('test_liquidity_pulse_candidate.py', 'test_liquidity_pulse_retained_acceptance.py'):
    ...
    with report('ops_6119_liquidity_pulse_retained_arithmetic') as r:
        assert '**Status:** success' in (ROOT/'aws/ops/reports/latest/ops_6118_liquidity_pulse_original_baseline.md').read_text(encoding='utf-8')
        ...
        try:
            ...
            assert proof['requested_series'] == 11 and proof['missing_series'] == 0
            assert proof['fresh_series'] == 11 and proof['calendar_comparisons'] == 44
            profile = {'compile_and_verify_seconds': round(time.monotonic()-started, 3), 'candidate_bytes': len(baseline.encoded(output)),
            ...
            final = {'baseline': BASELINE, 'artifacts': refs, 'proof': proof, 'privacy': privacy,
                ...
                'profile': profile, 'quality': output['quality'],
                'series_evidence': {sid: {'native_unit': row['unit'], 'frequency': row['frequency'],
                    'observation_date': row['latest_date'], 'acquired_at': row['acquired_at'],
                    'original_rows': len(row['history']), 'status': row['quality']['status']} for sid, row in output['series'].items()},
