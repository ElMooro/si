...
FUNCTIONS = ('justhodl-fifx-vol-migration', 'justhodl-term-premium')
...
def main():
    ...
    expected = subprocess.check_output(['git', 'log', '-1', '--format=%H', '--',
        'aws/lambdas/justhodl-fifx-vol-migration/source/fifx_store.py'], cwd=ROOT, text=True).strip()
    ...
    with report('ops_6149_native_json_integrity_verify') as r:
        rows = []
        for function in FUNCTIONS:
            ...
            if actual['memory_mb'] != config['memory'] or actual['timeout'] != config['timeout']: raise ValueError('Runtime reserve differs')
            ...
            if not clocks or any(row != (config['schedule'], 'ENABLED', 'UTC') for row in clocks):
