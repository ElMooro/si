"""Requalify bounded-memory Dollar replay using the exact rejected inputs.

The prior compiler/output are verified but never executed dynamically. New
output must be byte-identical; the original memory/resource gate is unchanged.
No new collection, invocation or mutable public-head publication.
"""
...
REQUEST='chatgpt-dollar-bounded-candidate-6024';STATUS=store.request_key(REQUEST)
...
REJECTED={'manifest_key':'data/dollar-research/runs/372e397964ba093b01d74a881d8b23a0b4e58ae61711824badb595d57ef12839.json',
    'output_sha256':'aa871597f3adfa7f417a568806b582ffae34524d52172d67de6d894b63c816d5'}
PRIOR_COMPILER_COMMIT='fd8e14785c716683a1857172f6e40c9370ed3553'
def prior_record(read,source_bytes=None):
    raw=read(REJECTED['manifest_key']);run=json.loads(raw)
    assert store.identity(raw,'runs')['key']==REJECTED['manifest_key'] and run['contract']=='dollar-original-replay.v1'
    assert run['output_sha256']==REJECTED['output_sha256'] and run['output']['sha256']==REJECTED['output_sha256']
    assert set(run['compilers'])=={m.__name__ for m in store.COMPILERS}
    for module in store.COMPILERS:
        expected=source_bytes[module.__name__] if source_bytes is not None else subprocess.check_output(
            ['git','show',PRIOR_COMPILER_COMMIT+':aws/shared/'+module.__name__+'.py'],cwd=ROOT)
        assert run['compilers'][module.__name__]==store.identity(expected,'compilers')
        assert read(run['compilers'][module.__name__]['key'])==expected
    inputs=store.checked(run['input'],'inputs',read);out=store.checked(run['output'],'outputs',read)
    assert model.sha(model.encoded(out))==REJECTED['output_sha256'] and inputs['generated_at']==out['generated_at']==run['generated_at']
    return inputs,out
...
def main():
    ...
    with report('ops_6024_dollar_bounded_candidate') as r:
        for test in ('tests/test_dollar_research_model.py','tests/test_dollar_research_store.py','tests/test_dollar_original_audit.py','tests/test_dollar_bounded_candidate.py'):
        ...
        if prior:
            ...
            inputs,prior_output=prior_record(bound)
            assert inputs==recorded_inputs(before,source,inputs['generated_at'])
            ...
            assert output==prior_output and model.sha(model.encoded(output))==REJECTED['output_sha256'],'Memory change altered the recorded calculation'
            del prior_output
            ...
            r.kv(candidate_seconds=round(elapsed,3),peak_runner_rss_kib=peak,original_arithmetic_counts=counts,prior_output_byte_identical=True)
            ...
            candidate={'contract':'dollar-original-candidate.v1','generated_at':store.now(),'baseline':BASELINE,'source_qualification':QUALIFIED,
                ...
                'recorded_replay_verified':True,'prior_output_byte_identical':True,'prior_rejected_replay':REJECTED,'compiler_commit':subprocess.check_output(['git','rev-parse','HEAD'],cwd=ROOT,text=True).strip()}
        ...
        r.kv(accepted_candidate=accepted,candidate_replay=candidate['candidate_replay'],generated_at=output['generated_at'],counts=candidate['counts'],
            ...
            compiler_commit=candidate['compiler_commit'],prior_output_byte_identical=True,prior_rejected_replay=REJECTED,original_arithmetic_independently_checked=True,recorded_replay_verified=True,
