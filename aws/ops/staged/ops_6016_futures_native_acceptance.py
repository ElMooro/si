"""Exact native futures/consumer packages, one recovery, live bytes and replay."""
...
CONSUMERS=tuple('justhodl-'+name for name in ('prediction-snapshotter','prepump-alerts-router'))
from ops_6013_futures_holiday_calendar_candidate import independent
...
QUALIFIED = {'key': 'audit-private/20260909-originals/futures-research/3b993b567f005266e9e3e813d0016c64bb4564827e9b6d23a1306eafe1669211.bin', 'sha256': '3b993b567f005266e9e3e813d0016c64bb4564827e9b6d23a1306eafe1669211', 'bytes': 14245}
RECOVERY = {'manifest_key': 'data/futures-research/runs/d76d6a33e2bedf21f173477223bfcabd3fe0ef342fdce7bbbd16f6aa935ff36c.json', 'output_sha256': '2200014acf7c542427b7446988e08738f0abe4fb22af75a3693188ac74836c90'}
ASSETS = ('futures-research.html','jh-futures-research.js','jh-futures-research-page.js','jh-futures-research.css',
    'jh-option-research.js','jh-option-research.css','market-evidence.html','jh-data-feeds.js','why.html')
PROOF = 'data/futures-research-verification.json'
...
def completed_request(s3, packet):
    ...
    for item in sorted(items, key=lambda x: x['LastModified'], reverse=True)[:100]:
        ...
        if (doc.get('contract') == 'futures-original-request.v1' and doc.get('status') == 'complete'
...
def invoke(lam, s3, commit):
    ...
    request = 'chatgpt-futures-native-'+commit[:12]+'-1'; key = store.request_key(request); dispatch = store.request_key(request+'-dispatch')
    claim = {'contract': 'futures-original-native-dispatch.v1', 'request_id': request,
...
def profile(logs, status):
    ...
    assert all(v.get('status') not in ('error', 'timeout') and v.get('memory_mb') == 512
        and v.get('max_memory_mb', 512) < 512 and v.get('duration_ms', 90000) < 90000 for v in values)
...
def main():
    ...
    with report('ops_6016_futures_native_acceptance') as r:
        ...
        assert actual['receipt'] == {'status': 'matched', 'commit': commit} and actual['memory_mb'] == 512 and actual['timeout'] == 90
        ...
        for name in ASSETS:
            ...
            if name == 'futures-research.html': assert b'/jh-futures-research-page.js?v=' in body
        ...
        proof = {'contract': 'futures-native-acceptance.v1', 'generated_at': store.now(), 'commit': commit,
