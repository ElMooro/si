...
FUNCTIONS = ('justhodl-signal-board', 'justhodl-ai-website-synthesis', 'justhodl-ask-desk', 'justhodl-page-ai-commentary', 'justhodl-strategist', 'justhodl-regime-conditional-router')


def main():
    subprocess.run([sys.executable, str(ROOT/'tests/test_signal_board_interpretation_consumers.py')], cwd=ROOT, check=True)
    expected = subprocess.check_output(['git', 'log', '-1', '--format=%H', '--', 'aws/lambdas/justhodl-page-ai-commentary/source/lambda_function.py'], cwd=ROOT, text=True).strip()
    if len(expected) != 40:
        raise ValueError('Full deployment commit required')
    lam, s3 = (boto3.client(n, region_name='us-east-1') for n in ('lambda', 's3'))
    with report('ops_6145_signal_board_interpretation_code_verify') as r:
        ...
        for row in rows:
            ...
            row['authority_module_checked'] = fn == 'justhodl-signal-board' or any(f['member'] == 'signal_board_authority.py' and f['match'] for f in row['files'])
