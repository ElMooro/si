"""Preserve FOMC reaction/calibration predecessors and replay existing canonical sources without invocation.
...
FN='justhodl-fomc-reaction';BUCKET='justhodl-dashboard-live'
PREFIX='audit-private/20260909-originals/fomc-research/'
PACKETS=('data/fomc-reaction.json','data/fomc-calibration.json','data/fedwatch.json','data/report-measurements.json')
SERIES=('DGS2','DGS10','SP500','NASDAQCOM')
...
def main():
    ...
    with report('ops_5956_fomc_reaction_source_preflight') as r:
        ...
        for key in PACKETS:
            ...
            if key=='data/fomc-reaction.json' and isinstance(p,dict):
                metadata[key]['claims']={k:p.get(k) for k in ('version','meeting_date','is_decision_day','calibration','self_grading','honesty')}
                metadata[key]['claims']['surprise']={k:p.get('surprise',{}).get(k) for k in ('label','basis','preliminary','d2y_change_bp','two_y_fresh','as_of_2y')}
                metadata[key]['reaction_assets']=list(p.get('reaction_map',{}))
            if key=='data/fomc-calibration.json' and isinstance(p,dict):
                metadata[key]['claims']={k:p.get(k) for k in ('built_at','method','horizons_trading_days','n_events_classified','events_by_sign','fomc_days_used')}
            if key=='data/fedwatch.json' and isinstance(p,dict):
                metadata[key]['calendar']={k:p.get('calendar',{}).get(k) for k in ('available','years','scheduled_counts_by_year','source_received_at','original')}

        # Retain the calendar's actual body and the bounded public event log history.
        fw=packets['data/fedwatch.json'];assert fw.get('contract')=='fedwatch-native-research.v1'
        calref=fw['calendar']['original'];assert re.fullmatch(r'audit-private/20260909-originals/fedwatch-research/[a-f0-9]{64}\.bin',calref['key'])
        raw=bounded(s3.get_object(Bucket=BUCKET,Key=calref['key'])['Body']);assert sha(raw)==calref['sha256'] and len(raw)==calref['bytes']
        calendar_original=retain(s3,raw)
        listing=s3.list_objects_v2(Bucket=BUCKET,Prefix='data/fomc-reaction-log/',MaxKeys=200)
        assert not listing.get('IsTruncated'),'Review log inventory bound before expanding access'
        logs={};log_summary={}
        for item in listing.get('Contents',[]):
            key=item['Key'];assert re.fullmatch(r'data/fomc-reaction-log/[0-9]{4}-[0-9]{2}-[0-9]{2}\.json',key)
            raw=bounded(s3.get_object(Bucket=BUCKET,Key=key)['Body']);logs[key]=retain(s3,raw);doc=json.loads(raw)
            log_summary[key]={k:doc.get(k) for k in ('generated_at','meeting_date','version','contract')}
        ...
        manifest={'contract':'fomc-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtime':runtime,
            'packets':refs,'metadata':metadata,'canonical_originals':originals,'canonical_inventory':available,
            'calendar_original':calendar_original,'logs':logs,'log_summary':log_summary}
        ref=retain(s3,encoded(manifest))
        for item in (ref,*refs.values(),calendar_original,*logs.values()):
        ...
        r.kv(runtime=runtime,retained_manifest=ref,packet_inventory=metadata,canonical_inventory=available,
            calendar_original=calendar_original,public_log_inventory=log_summary,
            ...
            next_work='Use official dated events, exact observation joins, actual horizons and source-defined asset identities. Expose sample membership/overlap and retrospective conditioning. Daily 2Y moves and LLM tone do not identify a policy shock or calibrated forecast.')
