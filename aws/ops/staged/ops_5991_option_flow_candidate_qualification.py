"""One bounded complete option candidate, without a mutable public publication.
...
def main():
    ...
    with report('ops_5991_option_flow_candidate_qualification') as r:
        ...
        assert secret,'Existing managed option provider configuration required'
        ...
        counts=Counter();coverage={};protected=original_keys(inputs)|{store.request_key(REQUEST)}
        for symbol,row in output['chains'].items():
            ...
            protected.add(store.request_key(REQUEST)[:-5]+'/chains/'+symbol+'.json')
        ...
        evidence={'contract':'option-flow-candidate-qualification.v1','generated_at':output['generated_at'],'candidate_replay':result['replay'],
            ...
            'production_path_seconds':elapsed,'runner_peak_rss_kib':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss,
            'runtime_budget_pass':elapsed<780,'memory_budget_pass':resource.getrusage(resource.RUSAGE_SELF).ru_maxrss<3*1024*1024,
            ...
            'mutable_current_published':False,'engine_invocations':0,'private_account_reads':0,'paid_ai_calls':0,'notifications_sent':0,'portfolio_writes':0,'schedules_changed':0}
