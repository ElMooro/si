"""Inspect the retained batch-2 source failure, without a provider retry."""
...
def main():
    ...
    with report('ops_6091_share_structure_batch_diagnosis') as r:
        key=source.request_key(batches.PARENT,'batch:2')
        state=campaign.read_journal(s3,key)
        assert state['status']=='failed' and state['part']==2 and state['source_errors']
        document=json.loads(source.read(s3,state['plan']))
        expected={spec['url']:spec for spec in campaign.specifications(document,2)}
        original=source.retain(s3,source.encode(state))
        report_raw=(ROOT/'aws/ops/reports/latest/ops_6084_share_structure_sources_part_2.md').read_bytes()
        ...
        r.kv(failed_request=batches.PARENT,failed_part=2,whole_failed_journal=original,whole_failed_report=report_ref,
