...
def main():
    ...
    with report('ops_5972_sector_fusion_native_recovery_acceptance') as r:
        ...
        # 5971 stopped before any invocation: the audit reconstructed IVV dates
        # but did not retain the separate content-addressed calendar projection.
        # Add only that exact upstream-bound artifact, with full byte verification.
        etf=json.loads(protected(audit['packets']['data/etf-true-flows.json'],read))
        calendar_ref=etf['reference_calendar'];store.issuer.checked(calendar_ref,'histories',read)
        calendar_raw=read(calendar_ref['key']);digest=model.sha(calendar_raw)
        assert digest==calendar_ref['sha256'] and len(calendar_raw)==calendar_ref['bytes']
        calendar_key=model.PRIVATE+digest+'.bin';store.immutable(s3,BUCKET,calendar_key,calendar_raw,'application/octet-stream')
        extra={calendar_ref['key']:{'key':calendar_key,'sha256':digest,'bytes':len(calendar_raw)}}
        protected_keys.add(calendar_key);r.kv(additional_reference_calendar_retained=extra)
        def audit_read(key):
            ref=audit['source_artifacts'].get(key) or references.get(key) or extra.get(key)
