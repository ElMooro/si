...
from ops_6018_cross_asset_composition_candidate import independent
...
QUALIFIED = {'key': 'audit-private/20260909-originals/massive-research/443c9a7ecee6815975a0b34624197148c711afab5e9582d05575a8109e973212.bin',
    'sha256': '443c9a7ecee6815975a0b34624197148c711afab5e9582d05575a8109e973212', 'bytes': 15193}
RECOVERY = {'manifest_key': 'data/massive-research/runs/79d2f7bbbbbd58a9566e28b33188e1402eb243622c09ff6e95985d2bf43d108b.json',
    'output_sha256': '0173bc7cc3ba8cfbdb473eb0f8ccd2c1d26c9455700040c5d285e700b85dc397'}
ASSETS = ('market-evidence.html', 'jh-massive-cross-asset.js', 'jh-massive-research.js', 'jh-massive-research-page.js',
    'jh-massive-research.css', 'jh-option-research.js', 'jh-option-research.css', 'fx-research.html', 'futures-research.html')
...
def current(s3):
    try: packet = model.strict(store.bounded(s3.get_object(Bucket=BUCKET, Key=model.CURRENT)['Body']))
        ...
        raise
    if packet.get('contract') == model.legacy.CONTRACT:
        model.permissions(packet); return None
...
def completed_request(s3, packet):
    ...
    for item in sorted(items, key=lambda x: x['LastModified'], reverse=True)[:100]:
        ...
        if (doc.get('contract') == 'massive-composite-request.v2' and doc.get('status') == 'complete'
...
def invoke(lam, s3, commit):
    ...
    request = 'chatgpt-cross-asset-native-'+commit[:12]+'-1'; key = store.request_key(request); dispatch = store.request_key(request+'-dispatch')
    claim = {'contract': 'massive-composite-native-dispatch.v2', 'request_id': request,
...
def main():
    ...
    with report('ops_6019_cross_asset_native_acceptance') as r:
        ...
        assert qualification['composition_replay_verified'] is True and qualification['prior_v1_replay_verified'] is True and qualification['candidate_replay'] == RECOVERY
        ...
        for name in ASSETS:
            ...
            if name == 'market-evidence.html': assert b'/jh-massive-research-page.js' in body and b'/jh-massive-cross-asset.js' in body
        ...
        if previous['contract'] == model.legacy.CONTRACT:
            assert store.legacy_store.replay(previous['replay'], read) == {k: v for k, v in previous.items() if k != 'replay'}
        protected = {QUALIFIED['key'], inputs['prior_composite']['original']['key'], request['status_key'], *(c['original']['key'] for c in [*inputs['sources'].values(), *inputs['predecessors'].values()] if c['original'])}
        ...
        proof = {'contract': 'massive-native-acceptance.v2', 'generated_at': store.now(), 'commit': commit,
            ...
            'composition_replay_matches': True, 'prior_version_replay_preserved': True, 'instrument_namespaces_verified': True, 'parent_outputs_independently_checked': True,
