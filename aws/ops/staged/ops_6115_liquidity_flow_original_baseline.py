"""Preserve native liquidity-flow and replay its existing canonical FRED roots.
...
FUNCTION = 'justhodl-liquidity-flow'
PRIVATE = 'audit-private/20260909-originals/liquidity-flow-research/'
REQUEST = 'chatgpt-liquidity-flow-original-baseline-6115'
...
INPUTS = ('data/liquidity-flow.json', 'data/settlement-fails.json', 'data/report-measurements.json',
          'data/global-liquidity.json', 'data/ops/releases/' + FUNCTION + '.json')
...
def main():
    ...
    with report('ops_6115_liquidity_flow_original_baseline') as r:
        progress = {'contract': 'liquidity-flow-source-baseline.v1', 'request_id': REQUEST,
        ...
        try:
            ...
            originals = canonical.restore(packet, ('WALCL', 'WTREGEN', 'RRPONTSYD'), read)
            summary = {}
            ...
            paths = subprocess.check_output(['git', 'grep', '-l', '-F', 'liquidity-flow.json', '--',
                'aws/lambdas/*/source/*.py', 'aws/shared/*.py', '*.html', '*.js', 'assets/*.js'], cwd=ROOT, text=True).splitlines()
            paths = set(paths) | {'aws/lambdas/' + FUNCTION + '/config.json', 'aws/shared/canonical_fred_replay.py',
                'aws/shared/report_observations.py', 'aws/shared/research_brief_model.py', 'aws/shared/evidence_store.py',
                'aws/shared/pd_fails_context.py', 'liquidity.html'}
            ...
            privacy = access.summarize(outcomes); r.kv(**privacy)
            ...
            r.kv(manifest=manifest, native_predecessor=progress['native_predecessor'], captures=progress['captures'],
                ...
                all_predecessor_bytes_preserved=True, canonical_originals_replayed=True, native_package_unchanged=True,
