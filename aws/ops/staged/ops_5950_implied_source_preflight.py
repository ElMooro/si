"""Preserve implied-probability predecessor and replay existing canonical sources without invocation.
...
FN='justhodl-implied-prob';BUCKET='justhodl-dashboard-live'
PREFIX='audit-private/20260909-originals/implied-research/'
PACKETS=('data/implied-prob.json','data/report-measurements.json')
SERIES=('DFEDTARU','DFEDTARL','FEDFUNDS','DFF','DGS1MO','DGS3MO','DGS6MO','DGS1','RECPROUSM156N','T10Y3M','BAMLH0A0HYM2','VIXCLS','VXVCLS','VXNCLS','RVXCLS','VXDCLS','GVZCLS','OVXCLS')
...
def main():
    ...
    with report('ops_5950_implied_source_preflight') as r:
        ...
        for key in PACKETS:
            ...
            if key=='data/implied-prob.json' and isinstance(p,dict):
                metadata[key]['sections']={k:{'keys':sorted(v)} for k,v in p.items() if isinstance(v,dict)}
                metadata[key]['claims']={'fed_stance':p.get('fed',{}).get('near_term_stance'),'recession_label':p.get('recession',{}).get('composite_label'),'source_named_ny_fed':p.get('recession',{}).get('ny_fed_date'),'earnings_rows':len(p.get('earnings_implied') or [])}
        def read(key):
        ...
        manifest={'contract':'implied-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtime':runtime,
            'packets':refs,'metadata':metadata,'canonical_originals':originals,'canonical_inventory':available}
        ref=retain(s3,encoded(manifest))
        for item in (ref,refs['data/implied-prob.json']):
        ...
        r.kv(runtime=runtime,retained_manifest=ref,packet_inventory=metadata,canonical_inventory=available,
            ...
            next_work='Separate coincident recession estimates, dated rate observations and explicit distribution assumptions. Bills are not futures-implied meeting probabilities; ETF RV is not BTC IV.')
