"""Preserve the complete 23-series Yield Curve engine and existing canonical roots.
...
FUNCTION = 'justhodl-yield-curve'
PRIVATE = 'audit-private/20260909-originals/yield-curve-research/'
REQUEST = 'chatgpt-yield-curve-original-baseline-6127'
...
INPUTS = ('data/yield-curve.json', 'data/term-premium.json', 'data/report-measurements.json')
SERIES = ('DGS1MO', 'DGS3MO', 'DGS6MO', 'DGS1', 'DGS2', 'DGS3', 'DGS5', 'DGS7', 'DGS10', 'DGS20', 'DGS30', 'DFII5', 'DFII7', 'DFII10', 'DFII20', 'DFII30', 'T5YIE', 'T10YIE', 'T5YIFR', 'DFF', 'DFEDTARU', 'DFEDTARL', 'SOFR30DAYAVG')
...
def main():
    subprocess.run([sys.executable, str(ROOT/'tests/test_yield_curve_original_baseline.py')], cwd=ROOT, check=True)
    s3, lam, events, scheduler = (boto3.client(name, region_name='us-east-1') for name in ('s3', 'lambda', 'events', 'scheduler'))
    with report('ops_6127_yield_curve_original_baseline') as r:
        progress = {'contract': 'yield-curve-source-baseline.v1', 'request_id': REQUEST,
        ...
        try:
            ...
            # No config is tracked for this predecessor. Discover qualified rule
            # targets as well, preserving each actually addressed code package.
            aliases=[]
            ...
            for key in INPUTS:
                try:response = s3.get_object(Bucket=BUCKET, Key=key)
                    ...
                    if key!='data/term-premium.json' or str(getattr(exc,'response',{}).get('Error',{}).get('Code')) not in ('NoSuchKey','404'):raise
            ...
            paths = subprocess.check_output(['git', 'grep', '-l', '-F', 'yield-curve.json', '--',
            ...
            paths = set(paths) | {'aws/lambdas/' + FUNCTION + '/source/lambda_function.py', 'aws/shared/canonical_fred_replay.py',
                ...
                'aws/shared/donor_contract.py', 'aws/lambdas/justhodl-yield-curve/source/_fred_shim.py', 'yield-curve.html'}
            ...
            access_ref = retain(s3, encoded({'contract': 'yield-curve-baseline-access.v1', 'outcomes': outcomes}))
