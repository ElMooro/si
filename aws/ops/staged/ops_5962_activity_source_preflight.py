"""Preserve Activity Nowcast predecessor and reconstruct canonical source definitions without invocation.
...
FN='justhodl-activity-nowcast';BUCKET='justhodl-dashboard-live'
PREFIX='audit-private/20260909-originals/activity-research/'
PACKETS=('data/activity-nowcast.json','data/report-measurements.json','data/indicator-bus.json','data/macro-nowcast.json')
SERIES=('WEI','ICSA','CCSA','NFCI','STLFSI4','BAA10Y')
...
def main():
    ...
    with report('ops_5962_activity_source_preflight') as r:
        ...
        for key in PACKETS:
            ...
            if key=='data/activity-nowcast.json' and isinstance(p,dict):
                metadata[key]['claims']={k:p.get(k) for k in ('schema_version','method','activity_index','activity_z','regime','momentum','components','divergence','bus_hard','errors','call','calls_eligible','forecast_qualified')}
            if isinstance(p,list):metadata[key]['rows']=len(p)
            if key=='data/report-measurements.json':metadata[key]['canonical_measurement_count']=len(p.get('measurements',{}))

        snapshots={};total=0
        for page in s3.get_paginator('list_objects_v2').paginate(Bucket=BUCKET,Prefix='data/activity-nowcast/snapshots/'):
            for item in page.get('Contents',[]):
                key=item['Key'];assert re.fullmatch(r'data/activity-nowcast/snapshots/\d{4}-\d{2}-\d{2}\.json',key)
                assert len(snapshots)<500,'Snapshot inventory exceeds reviewed bound'
                raw=bounded(s3.get_object(Bucket=BUCKET,Key=key)['Body'],1024*1024);total+=len(raw)
                assert total<=32*1024*1024,'Snapshot bytes exceed reviewed bound'
                json.loads(raw);snapshots[key]=retain(s3,raw)
        ...
        manifest={'contract':'activity-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtime':runtime,
            'packets':refs,'metadata':metadata,'legacy_snapshots':snapshots,'legacy_snapshot_bytes':total,'canonical_originals':originals,'canonical_inventory':available}
        ref=retain(s3,encoded(manifest))
        for item in (ref,*refs.values(),*originals.values(),*snapshots.values()):
