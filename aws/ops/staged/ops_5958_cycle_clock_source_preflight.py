"""Preserve Cycle Clock predecessor and every declared public dependency; replay canonical sources without invocation.
...
FN='justhodl-cycle-clock';BUCKET='justhodl-dashboard-live'
PREFIX='audit-private/20260909-originals/cycle-research/'
LEGACY=ROOT/'aws/lambdas/justhodl-cycle-clock/source/lambda_function.py'
TREE=ast.parse(LEGACY.read_text(encoding='utf-8'))
DEPENDENCIES=tuple(sorted({n.args[0].value for n in ast.walk(TREE) if isinstance(n,ast.Call) and isinstance(n.func,ast.Name) and n.func.id=='load' and n.args and isinstance(n.args[0],ast.Constant) and isinstance(n.args[0].value,str)}))
assert 60<=len(DEPENDENCIES)<=100
PACKETS=('data/cycle-clock.json','data/cycle-clock-history.json','data/report-measurements.json',*DEPENDENCIES)
SERIES=('INDPRO','PAYEMS','CPIAUCSL','PCEPILFE','UNRATE','SAHMREALTIME','SAHMCURRENT','WALCL','WTREGEN','RRPONTSYD','ECBASSETSW','JPNASSETS','MCUMFN','SP500')
...
def main():
    ...
    with report('ops_5958_cycle_clock_source_preflight') as r:
        ...
        for key in PACKETS:
            ...
            if key=='data/cycle-clock.json' and isinstance(p,dict):
                metadata[key]['claims']={k:p.get(k) for k in ('version','cycle','risk','synthesis','data_integrity','track_record','call','calls_eligible','forecast_qualified')}
        ...
        manifest={'contract':'cycle-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtime':runtime,
        ...
        for item in (ref,*refs.values(),*originals.values()):
        ...
        r.kv(runtime=runtime,retained_manifest=ref,packet_inventory=metadata,canonical_inventory=available,
            ...
            next_work='Build exact monthly comparisons and current-vintage descriptive coordinates from retained sources; use correctly defined Sahm series, bounded backward liquidity joins and an explicit dependency inventory. Recession probabilities, investment-clock asset leadership, independent votes and forecast performance need separate qualification.')
