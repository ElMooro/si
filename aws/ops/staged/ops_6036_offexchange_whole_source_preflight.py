"""Whole-source inventory after read-only 6035 measured a 75,680,388-byte history.

6034 remains failed and is never rerun; this is a distinct 128-MiB-bounded claim.
No legacy invocation or public-head write.
...
REQUEST='chatgpt-offexchange-source-whole-source-preflight-6036';STATUS=PRIVATE+'requests/'+hashlib.sha256(REQUEST.encode()).hexdigest()+'.json'
MAX=128*1024*1024;SOURCE_MAX=8*1024*1024
...
def main():
    ...
    with report('ops_6036_offexchange_whole_source_preflight') as r:
        ...
        if prior:
            ...
            for key in PACKETS:
                ...
                status(s3,{'request_id':REQUEST,'status':'retaining_parents','parents':parents})
            ...
            manifest={'contract':'offexchange-source-preflight.v1','generated_at':now(),'parent_byte_bound':MAX,'diagnosis':'ops_6035_offexchange_retention_diagnosis','supersedes_failed_request':'chatgpt-offexchange-source-preflight-6034','runtime':actual,'parents':parents,'captures':captures,'provider_bytes':total,
