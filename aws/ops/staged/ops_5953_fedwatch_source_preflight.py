"""Preserve Fed-funds futures predecessor and replay existing canonical sources without invocation.
...
def main():
    ...
    with report('ops_5953_fedwatch_source_preflight') as r:
        ...
        for key in PACKETS:
            ...
            if key=='data/fedwatch.json' and isinstance(p,dict):
                metadata[key]['claims']={'version':p.get('version'),'current_fed_funds_range':p.get('current_fed_funds_range'),
                    'next_6mo_summary':p.get('next_6mo_summary'),'meetings':[{k:m.get(k) for k in ('date','status','futures_contract','futures_close','implied_post_meeting_rate_pct','implied_move_bps','probabilities_pct','observed_at','as_of')} for m in p.get('meetings_ahead',[])],
        ...
        manifest={'contract':'fedwatch-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtime':runtime,
            'packets':refs,'metadata':metadata,'canonical_originals':originals,'canonical_inventory':available}
        ref=retain(s3,encoded(manifest))
        for item in (ref,refs['data/fedwatch.json']):
        ...
        r.kv(runtime=runtime,retained_manifest=ref,packet_inventory=metadata,canonical_inventory=available,
            ...
            next_work='Retain exact contract identity, original response and quote timestamps; distinguish close from settlement; reconstruct monthly average EFFR before any meeting probability model. Audit non-meeting anchors, effective dates and complete state support before using probabilities.')
