...
def profile(logs, status):
    ...
    while True:
        ...
        time.sleep(3)
    assert all(v.get('status') not in ('error', 'timeout') and v.get('memory_mb') == 512 and v.get('max_memory_mb', 512) < 512 and v.get('duration_ms', 300000) < 300000 for v in values)
...
def main():
    ...
    read = store.reader(s3, BUCKET)
    with report('ops_6056_short_interest_native_acceptance') as r:
        ...
        protected = {BASELINE['key'], inputs['predecessor']['key'], primary['status_key'], *(v['original']['key'] for v in inputs['captures'].values())}
