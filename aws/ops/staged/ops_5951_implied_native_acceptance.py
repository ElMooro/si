"""Verify exact Implied packages, original replay and public page; invoke two reviewed public research producers.
...
def canonical_refresh(lam,s3):
    # Claim before invocation. Ambiguous network results must not trigger a second
    # collector; its source clocks/run manifest can be inspected in recovery.
    request='chatgpt-implied-canonical-'+COMMIT[:12]+'-1';key=store.request_key(request)
    claim={'contract':'implied-canonical-acceptance-request.v1','request_id':request,'status':'claimed',
        'started_at':datetime.now(timezone.utc).isoformat(),'action':'research_measurements'}
    try:store.status_write(s3,BUCKET,key,claim,IfNoneMatch='*')
    except Exception as exc:
        if not store.conflict(exc):raise
        old=json.loads(store.bounded(s3.get_object(Bucket=BUCKET,Key=key)['Body']))
        assert old.get('status')=='complete','Prior canonical invocation requires evidence review; no duplicate sent'
        return {'invoke_sent':False,**old}
    ...
    result=json.loads(store.bounded(response['Payload']));assert not response.get('FunctionError') and result.get('statusCode')==200,'Canonical collector failed'
    out=json.loads(result['body']);assert out.get('published') is True
    complete={**claim,'status':'complete','completed_at':datetime.now(timezone.utc).isoformat(),'response':out,'throttle_rejections':rejected}
    store.status_write(s3,BUCKET,key,complete)
    return {'invoke_sent':True,**complete}
...
def main():
    ...
    with report('ops_5951_implied_native_acceptance') as r:
        ...
        assert macro['generated_at']==canonical['response']['generated_at'] and macro['replay']==canonical['response']['replay']
