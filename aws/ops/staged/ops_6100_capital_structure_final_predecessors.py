"""Retain two complete downstream packages before capital-structure guards.
...
REQUEST='chatgpt-capital-structure-final-predecessors-6100'
STATUS=source.request_key(REQUEST,'baseline')
CONSUMERS=('justhodl-buyback-engine','justhodl-equity-research')


def main():
    s3,lam,events,scheduler=(boto3.client(name,region_name='us-east-1') for name in ('s3','lambda','events','scheduler'))
    with report('ops_6100_capital_structure_final_predecessors') as r:
