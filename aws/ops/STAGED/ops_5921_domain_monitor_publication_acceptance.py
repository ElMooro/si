"""Verify the new scheduled-time public Domain Monitor packet without invoking its private-context producer."""
...
def main():
    ...
    with report('ops_5921_domain_monitor_publication_acceptance') as r:
        commit='369692d28ea19955b287d04ec407da4ea765eb95'
        ...
        if subprocess.check_output(['git','rev-parse','--is-shallow-repository'],cwd=ROOT,text=True).strip()=='true':
            subprocess.run(['git','fetch','--deepen=75','--no-tags','origin','main'],cwd=ROOT,check=True)
            r.kv(checkout_history_deepened_by=75)
        subprocess.run(['git','merge-base','--is-ancestor',commit,actual],cwd=ROOT,check=True)
        subprocess.run(['git','diff','--quiet',commit,actual,'--',*PAGES],cwd=ROOT,check=True)
        for name in PAGES:
        ...
        current_raw=public('data/domain-barometers.json');current=json.loads(current_raw);stamp=current.get('generated_at')
        ...
        assert qualified and stamp>='2026-09-20T12:20:00','The new public contract has not yet been observed'
        if qualified:
        ...
        proof={'contract':'domain-monitor-verification.v1','generated_at':datetime.now(timezone.utc).isoformat(),
            ...
            'public_generated_at':stamp,'public_packet_sha256':sha(current_raw),'trigger_identity_verified':False,'publication_acceptance':'observed_new_contract' if qualified else 'awaiting_natural_publication',
        ...
        assert public('data/domain-barometers.json')==current_raw,'Public packet changed during acceptance'
