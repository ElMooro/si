"""Accept native Plumbing deployment, public source acquisition and original-source replay."""
...
def main():
    lam=boto3.client('lambda',region_name='us-east-1',config=Config(read_timeout=960,retries={'max_attempts':0}))
    s3=boto3.client('s3',region_name='us-east-1');events=boto3.client('events',region_name='us-east-1')
    with report('ops_5914_plumbing_research_acceptance') as r:
        ...
        macro=json.loads(public('data/report-measurements.json'))
        assert set(model.SERIES)<=set(macro['catalog']) and macro['replay']==source_result['replay']
        ...
        proof={'contract':'plumbing-research-verification.v1','generated_at':datetime.now(timezone.utc).isoformat(),'runtimes':runtimes,
            ...
            'public_only':True,'controlled_source_acquisition':acquisition,'controlled_invocation':invocation,
