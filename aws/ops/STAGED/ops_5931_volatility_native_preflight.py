...
def main():
    with report('ops_5931_volatility_native_preflight') as r:
        ...
        receipt=None;receipt_status='public_verified'
        try:receipt=json.loads(fetch('https://justhodl.ai/data/ops/releases/'+FN+'.json'))
        except urllib.error.HTTPError as exc:
            if exc.code not in (401,403,404):raise
            receipt_status='public_unavailable_'+str(exc.code)
            try:
                receipt=json.loads(bounded(s3.get_object(Bucket=BUCKET,Key='data/ops/releases/'+FN+'.json')['Body']))
                receipt_status+='; protected_receipt_read'
            except Exception as source_error:
                code=str(getattr(source_error,'response',{}).get('Error',{}).get('Code',''))
                if code not in ('404','NoSuchKey'):raise
                receipt_status+='; no_receipt_object'
        if receipt is not None:assert receipt['code_sha256']==cfg['CodeSha256']
        # A legacy package can precede release receipts. Exact AWS ZIP/source
        # bytes above are required even then; native acceptance still requires
        # its new commit-specific public receipt after deployment.
        ...
        r.kv(runtime={'receipt_commit':receipt.get('commit') if receipt else None,'receipt_status':receipt_status,'code_sha256':cfg['CodeSha256'],
