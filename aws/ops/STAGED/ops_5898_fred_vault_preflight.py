"""Inspect FRED vault and preserve the complete preceding public publication."""
...
def main():
    fn, bucket = 'justhodl-tradingview', 'justhodl-dashboard-live'
    ...
    def raw(key):
        ...
        return value
    with report('ops_5898_fred_vault_preflight') as r:
        ...
        preserved = []
        for key in ('data/tradingview.json',):
            ...
            private = 'audit-private/20260909-originals/fred-vault/'+sha+'.bin'
            ...
            item.update(symbol_rows=len(packet.get('symbols') or []), version=packet.get('version'), contract=packet.get('contract'))
...
if __name__ == '__main__':
    try: main()
        ...
        print('FRED vault preflight failed; inspect the committed report before changing production.')
        sys.exit(1)
