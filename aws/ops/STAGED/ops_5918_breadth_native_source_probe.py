...
import boto3
from ops_5917_market_cycle_source_preflight import bounded, preserve, BUCKET
...
def main():
    with report('ops_5918_breadth_native_source_probe') as r:
        ...
        credential = (cfg.get('Environment') or {}).get('Variables', {}).get('POLYGON_KEY')
        assert credential, 'Existing breadth provider credential is not configured'
        for day in DATES:
            safe_url = 'https://api.polygon.io/v2/aggs/grouped/locale/us/market/stocks/'+day+'?adjusted=true&include_otc=false'
