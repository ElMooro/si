"""Accept retained-source holdings runtime without invoking the legacy producer."""
...
PROBE_SHA = 'c970a8b27d4f9fe48d5ad90968e88b8d9a843250d4686dc56da871396936d2ec'
PROBE = {'key': model.PREFIX + 'probes/' + PROBE_SHA + '.json', 'sha256': PROBE_SHA, 'bytes': 231002}
...
def main():
    ...
    with report('ops_5876_holdings_native_acceptance') as r:
        ...
        r.kv(runtime={'commit': expected, 'code_sha256': conf['CodeSha256']}, action='holdings_research_refresh',
             legacy_collector_invoked=False, source_probe=PROBE)
        before = {}
        ...
        response, waits = invoke_when_available(lam, dict(FunctionName=fn, InvocationType='RequestResponse',
            Payload=model.encoded({'action': 'holdings_research_refresh', 'probe': PROBE, 'notify': False})))
        ...
        assert all(result[k] == 0 for k in ('signals_emitted', 'paid_ai_calls', 'notifications_sent', 'private_account_reads', 'portfolio_writes'))
        ...
        assert output['source_generated_at'] == '2026-09-19T21:03:02.340304+00:00'
        ...
        proof = {'contract': 'holdings-native-verification.v1', 'generated_at': datetime.now(timezone.utc).isoformat(),
                 ...
                 'source_probe': PROBE, 'filings': originals, 'native_rows': native_rows, 'comparison_rows': comparable_rows,
                 ...
                 'remaining': 'Scheduled acquisition, legacy producer and downstream consumer migration are not accepted by this source-snapshot proof.'}
