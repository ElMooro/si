"""Audit the credit runtime and privately retain original FRED research inputs.
...
import ast, base64, hashlib, io, json, os, re, subprocess, sys, time
...
def fetch(url):
    with urllib.request.build_opener(NoRedirect()).open(urllib.request.Request(url,headers={
            'User-Agent':'JustHodl-credit-research-audit/1.0'}),timeout=30) as response:
...
def main():
    with report('ops_5928_credit_native_preflight') as r:
        ...
        retained=[]
        for key in ('data/credit-stress.json','data/credit-stress-history.json'):
        ...
        for item in ast.parse((directory/'lambda_function.py').read_text(encoding='utf-8')).body:
            if isinstance(item,ast.Assign) and any(isinstance(t,ast.Name) and t.id in ('SERIES','EY_RATINGS','EY_CURVE') for t in item.targets):
                catalog.update(ast.literal_eval(item.value))
        ...
        for sid in sorted(catalog):
            ...
            assert ds[0]['units']=='Percent' and ds[0]['frequency_short']=='D'
            ...
            coverage.append({'series_id':sid,'title':ds[0]['title'],'unit':ds[0]['units'],'frequency':ds[0]['frequency_short'],
                ...
                'latest_numeric_percent':numeric[-1]['value'],'weekend_rows':sum(date.fromisoformat(o['date']).weekday()>=5 for o in numeric),
                'three_year_limit_in_definition':bool(re.search(r'(3|three)[ -]year',notes,re.I)),
                'definition_original':sources['definition:'+sid],'observations_original':sources['observations:'+sid]})
        manifest={'contract':'credit-source-preflight.v1','generated_at':datetime.now(timezone.utc).isoformat(),
            'evaluation_date':str(evaluation),'sources':sources,'coverage':coverage,'runtime_code_sha256':cfg['CodeSha256']}
        ...
        for ref in (retained[0],sources['observations:BAMLH0A0HYM2'],manifest_ref):
        ...
        r.kv(next_work='Native typed credit observations, same-date differences, bounded-history descriptive statistics, private original replay and explicit forecast abstention. All 28 source identities retained; no source removal or producer invocation.')
