...
COMMIT='a7b37bb7894a77130c57dfa978b6acd360e8b929'
BASE_COMMIT='e349fe43a1e3038a17e2e8b95e1140711652698e'
...
def main():
    ...
    with report('ops_5910_global_stress_portable_acceptance') as r:
        # The fixed original-response fixture digest was computed on Windows.
        # Linux must match it before any controlled invocation.
        subprocess.run([sys.executable,str(SOURCE.parent/'tests/run_tests.py')],cwd=ROOT,check=True)
        portable_reference=(SOURCE.parent/'tests/fixtures/expected-output.sha256').read_text().strip()
        assert portable_reference=='3c7911c99a543be5bf015c1083648a49f32560af7b02e466a41fe5a2d75402d9'
        r.kv(portable_reference_digest=portable_reference,linux_fixture_replay='exact_match')
        runtimes={}
        for fn in FUNCTIONS:
            receipt=json.loads(public('data/ops/releases/'+fn+'.json'));cfg=lam.get_function_configuration(FunctionName=fn)
            expected_commit=COMMIT if fn==FN else BASE_COMMIT
            assert receipt['commit']==expected_commit and receipt['code_sha256']==cfg['CodeSha256'],'Exact runtime receipt required: '+fn
            ...
            runtimes[fn]={'commit':expected_commit,'code_sha256':cfg['CodeSha256'],'packaged_sources_match':True}
        ...
        assert packet['version']=='2.0.1' and packet['numerical_policy']['derived_output_decimal_places']==12
        ...
        for fn,accepted in runtimes.items():
            ...
            assert json.loads(public('data/ops/releases/'+fn+'.json'))['commit']==accepted['commit']
        proof={'contract':'global-stress-research-verification.v1','generated_at':datetime.now(timezone.utc).isoformat(),
            'runtimes':runtimes,'portable_reference_digest':portable_reference,'linux_fixture_replay':'exact_match',
            'numerical_policy':packet['numerical_policy'],'pages_commit':pages_commit,'assets':{k:build['files_sha256'][k] for k in ASSETS},
...
if __name__=='__main__':
    try:main()
        ...
        print('Global Stress portable acceptance failed; inspect its committed report before any repeat invocation.')
        sys.exit(1)
