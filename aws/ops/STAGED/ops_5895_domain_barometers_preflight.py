"""Inspect domain barometers and preserve the complete preceding public publication."""
...
def main():
    fn, bucket = 'justhodl-domain-barometers', 'justhodl-dashboard-live'
    ...
    def raw(key):
        ...
        return value
    with report('ops_5895_domain_barometers_preflight') as r:
        ...
        preserved = []
        for key in ('data/domain-barometers.json',):
            ...
            with urllib.request.urlopen(urllib.request.Request('https://justhodl.ai/'+key, headers={'User-Agent':'justhodl-verify-release/1.0'}),timeout=45) as response:
                public_body = response.read(64*1024*1024+1)
            assert json.loads(public_body) == json.loads(body), 'Preserve only the already-public product'
            private = 'audit-private/20260909-originals/domain-barometers/'+sha+'.bin'
            ...
            item.update(sized_rows=len(packet.get('sized_positions') or []), version=packet.get('version'), contract=packet.get('contract'))
...
if __name__ == '__main__':
    try: main()
        ...
        print('Domain barometer preflight failed; inspect the committed report before changing production.')
        sys.exit(1)
