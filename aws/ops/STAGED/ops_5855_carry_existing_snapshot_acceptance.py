"""Accept the already-published portable carry snapshot; never invoke its collector."""
...
def main():
    ...
    with report('ops_5855_carry_existing_snapshot_acceptance') as r:
        ...
        # The prior synchronous connection closed, but its invocation published.
        # Inspect that result; submitting the collector again is unnecessary.
        started='2026-09-19T15:23:06+00:00'
        r.kv(collector_invoked_by_this_acceptance=False, prior_invocation_outcome='existing public snapshot independently verified below')
        ...
        proof={'contract':'carry-research-verification.v1','generated_at':datetime.now(timezone.utc).isoformat(),
            ...
            'collector_invoked_by_this_acceptance':False,'accepted_existing_publication':True,
            'acceptance_scope':'Two exact runtimes; existing carry publication verified and only read-only HTTP branch invoked. Stress-index abstention guard tested offline, not invoked. Research and user-entered scenarios only; no qualified strategy.'}
...
if __name__=='__main__':
    try:main()
        ...
        print('Existing carry acceptance failed; inspect committed runner report; do not blindly reinvoke collection.')
        sys.exit(1)
