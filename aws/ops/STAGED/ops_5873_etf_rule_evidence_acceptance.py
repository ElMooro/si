...
def main():
    ...
    with report('ops_5873_etf_rule_evidence_acceptance') as r:
        ...
        r.kv(commit=expected,code_sha256=conf['CodeSha256'],collection_reinvoked=False,prior_transport_failure='ops5870 connection closed after publication')
        started='2026-09-19T19:46:46+00:00'
        ...
        for ticker,value in output['by_etf'].items():
            ...
            if ticker in ('KRE','XOP'):
                zeros=[v for v in rows if v['nav_decimal'] is not None and native.dec(v['nav_decimal'])==0];assert zeros and all(v['nav_valued_share_change_decimal'] is None for v in zeros);zero_rows+=len(zeros)
        ...
        events=boto3.client('events',region_name='us-east-1');schedule=events.describe_rule(Name='justhodl-etf-true-flows-daily')
        assert schedule['ScheduleExpression']=='cron(45 15 * * ? *)' and schedule['State']=='ENABLED'
        targets=events.list_targets_by_rule(Rule=schedule['Name'])
        ...
        proof={'contract':'etf-research-verification.v1','generated_at':datetime.now(timezone.utc).isoformat(),'commit':expected,'code_sha256':conf['CodeSha256'],
          ...
          'collection_reinvoked':False,'accepted_publication_after_transport_failure':'ops5870','protected_legacy_verified':backups,'http_reads_did_not_recollect':True,'source_errors':output['source_status_codes'],
          'schedule':{'kind':'events','name':schedule['Name'],'cron':schedule['ScheduleExpression'],'state':schedule['State']},
