...
def main():
    ...
    expected = subprocess.check_output(['git', 'log', '-1', '--format=%H', '--', 'aws/shared/treasury_fiscal_model.py'], text=True).strip()
    def raw(key):
    ...
    def invoke(short, event=None):
        ...
        return result
    with report('ops_5728_treasury_dimensions_replay_verify') as r:
