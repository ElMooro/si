"""ops 3686 — 4-FACTOR + 5 NEW PAIRS + GAP FIXES. portwatch v1.4 (Taiwan
...
with report("3686_chile_land") as rep:
    rep.heading("ops 3686 — 4-factor + new pairs + TW/CL fixes")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3686.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        dep("justhodl-portwatch", "portwatch v1.4.2 Chile names + canary priority")
    ...
    Path("aws/ops/reports/3686.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
