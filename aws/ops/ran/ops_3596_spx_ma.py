"""ops 3596 — justhodl-spx-ma v1 live: the dedicated S&P 500 MA engine (index
...
with report("3596_spx_ma") as rep:
    rep.heading("ops 3596 — S&P 500 MA Command engine")
    out = {"gates": {}}
    fails = []
    ...
    try:
        ...
        if isinstance(pl, dict) and pl.get("errorMessage"):
            ...
            b = j.get("breadth") or {}
            ok3 = (b.get("n_priced") or 0) >= 400 and isinstance(b.get("above200_pct"), (int, float))
            gate("G3_breadth_50_200", ok3,
                 f"n_priced={b.get('n_priced')}/{b.get('n_members')} above50={b.get('above50_pct')}% "
                 f"above200={b.get('above200_pct')}% spread={b.get('spread_50_200')} narrow={b.get('divergence_narrow_market')}")
            w = b.get("warming") or {}
            gate("G4_ledger_bootstrap", (w.get("ledger_days") or 0) >= 20,
    ...
    Path("aws/ops/reports/3596.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
