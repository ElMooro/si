"""ops 3678 — dedicated pages + sidebar for the geo-intel stack: geo-risk.html
...
from _lambda_deploy_helpers import deploy_lambda  # noqa: F401 (runner path bootstrap)
from ops_report import report

with report("3678_bs_page") as rep:
    rep.heading("ops 3678 — geo-stack dedicated pages + sidebar")
    out = {"gates": {}}
    fails = []
    Path("aws/ops/reports/3678.json").write_text(json.dumps({"verdict": "STARTED"}))
    ...
    PAGES = {
        "boom-stage.html": ["Boom-Stage Lens", "EARLY_PRICE_LED",
                             "boom-stage.json", "jh-nav-drawer.js",
                             "Lifecycle"],
    }
    ...
    Path("aws/ops/reports/3678.json").write_text(json.dumps(out, indent=2))
    sys.exit(0)
