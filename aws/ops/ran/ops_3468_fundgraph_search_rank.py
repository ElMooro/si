"""ops 3468 — close R1 from ops 3467 (search relevance).
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-3468",
...
with report("3468_fundgraph_search_rank") as rep:
    out = {"ops": 3468, "gates": {}}
    fails = []
    ...
    rep.heading("ops 3468 — search relevance close (name-first ranked merge)")
    ...
    deploy_lambda(
        ...
        description="Fundamental Graphs API v1.1.3 ranked search (ops 3468)",
    ...
    try:
        a = syms("apple")
        gate("R1b_apple_top3", "AAPL" in a[:3], a[:5])
        m = syms("micro")
        gate("R1c_micro_has_msft", "MSFT" in m, m[:6])
        t = syms("tesla")
        gate("R1d_tesla_top3", "TSLA" in t[:3], t[:5])
    except Exception as e:  # noqa: BLE001
        gate("R1b_apple_top3", False, str(e)[:220])

    out["status"] = "ALL PASS" if not fails else f"FAILS: {fails}"
    (REPO / "aws" / "ops" / "reports" / "3468.json").write_text(
