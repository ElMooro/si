"""ops_5100 -- global cycle v3: birth the feature store, run it on the real
...
def main():
    with report("5100-global-cycle-v3-birth") as r:
        r.heading("ops 5100 -- global cycle v3: feature store birth + v3.0.0 engine verification")
        ...
        deploy_lambda(report=r, function_name=FN_FEAT, source_dir=SRC_FEAT, env_vars={"S3_BUCKET": B},
                      timeout=600, memory=2048, create_function_url=False, smoke=False,
                      description=("Cycle feature store v1.0.0: per-country monthly business-cycle features from OECD "
        ...
        if not man:
            ...
            for k, need in (("cli_oecd", 12), ("bci", 25), ("ip_yoy", 25), ("credit_impulse", 25)):
                if cnt.get(k, 0) < need:
        ...
        while time.time() - t0 < 900:
            ...
            if "v3.0.0" in (c.get("Description") or "") and c.get("LastUpdateStatus") == "Successful":
                deployed = True
                r.ok(f"GBC v3.0.0 deployed {c.get('LastModified')} after {time.time() - t0:.0f}s")
                break
            r.log(f"  waiting for deploy-lambdas.yml… has_v3={'v3.0.0' in (c.get('Description') or '')} status={c.get('LastUpdateStatus')}")
            time.sleep(25)
        if not deployed:
            fails.append("GBC v3.0.0 not deployed within 15 min")
        ...
        if after is None:
            fails.append("no v3.0.0 feed landed within 15 min")
