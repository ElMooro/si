"""ops_5083 -- justhodl-fortress v1.0.2: the fortress rule made literal.

The 5082 diagnostics showed the median-of-3-episodes capture letting names
through that had fallen 15-35% inside the biggest SPY dump (UNCY -15% in
the -9% Jan-Mar episode, RGR -35% in the Oct-Nov -5% episode, ATEC -28%)
because two smaller episodes went their way. Khalid's brief is names that
barely dipped, stayed flat or rose during the BIG dumps, so v1.0.2:
  * dump_capture = size-weighted mean of episode captures (the deepest
    dump counts most), capture_worst published per name;
  * the dump gate now also requires the name never to have fallen MORE
    than the market in any single episode (capture_worst <= 1.0), and the
    worst episode is spelled out in the reasons when it disqualifies;
  * BIG_DUMP_PROOF flag when capture inside >= 8% dumps is <= 0.35;
  * resilience pillar penalises the worst episode;
  * ETF board: equity wrappers only -- the fleet-universe exemption had
    let TLT / VGLT / EDV (US Bonds) and UNG (commodity) back on.
Local harness green; page re-rendered with the new fields.
...
def main():
    with report("5083-fortress-v102") as r:
        r.heading("ops 5083 -- justhodl-fortress v1.0.2 redeploy + verified re-run")
        ...
        if not env_vars:
            ...
            sys.exit(1)
        env_vars["FORTRESS_VERSION"] = "1.0.2"
        ...
        json.dump({"ops": 5083, "engine": FN, "session": payload.get("session"),
                   "n_scored": payload.get("n_scored"), "tiers": payload.get("tiers"),
                   "coverage_pct": covs, "as_of": payload.get("as_of")},
                  open(str(ROOT / "aws" / "ops" / "reports" / "5083.json"), "w"), indent=1)
    sys.exit(0)
