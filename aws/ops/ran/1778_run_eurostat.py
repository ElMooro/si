...
for s in d["confidence"]+d["production_yoy"]:
    print(f"  {s['id']:22} {str(s['latest']):>9} ({s['latest_date']}) {s['first_date']}→ n={s['n_obs'] if 'n_obs' in s else s.get('n_points')} pctile={s['percentile']} | {s['label'][:42]}")
# heal manifest so they appear + are chartable now
print("heal:", lam.invoke(FunctionName="justhodl-ecb-history",InvocationType="RequestResponse",Payload=b'{}',
      ).get("StatusCode"))
