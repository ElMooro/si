...
def main():
    with report("3855_flows_page_gatefix") as rep:
        rep.heading("ops 3855 — flows.html render fix proven on the edge, live feed")
        fails = []
        ...
        checks = [
            ...
            ("nav-drawer script tag is self-closed (no trapped body)",
             bool(re.search(r'<script[^>]*jh-nav-drawer\.js[^>]*>\s*</script>', html))),
        ...
        FLOOR = {"meta-bar": 40}          # one-line status string, not a container
        for tid in TARGETS:
            b, rows = res[tid]["bytes"], res[tid]["rows"]
            ok = b > FLOOR.get(tid, 200)
