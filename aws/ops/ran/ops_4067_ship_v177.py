"""
ops_4067 — ship v1.5.0: rebuild the zip Khalid downloads, settle the
...
def main():
    with report("4067_ship_v177") as rep:
        rep.heading("ops 4016 — extension v1.7.7 + sources persistence")
        checks = []
        ...
        checks.append(("zip carries v1.5.0", man.get("version") == "1.7.7"))
        ...
        checks.append(("zip content is FULLY AUTONOMOUS",
                       "scanner.tradingview.com/symbol" in cjs and "sc2_ok" in cjs))
        bjs = chk.read("background.js" if rooted
                       else "chrome-extension/background.js").decode()
        checks.append(("background proxies symsearch", "ssfetch" in bjs))
