"""ops_5205 -- KATLIN v1.1.0 verify: redeploy from the runner, async run + freshness gate, war-room read, tiers, joins,
...
with report("ops_5205_katlin_v110_verify") as R:
    R.heading("ops 5205 -- KATLIN v1.1.0 verify")
    ...
    if not D:
        ...
        R.section("v1.1.0 lanes")
        if D.get("version") != "1.1.0":
            FAILS.append("artifact version %s != 1.1.0 (stale code ran)" % D.get("version"))
