"""
ops/853 - justhodl-merger-arb-risk RE-VERIFY (loss-cap + ordering fix).
...
Writes aws/ops/reports/853_merger_arb_risk_reverify.json.
"""
import io
import json
import time
...
rep = {
    ...
    "subject": "Re-verify justhodl-merger-arb-risk (the Merger-Arb "
...
if None not in (cb, wq, me, fc):
    # break loss is capped at zero, the worst-quartile is a pure break-loss
    # sum and the model number is a probability blend, so these invariants
    # hold by construction on any real book:
    #   cluster <= 0 ; cluster <= worst_quartile <= 0 ;
    #   cluster <= model_expected <= full_close ; cluster <= full_close
    ordered = (cb <= E and cb <= wq + E <= E
               and cb <= me + E and me <= fc + E and cb <= fc + E)
else:
    ordered = False
check("scenario_ordering_sane", ordered,
      f"cluster={cb} worst_q={wq} model={me} full_close={fc} "
      f"(need cluster<=0, cluster<=worst_q<=0, cluster<=model<=full_close)")
...
for p in live:
    ...
    # for a long arb leg with a genuine downside the break loss must be <= 0
    if (bl is not None and dn is not None and pos is not None
            and pos > 0 and dn < 0 and bl > 1e-6):
        econ_bad.append(f"{p.get('symbol')}:break_loss>0")
    # implied break probability, when defined, must be a probability
    if ib is not None and not (0.0 <= ib <= 1.0):
        econ_bad.append(f"{p.get('symbol')}:impl_break={ib}")
check("break_economics_sane", not econ_bad,
      "all live deals sane (negative carry on a bump name is allowed)"
      if not econ_bad else "; ".join(econ_bad[:4]))
...
print(out)
try:
    with open("aws/ops/reports/853_merger_arb_risk_reverify.json", "w") as f:
        f.write(out)
except Exception as e:
