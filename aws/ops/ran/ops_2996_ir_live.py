#!/usr/bin/env python3
"""ops 2996 -- industry-rotation FINAL FIX: the fleet schedules via
...
def get(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-2996",
...
def main():
    fails, warns = [], []
    out = {"ops": 2996, "ts": datetime.now(timezone.utc).isoformat()}
    with report("2996_ir_live") as rep:
...
def _w(rep, out, fails, warns):
    ...
    (AWS_DIR / "ops" / "reports" / "2996.json").write_text(
...
try:
    ...
    import traceback
    (AWS_DIR / "ops" / "reports" / "2996.json").write_text(json.dumps(
        {"ops": 2996, "verdict": "FAIL",
