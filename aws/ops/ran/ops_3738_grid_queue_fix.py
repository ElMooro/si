#!/usr/bin/env python3
"""ops 3738 — SHIP justhodl-grid-queue v1.0 (canary list item #2).
...
with report("3738_grid_queue_fix") as rep:
    rep.heading("ops 3738 — SHIP justhodl-grid-queue v1.0 (power buildout canary)")
    fails = []
    out = {"gates": {}}
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3738.json").write_text(json.dumps({"verdict": "STARTED"}))
    ...
    try:
        ...
        if doc:
            ...
            # ops 3737 GROUND TRUTH: CAISO's ACTIVE queue sheet holds 277
            # body rows (sheet1), not thousands — the 1,768-row sheet is
            # WITHDRAWN. Capacity lives in mw-1/2/3, not 'net mws to grid'.
            # Threshold set to observed reality, not to a guess.
            plausible = n_act >= 150 and mw >= 5000
        ...
        Path("aws/ops/reports/3738.json").write_text(json.dumps(out, indent=2))
        ...
        Path("aws/ops/reports/3738.json").write_text(
