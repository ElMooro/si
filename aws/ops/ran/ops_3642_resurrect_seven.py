"""ops 3642 — RESURRECTION SEVEN: the remaining true-breakage engines — same diagnose/invoke/freshness/auto-Scheduler pattern, cadence-correct crons (CFTC weekly Fri, symbol-dictionary weekly Sun, tv-tracker post-crawler 06:30). Per engine: config + CW 7d inv/err + Scheduler
...
SEVEN = [
    ("justhodl-treasury-noise", ["data/treasury-noise.json",
                                 "data/treasury-noise-history.json"],
     "cron(45 21 ? * MON-FRI *)"),
    ("justhodl-forward-orders", ["data/forward-orders.json"],
     "cron(50 21 ? * MON-FRI *)"),
    ("justhodl-supply-chain-graph", ["data/supply-chain-graph.json"],
     "cron(55 21 ? * * *)"),
    ("justhodl-cftc-deep-view", ["data/cftc-deep-view.json"],
     "cron(10 22 ? * FRI *)"),
    ("justhodl-tv-watchlist-tracker", ["data/tv-watchlist-tracker.json"],
     "cron(30 6 * * ? *)"),
    ("justhodl-thesis-engine", ["data/thesis-engine.json"],
     "cron(0 22 * * ? *)"),
    ("justhodl-symbol-dictionary", ["data/symbol-dictionary.json"],
     "cron(15 7 ? * SUN *)"),
]

with report("3642_resurrect_seven") as rep:
    rep.heading("ops 3642 — remaining seven true-breakage engines")
    out = {"gates": {}, "engines": {}}
    fails = []
            ...
            if freshened and not E["err"] and not scheds:
                try:
                    ...
                    EBS.create_schedule(
                        ...
                        Description=f"ops 3642 resurrection: {fn} daily",
    ...
    Path("aws/ops/reports/3642.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
