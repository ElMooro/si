"""ops/4930 -- justhodl-usd-funding v1.0.0 launch + acceptance.
...
UA = "JustHodl ops4930 edge-acceptance"
...
def main():
    ...
    fails, notes = [], []
    with report("4930-usd-funding-accept") as r:
        r.heading("ops 4930 — justhodl-usd-funding v1.0.0 launch")
        ...
        if fails:
            ...
            r.ok("all gates green" + (" (notes: %s)" % ", ".join(notes)
                                      if notes else ""))
        (ROOT / "aws" / "ops" / "reports" / "4930.json").write_text(
