"""ops 3647 — NEW justhodl-portwatch (IMF PortWatch chokepoints, discovered
...
with report("3647_portwatch") as rep:
    rep.heading("ops 3647 — IMF PortWatch chokepoint engine")
    out = {"gates": {}, "catalog": {
    ...
    try:
        deploy_lambda(report=rep, function_name=FN,
                      ...
                      description="portwatch v1.0: IMF chokepoint transits, z + disruption flags"[:200],
        ...
        gate("G1_data", ok1,
             ...
             f"disrupted={j.get('n_disrupted')} sample={[(c['name'], c['z'], c['vs_baseline_pct'], c['status']) for c in cps[:5]]} "
    ...
    while time.time() < dl:
        try:
            ...
            det = f"row={'Chokepoints' in h} fetch={'portwatch.json' in h} mi_zip={mi_ok}"
    ...
    Path("aws/ops/reports/3647.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
