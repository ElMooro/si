"""ops 3629 — forensic + hardened retry. Unzipped-marker check on the
...
with report("3629_forensic_v171") as rep:
    rep.heading("ops 3629 — deployed-zip forensic + v1.7.1/v2.5")
    out = {"gates": {}}
    fails = []
    ...
    try:
        err = dep_inv("justhodl-asia-leads", 300, 512,
                      "asia-leads v1.7.1: unconditional stage-6 keys + brute-SN")
        ...
        gate("G1_tw", all(mk.values()) and not err and
             ...
             f"s6_sample={[{k: r0.get(k) for k in ('sn', 'hit', 'titles')} for r0 in s6[:3]]} "
    ...
    try:
        err = dep_inv("justhodl-china-liquidity", 300, 768,
                      "china-liquidity v2.5: W020 JS-file content follow")
        ...
        gate("G2_cn", (got or cn.get("js_files") is not None
             ...
             f"js_files={cn.get('js_files')} via={cn.get('via')} note={cn.get('error')}")
        out["cn"] = cn
    except Exception as e:
    ...
    Path("aws/ops/reports/3629.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
