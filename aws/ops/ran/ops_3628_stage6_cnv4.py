"""ops 3628 — clean regate: CN national-node/attachment (v2.3) + TW stage-5
...
with report("3628_stage6_cnv4") as rep:
    rep.heading("ops 3628 — CN national + TW stage-5 (frame-body scan)")
    out = {"gates": {}}
    fails = []
    ...
    try:
        err = dep_inv("justhodl-china-liquidity", 300, 768,
                      "china-liquidity v2.4: newest-monthly across pages + stripped-phrase attachment check")
    ...
    try:
        err = dep_inv("justhodl-asia-leads", 240, 512,
                      "asia-leads v1.7: stage-6 GetPointData SitesSN sweep")
        ...
        gate("G2_tw5", (got or len(eps) >= 1 or len(s5) >= 1
             ...
             f"s6_hit={tw.get('stage6_hit')} s5_hit={tw.get('stage5_hit')} eps={[str(e.get('u'))[-44:] for e in eps[:5]]} "
             f"s5={[{k: t.get(k) for k in ('sn', 'via', 'hit', 'titles', 'item_blob', 'head')} for t in s5[:2]]} "
    ...
    Path("aws/ops/reports/3628.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
