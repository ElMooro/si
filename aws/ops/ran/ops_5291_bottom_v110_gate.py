"""ops_5291 -- BOTTOM v1.1.0 fix-forward gate (after the ops 5290 launch ran GREEN on every engine/page gate and tripped only on a list/dict slip in the fusion proof).
...
UA = {"User-Agent": "justhodl-ops-5291", "Cache-Control": "no-cache", "Pragma": "no-cache"}
FAILS, WARNS = [], []
PUSH_TS = int(subprocess.run(["git", "log", "-1", "--format=%ct", "--grep=ops 5291", "--", "aws/lambdas/justhodl-bottom/source/lambda_function.py"], cwd=ROOT, capture_output=True, text=True).stdout.strip() or "0")
...
with report("ops_5291_bottom_launch") as R:
    R.heading("ops 5291 -- BOTTOM launch (Wyckoff bottom desk)")
    ...
    arn = cfg["FunctionArn"]
    name, expr, desc = "justhodl-bottom-daily", "cron(45 3 ? * TUE-SAT *)", "BOTTOM daily 03:45 UTC Tue-Sat after polygon-full + fortress, before katlin (ops 5291)"
    ...
    if live and D:
        ...
        with sync_playwright() as p:
            ...
            for width, height in ((1440, 1100), (390, 844)):
                ...
                pg.screenshot(path=str(SHOTS / f"ops5291_bottom_{width}.png"))
                R.log("   %4dpx: %s errors=%s" % (width, json.dumps(facts)[:420], errors[:2]))
                if facts["funnel"] != 7 or facts["helps"] < 15 or facts["defs"] < 10 or errors or facts["err"] or facts["rates_cards"] != 2:
