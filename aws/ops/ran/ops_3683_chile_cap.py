"""ops 3683 — 4-FACTOR + 5 NEW PAIRS + GAP FIXES. portwatch v1.4 (Taiwan
...
with report("3683_chile_cap") as rep:
    rep.heading("ops 3683 — 4-factor + new pairs + TW/CL fixes")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3683.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        dep("justhodl-portwatch", "portwatch v1.4.1 ids cap 120")
    ...
    Path("aws/ops/reports/3683.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
