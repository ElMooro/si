...
def main():
    with report("3978_census_v11_close") as rep:
        ...
        rep.section("A1. resources + engine v1.1")
        ...
        for i in range(30):
            ...
            if c.get("State") == "Active" and c.get("LastUpdateStatus") != "InProgress":
                break
            time.sleep(10)
        checks.append(("v1.1 resources applied",
                       c.get("Timeout") == 900 and c.get("MemorySize") == 3008))
        ...
        if not fresh:
            ...
            lam.invoke(FunctionName=FN, InvocationType="Event",
                       Payload=json.dumps({"source": "ops3978"}).encode())
            for i in range(45):
                time.sleep(20)
                doc = read_out()
