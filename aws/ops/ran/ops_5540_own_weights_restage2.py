"""ops 5540 -- restage after forensics (ops 5539): both jobs died in 80 s because the code object was uploaded as
stage_base_weights-<sha>.py while the entrypoint ran stage_base_weights.py. The script now lives under a sha-named
PREFIX with its plain name, and the entrypoint runs that. Nothing else changed.

ops 5537 -- staging job re-launch after jh-stage-...-214158 failed with exit 2 and (EndOfJob upload) left no manifest
behind: outputs now upload CONTINUOUSLY so the manifest/reason survives a refusal, the stage script also reads the
LICENSE file and drops a removed huggingface_hub kwarg; the runner group also gets read-only pull on the AWS DLC
registry (763104351884) so the ECR mirror can copy. The previous job's CloudWatch log tail is fetched when permitted.

ops 5535 -- re-arm of 5528-5534: AddTags still denied 45 s after the group policy landed (propagation); this run
waits 90 s, widens the AddTags resource to every processing job, and if the tagged create is still denied it
launches untagged once (attribution kept in the staging record) rather than losing another cycle.

ops 5534 -- re-arm of 5528-5533: pricing works ($0.23/h, cap $0.46); CreateProcessingJob was denied on
sagemaker:AddTags (cost-attribution tags). The runner group gets a processing-job policy (Create/Describe/Stop/
AddTags/ListTags on processing jobs + PassRole for the SageMaker execution role), then the staging job launches.

ops 5533 -- re-arm of 5528-5532: the runner user had no pricing:GetProducts at all (cost_guard swallowed the
AccessDenied into "no live price"). The same IAM group that unlocked ECR gets a pricing policy first; then the live
probe runs. ECR repo now exists (5532).

ops 5532 -- re-arm of 5528-5531: the Price List has no `instanceName=ml.m5.xlarge` row for the Processing/Training
lanes; those rows are keyed `instanceType=ml.m5.xlarge-Processing` (older vintage). This op probes the live Price
List with several attribute keys, all pages, and records which one priced the job. Still refuses if none does.
The ECR group grant from 5531 should have propagated by now.
...
def main() -> int:
    ...
    with report("ops_5540_own_weights_restage2") as R:
        R.heading("ops 5540 (restage 2) -- owned training lane: ECR repo, pinned recipe bundle, base-weight staging job (CPU, priced), no flip")
        R.kv(head=head[:10])
        ...
        # the runner user needs pricing:GetProducts (global); grant through the runner group (no per-user quota)
        try:
            iam = boto3.client("iam")
            user = boto3.client("sts").get_caller_identity()["Arn"].split("/")[-1]
            parn = "arn:aws:iam::%s:policy/justhodl-runner-pricing" % ACCOUNT
            try:
                iam.create_policy(PolicyName="justhodl-runner-pricing", Description="Runner: read the AWS Price List (cost guard)",
                                  PolicyDocument=json.dumps({"Version": "2012-10-17", "Statement": [{"Effect": "Allow", "Action": ["pricing:GetProducts", "pricing:DescribeServices", "pricing:GetAttributeValues"], "Resource": "*"}]}))
            except iam.exceptions.EntityAlreadyExistsException:
                pass
            try:
                iam.create_group(GroupName="justhodl-runner-ecr")
            ...
            darn = "arn:aws:iam::%s:policy/justhodl-runner-dlc-pull" % ACCOUNT
            try:
                iam.create_policy(PolicyName="justhodl-runner-dlc-pull", Description="Runner: pull AWS Deep Learning Containers (mirror into owned ECR)",
                                  PolicyDocument=json.dumps({"Version": "2012-10-17", "Statement": [{"Effect": "Allow", "Action": ["ecr:BatchGetImage", "ecr:GetDownloadUrlForLayer", "ecr:BatchCheckLayerAvailability", "ecr:DescribeImages"],
                                                                                                     "Resource": "arn:aws:ecr:us-east-1:763104351884:repository/*"}]}))
            except iam.exceptions.EntityAlreadyExistsException:
                pass
            iam.attach_group_policy(GroupName="justhodl-runner-ecr", PolicyArn=darn)
            jarn = "arn:aws:iam::%s:policy/justhodl-runner-processing-v2" % ACCOUNT
            try:
                iam.create_policy(PolicyName="justhodl-runner-processing-v2", Description="Runner: create/describe tagged SageMaker processing jobs for weight staging",
                                  PolicyDocument=json.dumps({"Version": "2012-10-17", "Statement": [
                                      {"Effect": "Allow", "Action": ["sagemaker:CreateProcessingJob", "sagemaker:DescribeProcessingJob", "sagemaker:StopProcessingJob",
                                                                     "sagemaker:AddTags", "sagemaker:ListTags"],
                                       "Resource": "arn:aws:sagemaker:%s:%s:processing-job/*" % (REGION, ACCOUNT)},
                                      {"Effect": "Allow", "Action": ["iam:PassRole"], "Resource": ROLE,
                                       "Condition": {"StringEquals": {"iam:PassedToService": "sagemaker.amazonaws.com"}}}]}))
            except iam.exceptions.EntityAlreadyExistsException:
                pass
            iam.attach_group_policy(GroupName="justhodl-runner-ecr", PolicyArn=jarn)
            iam.add_user_to_group(GroupName="justhodl-runner-ecr", UserName=user)
            R.ok("pricing + processing-job (jh-stage-*) policies attached to group justhodl-runner-ecr for %s; waiting 45 s" % user)
            time.sleep(90)
        except Exception as e:  # noqa: BLE001
            warn.append("pricing-grant"); R.warn("pricing grant failed: %s" % str(e)[:160])
        ...
        if not hourly:
            def probe(filters, want):
                token = None
                for _ in range(8):
                    kw = {"ServiceCode": "AmazonSageMaker", "MaxResults": 100, "Filters": filters}
                    if token:
                        kw["NextToken"] = token
                    r = pricing.get_products(**kw)
                    for raw in r.get("PriceList") or []:
                        prod = json.loads(raw)
                        attrs = (prod.get("product") or {}).get("attributes") or {}
                        blob = json.dumps(attrs).lower()
                        if INSTANCE.lower() not in blob or want not in blob:
                            continue
                        for t in ((prod.get("terms") or {}).get("OnDemand") or {}).values():
                            for d in (t.get("priceDimensions") or {}).values():
                                try:
                                    v = float((d.get("pricePerUnit") or {}).get("USD") or 0)
                    ...
                    if not token:
                        break
                return None, None
            region = [{"Type": "TERM_MATCH", "Field": "regionCode", "Value": "us-east-1"}]
            for label, filters, want in (
                ("instanceType=ml.m5.xlarge-Processing", region + [{"Type": "TERM_MATCH", "Field": "instanceType", "Value": INSTANCE + "-Processing"}], "process"),
                ("component=Processing", region + [{"Type": "TERM_MATCH", "Field": "component", "Value": "Processing"}], INSTANCE.lower()),
                ("instanceType=ml.m5.xlarge-Training", region + [{"Type": "TERM_MATCH", "Field": "instanceType", "Value": INSTANCE + "-Training"}], "train"),
                ("component=Training", region + [{"Type": "TERM_MATCH", "Field": "component", "Value": "Training"}], INSTANCE.lower()),
            ):
                try:
                    v, attrs = probe(filters, want)
                except Exception as e:  # noqa: BLE001
                    R.log("price probe %s failed %s" % (label, str(e)[:100])); continue
                if v:
                    hourly = v
                    price = {"usd_per_hour": v, "source": "aws-price-list:%s" % label, "attributes": {k: attrs.get(k) for k in ("instanceType", "usagetype", "component")}}
                    R.ok("priced by %s: $%.4f/h %s" % (label, v, json.dumps(price["attributes"])))
                    break
        if not hourly:
        ...
        code_key = "factory/training/code/%s/stage_base_weights.py" % head[:10]
        ...
        try:
            ...
            streams = logs.describe_log_streams(logGroupName="/aws/sagemaker/ProcessingJobs", logStreamNamePrefix="jh-stage-", orderBy="LastEventTime", descending=True, limit=3)
            for st in streams.get("logStreams", [])[:1]:
                ev = logs.get_log_events(logGroupName="/aws/sagemaker/ProcessingJobs", logStreamName=st["logStreamName"], limit=25, startFromHead=False)
                tail = " | ".join(e.get("message", "")[:160] for e in ev.get("events", [])[-8:])
                R.log("previous job log tail (%s): %s" % (st["logStreamName"][:50], tail[:900]))
        except Exception as e:  # noqa: BLE001
            R.log("log tail unavailable: %s" % str(e)[:100])
        ...
        if existing and existing.get("status") == "staged":
            ...
            def _create(tagged):
                kw = dict(ProcessingJobName=name, RoleArn=ROLE,
                ...
                ProcessingInputs=[{"InputName": "code", "S3Input": {"S3Uri": "s3://%s/%s" % (PRI, code_key.rsplit("/", 1)[0] + "/"), "LocalPath": "/opt/ml/processing/input/code",
                ...
                if tagged:
                    kw["Tags"] = cg.tags("factory-base-weights", 3) + [{"Key": "jh-factory", "Value": "stage-base-weights"}]
                sm.create_processing_job(**kw)
            try:
                _create(True); tagged = True
            except Exception as e:  # noqa: BLE001
                if "AddTags" not in str(e):
                    raise
                R.warn("tagged create denied (AddTags not yet propagated); launching untagged once -- attribution in the staging record")
                _create(False); tagged = False
