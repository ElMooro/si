"""ops 1134 — Redeploy router with:
  (a) build_names_prompt supports dotted paths (rankings.composite_top_50 for momentum)
  (b) ticker_id_field config so each record gets normalized "ticker" key
  (c) better fallback list keys (top_100, all_qualifying, clusters, top_setups)
  (d) bigger ticker record budget (700 chars) + bigger prompt budget (7500 chars)
Plus uploads the patched 30-context registry and reinvokes the 7 per-name contexts.
"""
...
def main():
    ...
    try:
        ...
        # Show the patched config for each per-name context so we have a record
        rpt["patched_ctx_config"] = {}
        for ctx_id in NEW_NAMES:
            c = registry["contexts"].get(ctx_id, {})
            rpt["patched_ctx_config"][ctx_id] = {
                "primary_feed":         c.get("primary_feed"),
                "primary_tickers_field": c.get("primary_tickers_field"),
                "primary_score_field":  c.get("primary_score_field"),
                "ticker_id_field":      c.get("ticker_id_field"),
                "top_n":                c.get("top_n"),
            }

        print(f"[1134] reinvoking router for {len(NEW_NAMES)} per-name contexts with patched config")
        ...
        for ctx_id in NEW_NAMES:
            ...
            verify.append(row)
        rpt["verify"] = verify
        rpt["n_briefs"] = sum(1 for r in verify if r.get("status") == "OK" and r.get("fresh") and (r.get("n_names") or 0) > 0)
    ...
    p = os.path.join(REPO_ROOT, "aws/ops/reports/1134.json")
