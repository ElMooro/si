...
out["verify"] = {"version": d.get("version"), "duration_s": d.get("duration_s"),
                  ...
                  "aapl": {k: aapl.get(k) for k in ("roe", "ev_ebitda", "ev_s", "ev_fcf",
                                                      "ps", "pe", "label")},
                  "ev_s_coverage": sum(1 for x in sp if x.get("ev_s") is not None),
                  "ev_fcf_coverage": sum(1 for x in sp if x.get("ev_fcf") is not None),
                  "hp_top_ev": [{"t": x.get("t"), **{k: (x.get("metrics") or {}).get(k)
                                  for k in ("ev", "ev_s", "net_cash", "cash", "debt")}}
                                 for x in (d.get("hp") or [])[:4]],
...
open("aws/ops/reports/1626_valuations_ev.json", "w").write(json.dumps(out, indent=1, default=str))
