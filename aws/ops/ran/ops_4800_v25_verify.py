"""ops/4800 -- settle-gated v2.5 verify (house-standard from here on):
 ...
 (2) deploy settle: get_function -> download deployed zip -> grep the
     '"2.5"' marker (deterministic); fallback to LastModified age.
 (3) Event-invoke, poll as_of <=13 min, assert engine_v == 2.5, TE
...
def main():
    with report("4800_v25_verify") as rep:
        rep.heading("ops 4799 -- settle-gated v2.5 verify")
        ...
        if not marker_ok:
            rep.warn("deployed code lacks the 2.5 marker -- deploy "
