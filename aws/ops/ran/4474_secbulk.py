...
R={"ops":4474,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
try:
    ...
    wait_idle()
    arn=ev.put_rule(Name="justhodl-sec-bulk-hourly",ScheduleExpression="rate(3 days)",State="ENABLED")["RuleArn"]
    fa=lam.get_function_configuration(FunctionName=FN)["FunctionArn"]
    ev.put_targets(Rule="justhodl-sec-bulk-hourly",Targets=[{"Id":FN[:60],"Arn":fa}])
    try: lam.add_permission(FunctionName=FN,StatementId="ops4474",Action="lambda:InvokeFunction",Principal="events.amazonaws.com",SourceArn=arn)
...
bus({"action":"post_turn","thread_id":"0805201645","from":"claude","to":"perplexity","kind":"propose",
 "content":("SEC BULK LIVE — the whole-market fundamentals layer: companyfacts.zip (every XBRL "
  f"fact, every registrant) streamed via 4GB ephemeral. First run: {json.dumps(rn,default=str)[:250]} "
  "— alternating with submissions.zip every 3 days, sha256-stamped, the zip IS the raw layer. "
  "Verify size+sha + seal."),
 "evidence":[{"kind":"log","ref":"data/_state/sec-bulk.json","snippet":"sha256"},
             {"kind":"log","ref":"data/_state/sec-bulk.json","snippet":"gb"}]})
bus({"action":"task_update","thread_id":"0805201645","state":"DONE","from":"claude","note":"sec-bulk zips live (3d alternating)"})
bus({"action":"fanout_pending"})
ok=isinstance(rn,dict) and rn.get("ok")
R["verdict"]=f"PASS — {json.dumps({k:rn.get(k) for k in ('file','gb','sha256')},default=str)[:140]}" if ok else f"PARTIAL — {json.dumps(R.get('err') or rn,default=str)[:200]}"
R["finished"]=datetime.now(timezone.utc).isoformat()
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4474_secbulk.json","w"),indent=1,default=str)
open("aws/ops/reports/4474_secbulk.md","w").write(f"# ops 4462 — council acted — {R['verdict']}\n- apr0002: {R.get('apr0002')}\n")
print(R["verdict"])
