"""ops_5155 -- 5154 follow-up: the probe left "10-year treasury" in the explorer search before switching to NY Fed (0 rows was the filter, not a bug). hub-catalog fallback generalized (gdelt lists its 402k files), explorer + probe waits for rows.
...
def http(url, timeout=180):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5155", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5155-tv-shell-explorer") as r:
        r.heading("ops 5155 -- TradingView shell + Data Explorer (final verification)")
        fails = []
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5155-tv-shell.jpg"), type="jpeg", quality=55)
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5155-explorer-chart.jpg"), type="jpeg", quality=55)
                ...
                for _ in range(20):
                    ...
                    if page.evaluate("() => document.querySelectorAll('#dx-list .dx-row .hs-add').length") > 0:
                        break
                nrows = page.evaluate("() => ({ rows: document.querySelectorAll('#dx-list .dx-row').length, adds: document.querySelectorAll('#dx-list .dx-row .hs-add').length, count: document.getElementById('dx-count').textContent })")
                ...
                r.log(f"  nyfed list {json.dumps(nrows)} -> + from explorer -> watchlist: {wl}")
                if not wl:
