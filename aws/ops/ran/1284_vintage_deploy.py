"""1284 — deploy + schedule + invoke justhodl-vintage-fred (point-in-time macro)."""
import json, os, time, zipfile, io
from datetime import datetime, timezone
import boto3
from botocore.config import Config
REPORT="aws/ops/reports/1284_vintage.json"; BUCKET="justhodl-dashboard-live"
...
try:
    ...
    try: lam.get_function_configuration(FunctionName=LAMBDA); lam.update_function_code(FunctionName=LAMBDA,ZipFile=zb); act="updated"
        ...
        lam.create_function(FunctionName=LAMBDA,Runtime="python3.12",Role=ROLE,Handler="lambda_function.lambda_handler",
            Code={"ZipFile":zb},Description="Point-in-time vintage macro (FRED ALFRED)",Timeout=300,MemorySize=512,
            Architectures=["x86_64"],Environment={"Variables":{"FRED_KEY":FRED,"S3_BUCKET":BUCKET}},Publish=False); act="created"
    for _ in range(30):
        time.sleep(2); c=lam.get_function_configuration(FunctionName=LAMBDA)
        if c.get("State")=="Active" and c.get("LastUpdateStatus") in ("Successful",None): break
    # ensure env has FRED key
    lam.update_function_configuration(FunctionName=LAMBDA,Environment={"Variables":{"FRED_KEY":FRED,"S3_BUCKET":BUCKET}})
    time.sleep(3)
    out["deploy"]=act
except Exception as e: out["deploy_err"]=str(e)[:300]
try:
    events.put_rule(Name=RULE,ScheduleExpression=SCHED,State="ENABLED",Description="Daily vintage macro snapshot")
...
try:
    ...
    out["vintage_index"]={"n":len(idx) if isinstance(idx,(list,dict)) else "?"}
except Exception as e: out["vintage_index"]={"err":str(e)[:120]}
out["finished"]=datetime.now(timezone.utc).isoformat()
open(REPORT,"w").write(json.dumps(out,indent=2,default=str)); print(json.dumps(out,indent=2,default=str)[:700])
