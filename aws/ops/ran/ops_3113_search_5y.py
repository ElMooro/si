#!/usr/bin/env python3
"""ops 3113 -- SEARCH = FULL 5-YEAR READ (Khalid: typing a stock must
...
350-500B, pe present, sector medians non-empty. 3112 lesson: page markers must be static-source literals, not template-rendered strings -- "vsBox(" is split by ${med} interpolation."""
...
UA = {"User-Agent": "Mozilla/5.0 ops-3113", "Cache-Control": "no-cache"}
...
def main():
    fails, warns = [], []
    with report("3113_search_5y") as rep:
        ...
        try:
            ...
            if (datetime.now(timezone.utc) - o["LastModified"]
                ...
                if cand.get("version") == "2.2.0":
                    d = cand
                    rep.kv(doc_mb=round(len(json.dumps(cand)) / 1e6,
                                        2), source="existing")
        except Exception:
            pass
        if not d:
            L.invoke(FunctionName=FN, InvocationType="Event",
                     Payload=b"{}")
        for _ in range(50 if not d else 0):
            time.sleep(20)
            try:
                o = S3.get_object(Bucket=BUCKET,
                                  Key="data/forensic-screen.json")
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3113.json").write_text(json.dumps(
        {"ops": 3113, "verdict": "FAIL" if fails else "PASS",
