...
v2 math (engine v2.4.2), no future information anywhere:
...
Gates: G1 v2.4.2 settled · G2 fresh GSSI: breadth+comove numeric, weekly b/c
...
with report("3391_gssi_math_v242") as rep:
    ...
    while time.time() < dl:
        try:
            if LAM.get_function_configuration(FunctionName=FN).get("LastUpdateStatus") == "Successful":
                src = zsrc(FN)
                if 'VERSION = "2.4.2"' in src and "dispersion identity" in src and "_expz" in src and "krw_canary" in src:
                    ok1 = True; break
    ...
    while time.time() < dl:
        try:
            ...
            if j.get("version") == "2.4.2" and (j.get("generated_at") or "") > t_inv:
                g = j; break
        except Exception: pass
        time.sleep(20)
    ...
    Path("aws/ops/reports/3391.json").write_text(json.dumps(out, indent=2))
    sys.exit(0)
