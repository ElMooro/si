...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-nuke"
code=r'''
...
def lambda_handler(e,c):
    ...
    time.sleep(2)
    # verify empty + fast
    t0=time.time(); g=call("/brain?uid="+UID+"&t=9")
    out["after"]={"notes":len(g.get("notes",[])) if isinstance(g.get("notes"),list) else g,"secs":round(time.time()-t0,2)}
    return out
'''
buf=io.BytesIO()
...
open("aws/ops/reports/1429_n.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
