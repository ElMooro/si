"""ops 3630 — forensic + hardened retry. Unzipped-marker check on the
...
with report("3630_xerr_dump") as rep:
    rep.heading("ops 3630 — deployed-zip forensic + v1.7.1/v2.5")
    out = {"gates": {}}
    fails = []
    ...
    try:
        err = dep_inv("justhodl-asia-leads", 300, 512,
                      "asia-leads v1.7.2: blob-first + per-item xerr surfaced")
    ...
    Path("aws/ops/reports/3630.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
