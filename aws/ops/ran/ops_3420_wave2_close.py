...
with report("3420_wave2_close") as rep:
    ...
    gate("G0_all_settled",
         ...
         settled("justhodl-proven-portfolio",'VERSION = "1.2.3"') and
         settled("justhodl-best-setups","gamma_levels") and
         settled("justhodl-feed-registry",'VERSION = "1.2.0"') and
    ...
    gate("G2_registry", (fr.get("generated_at") or "")>t0 and 150<=fr.get("n_feeds",0)<=1200,
    ...
    while time.time()<dl:
        ...
        if pp.get("version")=="1.2.3" and (pp.get("generated_at") or "")>t0: break
        time.sleep(10)
    ...
    gate("G5_setups_context", n_sec>=8 and len(pbc)>=1 and any((pbc[0] or {}).values()),
         f"sector_ctx={n_sec}/25 walls={n_wall}/25 playbook_rules={len(pbc)} sample_pb={json.dumps(pbc[:1])[:120]}")
    out["verdict"]="PASS_ALL" if not fails else "GAPS: "+",".join(fails)
    print("\nVERDICT:",out["verdict"]); rep.log("VERDICT: "+out["verdict"])
    Path("aws/ops/reports/3420.json").write_text(json.dumps(out,indent=2,default=str)); sys.exit(0)
