...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-dn"
code=r'''
...
def lambda_handler(e,c):
    out={}
    out["dedup"]=call("/brain?dedupnow=1&token="+T+"&uid="+NK)
    time.sleep(2)
    g=call("/brain?uid="+NK+"&t=9")
    notes=g.get("notes",[]) if isinstance(g.get("notes"),list) else []
    seen={}
    for n in notes:
        k=(n.get("text") or "")[:60].lower().strip(); seen[k]=seen.get(k,0)+1
    out["final"]={"total":len(notes),"unique":len(seen),"dup_groups":sum(1 for v in seen.values() if v>1)}
    return out
'''
buf=io.BytesIO()
...
open("aws/ops/reports/1448_dn.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
