"""ops 3711 — Stage 1.5: make consensus observable so TWICE_UNPRICED can be earned.
...
with report("3711_consensus_coverage") as rep:
    rep.heading("ops 3711 — consensus observability, stage 1.5")
    out = {"gates": {}}
    fails = []
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3711.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        while time.time() - t0 < 300:
            try:
                ...
                if 'VERSION = "1.2.0"' in src and "load_analyst_actions" in src:
                    print("  artifact ready after", round(time.time() - t0), "s")
                    break
            except Exception as e:
                print("  retry:", str(e)[:70])
            time.sleep(20)
        gate("G1_shipped", 'VERSION = "1.2.0"' in src and "load_analyst_actions" in src
             and "consensus_snapshot" in src and "consensus_observed" in src,
        ...
        cov = d.get("consensus_coverage") or {}
    ...
    Path("aws/ops/reports/3711.json").write_text(json.dumps(out, indent=2, default=str))
