...
with report("3436_close2") as rep:
        ...
        try:
            if LAM.get_function_configuration(FunctionName="justhodl-ai-brief-router").get("LastUpdateStatus")=="Successful":
                ...
                with urllib.request.urlopen(urllib.request.Request(info["Code"]["Location"],headers=UA),timeout=60) as r:
                    if '"regime_3m"' in zipfile.ZipFile(io.BytesIO(r.read())).read("lambda_function.py").decode("utf-8","replace"):
                        ok2=True; break
    ...
    Path("aws/ops/reports/3436.json").write_text(json.dumps(out,indent=2,default=str)); sys.exit(0)
