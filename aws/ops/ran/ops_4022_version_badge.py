"""ops_4022 — dynamic version badge from the served zip's own manifest."""
import io
import json
import sys
import time
...
def main():
    with report("4022_version_badge") as rep:
        ...
        checks = [("zip is v1.6.0", ver == "1.6.0"),
                  ("version.json publicly serves it",
                   got.get("version") == "1.6.0")]
        n, htm = 0, ""
        for i in range(9):
            try:
