...
noise={"EQ","WCUFF","NUWE","AGMH","TPET","LTUM"}
present={x["symbol"] for x in d["deals"]}
print("prior-noise still present:", sorted(noise & present) or "NONE (clean)")
print("\nALL DEALS (post precision-2):")
for x in d["deals"][:20]:
    print("  %-6s %-7s mat=%s%% val=%s age=%sh sc=%s | %s"%(x["symbol"],x["cap_bucket"] or "?",x["materiality_pct"],x["deal_value_str"],x["age_h"],x["score"],x["title"][:70]))
