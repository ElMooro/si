"""ops_5139 -- chart-pro: deferred reap (two frames) closes the last "Object is disposed"; clean-console gate.
...
def http(url, timeout=60):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5139", "Accept-Encoding": "identity"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def main():
    with report("5139-chartpro-clean") as r:
        r.heading("ops 5139 -- chart-pro: zero page errors on native renders")
        fails = []
        ...
        while time.time() - t0 < 900 and not all(got.values()):
            try:
                ...
                got["chart"] = b"two frames later, so a chart" in c
        ...
        if got["chart"]:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5139-chartpro-native.jpg"), type="jpeg", quality=60)
