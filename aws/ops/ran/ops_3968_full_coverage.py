"""
ops_3968 — race-safe settle + verify of domain-barometers v1.1.
...
MARK = "domain-barometers v1.2 ops3968 full-coverage"
...
def main():
    with report("3968_full_coverage") as rep:
        ...
        r = lam.invoke(FunctionName=FN, InvocationType="RequestResponse",
                       Payload=json.dumps({"source": "ops3968"}).encode())
        ...
        rep.section("D2. the ops-3967 defects must now be closed")
        for s_ in ("MOVE", "FEDFUNDS", "DTWEXBGS", "RBUSBIS", "USIRYY", "CPFF"):
            x = idx.get(s_) or {}
            rep.log(f"  {s_:10s} pol={x.get('polarity')} basis={x.get('polarity_basis')} "
                    f"chg={x.get('chg_pct')} status={x.get('status')}")
        mv = idx.get("MOVE") or {}
        ff = idx.get("FEDFUNDS") or {}
        dx = idx.get("DTWEXBGS") or {}
        checks += [
            ("MOVE has a direction (42 notes, was silently dropped)",
             bool(mv.get("polarity"))),
            ("FEDFUNDS no longer guarded to 0 (174 notes)", bool(ff.get("polarity"))),
            ("DTWEXBGS broad dollar now signed (22 notes)", bool(dx.get("polarity"))),
        ]
        cov = {d: (bar.get(d) or {}).get("coverage") or {} for d in
               ("MACRO", "LIQUIDITY", "RISK")}
        for d, c in cov.items():
            rep.log(f"  {d:9s} coverage {json.dumps(c)}")
        checks.append(("every barometer publishes its coverage denominator",
                       all(c.get("voting") is not None for c in cov.values())))
        tot_vote = sum(c.get("voting") or 0 for c in cov.values())
        rep.kv(total_voting_now=tot_vote, was_before=156,
               ledger_prev=json.dumps(doc.get("ledger_prev_values")))
        checks.append(("more indicators voting than the 156 baseline", tot_vote > 156))
