"""ops_5085 -- justhodl-fortress v1.0.4: remaining income-overlay wrappers out
of the ETF board (OMAH "Target 15 ... Select Income" survived the 5084 regex);
same gates, same verification, diagnostics dump retained.
...
def main():
    with report("5085-fortress-v104") as r:
        r.heading("ops 5085 -- justhodl-fortress v1.0.4 redeploy + verified re-run")
        ...
        if not env_vars:
            ...
            sys.exit(1)
        env_vars["FORTRESS_VERSION"] = "1.0.4"
        ...
        json.dump({"ops": 5085, "engine": FN, "session": payload.get("session"),
                   "n_scored": payload.get("n_scored"), "tiers": payload.get("tiers"),
                   "coverage_pct": covs, "as_of": payload.get("as_of")},
                  open(str(ROOT / "aws" / "ops" / "reports" / "5085.json"), "w"), indent=1)
    sys.exit(0)
