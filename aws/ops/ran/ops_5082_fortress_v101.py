"""ops_5082 -- justhodl-fortress v1.0.1: gate consistency + equity-only ETF
board, redeployed and re-run on real data with a diagnostics dump.

What the 5081 first run showed and what v1.0.1 changes:
  * ETF board was topped by CLO / cash-like wrappers (AAA, PCMM, LDRT ...):
    trivially "resilient". Now equity ETFs only (finviz etf_type Equities*
    or fleet rotation/flow universe), 100d vol >= 8%, AUM >= $50M.
  * dump gate could be passed by a name that rose through the drawdown
    episodes but bled -260 bps on the single worst days (AVNW). Episode and
    worst-day reads may no longer contradict (capture <= 0.35 needs worst-
    day excess >= -75 bps; day-path needs capture <= 0.7).
  * coil gate accepted a 1-session TTM squeeze; now >= 3 sessions or a
    bandwidth percentile <= 20.
  * negative capture is worded as "ROSE while SPY dumped", thin liquidity
    is a risk line, the board carries up to 800 rows so WATCH is visible.
...
def main():
    with report("5082-fortress-v101") as r:
        r.heading("ops 5082 -- justhodl-fortress v1.0.1 redeploy + verified re-run")
        ...
        if not env_vars:
            ...
            sys.exit(1)
        env_vars["FORTRESS_VERSION"] = "1.0.1"
        ...
        for x in (payload.get("etfs") or [])[:14]:
            r.log("etf %s %s comp=%s cap=%s worst=%s ema=%s bbw=%s flow=%s vol100=%s aum=%s type=%s | %s" % (
                ...
                (x.get("name") or "")[:40]))
        for x in board[:14]:
            r.log("row %s %s %s %s cap=%s n_ep=%s worst=%s green=%s dc=%s bbw=%s sq=%s vol100=%s adv=%s px=%s eps=%s risks=%s" % (
                x.get("tier"), x.get("ticker"), x.get("cap_bucket"), (x.get("industry") or "")[:22],
                x.get("dump_capture"), x.get("n_episodes"), x.get("worst_days_excess_bps"),
        ...
        bad_etf = [x["ticker"] for x in (payload.get("etfs") or [])[:50]
        ...
        json.dump({"ops": 5082, "engine": FN, "session": payload.get("session"),
                   "n_scored": payload.get("n_scored"), "tiers": payload.get("tiers"),
                   "coverage_pct": covs, "as_of": payload.get("as_of")},
                  open(str(ROOT / "aws" / "ops" / "reports" / "5082.json"), "w"), indent=1)
    sys.exit(0)
