"""ops/4804 -- settle-gated v2.7 verify (house-standard from here on):
 ...
 (2) deploy settle: get_function -> download deployed zip -> grep the
     '"2.7"' marker (deterministic); fallback to LastModified age.
 (3) Event-invoke, poll as_of <=13 min, assert engine_v == 2.7, TE
...
def main():
    with report("4804_v27_verify") as rep:
        rep.heading("ops 4799 -- settle-gated v2.7 verify")
        ...
        if not marker_ok:
            rep.warn("deployed code lacks the 2.7 marker -- deploy "
