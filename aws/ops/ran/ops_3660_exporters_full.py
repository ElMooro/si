"""ops 3660 — exporters full coverage (Khalid: port calls in ALL major
exporting countries). portwatch v1.3: name variants (pusan/hai phong/yangshan/
...
with report("3660_exporters_full") as rep:
    rep.heading("ops 3660 — exporters full-coverage v1.3")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3660.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        deploy_lambda(report=rep, function_name="justhodl-portwatch",
                      ...
                      description="portwatch v1.3: full exporter coverage"[:200],
        ...
        gate("G1_full", ok1,
             f"err={err} ports_n={len(j.get('ports') or [])} nations={len(ex)} "
    ...
    Path("aws/ops/reports/3660.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
