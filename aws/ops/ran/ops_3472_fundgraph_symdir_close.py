"""ops 3472 — symdir close: multi-endpoint tolerant builder + ?symdir=1 diagnostics.
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-3472",
...
with report("3472_fundgraph_symdir_search") as rep:
    out = {"ops": 3472, "gates": {}}
    fails = []
    ...
    rep.heading("ops 3472 — symdir ranked search + memory-archive 34")
    ...
    deploy_lambda(report=rep, function_name=FN,
                  ...
                  description="Fundamental Graphs v1.1.6 symdir tolerant+diag (ops 3472)",
    ...
    try:
        ...
        gate("D1_symdir_built", sd.get("n", 0) >= 6000 and d0.get("n", 0) >= 6000,
    ...
    (REPO / "aws" / "ops" / "reports" / "3472.json").write_text(
