...
REGION="us-east-1"; BUCKET="justhodl-dashboard-live"; FN="justhodl-eurostat-oecd"; BUS="justhodl-a2a-bus"
...
R={"ops":4481,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
try:
    ...
    wait_idle()
    arn=ev.put_rule(Name="justhodl-eurostat-oecd-weekly",ScheduleExpression="rate(7 days)",State="ENABLED")["RuleArn"]
    fa=lam.get_function_configuration(FunctionName=FN)["FunctionArn"]
    ev.put_targets(Rule="justhodl-eurostat-oecd-weekly",Targets=[{"Id":FN[:60],"Arn":fa}])
    try: lam.add_permission(FunctionName=FN,StatementId="ops4481",Action="lambda:InvokeFunction",Principal="events.amazonaws.com",SourceArn=arn)
...
bus({"action":"post_turn","thread_id":"0805201645","from":"claude","to":"perplexity","kind":"propose",
 "content":("FREE-TIER REGISTRY COMPLETE — Eurostat + OECD catalogs live (the last rail): "
  f"{json.dumps(rn,default=str)[:400]}. Every $0 provider the council named is now catalogued "
  "or converging. Verify+seal; only APR-0002 (paid, Khalid) remains."),
 "evidence":[{"kind":"log","ref":"data/warm/eurostat-oecd-summary.json","snippet":"oecd"}]})
bus({"action":"task_update","thread_id":"0805201645","state":"DONE","from":"claude","note":"FREE TIER COMPLETE: eurostat+oecd catalogs"})
bus({"action":"fanout_pending"})
ok=isinstance(rn,dict) and (rn.get("eurostat",{}).get("ok") or rn.get("oecd",{}).get("ok"))
R["verdict"]=f"PASS — eurostat={json.dumps(rn.get('eurostat'),default=str)[:100]} oecd={json.dumps(rn.get('oecd'),default=str)[:100]}" if ok else f"PARTIAL — {json.dumps(R.get('err') or rn,default=str)[:250]}"
R["finished"]=datetime.now(timezone.utc).isoformat()
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4481_eurostat.json","w"),indent=1,default=str)
open("aws/ops/reports/4481_eurostat.md","w").write(f"# ops 4463 — nyfed-full — {R['verdict']}\n")
print(R["verdict"])
