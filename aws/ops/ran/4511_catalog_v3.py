...
R={"ops":4511,"started":datetime.now(timezone.utc).isoformat()}
for _ in range(20):
...
try:
    ...
    R["top_by_keys"]=[(p["slug"],p["n_keys"],p.get("hot_feeds")) for p in sorted(hub["providers"],key=lambda x:-x["n_keys"])[:8]]
...
bus({"action":"post_turn","thread_id":"0806-master","from":"claude","to":"perplexity","kind":"propose",
 "content":("HUB v3 — PLATFORM-WIDE (Khalid: way more datasets): rollup-married, legacy hot feeds attributed per provider: candidate prefixes for DDP/ECB + discovery. "
  f"Discovered: {json.dumps(R.get('discovery'),default=str)[:260]} · new totals {json.dumps(R.get('totals'))} · "
  f"top={json.dumps(R.get("top_by_keys"))[:220]} still_zero={R.get('still_zero')} (occ/midas/chicago are the known blocks; fred-canary fills as its warm lands). Verify hub+seal."),
 "evidence":[{"kind":"log","ref":"data/provider-catalog.json","snippet":"totals"}]})
bus({"action":"fanout_pending"})
R["verdict"]=f"top={json.dumps(R.get('top_by_keys'))[:200]} totals={json.dumps(R.get('totals'))} top={json.dumps(R.get("top_by_keys"))[:220]} still_zero={R.get('still_zero')} disc={json.dumps(R.get('discovery'),default=str)[:200]}"
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4511_catalog_v3.json","w"),indent=1,default=str)
open("aws/ops/reports/4511_catalog_v3.md","w").write(f"# ops 4508 — hub v2 — {R['verdict']}\n")
print(R["verdict"])
