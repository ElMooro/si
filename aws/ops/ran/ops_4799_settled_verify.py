"""ops/4799 -- settle-gated v2.4 verify (house-standard from here on):
 ...
 (2) deploy settle: get_function -> download deployed zip -> grep the
     '"2.4"' marker (deterministic); fallback to LastModified age.
 (3) Event-invoke, poll as_of <=13 min, assert engine_v == 2.4, TE
...
def main():
    with report("4799_settled_verify") as rep:
        rep.heading("ops 4799 -- settle-gated v2.4 verify")
        ...
        if not marker_ok:
            rep.warn("deployed code lacks the 2.4 marker -- deploy "
        ...
        for mid in ("DE10Y_TE", "IT10Y_TE", "FR10Y_TE", "ES10Y_TE",
                     "D_BTP_BUND_D", "WREPOFOR", "WLRRAFOIAL"):
            s0 = allrows.get(mid)
            if s0:
