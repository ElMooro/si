...
fails = []
with report("4308_long_stitched") as r:
    r.heading("ops 4308 -- rehypo barometer + 1996 long history")
    ...
    if str(st).startswith("FAIL"):
        ...
        try:
            ...
            if not L.get("actual_start") or \
                    L["actual_start"][:4] > "2014":
                fails.append("actual_start %s later than 2014 -- "
                             "even proven-keyid segments unreachable"
                             % L.get("actual_start"))
