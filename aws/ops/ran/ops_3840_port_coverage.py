"""
ops_3840 — justhodl-global-recession v1.0.0 + chokepoint day-two re-read
...
def main():
    with report("3840_port_coverage") as rep:
        rep.heading("ops 3840 — v1.3.1 self-maintaining port->ISO map: global recession ensemble + chokepoint day-two")
        ...
        # v1.3: the whole point — at least SOME country must now be confirmed
        # against physical/credit data rather than nothing.
        chk("CONFIRMED count > 0 (hard legs are working)",
            (cnt.get("CONFIRMED", 0) + cnt.get("DIVERGENT", 0)) > 1,
            f"confirmed={cnt.get('CONFIRMED')} divergent={cnt.get('DIVERGENT')}")
        hard = [c for c in cs if "physical" in
                json.dumps(c.get("confirmation_detail") or {}).lower()
                or "credit impulse" in
                json.dumps(c.get("confirmation_detail") or {}).lower()]
        rep.log(f"    ports mapped to {cf.get('ports_countries')} countries")
        if cf.get("ports_unmapped"):
            rep.warn(f"    still unmapped port countries: {cf.get('ports_unmapped')}")
        chk("port coverage >= 8 countries",
            (cf.get("ports_countries") or 0) >= 8,
            f"= {cf.get('ports_countries')}")
        chk("at least 6 countries on a hard leg", len(hard) >= 6,
            f"= {[c['iso3'] for c in hard][:12]}")
        chk("unconfirmed share fell below 84.3% (v1.2.1 baseline)",
            (cf.get("unconfirmed_share_of_global_pct") or 100) < 84.3,
            f"= {cf.get('unconfirmed_share_of_global_pct')}%")
        for c in hard[:8]:
            rep.log(f"    {c['iso3']}: {c['confirmation']} p={c['recession_prob_pct']}% "
                    f"{json.dumps(c.get('confirmation_detail'))[:150]}")
