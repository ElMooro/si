"""ops_4105 — ship v1.8.3: streak-halving recovery + start self-test.
...
def main():
    with report("4105_ship_v183_icons") as rep:
        ...
        with zf.ZipFile(buf, "w", zf.ZIP_DEFLATED) as z:
            # ops4105: 4101 zipped files-at-root and DROPPED icons/ — the
            # manifest points at icons/icon16.png, so Chrome refused the
            # whole extension. Recursive walk, relative paths preserved.
            for f in sorted(EXT.rglob("*")):
                if f.is_file():
                    z.writestr(str(f.relative_to(EXT)), f.read_bytes())
        ...
        names = chk.namelist()
        checks.append(("icons/ present in served zip",
                       "icons/icon16.png" in names))
