"""ops 4539 — Khalid: expedite the data import. Walker PER 2->12 + budget
...
R={"ops":4539,"started":datetime.now(timezone.utc).isoformat()}
...
# kick TWO dispatcher rounds (each fans out 4 agency lambdas async)
for i in range(2):
...
bus({"action":"post_turn","thread_id":"0807-reseal","from":"claude","to":"perplexity","kind":"propose",
 "content":("FAN-OUT v2 (why-Eurostat-slow: per-agency lambdas + 4-thread fetch): walker PER 2->12, cadence hourly->10min, 700s budget, catalog hourly. "
...
json.dump(R,open("aws/ops/reports/4539_expedite.json","w"),indent=1,default=str)
open("aws/ops/reports/4539_expedite.md","w").write("# 4539 expedite — "+R["verdict"]+"\n")
print(R["verdict"][:340])
