...
cfg=Config(read_timeout=130,retries={"max_attempts":1})
lam=boto3.client("lambda",region_name="us-east-1",config=cfg)
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-vc"
code=r'''
...
def lambda_handler(e,c):
    ...
    out["t0"]=call("/brain-purge?uid="+UID+"&token="+T+"&max=1&offset=0").get("total_now")
    time.sleep(95)
    out["t95"]=call("/brain-purge?uid="+UID+"&token="+T+"&max=1&offset=0").get("total_now")
    return out
'''
buf=io.BytesIO()
...
try:
    try: lam.get_function_configuration(FunctionName=FN); lam.update_function_code(FunctionName=FN,ZipFile=buf.getvalue())
        ...
        lam.create_function(FunctionName=FN,Runtime="python3.12",Role=ROLE,Handler="lambda_function.lambda_handler",Code={"ZipFile":buf.getvalue()},Timeout=130,MemorySize=128)
    for _ in range(20):
...
open("aws/ops/reports/1433_vc.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
