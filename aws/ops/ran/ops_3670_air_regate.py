"""ops 3670 — AIR-CARGO canary + freight page + sidebar: [A] NEW
...
with report("3670_air_regate") as rep:
    rep.heading("ops 3670 — HKIA air canary + freight page + sidebar")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3670.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        gate("G1_air", (not err) and got,
             ...
             f"errs={aj.get('errors')} row={str(aj.get("xlsx_row"))[:70]} tail={aj.get("xlsx_tail")} n={aj.get("xlsx_n")} xp={str(aj.get("xlsx_probe"))[:180]} cadsp={str(aj.get("cad_sub_probe"))[:200]} sm={aj.get("sitemap_hits")}")
    ...
    Path("aws/ops/reports/3670.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
