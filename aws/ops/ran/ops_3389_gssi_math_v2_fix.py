...
v2 math (engine v2.4.1), no future information anywhere:
...
Gates: G1 v2.4.1 settled · G2 fresh GSSI: breadth+comove numeric, weekly b/c
...
with report("3389_gssi_math_v2_fix") as rep:
    ...
    while time.time() < dl:
        try:
            if LAM.get_function_configuration(FunctionName=FN).get("LastUpdateStatus") == "Successful":
                src = zsrc(FN)
                if 'VERSION = "2.4.1"' in src and "dispersion identity" in src and "_expz" in src:
                    ok1 = True; break
    ...
    while time.time() < dl:
        try:
            ...
            if j.get("version") == "2.4.1" and (j.get("generated_at") or "") > t_inv:
                g = j; break
        except Exception: pass
        time.sleep(20)
    ...
    Path("aws/ops/reports/3389.json").write_text(json.dumps(out, indent=2))
    sys.exit(0)
