...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-wn"
code=r'''
import json,urllib.request
def lambda_handler(e,c):
    try:
        h=urllib.request.urlopen(urllib.request.Request("https://justhodl.ai/why-now.html?t=9",headers={"User-Agent":"jh"}),timeout=15).read().decode()
        return {"served":"Why" in h and "compound-signals.json" in h and "meaningful state changes" in h,"bytes":len(h)}
...
open("aws/ops/reports/1467_wn.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
