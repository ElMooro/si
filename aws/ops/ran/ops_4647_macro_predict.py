...
def main():
    misses = 0
    with report("4647_macro_predict") as r:
        r.heading("ops 4643 — DXY predict-the-future engine")

        r.section("pre-dump: macro-predict list candidates")
        ...
        with zf2.ZipFile(buf, "w", zf2.ZIP_DEFLATED) as z:
            z.write("aws/lambdas/justhodl-macro-predict/source/"
        ...
        pl = json.loads(s3.get_object(
            Bucket=B, Key="data/macro-predict.json")["Body"].read())
        dx = pl.get("macro") or {}
        rows = pl.get("rows") or []
        ...
        for att in range(9):
            try:
                pg = http_get("https://justhodl.ai/"
                              ...
                              % time.time()).decode("utf-8",
                                                    "replace")
                page_ok = ("data/macro-predict.json" in pg)
                jd = json.loads(http_get(
                    "https://justhodl.ai/data/macro-predict.json"
                    "?cb=%d" % time.time()))
                pay_ok = bool((jd.get("macro")
                               or {}).get("trend_label"))
        ...
        if misses:
            ...
            sys.exit(1)
        r.ok("MACRO PREDICT LIVE — list '%s': %s/%s resolved, %d "
             "signed rows · TREND %s (%s) · REVERSAL %s (%s) · "
             "https://justhodl.ai/macro-predict.html"
