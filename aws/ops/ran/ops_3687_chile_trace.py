"""ops 3687 — 4-FACTOR + 5 NEW PAIRS + GAP FIXES. portwatch v1.4 (Taiwan
...
with report("3687_chile_trace") as rep:
    rep.heading("ops 3687 — 4-factor + new pairs + TW/CL fixes")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3687.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        dep("justhodl-portwatch", "portwatch v1.4.3 chile trace + guard 20")
        ...
        gate("G1_ports", ok1,
             f"matched={pj.get('ports_ref_matched_total')} nations={len(ex)} TW={ex.get('Taiwan')} CL={ex.get('Chile')} "
             f"CN_ports={(ex.get('China') or {}).get('n_ports')} "
             f"slowing={pj.get('exporters_slowing')} TRACE={pj.get('chile_trace')}")
    ...
    Path("aws/ops/reports/3687.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
