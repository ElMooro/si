"""ops_5096 -- GBC v2.1.1: supplement scale guard + what FRED actually serves.
...
def main():
    with report("5096-gbc-v211-supplement-guard") as r:
        r.heading("ops 5096 -- GBC v2.1.1 supplement scale guard + FRED series metadata")
        fails = []
        ...
        while time.time() - t0 < 900:
            ...
            if "v2.1.1" in (cfg.get("Description") or "") and cfg.get("LastUpdateStatus") == "Successful":
                deployed = True
                r.ok(f"deployed {cfg.get('LastModified')} after {time.time() - t0:.0f}s")
                break
            r.log(f"  waiting… desc_has_v2.1.1={'v2.1.1' in (cfg.get('Description') or '')} status={cfg.get('LastUpdateStatus')}")
            time.sleep(25)
        if not deployed:
            fails.append("v2.1.1 not deployed within 15 min")
        ...
        if after is None:
            fails.append("no v2.1.1 feed landed within 15 min")
        ...
        if st.startswith("scale-unrecognised") or st.startswith("stale") or st in ("none", "unavailable"):
            ...
            r.ok(f"USA supplement recognised and blended: value={usa.get('supplement_value')}")
        else:
