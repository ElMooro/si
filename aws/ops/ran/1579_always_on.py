...
out["manifest"] = {"n": len(ser), "missing_stats": missing_stats,
                    "healed_n": sum(1 for x in ser if x.get("healed_from_file")),
                    "has_balance_sheet": any(x["id"]=="balance_sheet" and x.get("percentile") is not None for x in ser),
...
out["radar"] = {"version": d.get("version"), "n_sparks": d.get("n_sparks"),
                 ...
                 "always_on": {k: ({kk: (ind.get(k) or {}).get(kk) for kk in
                     ("signal","official_yoy","tracker_fwd_yoy","nfc_loans_yoy","impulse_6m_pp",
                      "m3_yoy","spf_longterm_pct","it_de_bp","as_of")
                     if kk in (ind.get(k) or {})} if isinstance(ind.get(k), dict) else "ABSENT")
                     for k in ("wage_persistence","credit_contraction",
                                "expectations_deanchoring","fragmentation_stress")},
                 "spreads_bp": (ind.get("fragmentation_stress") or {}).get("spreads_bp")}
open("aws/ops/reports/1579_always_on.json","w").write(json.dumps(out,indent=2,default=str))
