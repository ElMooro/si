"""ops 3144 — Flow-Price Divergence layer in etf-fund-flows.
...
with report("3144_etf_divergence_clean") as rep:
    fails, warns = [], []
    t0 = datetime.now(timezone.utc)
    rep.heading("ops 3144 — Flow-Price Divergence layer")
    ...
    lev = board.get("leveraged_extremes")
    if lev is None:
        fails.append("leveraged_extremes missing — split not deployed")
    else:
        rep.ok(f"leveraged extremes split out: {len(lev)} "
               f"({', '.join(m['ticker'] for m in lev[:4])})")
    # emission fix: nav now in metrics; if any clean candidate |z|>=1.5
    # existed, at least one signal must have logged
    clean_cand = [m for m in ms if not m.get("leveraged")
                  and m.get("quadrant") in ("STEALTH_ACCUMULATION",
                                             "DISTRIBUTION_RALLY")
                  and abs(m.get("flow_zscore_90d") or 0) >= 1.5]
    logged = comp.get("divergence_signals_logged") or 0
    rep.kv(clean_candidates=len(clean_cand), signals_logged_v2=logged)
    if clean_cand and logged == 0:
        fails.append(f"{len(clean_cand)} clean candidates but 0 signals "
                     "logged — nav/emit path still broken")
    elif clean_cand:
        rep.ok(f"emission live: {logged} signals into justhodl-signals")
    if ms and "nav" not in ms[0]:
        fails.append("nav field missing from metrics")
