"""ops/4801 -- settle-gated v2.6 verify (house-standard from here on):
 ...
 (2) deploy settle: get_function -> download deployed zip -> grep the
     '"2.6"' marker (deterministic); fallback to LastModified age.
 (3) Event-invoke, poll as_of <=13 min, assert engine_v == 2.6, TE
...
def main():
    with report("4801_v26_verify") as rep:
        rep.heading("ops 4799 -- settle-gated v2.6 verify")
        ...
        if not marker_ok:
            rep.warn("deployed code lacks the 2.6 marker -- deploy "
