...
fac=d.get("factors",{})
out["new_factors"]={k:{"risk":fac.get(k,{}).get("risk"),"note":fac.get(k,{}).get("note")} for k in ["inflation_print","etf_flows","ai_rotation","macro_risk"]}
out["top_drivers"]=[x["note"] for x in d.get("top_drivers",[])]
open("aws/ops/reports/1324_dr.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
