"""ops 4629 — fleet-join close: nested columnar + structural ceiling: the missing data from other
...
def main():
    misses = 0
    with report("4629_fleet_close") as r:
        r.heading("ops 4629 — fleet-join close: nested columnar + structural ceiling resolution")
        ...
        for att in range(16):
            try:
                ...
                if "justhodl-blackswan-watch v1.3.2" in src:
                    settled = True
                    break
        ...
        misses += contract(r, "resolution",
                           ...
                           "%s/500 resolved — remainder is TV-native (EUREX/ICE futures, NQ indexes, FX_IDC, MULTPL) awaiting extension vault sync" % pl.get("n_resolved"))
        misses += contract(r, "history-depth",
                           (pl.get("n_with_history") or 0) >= 215,
