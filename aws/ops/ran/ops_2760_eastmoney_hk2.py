"""ops 2760 — HK Southbound via Eastmoney (HKEX Akamai-blocked from AWS).
Deploy + invoke + dump Eastmoney structure so extractor can be finalized.
Report: 2760_eastmoney_hk.json.
"""
...
R = {"ops": 2760, "ts": datetime.now(timezone.utc).isoformat()}
def zip_fn(fn):
...
json.dump(R, open("aws/ops/reports/2760_eastmoney_hk.json", "w"), indent=1, ensure_ascii=False, default=str)
print("OPS 2760 COMPLETE")
