...
UA = "JustHodl ops4933 edge-acceptance"
...
def main():
    ...
    fails, notes = [], []
    with report("4933-usd-funding-nyfix") as r:
        r.heading("ops 4933 — justhodl-usd-funding deploy (in-ops) + accept")
        ...
        if fails:
            ...
            r.ok("all gates green" + (" (notes: %s)" % ", ".join(notes)
                                      if notes else ""))
        (ROOT / "aws" / "ops" / "reports" / "4933.json").write_text(
