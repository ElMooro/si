"""ops 3679 — NEW justhodl-boom-stage (Khalid: use port activity to prove an
...
with report("3679_bs_signals") as rep:
    rep.heading("ops 3679 — boom-stage v1.1 SIGNALS: 6 pairs + transitions/sliding + trades + sentinel")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3679.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        gate("G1_engine", ok1,
             ...
             f"signals={[(x['type'], x['pair']) for x in j.get('signals') or []]} hist_prev={((j.get('pairs') or [{}])[0].get('trajectory') or {}).get('prev_day')} stages={[(p['id'], p['stage'], (p.get('value') or {}).get('yoy_pct'), (p.get('volume') or {}).get('vs_baseline_pct')) for p in j.get('pairs') or []]}")
        ...
        cfg3 = LAM.get_function_configuration(FunctionName="justhodl-alert-sentinel")
        deploy_lambda(report=rep, function_name="justhodl-alert-sentinel",
                      source_dir=Path(__file__).resolve().parents[2] / "lambdas" / "justhodl-alert-sentinel" / "source",
                      env_vars=(cfg3.get("Environment") or {}).get("Variables") or {},
                      timeout=cfg3.get("Timeout", 180), memory=cfg3.get("MemorySize", 512),
                      description=(cfg3.get("Description") or "sentinel")[:200],
                      create_function_url=False)
        loc2 = LAM.get_function(FunctionName="justhodl-alert-sentinel")["Code"]["Location"]
        zf2 = zipfile.ZipFile(io.BytesIO(urllib.request.urlopen(loc2, timeout=60).read()))
        blob2 = b"".join(zf2.read(n) for n in zf2.namelist() if n.endswith(".py"))
        mi_ok = mi_ok and (b"boom_signals" in blob2)
        ...
        while time.time() < dl:
            try:
                ...
                mk = {"signals_panel": "Signals" in h and "SLIDING_WATCH" in h,
                      "trade_lines": "stage trade" in h,
                      "traj": "trajectory vs" in h,
    ...
    Path("aws/ops/reports/3679.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
