...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-walk"
code=r'''
...
def lambda_handler(e,c):
    out={"deleted":0,"rounds":0}; off=0
    for i in range(60):
        d=call("/brain-purge?uid="+UID+"&token="+T+"&garble=1&max=150&offset="+str(off))
        out["rounds"]+=1
        if d.get("err"): out["err"]=d["err"]; break
        out["deleted"]+=d.get("deleted",0); off=d.get("next_offset",off+150)
        out["total_now"]=d.get("total_now")
        if not d.get("more"): out["done"]=True; break
    return out
'''
buf=io.BytesIO()
...
open("aws/ops/reports/1410_w.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
