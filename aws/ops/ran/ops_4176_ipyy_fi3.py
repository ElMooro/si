"""ops_4176 — IPYY (self-computed yoy) + FI (CPI COICOP self-discovery)
...
def main():
    with report("4176_ipyy_fi3") as rep:
        ...
        rank = ("YOY_PCH_PA_PT", "PC_CP_A_PT", "PCH_CP_A_PT")
        ...
        if "IPYY-FI ops4176" not in fs:
            ...
            block = (
                ...
                "OECD/MEI/.PRINTO01.IXOBSA.M?observations=1&limit=500\")\n"
                "    try:\n"
            ...
            fs = fs.replace(
                ...
                'MARKER = "families-feed v1.6 ops4176 ipyy-fi"', 1)
            import ast as _ast
            _ast.parse(fs)
            fp.write_text(fs)
        buf = io.BytesIO()
        ...
        checks.append(("feed v1.6 settled",
                       settle(rep, "justhodl-families-feed",
                              "families-feed v1.6 ops4176 ipyy-fi",
                              buf.getvalue())))
        ...
        if "|IPYY|FI|" not in vs:
            ...
            vs = vs.replace(
                ...
                'honest-labels"',
                'MARKER = "tradingview-vault v3.24.0 ops4176 ipyy-fi"', 1)
            import ast as _ast
            _ast.parse(vs)
            vp.write_text(vs)
        vb = io.BytesIO()
        ...
        checks.append(("vault v3.24.0 settled",
                       settle(rep, "justhodl-tradingview",
                              "tradingview-vault v3.24.0 ops4176 ipyy-fi",
                              vb.getvalue())))
