"""ops_4070 — CHECK AND DELIVER: diag verdict; if real attribution flows,
...
KNOWN = {
    ...
    "COINGECKO": ("coingecko",),
    "MARKET-VENUES": ("tvc", "sgx", "tpex", "xetr", "omx", "six", "bme", "tradegate", "nasdaq", "nyse", "cboe", "cme", "ice ", "eurex",
...
def main():
    with report("4070_deliver_normalized") as rep:
        ...
        def norm(src):
            return PFX_RX.sub("", str(src or "")).strip()

        real = {}
        for k, v in m.items():
            n0 = norm(v.get("source"))
        ...
        econ = {k: v["source"] for k, v in real.items()
                if k.upper().startswith("ECONOMICS")}
        rep.section(f"ECONOMICS symbols walked so far: {len(econ)} — the "
                    "agency payoff preview")
        eby = Counter(econ.values())
        for src, n in eby.most_common(15):
            rep.log(f"  {n:4d}  {src[:60]}")
        ...
        s3.put_object(Bucket=BUCKET, Key="data/source-map.json",
                      Body=json.dumps({
                          ...
                          "marker": "source-map v1.3 ops4070 normalized",
