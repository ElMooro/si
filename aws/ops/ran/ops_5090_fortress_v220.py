"""ops_5090 -- justhodl-fortress v2.2.0: the engine learns which of its legs
carry information.

  * the walk-forward now scores the v2 accumulation / structure / momentum
    legs point-in-time from bars (OBV slope, up/down volume, absorption on
    the worst days, RS line vs SPY, higher lows, 3m/12m returns, beta) and
    reports each leg's Spearman rank-IC per test date -- mean, t-stat across
    dates, hit rate -- for 21s/63s excess (all windows) and for forward
    return in SPY-down windows
  * effective composite weights: each PRICE pillar's spec weight is scaled
    0.5-1.5x by its legs' IC t-stat (resilience judged on the down windows --
    protection is the thesis; the others on 63s excess), renormalised to 100,
    published as weights_effective + weights_evidence; fundamentals keep spec
Gates as 5089 (deploy, daily, backtest, daily re-run) plus: leg_ic table
present with >= 10 dates for the core legs; weights_effective sums to 100;
every IC row and the resolved weights are logged.
...
def main():
    with report("5090-fortress-v220") as r:
        r.heading("ops 5090 -- justhodl-fortress v2.2.0: leg ICs + evidence-scaled weights, verified")
        ...
        if not env_vars:
            ...
            sys.exit(1)
        env_vars["FORTRESS_VERSION"] = "2.2.0"
        ...
        json.dump({"ops": 5090, "engine": FN, "session": payload.get("session"),
                   "n_scored": payload.get("n_scored"), "tiers": payload.get("tiers"),
                   "coverage_pct": covs, "as_of": payload.get("as_of")},
                  open(str(ROOT / "aws" / "ops" / "reports" / "5090.json"), "w"), indent=1)
    sys.exit(0)
