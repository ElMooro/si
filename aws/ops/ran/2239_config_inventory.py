...
def wait_active():
    for _ in range(40):
        ...
        if c.get("State")=="Active" and c.get("LastUpdateStatus") in ("Successful",None): return c
        time.sleep(4)
    return lam.get_function(FunctionName=FN)["Configuration"]
c=wait_active(); print("state:",c.get("State"),c.get("LastUpdateStatus"))
...
lam.update_function_configuration(FunctionName=FN,Environment={"Variables":{"FRED_API_KEY":fred}},
    MemorySize=512,Timeout=180)
wait_active(); print("env set, FRED present:",bool(fred))
rule="justhodl-inventory-drawdown-weekly"
ev.put_rule(Name=rule,ScheduleExpression="cron(30 23 ? * TUE *)",State="ENABLED",Description="Inventory drawdown weekly")
...
ev.put_targets(Rule=rule,Targets=[{"Id":"1","Arn":arn}]); print("scheduled")
...
print("\nDRAWDOWN BOARD sample:")
for r in d.get("stock_drawdown_board",[])[:10]:
