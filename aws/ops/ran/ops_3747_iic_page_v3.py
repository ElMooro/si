#!/usr/bin/env python3
"""ops 3747 — import-canary.html + FIELD-COVERAGE AUDIT + nav pin.
...
with report("3747_iic_page_v3") as rep:
    rep.heading("ops 3747 — insider-industry-cluster page + field-coverage audit")
    fails = []
    out = {"gates": {}}
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3747.json").write_text(json.dumps({"verdict": "STARTED"}))
    ...
    try:
        ...
        for attempt in range(20):
            try:
                ...
                if "Insider Industry Cluster" in html and "thin universe (" in html:
        ...
        out["verdict"] = verdict
        out["unrendered"] = gaps
        Path("aws/ops/reports/3747.json").write_text(json.dumps(out, indent=2))
        ...
        Path("aws/ops/reports/3747.json").write_text(
