"""ops 3474 — symdir mcap ranking: within-tier marketCap desc (Microsoft > MicroBot); key symdir2.json.

Engine v1.1.5 builds data/fundgraph/symdir2.json from FMP /stable stock-list
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-3474",
...
with report("3474_fundgraph_symdir_search") as rep:
    out = {"ops": 3474, "gates": {}}
    fails = []
    ...
    rep.heading("ops 3474 — symdir ranked search + memory-archive 34")
    ...
    deploy_lambda(report=rep, function_name=FN,
                  ...
                  description="Fundamental Graphs v1.1.8 mcap-ranked (ops 3474)",
    ...
    try:
        ...
        time.sleep(2)
        obj = s3.get_object(Bucket=BUCKET, Key="data/fundgraph/symdir2.json")
    ...
    (REPO / "aws" / "ops" / "reports" / "3474.json").write_text(
