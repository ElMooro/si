"""ops 3643 — NEW justhodl-geopolitical-risk engine (World Monitor-inspired,
...
with report("3643_geo_risk") as rep:
    rep.heading("ops 3643 — geopolitical-risk engine (news-flow country stress)")
    out = {"gates": {}}
    fails = []
    ...
    try:
        ...
        if isinstance(pl, dict) and pl.get("errorMessage"):
            ...
            ok = (src.get("feeds_responding", 0) >= 20
                  ...
                  and len(rk) >= 15
                  and all(isinstance(r0.get("stress_score"), (int, float)) for r0 in rk[:5]))
    ...
    Path("aws/ops/reports/3643.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
