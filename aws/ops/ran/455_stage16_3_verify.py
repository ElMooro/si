#!/usr/bin/env python3
"""Step 455 — Verify Stage 16.3: 52-fund inverted index after deploy.
Should see more flagship positions from Druckenmiller (62 pos), Tepper (39),
Hohn (9), Peltz (7), Cooperman (40), Marks (172), etc.
"""
import io, json, os, time, zipfile
from datetime import datetime, timezone
import boto3

REPORT = "aws/ops/reports/455_stage16_3_verify.json"
NAME = "justhodl-tmp-455"
...
def lambda_handler(event, context):
    ...
    # Also show top positions for the newly-added famous funds
    NEW_FUNDS_TO_HIGHLIGHT = [
        ("0001345471", "Trian (Peltz)"),
        ("0001647251", "TCI (Hohn)"),
        ("0001656456", "Appaloosa (Tepper)"),
        ("0001536411", "Duquesne (Druckenmiller)"),
        ("0000898382", "Omega (Cooperman)"),
        ("0001135778", "Miller Value"),
        ("0000949509", "Oaktree (Marks)"),
        ("0000807249", "GAMCO (Gabelli)"),
        ("0001512857", "Brevan Howard"),
    ]
    fund_top_positions = {}
    fund_by_cik = {f["cik"]: f for f in funds}
    for cik, label in NEW_FUNDS_TO_HIGHLIGHT:
        f = fund_by_cik.get(cik)
        if not f:
            fund_top_positions[label] = {"missing": True}
            continue
        top = f.get("top_holdings", [])[:5]
        fund_top_positions[label] = {
            "cik": cik,
            "n_holdings": f.get("n_holdings"),
            "total_value_b": round((f.get("total_value") or 0)/1e9, 2),
            "top_5": [{"sym": h.get("symbol"), "name": (h.get("name") or "")[:24],
                          "value_b": round((h.get("value") or 0)/1e9, 2),
                          "pct": h.get("pct_of_fund")}
                         for h in top],
        }
    out["new_fund_positions"] = fund_top_positions
