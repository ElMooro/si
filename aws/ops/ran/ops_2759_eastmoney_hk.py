"""ops 2759 — HK Southbound via Eastmoney (HKEX Akamai-blocked from AWS).
Deploy + invoke + dump Eastmoney structure so extractor can be finalized.
Report: 2759_eastmoney_hk.json.
"""
...
R = {"ops": 2759, "ts": datetime.now(timezone.utc).isoformat()}
def zip_fn(fn):
...
json.dump(R, open("aws/ops/reports/2759_eastmoney_hk.json", "w"), indent=1, ensure_ascii=False, default=str)
print("OPS 2759 COMPLETE")
