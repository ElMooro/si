#!/usr/bin/env python3
"""ops 3073 (marker fix) -- Chart Pro TERMINAL V2 ('improve as much as you can'):
...
UA = {"User-Agent": "Mozilla/5.0 ops-3073",
...
def main():
    fails, warns = [], []
    with report("3073_terminal_v2") as rep:
        ...
        for m in ('id="vp-btn"', 'id="log-btn"', 'id="rs-btn"',
                  ...
                  "static measureClick", "static async screenshot",
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3073.json").write_text(json.dumps(
        {"ops": 3073, "verdict": "FAIL" if fails else "PASS",
