"""ops 3662 — AIR-CARGO canary + freight page + sidebar: [A] NEW
...
with report("3662_air_page") as rep:
    rep.heading("ops 3662 — HKIA air canary + freight page + sidebar")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3662.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        probe = aj.get("body_probe") or aj.get("list_probe")
        gate("G1_air", (not err) and (got or bool(probe)),
             ...
             f"errs={aj.get('errors')} probe={(str(probe)[:260] if probe else None)}")
        ...
        while time.time() < dl:
            try:
                ...
                mk = {"page": "Freight & Trade Pulse" in h
    ...
    Path("aws/ops/reports/3662.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
