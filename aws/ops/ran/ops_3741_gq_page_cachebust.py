#!/usr/bin/env python3
"""ops 3741 — import-canary.html + FIELD-COVERAGE AUDIT + nav pin.
...
with report("3741_gq_page_cachebust") as rep:
    rep.heading("ops 3741 — grid-queue page + field-coverage audit")
    fails = []
    out = {"gates": {}}
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3741.json").write_text(json.dumps({"verdict": "STARTED"}))
    ...
    try:
        ...
        for attempt in range(20):
            try:
                ...
                if "Grid Queue" in html and "caiso_active_rows" in html:
        ...
        out["verdict"] = verdict
        out["unrendered"] = gaps
        Path("aws/ops/reports/3741.json").write_text(json.dumps(out, indent=2))
        ...
        Path("aws/ops/reports/3741.json").write_text(
