"""ops 3471 — own-directory search (close the "micro" issue) + archive 34.
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-3471",
...
with report("3471_fundgraph_symdir_search") as rep:
    out = {"ops": 3471, "gates": {}}
    fails = []
    ...
    rep.heading("ops 3471 — symdir ranked search + memory-archive 34")
    ...
    deploy_lambda(report=rep, function_name=FN,
                  ...
                  description="Fundamental Graphs v1.1.5 symdir search (ops 3471)",
    ...
    # first search call builds the directory (cold ~20-40s)
    try:
        d0 = fetch(f"{url}/?search=microsoft")
        time.sleep(2)
        obj = s3.get_object(Bucket=BUCKET, Key="data/fundgraph/symdir.json")
        sd = json.loads(obj["Body"].read())
        gate("D1_symdir_built", sd.get("n", 0) >= 6000 and d0.get("ok"),
             {"rows": sd.get("n"), "src0": d0.get("src")})
    ...
    (REPO / "aws" / "ops" / "reports" / "3471.json").write_text(
