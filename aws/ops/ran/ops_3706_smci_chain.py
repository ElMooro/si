"""ops 3706 (follow-on to 3705) — readthrough v1.0.4: the SMCI fix.
...
with report("3706_smci_chain") as rep:
    rep.heading("ops 3706 — SMCI bill-of-materials + adjacent-theme T3")
    out = {"gates": {}}
    fails = []
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3706.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        while time.time() - t0 < 240:
            try:
                src = deployed(FN)
                if "THEME_INFRA" in src and 'VERSION = "1.0.5"' in src:
        ...
        gsrc = ""
        t1w = time.time()
        while time.time() - t1w < 240:
            try:
                gsrc = deployed("justhodl-supply-chain-graph")
                if "SERVER-OEM INPUTS" in gsrc:
                    break
            except Exception:
                pass
            time.sleep(20)
        gate("G1_shipped", "THEME_INFRA" in src and 'VERSION = "1.0.5"' in src
             and "REL_CAPTURE_MOD" in src and "SERVER-OEM INPUTS" in gsrc,
             f"theme_infra={'THEME_INFRA' in src} capture_mod={'REL_CAPTURE_MOD' in src} "
             f"v1.0.5={chr(34)+'1.0.5' in src} graph_bom={'SERVER-OEM INPUTS' in gsrc}")

        # rebuild the graph first — readthrough reads its published output
        gb = None
        try:
            gb = S3C.head_object(Bucket=BUCKET, Key="data/supply-chain-graph.json")["LastModified"]
        except Exception:
            pass
        LAM.invoke(FunctionName="justhodl-supply-chain-graph", InvocationType="Event", Payload=b"{}")
        gt = time.time()
        while time.time() - gt < 420:
            time.sleep(15)
            try:
                h = S3C.head_object(Bucket=BUCKET, Key="data/supply-chain-graph.json")
                if gb is None or h["LastModified"] > gb:
                    break
            except Exception:
                pass
        print(f"  graph rebuilt in {round(time.time()-gt)}s")
        ...
        gate("G5_smci_chain", len(srows) >= 5 and len(t1) >= 6,
    ...
    Path("aws/ops/reports/3706.json").write_text(json.dumps(out, indent=2, default=str))
