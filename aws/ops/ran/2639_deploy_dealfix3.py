...
print("\nDEALS (post announcer-match gate):")
for d in (j.get("deals") or []):
    print(f"  {d['symbol']:6s} hl={str(d.get('highlight')):6s} ai={int(bool(d.get('ai_relevant')))} sz={d.get('deal_value_str') or '—':>10s} vsMC={d.get('vs_market_cap_pct')}% | {d['title'][:60]}")
print("DONE 2639")
