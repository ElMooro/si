...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-garble"
code=r'''
...
def lambda_handler(e,c):
    out={"deleted":0,"rounds":0}
    for i in range(40):
        d=call("/brain-purge?uid="+UID+"&token="+T+"&garble=1&max=400")
        out["rounds"]+=1
        if d.get("err"): out["err"]=d["err"]; break
        out["deleted"]+=d.get("deleted",0)
        if not d.get("more"): out["remaining"]=d.get("remaining_in_index"); break
    # verify load speed + count after
    t0=time.time(); g=call("/brain?uid="+UID+"&t=1",to=30)
    out["after_notes"]=len(g.get("notes",[])) if isinstance(g.get("notes"),list) else g
    out["load_secs"]=round(time.time()-t0,2)
    out["samples"]=[(n.get("text") or "")[:50] for n in g.get("notes",[])[:8]] if isinstance(g.get("notes"),list) else None
    return out
'''
buf=io.BytesIO()
...
open("aws/ops/reports/1409_g.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
