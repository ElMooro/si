#!/usr/bin/env python3
"""ops 3056 -- ARC B (items 4+5): EW/CW narrowness twins + dated MA-cross events, on top of Arc A of Khalid's 17-item IR upgrade (items 1+2+3):
...
def main():
    fails, warns = [], []
    with report("3056_ewcw_maevents") as rep:
        ...
        if d.get("version") != "3.2":
            fails.append("IR v=%s (want 3.2)" % d.get("version"))
        if len(ew) < 8:
        ...
        try:
            ...
            req = urllib.request.Request(
                ...
                % time.time(),
                headers={"User-Agent": "Mozilla/5.0 ops-3056"})
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3056.json").write_text(json.dumps(
        {"ops": 3056, "verdict": "FAIL" if fails else "PASS",
