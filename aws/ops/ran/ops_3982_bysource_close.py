"""
ops_3982 — census v1.4 IDEMPOTENT close (short-cycle, timeout-proof).
...
So this op is redesigned to be re-runnable and to ALWAYS land a report:
  ...
  pass 1: confirm v1.4 marker deployed (Deploy Lambdas pushed it 19:04);
          if artifact already fresh v1.4 -> full gates. else async invoke,
...
MARK = "data-census v1.4 ops3982 by-source"
...
def main():
    with report("3982_bysource_close") as rep:
        rep.heading("ops 3981 — census v1.4 idempotent close")
        checks = []
        ...
        rep.section("A. is v1.4 the deployed artifact?")
        ...
        if not have:
            ...
            for _ in range(24):
                ...
                if c.get("State") == "Active" and c.get("LastUpdateStatus") != "InProgress":
                    break
                time.sleep(8)
            rep.ok("  v1.4 pushed from runner")
        checks.append(("v1.4 deployed", True))
        ...
        if not fresh_v13:
            ...
            lam.invoke(FunctionName=FN, InvocationType="Event",
                       Payload=json.dumps({"source": "ops3982"}).encode())
            ...
            for i in range(20):
                ...
                if doc and doc.get("marker") == MARK:
                    fresh_v13 = True
                    rep.ok(f"  v1.4 wrote after ~{(i+1)*15}s")
                    break
            if not fresh_v13:
        ...
        rep.section("D. full gates on the v1.4 artifact")
        t = doc.get("totals") or {}
        rep.kv(**t)
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok(f"PASS_ALL — census v1.4: {t.get('artifacts')} artifacts / "
