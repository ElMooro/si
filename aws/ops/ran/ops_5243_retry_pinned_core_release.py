"""Deploy remaining audit source changes and the two confirmed recovery failures. No AWS API calls."""
...
BASE_SHA = "0c82670e9c24925e1b152549a1da167960830b3d"
WORKFLOW = "deploy-lambdas.yml"
REPORT = ROOT / "aws/ops/reports/latest/ops_5243_core_recovery_dispatch.json"
...
def main():
    ...
    report = {"operation": "5243", "workflow": WORKFLOW, "workflow_sha": commit, "base_sha": BASE_SHA,
              "requested_at": datetime.now(timezone.utc).isoformat(), "request_sent": True,
              "status": "REQUESTING", "aws_calls": 0, "expected_checkout_sha": commit,
              "checkout_verification": "REQUIRED_IN_RELEASE_JOB"}
    ...
    try:
        ...
        # The workflow checks commit ancestry and identical workflow bytes, then
        # checks out expected_sha exactly before credentials/tests/AWS mutation.
...
if __name__ == "__main__":
    ...
    with ops_report("ops_5243_retry_pinned_core_release") as rep:
        try:
            main()
