"""ops_4119 — deterministic deploy with NOTHING silent: current-zip truth,
...
def main():
    with report("4119_settle_once") as rep:
        ...
        rep.kv(new_zip_files=len(zf.ZipFile(io.BytesIO(buf.getvalue()))
                                 .namelist()))
        # ops4119 ROOT CAUSE of every settle failure tonight: the loop
        # re-issued update_function_code EVERY iteration, resetting
        # LastUpdateStatus to InProgress each time — livelocking my own
        # deploy. Update ONCE, then only poll.
        for attempt in range(6):
            try:
                lam.update_function_code(FunctionName=FN,
                                         ZipFile=buf.getvalue(), Publish=True)
                rep.ok(f"  update accepted (attempt {attempt})")
                break
            except Exception as e:
                rep.log(f"  update EXC: {type(e).__name__}: {str(e)[:140]}")
                time.sleep(10)
        ok = False
        for i in range(50):
            try:
