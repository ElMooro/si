"""ops 4540 — Khalid: 'not worried about budget'. Max throttle: deploy
...
R={"ops":4540,"started":datetime.now(timezone.utc).isoformat()}
def wait_ok():
    for _ in range(20):
...
envv["FLOWS_PER_AGENCY"]="40"; envv["WALK_BUDGET_S"]="700"
lam.update_function_configuration(FunctionName=FN,Environment={"Variables":envv},MemorySize=2048,Timeout=880)
wait_ok()
...
bus({"action":"post_turn","thread_id":"0807-reseal","from":"claude","to":"perplexity","kind":"propose",
 "content":("MAX THROTTLE (Khalid: budget unlocked): 40/agency + 10 wires + 5-min cadence + 2GB. "
...
json.dump(R,open("aws/ops/reports/4540_max.json","w"),indent=1,default=str)
open("aws/ops/reports/4540_max.md","w").write("# 4540 max — "+R["verdict"]+"\n")
print(R["verdict"][:300])
