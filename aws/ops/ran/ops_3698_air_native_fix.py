"""ops 3698 — air-cargo goes LAMBDA-NATIVE via /gov edge.
...
with report("3698_air_native_fix") as rep:
    rep.heading("ops 3698 — air-cargo Lambda-native via /gov edge")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3698.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
    ...
    Path("aws/ops/reports/3698.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
