"""ops_4113 — family verify-only: artifact truth or the vault's own logs."""
import json
import sys
...
def main():
    with report("4113_family_verify") as rep:
        ...
        try:
            ev = logs.filter_log_events(
                ...
                startTime=int((datetime.now(timezone.utc) -
                               timedelta(minutes=35)).timestamp() * 1000),
                filterPattern='?REPORT ?Task ?Error ?Traceback ?family',
                limit=40)
