"""ops_5185 -- auction desk v1.2.0: composite history 1996->today (detector math, vendored) and the
...
with report("ops_5185_auction_desk_reactions") as R:
    R.heading("ops 5185 -- auction desk v1.2.0: 1996 composite + cross-asset reactions")
    ...
    if not ser or (ch.get("first") or "9999") > "1996-06-01" or len(ser) < 1000:
        FAILS.append("composite history not 1996->: first=%s points=%d" % (ch.get("first"), len(ser)))
    R.section("reactions")
    ...
    for _ in range(40):
        try:
            with urllib.request.urlopen(urllib.request.Request("https://justhodl.ai/auctions.html", headers={"User-Agent": "ops5185", "Cache-Control": "no-cache"}), timeout=30) as r:
    ...
    if live:
        ...
        with sync_playwright() as p:
            ...
            pg.screenshot(path=str(SHOTS / "ops5185_auctions_top.png"))
