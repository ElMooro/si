"""1285 — deploy justhodl-vintage-fred (point-in-time macro) + invoke + verify."""
import json, os, time, zipfile, io
from datetime import datetime, timezone
import boto3
from botocore.config import Config
REPORT="aws/ops/reports/1285_vintage.json"; BUCKET="justhodl-dashboard-live"
...
try:
    ...
    # ensure env has FRED_KEY
    env=c.get("Environment",{}).get("Variables",{})
    if not env.get("FRED_KEY"):
        env["FRED_KEY"]="2f057499936072679d8843d7fce99989"
        lam.update_function_configuration(FunctionName=LAMBDA,Environment={"Variables":env}); time.sleep(5)
    out["deploy"]=act
...
try:
    ...
    out["index"]={"n_series":idx.get("n_series"),"series":idx.get("series",[])[:10]}
    # sample one series' vintage ledger
    if idx.get("series"):
        s0=idx["series"][0]
        v=json.loads(s3.get_object(Bucket=BUCKET,Key=f"data/vintage/{s0}.json")["Body"].read())
        vs=v.get("vintages",[])
        out["sample"]={"series":s0,"n_vintages":len(vs),"latest":vs[-1] if vs else None}
except Exception as e: out["index"]=str(e)[:200]
out["finished"]=datetime.now(timezone.utc).isoformat()
open(REPORT,"w").write(json.dumps(out,indent=2,default=str)); print(json.dumps(out,indent=2,default=str)[:900])
