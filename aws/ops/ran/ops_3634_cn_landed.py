"""ops 3634 — forensic + hardened retry. Unzipped-marker check on the
...
with report("3634_cn_landed") as rep:
    rep.heading("ops 3634 — deployed-zip forensic + v1.7.1/v2.5")
    out = {"gates": {}}
    fails = []
    ...
    try:
        err = dep_inv("justhodl-asia-leads", 300, 512,
                      "asia-leads v1.8.1: module BUCKET const (levels+korea caches persist)")
        ...
        gate("G1_tw", all(mk.values()) and not err and
             ...
             f"levels_err={str(tw.get('levels_err'))[:90]} levels={tw.get('levels_cached')} brute={tw.get('stage6_brute')} sns={tw.get('stage6_sitesns')} "
    ...
    try:
        err = dep_inv("justhodl-china-liquidity", 300, 768,
                      "china-liquidity v2.8: H1/9M/Q1 patterns + att-title/createDate year — VALUE-required")
        ...
        gate("G2_cn", (got and cn.get("js_files") is not None
             ...
             f"att_title={str(cn.get('att_title'))[:44]} cum={cn.get('cumulative')} phrase_ctx={str(cn.get('phrase_ctx'))[:220]} refs={[r[-34:] for r in (cn.get('content_refs') or [])[:3]]} probe={str(cn.get('body_probe'))[180:560]} via={cn.get('via')} note={cn.get('error')}")
        out["cn"] = cn
    except Exception as e:
    ...
    Path("aws/ops/reports/3634.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
