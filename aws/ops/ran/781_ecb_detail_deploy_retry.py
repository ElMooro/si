"""ops/781 — retry deploy + verify justhodl-ecb-detail.

ops 780 failed CreateFunction on an over-length Description (AWS caps it at
256 chars). Config shortened; this re-runs the create + schedule + invoke +
verify, truncating Description defensively.
"""
...
report = {"ops": 781, "ts": datetime.now(timezone.utc).isoformat(),
          "subject": "Retry deploy + verify justhodl-ecb-detail"}
...
report["output"] = {
    ...
    "eurodollar_read": out.get("eurodollar_read"),
...
report["verdict"] = (
    ...
    f"deposit rate {pr.get('deposit_facility_pct')}%. ECB Data Portal keys "
    "all resolve."
    if report["all_pass"] else
    "REVIEW — see checks[]/output.errors")

print(json.dumps(report, indent=2, default=str))
os.makedirs("aws/ops/reports", exist_ok=True)
with open("aws/ops/reports/781_ecb_detail_deploy_retry.json", "w") as f:
    json.dump(report, f, indent=2, default=str)
print("[ok] wrote aws/ops/reports/781_ecb_detail_deploy_retry.json")
