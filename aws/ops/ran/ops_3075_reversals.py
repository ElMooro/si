#!/usr/bin/env python3
"""ops 3075 -- REVERSAL TRANSITIONS on the accumulation radar
...
UA = {"User-Agent": "Mozilla/5.0 ops-3075",
...
def main():
    fails, warns = [], []
    with report("3075_reversals") as rep:
        ...
        for _ in range(50):
            ...
            try:
                ...
                if (datetime.now(timezone.utc)
                    ...
                    if d.get("version") == "1.4.0":
                        break
                    d = None
            except Exception:
                pass
        if not d:
            fails.append("no fresh v1.4.0 output")
            _fin(rep, fails, warns)
            sys.exit(1)
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3075.json").write_text(json.dumps(
        {"ops": 3075, "verdict": "FAIL" if fails else "PASS",
