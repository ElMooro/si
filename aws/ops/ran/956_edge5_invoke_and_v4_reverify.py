"""
ops 956 -- invoke Edge #5 (after FMP_KEY default fix) + v4 final re-verify
===========================================================================

After the source-level fix in commit 628b42ec (FMP_KEY default no longer ""),
the Russell recon Lambda should be able to call FMP /stable/company-screener
without any env-var setup.

Steps:
  1. wait briefly for CI to deploy
  2. invoke justhodl-russell-recon-frontrun
  3. verify S3 output appears
  4. run v4 unified re-verify of edges 5-10
     (v4 drops the env_vars_present check since inline defaults are used)
"""
...
EDGES = [
    ...
    },
    {
        ...
        "ssm_key": None, "state_field": "state",
        ...
        "data_age_max_h": 168, "min_why_now_len": 200,
    },
    {
    ...
    },
    {
        ...
        "extra_required": ["all_setups", "top_setups", "recommended_trade",
                           "current_readings"],
        "trade_field": "recommended_trade",
        "data_age_max_h": 168, "min_why_now_len": 100,
    },
    {
        ...
        "trade_field": "recommended_trade",
        "data_age_max_h": 48, "min_why_now_len": 200,
    },
]
...
def add(edge, name, passed, detail=""):
    ALL_CHECKS.append({"edge": edge, "name": f"e{edge}.{name}",
                       "passed": bool(passed), "detail": str(detail)[:300]})
...
def verify_edge(cfg):
    ...
    add(e, "state_valid", d.get(field) in cfg["valid_states"],
        f"field={field} got={d.get(field)}")
    ...
    add(e, "forward_horizons_complete",
        ...
        f"got={list(fe.keys())[:6] if isinstance(fe, dict) else 'n/a'} expected={fe_horizons}")
    ...
    if ssm_key:
        ...
        add(e, "ssm_state_present", True, "engine does not use SSM")

    try:
        req = urllib.request.Request(f"{PAGES_BASE}/{cfg['page']}",
                                     headers={"User-Agent": "ops/956"})
...
def main():
    print(f"ops 956 at {dt.datetime.utcnow().isoformat()}Z")

    # Phase 1: re-invoke Edge #5 (Russell) -- now that source has FMP_KEY default
    print("\n=== Phase 1: re-invoke Edge #5 after FMP_KEY fix ===")
    t0 = time.time()
    try:
        ...
        add(5, "post_fix_invoke", ok,
            f"dur={dur}s outer={r['StatusCode']} inner={inner} body={payload[:200]}")
    except ClientError as ex:
        add(5, "post_fix_invoke", False, str(ex)[:200])

    time.sleep(3)

    # Phase 2: per-edge verification
    print("\n=== Phase 2: per-edge verification ===")
    for cfg in EDGES:
    ...
    rep = {
        "ops": 956,
        "title": "Edge #5 post-fix invoke + v4 unified re-verify",
    ...
    with open("aws/ops/reports/956_edge5_invoke_and_v4_reverify.json", "w") as f:
        json.dump(rep, f, indent=2)

    print("\n=== PER-EDGE SUMMARY ===")
    for eid in sorted(per_edge):
        p = per_edge[eid]
        print(f"  Edge #{eid:2}  pass={p['passed']:2}/{p['total']:2}")
    print(f"\n=== OVERALL pass={op}/{ot} ({100*op//max(ot,1)}%) ===")
