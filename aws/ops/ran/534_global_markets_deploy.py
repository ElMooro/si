#!/usr/bin/env python3
"""534 — Direct-deploy justhodl-global-markets (BUILD 14) + schedule + verify."""
import io, json, os, time as _time, zipfile, base64
from datetime import datetime, timezone
import boto3

REPORT = "aws/ops/reports/534_global_markets_deploy.json"
NAME = "justhodl-global-markets"
SOURCE = "aws/lambdas/justhodl-global-markets/source/lambda_function.py"
...
def inherit_env():
    env = {}
    for src in ("justhodl-earnings-nlp", "justhodl-13f-positions"):
        try:
            cfg = lam.get_function_configuration(FunctionName=src)
            v = (cfg.get("Environment") or {}).get("Variables") or {}
            if v.get("FMP_KEY"): env["FMP_KEY"] = v["FMP_KEY"]; break
        except: pass
...
def main():
    ...
    try:
        try:
            lam.create_function(FunctionName=NAME, Runtime="python3.12",
                ...
                Description="Cross-asset regional ETF coherence engine",
    ...
    try:
        eb.put_rule(Name=RULE, ScheduleExpression=SCHEDULE, State="ENABLED",
                     Description="Global markets refresh every 3 hours")
    ...
    try:
        obj = s3.get_object(Bucket="justhodl-dashboard-live", Key="data/global-markets.json")
        body = obj["Body"].read()
        p = json.loads(body)
        comp = p.get("composite") or {}
        ranked = (comp.get("ranked_20d") or [])[:15]
        out["sidecar"] = {
            ...
            "spy_20d": comp.get("spy_20d"),
            "intl_avg_20d": comp.get("intl_avg_20d"),
            "us_minus_intl_20d_pp": comp.get("us_minus_intl_20d_pp"),
            "n_positive_20d": comp.get("n_positive_20d"),
            "pct_positive": comp.get("pct_positive"),
