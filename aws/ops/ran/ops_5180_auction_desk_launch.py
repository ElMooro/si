"""ops_5180 -- launch the Treasury auction desk engine + verify the rebuilt auctions.html.
...
with report("ops_5180_auction_desk_launch") as R:
    R.heading("ops 5180 -- Treasury auction desk: launch + verify")
    R.section("P1 deploy + first run")
    cfg = None
    for _ in range(45):
        try:
        ...
        time.sleep(20)
    if not cfg:
        R.fail("   function not deployed after 15 min")
        sys.exit(1)
    ...
    for name, expr, payload in plan:
        ...
        try:
            ...
            sch.update_schedule(Name=name, GroupName="default", ScheduleExpression=expr, ScheduleExpressionTimezone="America/New_York",
                                FlexibleTimeWindow={"Mode": "OFF"}, Target=tgt, State="ENABLED", Description="auction desk (ops 5180)")
    ...
    try:
        ...
        for _ in range(40):
            try:
                with urllib.request.urlopen(urllib.request.Request("https://justhodl.ai/auctions.html", headers={"User-Agent": "ops5180", "Cache-Control": "no-cache"}), timeout=30) as r:
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                for width, height in ((1440, 1000), (390, 844)):
                    ...
                    pg.screenshot(path=str(SHOTS / f"ops5180_auctions_{width}.png"), full_page=False)
