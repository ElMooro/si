"""
ops_4000 — v2.3 verify: the families that were structurally impossible
...
def main():
    with report("4000_v23_verify") as rep:
        rep.heading("ops 3992 — v2.3 vault-complete verify")
        checks = []
        doc = None
        for i in range(26):
            ...
            if doc.get("marker") == MARK:
                rep.ok(f"  v2.3 artifact after ~{i*30}s")
                break
            rep.log(f"  [{i}] {str(doc.get('marker'))[:40]}")
            time.sleep(30)
        checks.append(("artifact is v2.3", doc.get("marker") == MARK))
        if doc.get("marker") != MARK:
            for l, ok in checks:
                (rep.ok if ok else rep.fail)(f"  {l}")
            rep.fail("v2.3 never wrote in the window")
            sys.exit(1)
        ...
        checks.append(("vault actually walked (>=1500 paths, not skipped)",
                       (va.get("n_paths") or 0) >= 1500 and not va.get("skipped")))

        def flag(fam, sym, srckey=None):
            # v2.3: membership only — the substring pin produced the "bo" not
        ...
        checks += [
            ...
            ("OTHER near-zero (<100, truly unattributable only)", (per.get("OTHER") or 0) < 100),
        ]
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok(f"PASS_ALL — v2.3 complete: {tot} points / {len(bs)} families; "
