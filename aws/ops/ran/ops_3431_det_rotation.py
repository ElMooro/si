...
with report("3431_det_rotation") as rep:
    ...
    while time.time()<dl:
        ...
        time.sleep(20)
    gate("G2_rotation_live", fresh>=12 and det and det.get("mode")=="deterministic",
    ...
    S3C.put_object(Bucket="justhodl-dashboard-live",Key="config/feed-retired.json",
        Body=json.dumps({"generated_at":t0.isoformat(),"retired":RETIRED,
            "reason":"writerless orphans (census #2): writers deleted; zero page/engine consumers"},
            separators=(",",":")).encode(),ContentType="application/json")
    gate("G3_retired_list", True, f"{len(RETIRED)} published")
    out["verdict"]="PASS_ALL" if not fails else "GAPS: "+",".join(fails)
    print("\nVERDICT:",out["verdict"]); rep.log("VERDICT: "+out["verdict"])
    Path("aws/ops/reports/3431.json").write_text(json.dumps(out,indent=2,default=str)); sys.exit(0)
