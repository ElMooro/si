#!/usr/bin/env python3
"""ops 3804 — probe justhodl-supply-chain-linkage as a wider dependency source.
...
def main():
    with report("3804_probe_linkage_coverage") as rep:
        rep.heading("ops 3804 — can supply-chain-linkage widen dependency coverage?")
        ...
        if L is None:
            rep.warn("linkage feed absent — checking alternates before concluding")
            for alt in ("data/supply-chain-linkage.json", "data/supply_chain_linkage.json",
                        "data/linkage.json", "data/supply-chain-by-symbol.json"):
                try:
                    hh = s3.head_object(Bucket=BUCKET, Key=alt)
                    rep.log("  ALT %-40s %d bytes" % (alt, hh["ContentLength"]))
                except Exception:
                    rep.log("  absent %s" % alt)
            rep.warn("No wider feed available: expanding coverage means widening the "
                     "PRODUCER universe (justhodl-supply-chain-graph), which is its own build.")
            rep.ok("PASS_ALL — probe complete (negative result recorded)")
            return
        ...
        if not found:
            rep.warn("no directional supplier/customer fields — aggregates only")
        else:
            rep.ok("LINK.directional :: directional fields present")
