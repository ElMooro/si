"""ops 3702 (rerun of 3701) — neocloud/AI-DC cluster on the supply-chain graph + readthrough tier caps.
...
with report("3702_neocloud_edges") as rep:
    rep.heading("ops 3702 — neocloud cluster + readthrough tier caps")
    out = {"gates": {}}
    fails = []
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3702.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        rsrc = wait_for_markers(RT_FN, ("TIER_CAPS",))
    ...
    Path("aws/ops/reports/3702.json").write_text(json.dumps(out, indent=2, default=str))
