...
(git diff --quiet), and its code_sha256 must match live. Original text follows.

ops 5521 -- re-arm of ops 5520 after the IAM widen (Claude Ship 1, 2026-09-13).

ops 5520 proved everything except the public mirrors: the student role was widened at 17:59:56 and the two
ticks at 17:59:58/18:00:03 still saw AccessDenied -- IAM propagation, not a code defect. This re-arm polls the
tick for up to 10 minutes after the (now idempotent) widen until the projection lands, then re-checks the
public feed. Original op text follows.
...
def _same_source(commit, fn):
    """True when the receipt commit's function source + shared modules are byte-identical to HEAD."""
...
def main() -> int:
    ...
    with report("ops_5525_factory_discipline_prints_regate2") as R:
        ...
        while pending and time.time() < deadline:
            for fn in list(pending):
                ...
                if rc.get("commit") and _same_source(str(rc["commit"]), fn):
                    ...
                    (R.ok if ok else R.fail)("%s receipt commit=%s (source identical to HEAD %s) sha_match=%s run=%s" % (fn, str(rc["commit"])[:7], head[:7], ok, rc.get("run_id")))
                    if not ok:
        ...
        for fn in pending:
            ...
            R.fail("%s: no release receipt whose source matches HEAD %s within %d min" % (fn, head[:7], RECEIPT_WAIT_S // 60))
        if red:
        ...
        # 3. live ticks: poll until the widened role has propagated (no projection error), up to 10 minutes
        settle = time.time() + 10 * 60
        while time.time() < settle:
            raw = lam.invoke(FunctionName="justhodl-student-rsi", InvocationType="RequestResponse", Payload=b"{}")
            probe = json.loads(raw["Payload"].read() or b"{}")
            errs = probe.get("errors") or []
            if probe.get("status") != "already_running" and not any(e.get("phase") == "projection" for e in errs):
                R.ok("role propagated: tick state_version=%s without projection errors" % probe.get("state_version"))
                break
            time.sleep(30)
        else:
            R.warn("projection still denied after 10 minutes of polling")
        versions = []
        for i in range(2):
            out = None
