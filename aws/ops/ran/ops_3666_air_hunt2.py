"""ops 3666 — AIR-CARGO canary + freight page + sidebar: [A] NEW
...
with report("3666_air_hunt2") as rep:
    rep.heading("ops 3666 — HKIA air canary + freight page + sidebar")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3666.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
    ...
    Path("aws/ops/reports/3666.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
