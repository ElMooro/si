...
def lambda_handler(e,c):
    ...
    return {"total":len(notes),"secs":round(time.time()-t0,2),"unique":len(seen),"dup_groups":sum(1 for v in seen.values() if v>1)}
'''
buf=io.BytesIO()
...
open("aws/ops/reports/1447_cf.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
