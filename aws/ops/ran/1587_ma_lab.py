...
out["verify"]["crossings"]={"n_up":len(xx.get("stocks_up") or []),"n_down":len(xx.get("stocks_down") or []),
  "up_head":(xx.get("stocks_up") or [])[:6],"down_head":(xx.get("stocks_down") or [])[:6],
  "indexes":xx.get("indexes")}
# DDB sample of today's ma_cross logs
try:
    today=__import__("datetime").datetime.utcnow().strftime("%Y-%m-%d")
    sc=ddb.scan(TableName="justhodl-signals",FilterExpression="signal_type = :t AND contains(signal_id, :d)",
        ExpressionAttributeValues={":t":{"S":"ma_cross"},":d":{"S":today}},Limit=25)
    out["verify"]["ddb_ma_cross"]=[{"id":i["signal_id"]["S"],"dir":i["predicted_direction"]["S"],
        "conf":i["confidence"]["N"]} for i in sc.get("Items",[])][:8]
except Exception as e:
    out["verify"]["ddb_ma_cross"]=f"ERR {str(e)[:60]}"
L=d.get("reversion_lab") or {}
out["verify"]["lab"]={"headline":L.get("headline"),"regimes":L.get("regime_breakdown"),
  "heatmap_sample":(L.get("heatmap") or [])[:6],"ema_vs_sma":L.get("ema_vs_sma"),
  "qqq":L.get("qqq_headline"),"examples":L.get("trade_examples")}
ZS=d.get("stretch_scan") or {}
out["verify"]["stretch"]={"indexes":ZS.get("indexes"),
  "stocks_head":[r for r in (ZS.get("stocks") or []) if abs(r.get("z50",0))>=1.5][:8]}
...
open("aws/ops/reports/1587_ma_lab.json","w").write(json.dumps(out,indent=2,default=str))
