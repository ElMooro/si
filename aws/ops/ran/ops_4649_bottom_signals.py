...
def main():
    misses = 0
    with report("4649_bottom_signals") as r:
        r.heading("ops 4643 — DXY predict-the-future engine")

        r.section("pre-dump: bottom list candidates")
        ...
        with zf2.ZipFile(buf, "w", zf2.ZIP_DEFLATED) as z:
            z.write("aws/lambdas/justhodl-bottom-signals/source/"
            ...
            try:
                ...
                src = zf2.ZipFile(io.BytesIO(zb)).read(
                    "lambda_function.py").decode("utf-8",
                                                 "replace")
                if "justhodl-bottom-signals v1.0.0" in src:
                    settled = True
                    break
            except Exception:
                pass
        ...
        pl = json.loads(s3.get_object(
            Bucket=B, Key="data/bottom-signals.json")["Body"].read())
        dx = pl.get("bottom") or {}
        rows = pl.get("rows") or []
        ...
        for att in range(9):
            try:
                pg = http_get("https://justhodl.ai/"
                              ...
                              % time.time()).decode("utf-8",
                                                    "replace")
                page_ok = ("data/bottom-signals.json" in pg)
                jd = json.loads(http_get(
                    "https://justhodl.ai/data/bottom-signals.json"
                    "?cb=%d" % time.time()))
                pay_ok = bool((jd.get("bottom")
                               or {}).get("trend_label"))
        ...
        if misses:
            ...
            sys.exit(1)
        r.ok("BOTTOM SIGNALS LIVE — list '%s': %s/%s resolved, %d "
             "signed rows · TREND %s (%s) · REVERSAL %s (%s) · "
             "https://justhodl.ai/bottom-signals.html"
