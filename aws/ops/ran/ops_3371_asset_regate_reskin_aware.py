"""ops 3371 — asset cache-integrity RE-GATE, reskin-aware expected model.

3370 finding: reskin_site.py transforms .js/.css/.svg in _site (Amber
palette) BEFORE stamping — live bytes+stamps were self-consistent and
correct; only the gate's expected-computation skipped reskin. Local proof:
reskin-aware replica == live hashes EXACTLY (7/7). Doctrine: any
expected-hash replication must run the SAME pre-transforms as pages.yml.
...
UA = {"User-Agent": "Mozilla/5.0 (X11; Linux x86_64) ops-3371"}
...
def expected_versions():
    ...
    # mirror pages.yml EXACTLY: reskin transforms js/css BEFORE stamping
    import subprocess
    subprocess.run([sys.executable, "scripts/reskin_site.py", str(tmp)],
...
def main(rep):
    ...
    Path("aws/ops/reports/3371.json").write_text(json.dumps(out, indent=2))
    sys.exit(0)


with report("3371_asset_regate_reskin_aware") as _rep:
    _rep.heading("ops 3371 — reskin-aware asset re-gate")
    main(_rep)
