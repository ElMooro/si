#!/usr/bin/env python3
"""ops 2997 -- SOLDIERS FIX verify.
1) FMP endpoint corrected (etf/holdings not legacy etf-holder).
2) resilience join now ticker-intersection against real schema
   (all_resilient/top_picks/about_to_boom), not sector-string guess.
Re-invoke (no redeploy needed if deploy-lambdas already ran on this
push) and check for either real soldiers OR an honest, specific
failure reason per leader -- never silence.

Original ops 2996 -- industry-rotation FINAL FIX: the fleet schedules via
...
def get(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-2997",
...
def main():
    fails, warns = [], []
    out = {"ops": 2997, "ts": datetime.now(timezone.utc).isoformat()}
    with report("2997_soldiers_fix") as rep:

        rep.section("1. Deploy gate (env persists per 2968 fix)")
        time.sleep(75)
        cfg, env_n = {}, 0
        for _ in range(50):
            cfg = LAM.get_function_configuration(FunctionName=FN)
            ...
            if cfg.get("LastUpdateStatus") == "Successful" \
                    and age < 1800 and env_n >= 3:
                rep.kv(deploy_age_s=int(age), env_vars=env_n)
                break
            time.sleep(8)
        out["env_vars"] = env_n
        if env_n < 3 or cfg.get("LastUpdateStatus") != "Successful":
            fails.append("gate: env=%d status=%s"
                         % (env_n, cfg.get("LastUpdateStatus")))
        ...
        rep.section("5. Soldiers diagnostic")
        d2 = s3_json("data/industry-rotation.json")
        lds = d2.get("leaders") or []
        out["soldiers_detail"] = [
            {"etf": l["etf"], "holdings_n": len(l.get("holdings_top") or []),
             "reason": l.get("holdings_reason"),
             "resilient_n": len(l.get("resilient_names") or [])}
            for l in lds]
        rep.kv(soldiers=json.dumps(out["soldiers_detail"]))
        any_holdings = any(l.get("holdings_top") for l in lds)
        any_reason = all(l.get("holdings_reason") or l.get("holdings_top")
                         for l in lds)
        out["any_holdings"] = any_holdings
        if not any_holdings and not any_reason:
            fails.append("soldiers: neither real data nor honest "
                         "reasons on some leader(s)")
        idx_warn = next((w for w in (d2.get("warns") or [])
                         if "resilience_index" in w), None)
        out["resilience_index_size"] = idx_warn
        rep.kv(resilience_index=idx_warn)

        if not fails:
            rep.ok("SOLDIERS FIX VERIFIED: regime %s | ladder %d | "
                   "top %s | soldiers %s"
                   % (st, len(rows), json.dumps(out["top5"][:3]),
                      json.dumps(out["soldiers_detail"])))
        _w(rep, out, fails, warns)
...
def _w(rep, out, fails, warns):
    ...
    (AWS_DIR / "ops" / "reports" / "2997.json").write_text(
...
try:
    ...
    import traceback
    (AWS_DIR / "ops" / "reports" / "2997.json").write_text(json.dumps(
        {"ops": 2997, "verdict": "FAIL",
