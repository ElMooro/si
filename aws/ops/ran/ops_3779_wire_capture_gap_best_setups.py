#!/usr/bin/env python3
"""ops 3779 — wire capture_gap into best-setups (ONE consumer, verified anchors).
...
def main():
    with report("3779_wire_capture_gap_best_setups") as rep:
        rep.heading("ops 3779 — capture_gap overlay into best-setups")
        ...
        try:
            ...
            cur = bs.get("top_setups") or []   # ops 3778 probe: VERIFIED key, 50 rows, field "ticker"
        ...
        if "Capture-gap overlay (ops 3779)" in src:
        ...
        gate(rep, "SPLICE.present", "Capture-gap overlay (ops 3779)" in LF.read_text(), "marker in source")
        ...
        for i in range(12):
            ...
            with zipfile.ZipFile(io.BytesIO(blob)) as z:
                if "Capture-gap overlay (ops 3779)" in z.read("lambda_function.py").decode("utf-8", "replace"):
                    settled = True
        ...
        for k in ("structural_chokepoints", "top_setups", "triple_threats", "by_verdict"):
