"""ops/4805 -- settle-gated v2.8 verify (house-standard from here on):
 ...
 (2) deploy settle: get_function -> download deployed zip -> grep the
     '"2.8"' marker (deterministic); fallback to LastModified age.
 (3) Event-invoke, poll as_of <=13 min, assert engine_v == 2.8, TE
...
def main():
    with report("4805_v28_verify") as rep:
        rep.heading("ops 4799 -- settle-gated v2.8 verify")
        ...
        if not marker_ok:
            rep.warn("deployed code lacks the 2.8 marker -- deploy "
        ...
        for mid in ("DTCC-TREASURY-FAILS", "DTCC-AGENCY-FAILS",
                     "WREPOFOR", "SFTR-EU-outstanding-all-sfts-total-"
                     "sft-cash-value-eur-mn"):
            s0 = allrows.get(mid)
            if s0:
        ...
        lv = (allrows.get("DTCC-TREASURY-FAILS") or {}
               ).get("last_value")
        rep.kv(check="dtcc_tsy_sane_5bn_500bn",
                value=bool(lv and 5e9 <= lv <= 5e11), last_usd=lv)
        ...
        rep.section("opportunistic Wayback CDX retry")
        ...
        for i, w in enumerate((0, 20, 60)):
            if w:
                time.sleep(w)
            try:
                rows2 = json.loads(_u2.urlopen(_u2.Request(cdx,
                    headers={"User-Agent": "Mozilla/5.0"}),
                    timeout=50).read().decode())
                rep.kv(check="cdx_up_snapshots",
                        value=max(0, len(rows2) - 1))
                break
            except Exception as e:
                rep.log(f"cdx attempt {i}: {type(e).__name__}")
        else:
            rep.log("archive.org still down -- 4803 pattern will be "
                     "re-run in a later session")
