"""ops/4925 -- floor-audit v2.1.0: guards that protect a retail call.
...
Gates:
  ...
  G7 edge: page marker floor-audit-v2.1.0 + feed v2.0.0 (real UA)
"""
import json
import sys
import time
...
MARKER = "floor-audit-v2.1.0"
UA = "JustHodl ops4925 edge-acceptance"
...
def main():
    with report("4925-floor-v210-guards") as r:
        r.heading("ops 4925 -- floor-audit v2.1.0 retail guards")
        t0 = datetime.now(timezone.utc)

        r.section("G1 deploy")
        if ENG.VERSION != "2.1.0":
        ...
        for i in range(90):  # sweep + deep tier: allow 15 min
            time.sleep(10)
            cur = s3_json(OUT_KEY)
            if cur and cur.get("version") == "2.1.0" and (
        ...
        if bad_unbound:
            r.log("FAIL G5: buy call on an unbound debt leg: %s"
                  % bad_unbound)
            sys.exit(1)
        thin_buys = [(x["ticker"], round((x.get("adv_usd_20d") or 0)
                                         / 1000.0))
                     for x in okall
                     if x["recommendation"]["action"] == "BUY" and
                     (x.get("adv_usd_20d") or 0) < 250000.0]
        if thin_buys:
        ...
        r.kv(guards="PASS",
             ...
             thin_names=sum(1 for x in okall
                            if (x.get("adv_usd_20d") or 0) < 250000.0))
               ...
               "top_calls": [
                   ...
                   for x in sorted(
                       ok, key=lambda z: -(z["recommendation"]
                                           ["conviction"] or 0))[:15]],
               "edge": "LIVE"}
        (ROOT / "aws" / "ops" / "reports" / "4925.json").write_text(
