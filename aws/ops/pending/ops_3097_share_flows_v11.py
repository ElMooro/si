#!/usr/bin/env python3
"""ops 3097 -- (3096 raced: verify accepted the still-fresh v1.0.0 doc from the prior run; now gate on engine version). ops 3096 was -- SHARE-FLOWS v1.1.0 verify (3095 fails fixed:
...
UA = {"User-Agent": "Mozilla/5.0 ops-3097",
...
def main():
    fails, warns = [], []
    with report("3097_share_flows_v11") as rep:
        ...
        if n_sell == 0:
            fails.append("sell side still empty -- management "
                         "selling not tracked")
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3097.json").write_text(json.dumps(
        {"ops": 3097, "verdict": "FAIL" if fails else "PASS",
