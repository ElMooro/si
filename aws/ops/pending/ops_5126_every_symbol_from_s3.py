"""ops_5126 -- every symbol from the warehouse (5125 re-run after the symbol-feed type bug).
...
def http(url, timeout=180):
    req = urllib.request.Request(url, headers={"User-Agent": "justhodl-ops-5126", "Accept-Encoding": "identity"})
    t = time.time()
...
def main():
    with report("5126-every-symbol-from-s3") as r:
        r.heading("ops 5126 -- every symbol from the warehouse; no TradingView widget in AUTO mode (re-run)")
        fails = []
            ...
            with sync_playwright() as p:
                ...
                for sym in ("TVC:VIX", "AAPL", "SSE:000001", "ECONOMICS:DEUR", "NVDA"):
                    ...
                    pr = page.evaluate("""() => ({ active: State.activeTicker, meta: (document.getElementById('nch-meta-0')||{}).textContent, loading: (document.getElementById('nch-loading-0')||{}).textContent,
                        ...
                        name: (document.getElementById('active-name')||{}).textContent })""")
                    r.log(f"  page {sym}: {json.dumps(pr)[:400]}")
                    if pr.get("iframe") or pr.get("paywall"):
                        fails.append(f"{sym}: TradingView widget/paywall shown")
                    if "obs" not in str(pr.get("meta") or "") and "warehouse" not in str(pr.get("meta") or ""):
                        fails.append(f"{sym}: chart did not render natively: {json.dumps(pr)[:160]}")
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5126-chartpro-native.jpg"), type="jpeg", quality=60)
