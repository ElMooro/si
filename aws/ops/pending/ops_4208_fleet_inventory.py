"""ops_4208 — FLEET COVERAGE SCAN: mine the system's own artifacts for
...
def main():
    with report("4208_fleet_inventory") as rep:
        ...
        for k, sz in sorted(objs, key=lambda x: -x[1]):
            ...
            for bare, (val, asof, dep) in found.items():
                if bare not in fleet and len(fleet) < 9000:
        ...
        s3.put_object(Bucket=BUCKET, Key="data/fleet-inventory.json",
                      Body=json.dumps(
                          {"marker": "fleet-inventory v1 ops4208",
                           "n": len(fleet),
                           "prices": fleet}).encode(),
                      ContentType="application/json",
                      CacheControl="max-age=600")
        rep.ok(f"FLEET INVENTORY — {len(fleet)} remappable ticks banked")
