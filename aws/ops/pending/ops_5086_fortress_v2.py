"""ops_5086 -- justhodl-fortress v2.0.0: the evidence-grade rebuild, deployed,
scheduled (daily + weekly walk-forward backtest) and verified on real data.

What v2 adds (Khalid: "think very deeply ... improve it exponentially"):
  * three years of bars; dump capture weighted by depth and recency, read
    two ways (peak-to-trough and on the market's down days inside each
    dump) that must agree; worst-day t-statistics; evidence confidence
    that shrinks thin samples toward neutral
  * ACCUMULATION pillar from volume structure and smart money (OBV / A-D
    divergence, up/down volume, absorption on worst days, FINRA dark-pool
    state, 13F, insider clusters, Congress, ETF constituent pressure)
  * STRUCTURE pillar (higher lows, VCP, RS-line lead, base age, squeeze
    momentum); tail-risk safety (CVaR, downside deviation, Ulcer, gaps,
    Amihud, rate/dollar beta); options overlay (IV rank, VRP, skew)
  * rank-blended composite, conviction (geometric mean of the gate
    pillars), trade plan, risk-parity sizing, session changes, watch
    triggers, regime block
  * VALIDATION: event {"mode":"backtest"} walks the price-structure legs
    forward through ~1,250 sessions of the warehouse (every 15 sessions)
    and writes data/fortress-backtest.json; scheduled Sundays 09:00 UTC.

Gates: G1-G7 as before (memory 8192 for the full-tape backtest), plus
  G5b v2 coverage: obv_slope_63 / absorption_clv / conviction / trade_plan
      >= 60% of board rows; sessions_loaded >= 700; regime block present
  G8  weekly backtest schedule exists; backtest invoked and verified on
      disk: >= 20,000 observations, four gate cohorts, 3/3 cohort n >= 200,
...
def main():
    with report("5086-fortress-v2") as r:
        r.heading("ops 5086 -- justhodl-fortress v2.0.0 deploy + daily + weekly backtest, verified")
        ...
        if not env_vars:
            ...
            sys.exit(1)
        env_vars["FORTRESS_VERSION"] = "2.0.0"
        ...
        json.dump({"ops": 5086, "engine": FN, "session": payload.get("session"),
                   "n_scored": payload.get("n_scored"), "tiers": payload.get("tiers"),
                   "coverage_pct": covs, "as_of": payload.get("as_of")},
                  open(str(ROOT / "aws" / "ops" / "reports" / "5086.json"), "w"), indent=1)
    sys.exit(0)
