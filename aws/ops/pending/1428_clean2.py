...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-cl2"
code=r'''
import json,urllib.request,time
B="https://justhodl-data-proxy.raafouis.workers.dev"; T="jhpurge_9f48_2026"; UID="9f48a96b-1a1e-4867-9fc6-e6cc5054c56d"
def call(p):
    try:
        r=urllib.request.urlopen(urllib.request.Request(B+p,headers={"User-Agent":"jh","Origin":"https://justhodl.ai"}),timeout=50); return json.loads(r.read().decode())
...
def lambda_handler(e,c):
    ...
    while time.time()<deadline:
        ...
        off=d.get("scanned_to",off+400); out["kept"]=d.get("kept"); out["total"]=d.get("total_index"); out["scanned_to"]=off
        if d.get("done"): out["done"]=True; break
    # if done, fetch the clean load + samples
    if out.get("done"):
        time.sleep(2); g=call("/brain?uid="+UID+"&t=9")
        out["final_notes"]=len(g.get("notes",[])) if isinstance(g.get("notes"),list) else g
        out["samples"]=[(n.get("text") or "")[:55] for n in g.get("notes",[])[:10]] if isinstance(g.get("notes"),list) else None
    return out
'''
buf=io.BytesIO()
...
try:
    try: lam.get_function_configuration(FunctionName=FN); lam.update_function_code(FunctionName=FN,ZipFile=buf.getvalue())
        ...
        lam.create_function(FunctionName=FN,Runtime="python3.12",Role=ROLE,Handler="lambda_function.lambda_handler",Code={"ZipFile":buf.getvalue()},Timeout=780,MemorySize=256)
    for _ in range(25):
    ...
    for inv in range(3):
        try:
            ...
            out["invocations"].append({"invoke_err":str(ex)[:80],"resume_off":off}); break
...
open("aws/ops/reports/1428_c.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
