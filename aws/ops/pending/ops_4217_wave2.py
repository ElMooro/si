"""ops_4217 — WAVE-2: recession/sentinel/dollar bus-wrapped, verified, ledger+3."""
import io
import json
import re
import sys
...
def main():
    with report("4217_wave2") as rep:
        rep.heading("ops 4217 — wave-2 (recession, sentinel, dollar)")
        import base64
        checks = []
        TRIO = (("justhodl-global-recession", "bus_legs",
                 "data/global-recession.json"),
                ("justhodl-us10y-sentinel", "bus_cross",
                 "data/us10y-sentinel.json"),
                ("justhodl-dollar-radar", "bus_dollar",
        ...
        for name, blk, outk in TRIO:
            ...
            if name.endswith("recession") and has:
                bl = d[blk]
                rep.log("  recession bus_legs: " + json.dumps(bl)[:220])
                checks.append(("gdp_universe >= 100",
                               (bl.get("gdp_universe") or 0) >= 100))

        led = json.loads(s3.get_object(
            Bucket=BUCKET, Key="data/bus-consumers.json")["Body"].read())
        for name, blk, outk in TRIO:
            led["wired"].append({"engine": name, "mode": blk})
        led["candidates_next"] = [c for c in led.get(
            "candidates_next", []) if c not in
            [t[0] for t in TRIO]]
        ...
        rep.ok(f"  ledger: wired={len(led['wired'])} "
               f"next={len(led['candidates_next'])}")
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok("WAVE-2 LIVE — recession hard-legs armed, sentinel "
               "cross-checked, dollar on modern COT")
