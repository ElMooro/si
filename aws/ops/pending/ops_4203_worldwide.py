"""ops_4203 — world-wide finish: te v1.2 exotic sweep, sf v1.7 CN codes, fire."""
import io
import json
import re
import sys
...
def main():
    with report("4203_worldwide") as rep:
        rep.heading("ops 4203 — world-wide finish")
        checks = [("te v1.2 settled",
                   settle(rep, "justhodl-te-feed",
                          "te-feed v1.2 ops4203 world-wide")),
                  ("sf v1.7 settled",
                   settle(rep, "justhodl-symbol-feed",
                          "symbol-feed v1.7 ops4203 cn-codes"))]
        for k in (1, 2, 3):
            r = lam.invoke(FunctionName="justhodl-te-feed",
                           InvocationType="RequestResponse", Payload=b"{}")
            rep.kv(**{f"te{k}": r["Payload"].read().decode()[:60]})
        ...
        checks.append(("te n >= 8200", (td.get("n") or 0) >= 8200))
        r = lam.invoke(FunctionName="justhodl-symbol-feed",
                       InvocationType="RequestResponse", Payload=b"{}")
        rep.kv(sf=r["Payload"].read().decode()[:80])
        ...
        rep.ok("  vault fired — 4204 converts")
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok(f"WORLDWIDE — te={td.get('n')} sf={sd.get('resolved')}")
