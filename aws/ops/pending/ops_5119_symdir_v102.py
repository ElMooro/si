"""ops_5119 -- symbol directory v1.0.2: ranking refinements from the 5118 battery.
...
def http_json(url, timeout=120):
    req = urllib.request.Request(url, headers={"User-Agent": "justhodl-ops-5119"})
    t = time.time()
...
def main():
    with report("5119-symdir-v102") as r:
        r.heading("ops 5119 -- symbol directory v1.0.2: ranking refinements")
        fails = []
        ...
        if h.get("version") != "1.0.2":
            fails.append(f"version {h.get('version')} != 1.0.2")

        r.section("S2 rebuild + warm reload")
        t_inv = datetime.now(timezone.utc)
        # warm a container on the OLD index first, so the reload path is actually exercised
        ...
        r.ok(f"rebuilt docs={man.get('docs'):,} elapsed={man.get('elapsed_s')}s fred titles ledger={fs.get('titles_ledger')} (+{fs.get('titles_fetched_now')} now) untitled={fs.get('untitled_banked')}")
        ...
        if fails:
            sys.exit(1)
        r.ok(f"PASS_ALL: symdir v1.0.2 live, {man.get('docs'):,} docs, ranking battery green, warm containers reload newer builds")
