"""ops 4562 — pause cron, wait out in-flight, clean epoch reset (baseline
= current imported count, skip-set preserved), deploy lease code,
re-enable, one Event+poll round, expect reconciles=true."""
...
R={"ops":4562,"started":datetime.now(timezone.utc).isoformat()}
ev.disable_rule(Name="justhodl-fred-catalog-5min")
time.sleep(200)  # let any in-flight round finish
...
s3.put_object(Bucket=B,Key="data/_state/fred-scoped-import.json",
    Body=json.dumps({"cats_done":[],"series_seen":0,"series_excluded_stale":0,
                     ...
                     "excluded_ids":[],"imported_ids":keep,
                     "n_pages":st.get("n_pages",0),"buffer":[],
                     "reset_by":"ops4562_clean_epoch"}).encode(),
...
for _ in range(9):
    ...
    if st2.get("updated_at","")>R["started"] and not (st2.get("lease_until") or 0)>time.time():
        final={"cats_done":len(st2.get("cats_done") or []),"of":st2.get("n_categories_expanded"),
               ...
               "new_this_epoch":st2.get("series_imported",0)-R["baseline"],
...
json.dump(R,open("aws/ops/reports/4562.json","w"),indent=1,default=str)
open("aws/ops/reports/4562.md","w").write("# 4562 — "+json.dumps(final,default=str)+"\n")
