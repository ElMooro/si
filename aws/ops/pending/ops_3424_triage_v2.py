...
with report("3424_triage_v2") as rep:
    rep.heading("ops 3423 — negative-alpha evidence study")
    # source A: outcomes ON signal rows (older native path)
    ...
    while True:
        ...
        if not lek: break
    # source B: the separate justhodl-outcomes table ({signal_id}_{window})
    orow=[]
    try:
        otb=DDB.Table("justhodl-outcomes"); lek=None
        while True:
            kw={"FilterExpression":fe}
            if lek: kw["ExclusiveStartKey"]=lek
            r=otb.scan(**kw); orow+=r.get("Items") or []
            lek=r.get("LastEvaluatedKey")
            if not lek: break
    except Exception as e:
        print("[study] outcomes table:",str(e)[:90])
    # fold source-B rows into source-A shape: synth signal rows keyed by base id
    synth={}
    for o in orow:
        fam=o.get("signal_type"); sid=str(o.get("signal_id") or o.get("id") or "")
        wk=o.get("window_key") or o.get("window") or (sid.rsplit("_",1)[1] if "_day_" in sid else None)
        base=sid.rsplit("_",1)[0] if wk and sid.endswith(str(wk)) else sid
        if not fam or not wk: continue
        e=synth.setdefault(base,{"signal_type":fam,
                                 "logged_at":o.get("logged_at") or o.get("checked_at") or "",
                                 "predicted_direction":o.get("predicted_direction") or "UP",
                                 "outcomes":{}})
        e["outcomes"][str(wk)]={"excess_return_pct":o.get("excess_return_pct"),
                                 "return_pct":o.get("return_pct") or o.get("actual_return_pct")}
        if not e["logged_at"]: e["logged_at"]=str(o.get("checked_at") or "")
    rows+=list(synth.values())
    print(f"[study] rows: on-signal={len(rows)-len(synth)} outcomes-table-signals={len(synth)} (raw outcome rows {len(orow)})")
    rep.log(f"rows_total={len(rows)} synth={len(synth)}")
    ...
    Path("aws/ops/reports/3424.json").write_text(json.dumps(doc,indent=2))
    sys.exit(0)
