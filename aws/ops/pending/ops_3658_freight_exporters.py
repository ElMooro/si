"""ops 3658 — FREIGHT+EXPORTERS CANARIES (Khalid directive): [A] NEW
...
with report("3658_freight_exporters") as rep:
    rep.heading("ops 3658 — freight pulse + exporters pulse")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3658.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
    ...
    Path("aws/ops/reports/3658.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
