...
R={"ops":4455,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
try:
    ...
    try: lam.add_permission(FunctionName=FN,StatementId="ops4455",Action="lambda:InvokeFunction",Principal="events.amazonaws.com",SourceArn=arn)
...
json.dump(R,open("aws/ops/reports/4455_e10.json","w"),indent=1,default=str)
open("aws/ops/reports/4455_e10.md","w").write(f"# ops 4455 — E10 — {R['verdict']}\n- runs: {json.dumps({k:R.get(k) for k in ('run1','run2')},default=str)[:500]}\n")
print(R["verdict"])
