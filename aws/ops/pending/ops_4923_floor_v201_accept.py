"""ops/4923 -- floor-audit v2.0.1 acceptance: sweep without the sync smoke.
...
Gates:
  ...
  G7 edge: page marker floor-audit-v2.0.1 + feed v2.0.0 (real UA)
"""
import json
import sys
import time
...
MARKER = "floor-audit-v2.0.1"
UA = "JustHodl ops4923 edge-acceptance"
...
def main():
    with report("4923-floor-v201-accept") as r:
        r.heading("ops 4923 -- floor-audit v2.0.1 acceptance")
        t0 = datetime.now(timezone.utc)

        r.section("G1 deploy")
        if ENG.VERSION != "2.0.1":
        ...
        deploy_lambda(
            ...
            # ops 4922 trap: deploy_lambda smoke-tests SYNCHRONOUSLY.
            ...
            smoke_test=False, create_function_url=False,
        ...
        for i in range(90):  # sweep + deep tier: allow 15 min
            time.sleep(10)
            cur = s3_json(OUT_KEY)
            if cur and cur.get("version") == "2.0.1" and (
               ...
               "top_calls": [
                   ...
                   for x in sorted(
                       ok, key=lambda z: -(z["recommendation"]
                                           ["conviction"] or 0))[:15]],
               "edge": "LIVE"}
        (ROOT / "aws" / "ops" / "reports" / "4923.json").write_text(
