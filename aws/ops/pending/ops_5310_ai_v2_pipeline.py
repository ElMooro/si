"""ops_5308 -- deploy the merged AI engine v2.1.0: Perplexity's governance modules (fleet input registry over every wired feed, privacy classification,
release gates, signal envelope, eligibility) plus Claude's pipeline state machine, market read, lessons loop and scoreboard. Runs in
AI_ENVIRONMENT=review on the existing lambda-execution-role and buckets: deploys stay allowed under the cost guard, while the unprovisioned
MLOps stack (MLflow tracking server, Feature Store, prediction ledger table, CloudFormation canary) answers "not configured" until it is
explicitly approved -- an MLflow tracking server alone bills every hour. Also: the page's GET /inventory route (404 storm), serverless
endpoints no longer TTL-reaped, RoBERTa-SEC deployed on ml.g4dn.xlarge (libcuda), CPU TensorFlow cards for the serverless retrieval endpoint.
...
UA = {"User-Agent": "justhodl-ops-5310", "Cache-Control": "no-cache", "Pragma": "no-cache"}
...
def write_verdict(status, title, t_start):
    doc = {"ops": 5310, "status": status, "title": title, "finished_at": datetime.now(timezone.utc).isoformat(), "duration_s": round(time.time() - t_start, 1),
           "steps": STEPS, "fails": [f[:300] for f in FAILS], "warns": [w[:300] for w in WARNS],
           "report": "https://github.com/ElMooro/si/blob/main/aws/ops/reports/latest/ops_5310_ai_v2_pipeline.md"}
    try:
...
T_START = time.time()
with report("ops_5310_ai_v2_pipeline") as R:
    ...
    try:
        ...
        pass
    for stale in ("AI_EVENT_BUS", "SIGNAL_FEATURE_GROUP", "PREDICTION_FEATURE_GROUP", "PREDICTION_LEDGER_TABLE", "PREDICTION_LEDGER_ARCHIVE_BUCKET", "MODEL_PACKAGE_GROUP",
                  "MLFLOW_TRACKING_SERVER_NAME", "MLFLOW_TRACKING_SERVER_ARN", "MLFLOW_EXPERIMENT_NAME"):
        env.pop(stale, None)
    ...
    ctl = {"function_url": fn_url, "sagemaker_role_arn": SM_ROLE_ARN, "private_bucket": PRIVATE, "updated_at": datetime.now(timezone.utc).isoformat(), "ops": 5310}
    try:
    ...
    try:
        tok = env.get("JH_SERVICE_TOKEN", "")
        req = urllib.request.Request(fn_url + "inventory", method="GET", headers={"X-JH-Service-Token": tok, **UA})
        with urllib.request.urlopen(req, timeout=60) as r:
            j = json.loads(r.read().decode())
        R.ok("   GET /inventory (owner read model) -> %s keys %s" % (r.status, sorted(list((j.get("result") or j).keys()))[:12]))
    except Exception as e:
        FAILS.append("GET /inventory through the Function URL: %s" % str(e)[:160])
    step("owner read model (GET /inventory)", not any("GET /inventory" in f for f in FAILS))
    ...
    if D and not (D.get("inventory_errors") or {}) and (D.get("catalog") or {}).get("n"):
        try:
            ...
            ladder = [m for m in (cat.get("article_models_present") or []) if "base" in m and "wiki" not in m][:1] + [
                "tensorflow-tcembedding-bert-en-uncased-L-12-H-768-A-12-2", "huggingface-sentencesimilarity-all-MiniLM-L6-v2"]
            pstate = invoke("pipeline/start", {"force": True, "ladder": ladder})
    ...
    for _ in range(40):
        try:
            with urllib.request.urlopen(urllib.request.Request("https://justhodl.ai/ai.html?v=%d" % int(time.time()), headers=UA), timeout=30) as r:
                live = b"AI_DESK_V2" in r.read()
        except Exception:
            live = False
        if live:
            break
        time.sleep(15)
    R.log("   ai.html carries marker AI_DESK_V2 at the edge: %s" % live)
    if live:
        try:
        ...
        with sync_playwright() as p:
            ...
            for width, height in ((1440, 1100), (390, 844)):
                ...
                pg.evaluate("() => { var b = document.getElementById('pl-toggle'); if (b) b.click(); }")
                pg.wait_for_timeout(500)
                facts = pg.evaluate("""() => ({ plain: document.getElementById('pl-headline').textContent.slice(0, 90), voice: document.getElementById('pl-voice').textContent, headline: document.getElementById('wr-headline').textContent.slice(0, 90), legs: document.querySelectorAll('#wr-legs .leg').length,
                ...
                if errors or facts["err"] or not facts.get("plain") or facts["legs"] != 6 or facts["steps"] != 5:
    ...
    write_verdict("RED" if FAILS else "GREEN", "AI v2.2.2 -- review-mode training, pipeline restarted", T_START)
    if FAILS:
        sys.exit(1)
