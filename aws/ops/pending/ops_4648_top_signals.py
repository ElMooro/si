...
def main():
    misses = 0
    with report("4648_top_signals") as r:
        r.heading("ops 4643 — DXY predict-the-future engine")

        r.section("pre-dump: top list candidates")
        ...
        with zf2.ZipFile(buf, "w", zf2.ZIP_DEFLATED) as z:
            z.write("aws/lambdas/justhodl-top-signals/source/"
        ...
        pl = json.loads(s3.get_object(
            Bucket=B, Key="data/top-signals.json")["Body"].read())
        dx = pl.get("top") or {}
        rows = pl.get("rows") or []
        ...
        for pfx in ("GLASSNODE", "INTOTHEBLOCK", "CRYPTOCAP",
                    "USI"):
            samp = [str(x.get("symbol")) for x in rows
                    if str(x.get("symbol", "")).startswith(
                        pfx + ":")][:12]
            r.log("%s samples: %s" % (pfx, samp))
        cry = [x for x in rows
        ...
        for att in range(9):
            try:
                pg = http_get("https://justhodl.ai/"
                              ...
                              % time.time()).decode("utf-8",
                                                    "replace")
                page_ok = ("data/top-signals.json" in pg)
                jd = json.loads(http_get(
                    "https://justhodl.ai/data/top-signals.json"
                    "?cb=%d" % time.time()))
                pay_ok = bool((jd.get("top")
                               or {}).get("trend_label"))
        ...
        if misses:
            ...
            sys.exit(1)
        r.ok("TOP SIGNALS LIVE — list '%s': %s/%s resolved, %d "
             "signed rows · TREND %s (%s) · REVERSAL %s (%s) · "
             "https://justhodl.ai/top-signals.html"
