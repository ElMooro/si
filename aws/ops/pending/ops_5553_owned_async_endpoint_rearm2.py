...
def main() -> int:
    ...
    with report("ops_5553_owned_async_endpoint_rearm2") as R:
        R.heading("ops 5553 (re-arm of 5552) -- owned model async endpoint (scale-to-zero) from the staged weights; proven by one round trip")
        ...
        control = {"schema_version": "factory-inference-control.v1", "enabled": True, "endpoint_name": NAME, "mode": "async-scale-to-zero",
                   ...
                   "endpoint_config": cfg_name, "written_by": "ops 5553", "git_sha": head[:12], "at": datetime.now(timezone.utc).isoformat()}
