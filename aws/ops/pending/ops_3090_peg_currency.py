#!/usr/bin/env python3
"""ops 3090 -- PEG CURRENCY GUARD (Khalid caught TSM PE 0.9 / fPEG
0.09): external truth = trailing ~33-38x, forward ~23-28x. Root cause
verbatim: FMP analyst estimates for ADRs return EPS in LOCAL currency
(TSM ~368 TWD vs $11.5 USD ADR trailing) -> USD price / TWD earnings.
v3.8 fix: (a) trailing PE computed from quote price/eps when the pe
field is empty; (b) scale-sanity gate (fwd/trailing outside 0.2-5x =
unusable level) -> NORMALIZED forward EPS = trailing USD x (1 + est
growth), since the growth RATIO is currency-invariant; (c) bounds:
publish fwd_pe only in [3,150], PEG in [0.1,10]; basis flagged on the
chip tooltip. Asserts TSM/NVDA against external truth bands."""
import json
import sys
import time
...
UA = {"User-Agent": "Mozilla/5.0 ops-3090",
...
def main():
    ...
    with report("3090_peg_currency") as rep:
        ...
        for _ in range(45):
            ...
            try:
                ...
                if (datetime.now(timezone.utc) - o["LastModified"]
                    ...
                    if dj.get("version") == "3.8":
                        break
                    dj = None
            except Exception:
                pass
        if not dj:
            fails.append("no fresh v3.8 output")
            _fin(rep, fails, warns)
            sys.exit(1)
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3090.json").write_text(json.dumps(
        {"ops": 3090, "verdict": "FAIL" if fails else "PASS",
