"""ops_5211 -- KATLIN v2.2.0: split repair over the grouped store (banked Polygon split calendar), learned weight 30, model basket sized by the war room. Same two-phase run.
(prior docstring:) tighter CONFIRMED structure (real chain, margins, prior decline), history gate on PRIME/READY,
CRASH_BARBELL tier with its walk-forward base rate, learned weight 22. Same two-phase run as 5209, run the walk-forward FIRST (feature base rates, regime split, out-of-sample
check, survivorship-honest exits, random tradable universe), then the daily desk so today's names carry the learned prior; verify both.: redeploy from the runner, async run + freshness gate, war-room read, tiers, joins,
...
with report("ops_5211_katlin_v220") as R:
    R.heading("ops 5211 -- KATLIN v2.2.0")
    ...
    # ---- phase 1: walk-forward (async, ~10-13 min) -> data/katlin-backtest.json with feature_stats + oos
    BT = "data/katlin-backtest.json"
    bt_before = None
    try:
        bt_before = get_json(BT).get("as_of")
    except Exception:
        pass
    tb = time.time()
    lam.invoke(FunctionName=FN, InvocationType="Event", Payload=json.dumps({"mode": "backtest"}).encode())
    R.log("   walk-forward invoked async (prior as_of=%s)" % bt_before)
    B = None
    while time.time() - tb < 870:
        time.sleep(20)
        try:
            b = get_json(BT)
        except Exception:
            continue
        if b.get("as_of") and b.get("as_of") != bt_before:
            B = b
            break
    if not B:
        FAILS.append("no fresh walk-forward after %.0fs" % (time.time() - tb))
    else:
        R.section("walk-forward v%s: %s obs, %s dates, %ss, budget_hit=%s, universe %s" % (B.get("version"), B.get("n_obs"), B.get("n_dates"), B.get("elapsed_s"), B.get("budget_hit"), B.get("universe")))
        if B.get("version") != "2.2.0":
            FAILS.append("walk-forward version %s != 2.0.0" % B.get("version"))
        co = B.get("cohorts") or {}
        R.log("   %-28s %s" % ("cohort", " | ".join("%-30s" % h for h in ("63s n/mean/med/hit/p90", "126s", "252s"))))
        for lab, hz in co.items():
            cells = []
            for h in ("63s", "126s", "252s"):
                c = (hz or {}).get(h) or {}
                cells.append("%5s %6s %6s %4s %6s" % (c.get("n"), c.get("mean_excess_pct"), c.get("median_excess_pct"), c.get("hit_rate_pct"), c.get("p90_excess_pct")))
            R.log("   %-28s %s" % (lab, " | ".join("%-30s" % x for x in cells)))
        for h, x in (B.get("oos") or {}).items():
            R.log("   OOS %s: split %s train %s test %s spread %s corr %s top %s bottom %s below200-top %s" % (h, x.get("split_date"), x.get("train_obs"), x.get("test_obs"), x.get("spread_pct"), x.get("corr"), x.get("top_decile"), x.get("bottom_decile"), x.get("below200_top_decile")))
        fs = ((B.get("feature_stats") or {}).get("126s") or {})
        R.log("   feature_stats 126s grand mean %s over %s obs; features: %s" % (fs.get("grand_mean_excess_pct"), fs.get("n"), sorted((fs.get("buckets") or {}).keys())))
        for f in sorted((fs.get("buckets") or {}).keys()):
            bks = (fs.get("buckets") or {}).get(f) or {}
            R.log("   %-7s %s" % (f, " | ".join("%s: n=%s mean=%s hit=%s" % (bk, c.get("n"), c.get("mean_excess_pct"), c.get("hit_rate_pct")) for bk, c in bks.items())))
        rg = B.get("regime_126s") or {}
        for lab in ("below200", "below200+confirmed", "KATLIN_price_3of3", "knife", "above200"):
            R.log("   regime %-20s %s" % (lab, {mk: (c.get("n"), c.get("mean_excess_pct"), c.get("hit_rate_pct")) for mk, c in (rg.get(lab) or {}).items()}))
        if not (B.get("oos") or {}).get("126s"):
            FAILS.append("no 126s out-of-sample result")
        if not fs.get("buckets"):
            FAILS.append("feature_stats missing")
    # ---- phase 2: daily desk (async) + freshness gate
    before = None
    try:
    ...
    if not D:
        ...
        if D.get("version") != "2.2.0":
            FAILS.append("artifact version %s != 2.0.0 (stale code ran)" % D.get("version"))
        n_prior = sum(1 for r in picks if (r.get("alpha_prior") or {}).get("126s"))
        R.log("   learned prior on picks: %d / %d; history: %s" % (n_prior, len(picks), (D.get("history") or {}).get("text")))
        if picks and n_prior < 0.9 * len(picks):
            FAILS.append("learned prior missing on %d picks" % (len(picks) - n_prior))
        for r in picks[:8]:
            ap = (r.get("alpha_prior") or {}).get("126s") or {}
            R.log("   %-6s %-12s learned %6s%% (n_feat %s) top-lever %s" % (r.get("ticker"), r.get("tier"), ap.get("expected_excess_pct"), ap.get("n_features"), [(c.get("feature"), c.get("bucket"), c.get("delta_pct")) for c in (ap.get("contrib") or [])[:3]]))
        bb = [r for r in picks if r.get("tier") == "CRASH_BARBELL"]
        R.log("   CRASH_BARBELL: %d names; base rate %s" % (len(bb), D.get("barbell_base_rate")))
        for r in bb[:6]:
            R.log("      %-6s dd52 %s vol %s learned %s 3m %s | %s" % (r.get("ticker"), r.get("dd_52w_pct"), r.get("vol_ann_pct"), r.get("learned_excess_126s_pct"), r.get("ret_3m_pct"), (r.get("knife_why") or "")[:70]))
        st = {}
        for r in (D.get("picks") or []) + (D.get("watch") or []):
            st[r.get("structure_state")] = st.get(r.get("structure_state"), 0) + 1
        R.log("   structure states across published rows: %s" % st)
        hist_neg = [(r.get("ticker"), r.get("tier"), r.get("learned_excess_126s_pct")) for r in picks if r.get("tier") in ("KATLIN_PRIME", "READY") and (r.get("learned_excess_126s_pct") or 0) < -2]
        if hist_neg:
            FAILS.append("PRIME/READY names with a clearly negative history: %s" % hist_neg[:6])
        bk = D.get("basket") or {}
        R.log("   basket: posture %s cap %s core %s%% (%d names) barbell %s%% (%d) cash %s%%" % (bk.get("posture"), bk.get("exposure_cap_pct"), bk.get("core_pct"), len(bk.get("core") or []), bk.get("barbell_pct"), len(bk.get("barbell") or []), bk.get("cash_pct")))
        for x in (bk.get("core") or [])[:12]:
            R.log("      core %-6s %5s%% hist %6s vol %5s score %5s entry %s stop %s t1 %s 4h %s" % (x.get("ticker"), x.get("weight_pct"), x.get("expected_excess_126s_pct"), x.get("vol_ann_pct"), x.get("composite"), x.get("entry"), x.get("stop"), x.get("target_1"), x.get("sniper")))
        for x in (bk.get("barbell") or [])[:6]:
            R.log("      barbell %-6s %5s%% hist %6s dd %s vol %s" % (x.get("ticker"), x.get("weight_pct"), x.get("expected_excess_126s_pct"), x.get("dd_52w_pct"), x.get("vol_ann_pct")))
        if not bk or (bk.get("exposure_cap_pct") or 0) > 0 and not bk.get("core"):
            R.warn("   basket has no core names")
        for line in (D.get("log") or []):
            if "split repair" in line:
                R.log("   " + line[:200])
        try:
            sp = get_json("data/warm/katlin/splits.json")
            R.log("   split calendar banked: %s splits since %s (banked %s)" % (sp.get("n"), sp.get("since"), sp.get("banked_at")))
        except Exception as e:
            FAILS.append("split calendar not banked: %s" % str(e)[:80])
        vs = D.get("validation") or {}
        if not (vs.get("oos") or {}).get("126s") or not vs.get("feature_stats_126s"):
            FAILS.append("validation block lacks oos / feature_stats")
        ...
        etf_overlay = [r.get("ticker") for r in picks if r.get("asset_class") == "etf" and any(k in str(r.get("name") or "").lower() for k in ("option", "buffer", "hedged", "covered"))]
        if etf_overlay:
        ...
        with sync_playwright() as p:
            ...
            for width, height in ((1440, 1100), (390, 844)):
                ...
                facts = pg.evaluate("""() => ({ score: document.getElementById('wr-score').textContent, posture: document.getElementById('wr-posture').textContent,
                    ...
                    valextra: document.querySelectorAll('#val-extra table tbody tr').length, basket: document.querySelectorAll('#basket-core tbody tr').length, hist: (document.getElementById('wr-brief').textContent.indexOf('In this regime') >= 0),
                ...
                if not facts.get("valextra") or not facts.get("hist"):
                    FAILS.append("%dpx: validation extras / regime history not rendered: %s" % (width, json.dumps(facts)[:200]))
