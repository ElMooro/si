...
def main() -> int:
    ...
    with report("ops_5529_own_training_lane_rearm") as R:
        R.heading("ops 5529 (re-arm of 5528) -- owned training lane: ECR repo, pinned recipe bundle, base-weight staging job (CPU, priced), no flip")
        R.kv(head=head[:10])
        ...
        try:
            ...
            if "AccessDenied" in str(first):
                try:
                    ...
                    iam.put_user_policy(UserName=user, PolicyName="justhodl-factory-train-ecr", PolicyDocument=json.dumps(policy))
                    time.sleep(20)
