"""ops_5134 -- chart-pro: 4xx per URL during native probes (5133: 68 console 400s were /fundamentals + /news for non-equity ids; now gated).
...
def http(url, timeout=60):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5134", "Accept-Encoding": "identity"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def main():
    with report("5134-chartpro-clean-console") as r:
        r.heading("ops 5134 -- chart-pro: clean console on native charts")
        fails = []
        ...
        while time.time() - t0 < 900 and not all(got.values()):
            try:
                ...
                got["chart"] = b"ops 5134: data series / TV-universe ids have no FMP profile" in c
        ...
        if got["chart"]:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5134-chartpro-native.jpg"), type="jpeg", quality=60)
