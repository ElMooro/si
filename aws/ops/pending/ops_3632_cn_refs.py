"""ops 3632 — forensic + hardened retry. Unzipped-marker check on the
...
with report("3632_cn_refs") as rep:
    rep.heading("ops 3632 — deployed-zip forensic + v1.7.1/v2.5")
    out = {"gates": {}}
    fails = []
    ...
    try:
        err = dep_inv("justhodl-asia-leads", 300, 512,
                      "asia-leads v1.8 recheck: levels_err value dump")
    ...
    try:
        err = dep_inv("justhodl-china-liquidity", 300, 768,
                      "china-liquidity v2.6: generic same-host content-ref harvester")
        ...
        gate("G2_cn", (got or cn.get("js_files") is not None
             ...
             f"refs={cn.get('content_refs')} probe={str(cn.get('body_probe'))[180:560]} via={cn.get('via')} note={cn.get('error')}")
        out["cn"] = cn
    except Exception as e:
    ...
    Path("aws/ops/reports/3632.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
