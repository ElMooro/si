"""ops_4128 — family verify-only: artifact truth or the vault's own logs."""
import json
import sys
...
MARK = "tradingview-vault v3.15.6 ops4126 trust-cache"


def main():
    with report("4128_run2_fate") as rep:
        ...
        try:
            ...
            rep.kv(feed_counts=json.dumps(fd.get("counts")),
                   feed_elapsed=fd.get("elapsed_s"))
        except Exception:
            pass
        rep.section("vault's own story (last 40 min)")
        try:
            ev = logs.filter_log_events(
                logGroupName=f"/aws/lambda/justhodl-tradingview",
                startTime=int((datetime.now(timezone.utc) -
                               timedelta(minutes=25)).timestamp() * 1000),
                filterPattern='?REPORT ?families ?timed ?Traceback',
                limit=40)
