...
if d:
    ...
    rows=[(tk,r.get("confluence"),r.get("confluence_avail"),r.get("confluence_signals")) for tk,r in bt.items() if r.get("confluence") is not None]
    rows.sort(key=lambda x:-(x[1] or 0))
    print(f"\nconfluence present on {len(rows)}/{len(bt)} names. Top by confirmations:")
    for tk,cf,ca,sg in rows[:12]:
