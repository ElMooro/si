"""ops 3621 — WIRING SWEEP (Khalid: wire the huge improvements into the
...
with report("3621_wiring") as rep:
    ...
    try:
        ...
        mk = {m: (m.encode() in zb) for m in ("spx_regime", "kr_flash_yoy",
    ...
    try:
        ...
        pl = json.loads(r["Payload"].read() or b"{}")
        err = pl.get("errorMessage") if isinstance(pl, dict) else None
        st = json.loads(S3C.get_object(Bucket=B, Key="data/_alerts/last.json.snap")["Body"].read())
        ...
        gate("G2_sentinel_kr", ok2,
             f"err={err} state_period={per} yoy={(st.get('snap') or {}).get('kr_flash_yoy')} "
    ...
    Path("aws/ops/reports/3621.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
