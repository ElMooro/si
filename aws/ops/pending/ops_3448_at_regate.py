...
with report("3448_at_regate") as rep:
        ...
        try:
            if LAM.get_function_configuration(FunctionName="justhodl-auction-tail").get("LastUpdateStatus")=="Successful":
                ...
                with urllib.request.urlopen(urllib.request.Request(info["Code"]["Location"],headers=UA),timeout=60) as r:
                    if 'VERSION = "1.0.1"' in zipfile.ZipFile(io.BytesIO(r.read())).read("lambda_function.py").decode():
                        ok1=True; break
    ...
    gate("G2_feed_live", feed.get("ok") is True and (feed.get("generated_at") or "")>t0 and feed.get("n_graded_seen",0)>=1,
