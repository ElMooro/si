"""ops_5149 -- v1.6.0: FRED has no 2-/4-month CMT (verified 400 "series does not exist"); the Treasury par yield curve lane (home.treasury.gov, daily since 1990, all 13 tenors) now serves TVC:US02MY/US04MY, Bundesbank daily serves TVC:DE10Y; dropdown render guarded and console captured.
...
def http(url, timeout=120):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5149", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5149-search-sources") as r:
        r.heading("ops 5149 -- Treasury par curve + Bundesbank sources; search chips (v1.6.0)")
        fails = []
        ...
        if h.get("version") != "1.6.0":
            fails.append(f"version {h.get('version')} != 1.6.0")
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5149-us02my.jpg"), type="jpeg", quality=55)
