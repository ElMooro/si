"""
ops_3972 — wire EUBUND into the vault, then refresh the barometers.
...
MARK = "tradingview-vault v3.7 ops3972 EUBUND"
...
def main():
    with report("3972_wire_eubund") as rep:
        ...
        lam.invoke(FunctionName=FN_V, InvocationType="Event",
                   Payload=json.dumps({"source": "ops3972", "force": True}).encode())
        vault = None
        for i in range(40):
            time.sleep(20)
        ...
        checks += [
            ("EUBUND is LIVE", eb.get("status") == "LIVE"),
        ...
        r = lam.invoke(FunctionName=FN_B, InvocationType="RequestResponse",
                       Payload=json.dumps({"source": "ops3972"}).encode())
