"""ops_5147 -- v1.5.3: why does fred:DGS2MO fail on demand? (alternatives carry the resolver errors); direct /series?id=fred:DGS2MO probe.
...
def http(url, timeout=120):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5147", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5147-search-sources") as r:
        r.heading("ops 5147 -- on-demand FRED banking diagnosis + search sources (v1.5.3)")
        fails = []
        ...
        if h.get("version") != "1.5.3":
            fails.append(f"version {h.get('version')} != 1.5.3")

        r.section("S2 resolution + search")
        for sid in ("fred:DGS2MO", "fred:DGS3MO"):
            d = http_json(url + "/series?id=" + sid + "&nocache=1")
            r.log(f"  direct {sid}: n={d.get('n')} first={d.get('first')} src={str(d.get('source'))[:80]} err={str(d.get('error') or '')[:200]} trace={str(d.get('trace') or '')[-300:]}")
        for sid, want in (("TVC:US02MY", "fred:DGS2MO"), ("ECONOMICS:USINTR", "fred:FEDFUNDS"), ("TVC:DE10Y", "fred:IRLTLT01DEM156N"), ("TVC:US03MY", "fred:DGS3MO")):
        ...
        if not (tvrow and tvrow.get("sources") and tvrow["sources"][0]["id"] == "fred:DGS2MO"):
        ...
        while time.time() - t0 < 900:
            try:
                ...
                if b"ops 5146: rows the instant client-side list" in c:
                    live = True
                    break
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                if not (tv and tv.get("src") and "DGS2MO" in str(tv.get("src"))):
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5147-us02my.jpg"), type="jpeg", quality=55)
