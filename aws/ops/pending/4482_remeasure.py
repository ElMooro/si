...
R={"ops":4482,"started":datetime.now(timezone.utc).isoformat()}
BASE={"fleet-feed":50.8,"unmapped":14.4,"fred":11.0,"polygon":9.5,"llm-anthropic":4.5,"nyfed":0.8}
def wait_idle(fn):
    for _ in range(24):
...
try:
    ...
    mix=d.get("global_mix_pct") or d.get("global_mix") or {}
    ...
    R["delta"]={k:{"was":BASE.get(k),"now":mix.get(k)} for k in BASE}
...
json.dump(R,open("aws/ops/reports/4482_remeasure.json","w"),indent=1,default=str)
open("aws/ops/reports/4482_remeasure.md","w").write(f"# ops 4482 — remeasure — {R['verdict']}\n- delta: {json.dumps(R.get('delta'),default=str)[:400]}\n- new: {json.dumps(R.get('counts_new'),default=str)[:250]}\n")
print(R["verdict"])
