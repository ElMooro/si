"""ops_5132 -- site verification after the pages.yml repair.
...
def http(url, timeout=60):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5132", "Accept-Encoding": "identity"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def main():
    with report("5132-site-verify") as r:
        r.heading("ops 5132 -- site verification after the pages.yml repair")
        fails = []
        ...
        if got["chart"]:
            ...
            with sync_playwright() as p:
                ...
                for sym in ("AAPL", "NVDA", "SSE:000001", "TVC:VIX", "XETR:DAX", "X:BTCUSD"):
                    ...
                    import re as _re
                    m = _re.search(r"([\d,]+) (bars|obs)", meta)
                    if m and int(m.group(1).replace(",", "")) < 2000 and sym in ("AAPL", "NVDA", "XETR:DAX", "SSE:000001"):
                        fails.append(f"{sym}: only {m.group(1)} {m.group(2)} -- not daily history")
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5132-chartpro-native.jpg"), type="jpeg", quality=60)
                errs = [e for e in errors if "TradingView" not in e]
                r.log(f"  page errors: {errs[:3]}")
                if errs:
