"""ops_5122 -- symbol directory v1.1.1: memory, reload storm, ranking hints.
...
def http_json(url, timeout=120):
    req = urllib.request.Request(url, headers={"User-Agent": "justhodl-ops-5122"})
    t = time.time()
...
def main():
    with report("5122-symdir-v111") as r:
        r.heading("ops 5122 -- symbol directory v1.1.1: 6144MB, reload storm fixed, ranking hints")
        fails = []
        ...
        if h.get("version") != "1.1.1":
            fails.append(f"version {h.get('version')} != 1.1.1")
        ...
        checks = [("sofr", "nyfed:sofr", 1), ("dgs10", "fred:DGS10", 1), ("AAPL", "AAPL", 1), ("unrate", "fred:UNRATE", 1), ("TVC:VIX", "TVC:VIX", 1),
                  ...
                  ("money market funds", "ofr:", 12), ("euro dollar exchange rate", "fred:DEXUSEU", 5), ("japan cpi", "fred:", 3), ("fed balance sheet", "fred:WALCL", 5),
        ...
        if fails:
            sys.exit(1)
        r.ok(f"PASS_ALL: symdir v1.1.1 live, {man.get('docs'):,} docs, ranking battery green, warm containers reload newer builds")
