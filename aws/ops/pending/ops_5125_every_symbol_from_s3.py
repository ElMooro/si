"""ops_5125 -- every symbol from the warehouse: chart-pro never touches the TradingView widget.
...
def http(url, timeout=180):
    req = urllib.request.Request(url, headers={"User-Agent": "justhodl-ops-5125", "Accept-Encoding": "identity"})
    t = time.time()
...
def main():
    with report("5125-every-symbol-from-s3") as r:
        r.heading("ops 5125 -- every symbol from the warehouse; no TradingView widget in AUTO mode")
        fails = []
        ...
        if not page_ok:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5125-chartpro-native.jpg"), type="jpeg", quality=60)
