"""ops_4133 — streaming phase timeline: settle v3.15.4, invoke, watch the
...
MARK = "tradingview-vault v3.17.0 ops4133 fleet-join"


def main():
    with report("4133_fleetjoin_stream") as rep:
        ...
        if wrote:
            ...
            checks = [("INTR >=12 (majors native)", fams.get("family:INTR", 0) >= 12),
                      ("FER >=80", fams.get("family:FER", 0) >= 80),
                      ("WB trio >=250",
                       sum(fams.get("family:" + f, 0)
                           for f in ("GDPYY", "IRYY", "UR")) >= 250),
                      ("fleet:finviz >= 3000",
                       fams.get("fleet:finviz", 0) >= 3000),
                      ("total LIVE >= 5200", live >= 5200)]
