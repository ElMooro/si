"""ops_5151 -- v1.6.2: curated equivalents before the dictionary mapping (DE10Y -> Bundesbank daily); dropdown probe awaits the query promise and times the fetch.

5150 note -- v1.6.1 (5149 timed out banking 37 years of the Treasury par curve inside a request): the first bank runs as mode=ustbank (Event + daily 21:30 UTC schedule); requests never fetch history inline.
...
def http(url, timeout=120):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5151", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5151-search-sources") as r:
        r.heading("ops 5151 -- equivalents before dictionary; dropdown chips with evidence (v1.6.2)")
        fails = []
        ...
        if h.get("version") != "1.6.2":
            fails.append(f"version {h.get('version')} != 1.6.2")
        ...
        resp = lam.invoke(FunctionName=FN, InvocationType="RequestResponse", Payload=json.dumps({"mode": "ustbank"}).encode())
        ...
        for sid, want in (("TVC:US02MY", "ustpar:2M"), ("ECONOMICS:USINTR", "fred:FEDFUNDS"), ("TVC:DE10Y", "official-yields:de-10y-bbk"), ("TVC:US03MY", "fred:DGS3MO|fred:DTB3")):
            ...
            if not d.get("n") or d.get("via") not in want.split("|"):
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                waited = page.evaluate("""async () => { const t0 = performance.now(); const k = Array.from(SymDir._cache.keys()).find(x => x.startsWith('us02my|40')); let d = null; try { d = k ? await SymDir._cache.get(k) : null; } catch (e) { return { err: String(e) }; }
                    const ent = performance.getEntriesByType('resource').filter(e => e.name.includes('/symsearch?q=US02MY')).map(e => ({ ms: Math.round(e.duration), status: e.responseStatus }));
                    return { key: k, resolved_ms: Math.round(performance.now() - t0), facets: d && d.facets, failed: d && d.failed, rows: d && (d.rows || []).length, timing: ent }; }""")
                r.log(f"  query promise: {json.dumps(waited)[:400]}")
                page.wait_for_timeout(1500)
                r.log(f"  after await: _facets={json.dumps(page.evaluate('() => HeaderSearch._facets || null'))[:160]} groups={json.dumps(page.evaluate('() => Object.fromEntries(Object.entries(HeaderSearch._groups||{}).map(([k,v])=>[k,(v||[]).length]))'))} render_src={json.dumps(page.evaluate('() => HeaderSearch.render.toString().slice(0,120)'))}")
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5151-us02my.jpg"), type="jpeg", quality=55)
