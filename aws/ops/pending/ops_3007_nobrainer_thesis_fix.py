#!/usr/bin/env python3
"""ops 3007 -- Nobrainer Hunter L5 fix: the thesis board was 0-ok/12-fail
...
def main():
    fails, warns = [], []
    with report("3007_nobrainer_thesis_fix") as rep:
...
def _write(rep, fails, warns, extra):
    payload = {"ops": 3007, "fails": fails, "warns": warns,
               "verdict": "FAIL" if fails else "PASS",
               "ts": datetime.now(timezone.utc).isoformat()}
    payload.update(extra)
    (AWS_DIR / "ops" / "reports" / "3007.json").write_text(
