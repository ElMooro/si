...
UA = "JustHodl ops4931 edge-acceptance"
...
def main():
    ...
    fails, notes = [], []
    with report("4931-usd-funding-accept") as r:
        r.heading("ops 4931 — justhodl-usd-funding v1.0.0 launch")
        ...
        if fails:
            ...
            r.ok("all gates green" + (" (notes: %s)" % ", ".join(notes)
                                      if notes else ""))
        (ROOT / "aws" / "ops" / "reports" / "4931.json").write_text(
