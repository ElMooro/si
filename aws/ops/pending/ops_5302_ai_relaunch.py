"""ops_5302 -- RE-ARM of ops_5301 after its report: the RoBERTa-SEC card ships an uncompressed inference-prepack prefix; serverless
...
UA = {"User-Agent": "justhodl-ops-5302", "Cache-Control": "no-cache", "Pragma": "no-cache"}
...
def write_verdict(status, title, t_start):
    doc = {"ops": 5302, "status": status, "title": title, "finished_at": datetime.now(timezone.utc).isoformat(), "duration_s": round(time.time() - t_start, 1),
           "steps": STEPS, "fails": [f[:300] for f in FAILS], "warns": [w[:300] for w in WARNS],
           "report": "https://github.com/ElMooro/si/blob/main/aws/ops/reports/latest/ops_5302_ai_relaunch.md"}
    try:
...
T_START = time.time()
with report("ops_5302_ai_relaunch") as R:
    R.heading("ops 5302 -- AI relaunch v1.2.0: prepacked-prefix hosting fixed (script env + serverless tarball), ledger/LLM permissions, first market read, verdict on the page")
    ...
    ctl = {"function_url": fn_url, "sagemaker_role_arn": SM_ROLE_ARN, "private_bucket": PRIVATE, "updated_at": datetime.now(timezone.utc).isoformat(), "ops": 5302}
    try:
    ...
    write_verdict("RED" if FAILS else "GREEN", "AI relaunch v1.2.0 -- hosting fix, ledger permissions, first market read", T_START)
    if FAILS:
        sys.exit(1)
