"""
ops_3992 — v2.0 verify: the families that were structurally impossible
...
MARK = "data-census v2.0 ops3991 vault-complete"


def main():
    with report("3992_v20_verify") as rep:
        rep.heading("ops 3992 — v2.0 vault-complete verify")
        checks = []
        doc = None
        for i in range(26):
            ...
            if doc.get("marker") == MARK:
                rep.ok(f"  v2.0 artifact after ~{i*30}s")
                break
            rep.log(f"  [{i}] {str(doc.get('marker'))[:40]}")
            time.sleep(30)
        checks.append(("artifact is v2.0", doc.get("marker") == MARK))
        if doc.get("marker") != MARK:
            for l, ok in checks:
                (rep.ok if ok else rep.fail)(f"  {l}")
            rep.fail("v2.0 never wrote in the window")
            sys.exit(1)
        ...
        def flag(fam, sym, srckey):
            ...
            return bool(m) and srckey in str(m.get("pulled_from", "")).lower()
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok(f"PASS_ALL — v2.0 complete: {tot} points / {len(bs)} families; "
