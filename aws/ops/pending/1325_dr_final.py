...
fac=d.get("factors",{})
out["factor_keys"]=list(fac.keys())
for k in ["inflation_print","etf_flows","ai_rotation"]:
    out[k]={"risk":fac.get(k,{}).get("risk"),"note":(fac.get(k,{}).get("note") or "")[:100]}
open("aws/ops/reports/1325_dr.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
