...
out={}
# update code AND config (timeout/mem)
...
lam.update_function_configuration(FunctionName="justhodl-brain-sync",Timeout=120,MemorySize=512)
time.sleep(6)
try:
...
try:
    ...
    out["n_notes_seen"]=b.get("n_notes"); out["regime_read"]=b.get("regime_read")
    d=b.get("directive"); out["profile"]=(d.get("investor_profile") if d else None)
except Exception as e: out["mirror_err"]=str(e)[:100]
open("aws/ops/reports/1422_r.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
