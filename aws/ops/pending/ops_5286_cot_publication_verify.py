"""Refresh the reviewed COT audit mode after exact package/configuration comparison."""
import json
import os
...
FUNCTIONS = ('justhodl-cot-extremes-scanner',)


def main():
    report = {'ops': 5286, 'ok': False, 'status': 'NOT_STARTED', 'private_payloads_reported': 0,
              'raw_lambda_responses_reported': 0,
              'checkout_sha': subprocess.check_output(['git', 'rev-parse', 'HEAD'], cwd=ROOT, text=True).strip()}
    destination = ROOT / 'aws/ops/reports/5286_cot_publication.json'
    def checkpoint():
    ...
    try:
        ...
        verifier = ReleaseVerifier(ROOT, clients, parity_seconds=300, publication_seconds=420)
        report.update(verifier.report, ops=5286)
        ...
        while True:
            ...
            if not remaining or time.monotonic() >= deadline:
                break
            print(json.dumps({'ops': 5286, 'phase': 'publication', 'pending': remaining}), flush=True)
