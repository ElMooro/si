...
code = zs("aws/lambdas/justhodl-rotation-radar/source")
try:
    lam.get_function(FunctionName="justhodl-rotation-radar")
    rc(lambda: lam.update_function_code(FunctionName="justhodl-rotation-radar", ZipFile=code)); out["fn"]="updated"
except ClientError:
    role = lam.get_function(FunctionName="justhodl-historical-analogs")["Configuration"]["Role"]
    rc(lambda: lam.create_function(FunctionName="justhodl-rotation-radar", Runtime="python3.12", Role=role,
...
ready("justhodl-rotation-radar")
arn=lam.get_function(FunctionName="justhodl-rotation-radar")["Configuration"]["FunctionArn"]
ev.put_rule(Name="justhodl-rotation-radar-daily", ScheduleExpression="cron(45 13 * * ? *)", State="ENABLED")
ev.put_targets(Rule="justhodl-rotation-radar-daily", Targets=[{"Id":"1","Arn":arn}])
try:
    lam.add_permission(FunctionName="justhodl-rotation-radar", StatementId=f"rr{int(time.time())%99999}",
        Action="lambda:InvokeFunction", Principal="events.amazonaws.com",
        SourceArn=f"arn:aws:events:us-east-1:{ACC}:rule/justhodl-rotation-radar-daily")
except ClientError: pass

r = rc(lambda: lam.invoke(FunctionName="justhodl-rotation-radar", InvocationType="RequestResponse", Payload=b"{}"))
...
out["verify"]={"duration_s":d.get("duration_s"),"scores":d.get("scores"),
  "diagnostics":(d.get("crypto") or {}).get("diagnostics"),
  "crypto_live":(d.get("crypto") or {}).get("live"),
  "crypto_events":((d.get("crypto") or {}).get("event_study") or {}).get("events"),
  "crypto_tables":{k:((d.get("crypto") or {}).get("event_study") or {}).get(k) for k in ("alts_in_btc","eth_usd")},
  "equity":{k:{"n_events":len(v.get("events") or []),"latest":(v.get("events") or [{}])[-1].get("thrust_date") if v.get("events") else None,
               "iwm_rel_90":(v.get("iwm_rel") or {}).get("90"),"live":v.get("live")}
  ...
  "ai_alt":str((d.get("ai_brief") or {}).get("altseason_read",""))[:260],
...
out["board_row"]=next(({k:e2.get(k) for k in ("signal","signal_label","read")}
          for e2 in (sb.get("engines") or []) if e2.get("engine")=="Rotation Radar"), "MISSING")
...
m.setdefault("key_overrides",{})["data/rotation-radar.json"]=30; m["updated_by"]="ops-1586"
s3.put_object(Bucket=B,Key="data/_freshness-manifest.json",Body=json.dumps(m,indent=1).encode(),ContentType="application/json")
open("aws/ops/reports/1595_crypto_fix.json","w").write(json.dumps(out,indent=2,default=str))
