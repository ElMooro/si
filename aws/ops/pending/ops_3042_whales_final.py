...
re-serve). v1.1: duplicate-page signature guard + short-page break
comment + implausible-row drop (>1.5x fund book). New credibility
asserts: Citadel positions 8k-16k, max |conviction flow| < $30B.

Prior: ops 3040 -- WHALES engine + page (Khalid). Audit found 5 smart-money
...
def main():
    ...
    with report("3042_whales_final") as rep:
        ...
        max_flow = max((abs(r["conviction_flow_usd"]) for r in
        ...
        if max_flow > 30_000_000_000:
            fails.append("implausible flow magnitude: %s" % max_flow)
