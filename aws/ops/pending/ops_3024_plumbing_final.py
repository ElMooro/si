#!/usr/bin/env python3
"""ops 3024 -- plumbing FINAL. 3023 probe proved schema fine + data fresh
...
def main():
    fails, warns = [], []
    with report("3024_plumbing_final") as rep:
        ...
        pl = [c for c in cans if c.get("mechanism") == "plumbing"]
        mech_keys = [m.get("key") for m in (d.get("mechanisms") or [])]
        dupe_hit = [c["name"] for c in pl if any(t in c["name"] for t in (
            "Temp Help", "Manufacturing Capacity", "CISS —",
            "Broad Dollar Index (Nominal)", "Fed Liquidity Swaps"))]
        rep.kv(plumbing_rows=len(pl),
               plumbing_firing=sum(1 for c in pl if c["firing"]),
               mechanisms=mech_keys, dedupe_leaks=dupe_hit)
        if "plumbing" not in mech_keys:
            fails.append("plumbing mechanism missing")
        if len(pl) < 18:
            fails.append("plumbing rows=%d (<18)" % len(pl))
        if dupe_hit:
            fails.append("dedupe leaked: %s" % dupe_hit[:3])
        if (baro.get("n_votes") or 0) <= 250:
            fails.append("n_votes=%s did not grow past 250"
                         % baro.get("n_votes"))
...
def _finish(rep, fails, warns, extra):
    payload = {"ops": 3024, "fails": fails, "warns": warns,
               "verdict": "FAIL" if fails else "PASS",
               "ts": datetime.now(timezone.utc).isoformat()}
    payload.update(extra)
    (AWS_DIR / "ops" / "reports" / "3024.json").write_text(
