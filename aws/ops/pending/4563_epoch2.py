"""ops 4563 — pause cron, wait out in-flight, clean epoch reset (baseline
...
R={"ops":4563,"started":datetime.now(timezone.utc).isoformat()}
ev.disable_rule(Name="justhodl-fred-catalog-5min")
time.sleep(30)  # let any in-flight round finish
...
s3.put_object(Bucket=B,Key="data/_state/fred-scoped-import.json",
    Body=json.dumps({"cats_done":[],"series_seen":0,"series_excluded_stale":0,
                     ...
                     "reset_by":"ops4563_clean_epoch"}).encode(),
...
json.dump(R,open("aws/ops/reports/4563.json","w"),indent=1,default=str)
open("aws/ops/reports/4563.md","w").write("# 4563 — "+json.dumps(final,default=str)+"\n")
