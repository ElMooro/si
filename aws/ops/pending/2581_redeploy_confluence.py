"""ops 2581 — update attention-confluence code + invoke + verify recalibrated staging."""
...
for sec in ["stealth","igniting","undiscovered","crowded","distribution"]:
    ...
    ex=[(x["symbol"],f"s{x['smart_score']}",f"c{x['crowd_score']}",f"cf{x['confluence_smart']}",x.get("layer")) for x in rows[:5]]
    print(f"  {sec} ({len(rows)}): {ex}")
# show a couple full stealth records' why
for x in j.get("stages",{}).get("stealth",[])[:3]:
    print("   STEALTH why:", x["symbol"], "->", x.get("why"))
print("DONE 2581")
