...
def main() -> int:
    ...
    with report("ops_5551_owned_async_endpoint") as R:
        R.heading("ops 5551 -- owned model async endpoint (scale-to-zero) from the staged weights; proven by one round trip")
        ...
        if status != "InService":
            ...
            try:
                ...
                pass
            R.fail("endpoint status %s (%s); deleting so nothing bills" % (status, reason[:300]))
            try:
        ...
        try:
            ...
            warn.append("autoscaling"); R.warn("autoscaling setup failed (%s) -- endpoint stays at 1 instance until fixed; billing continues" % str(e)[:160])
        # round trip
        import factory_inference as fi
        rt = boto3.client("sagemaker-runtime", region_name=REGION)
        prompt = fi.qwen_chat_prompt(fi.SYSTEM, [], "Write a Python function is_palindrome(s) that ignores case and non-letters, with two example calls.")
        req_key = "factory/inference/requests/probe-%s.json" % stamp
        s3.put_object(Bucket=PRI, Key=req_key, Body=json.dumps({"inputs": prompt, "parameters": {"max_new_tokens": 300, "temperature": 0.2}}).encode(), ContentType="application/json")
        resp = rt.invoke_endpoint_async(EndpointName=NAME, InputLocation="s3://%s/%s" % (PRI, req_key), ContentType="application/json", InferenceId="probe-" + stamp, InvocationTimeoutSeconds=900)
        out_key = resp["OutputLocation"].split(PRI + "/", 1)[1]
        answer = None
        for _ in range(60):
            try:
                answer = s3.get_object(Bucket=PRI, Key=out_key)["Body"].read().decode("utf-8", "replace")
                break
            except Exception:  # noqa: BLE001
                time.sleep(10)
        if not answer:
            red.append("roundtrip"); R.fail("no output object within 10 min at %s" % out_key)
        else:
            try:
                text = json.loads(answer).get("generated_text", answer)
            except ValueError:
                text = answer
            R.ok("round trip OK: %s" % " ".join(str(text).split())[:300])
            control = {"schema_version": "factory-inference-control.v1", "enabled": True, "endpoint_name": NAME, "mode": "async-scale-to-zero",
                       ...
                       "endpoint_config": cfg_name, "written_by": "ops 5551", "git_sha": head[:12], "at": datetime.now(timezone.utc).isoformat()}
        ...
        try:
            ...
            warn.append("lambda-role"); R.warn("lambda role grant failed: %s" % str(e)[:160])
        if red:
