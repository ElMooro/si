...
UA = {"User-Agent": "Mozilla/5.0 ops-3081",
...
def main():
    ...
    with report("3081_ranks_leaders") as rep:
        ...
        for _ in range(30):
            try:
                ...
                if cfg.get("LastUpdateStatus") in ("Successful",
                                                   None) and                         (not need or lm >= T_START - _td(seconds=90)):
                    okf = True
                    break
            except Exception:
                pass
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3081.json").write_text(json.dumps(
        {"ops": 3081, "verdict": "FAIL" if fails else "PASS",
