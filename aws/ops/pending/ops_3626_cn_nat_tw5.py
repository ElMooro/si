"""ops 3626 — [CN] PBoC TSF (社会融资规模增量) NATIONAL-node + attachment-hop (china-liquidity v2.3) (EN report lags ~11mo; CN is same-week) + [TW] asia-leads
v1.6 STAGE-5 XHR-endpoint hunt (api/ajax literals from frame bodies, edge-probed). Gates =
...
with report("3626_cn_nat_tw5") as rep:
    ...
    try:
        err = dep_inv("justhodl-china-liquidity", 300, 768,
                      "china-liquidity v2.3: national 统计数据报告 node + attachment hop")
        ...
        gate("G1_cn_tsf", (got or probed) and not err,
             f"err={err} s5_hit={tw.get(chr(39)+"stage5_hit"+chr(39))} s5={[{k: t.get(k) for k in (chr(39)+"via"+chr(39),chr(39)+"json"+chr(39),chr(39)+"hit"+chr(39),chr(39)+"head"+chr(39))} for t in s5[:2]]} eps={[e.get(chr(39)+"u"+chr(39))[-40:] for e in eps[:4]]} VALUE={'%s trn CNY %s (yoyΔ %s)' % (cn.get('flow_trn_cny'), cn.get('period'), cn.get('yoy_delta_trn')) if got else 'none'} "
    ...
    try:
        err = dep_inv("justhodl-asia-leads", 240, 512,
                      "asia-leads v1.6: TW stage-5 XHR-endpoint hunt via edge")
        ...
        ok2 = (got or len(s5) >= 1 or len(eps) >= 1 or tw.get("stage4_block")) and not err
    ...
    Path("aws/ops/reports/3626.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
