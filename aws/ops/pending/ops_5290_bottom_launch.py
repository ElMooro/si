"""ops_5290 -- launch justhodl-bottom (the Wyckoff bottom desk) + verify bottom.html + arm the schedule + prove the joins.
...
UA = {"User-Agent": "justhodl-ops-5290", "Cache-Control": "no-cache", "Pragma": "no-cache"}
FAILS, WARNS = [], []
PUSH_TS = int(subprocess.run(["git", "log", "-1", "--format=%ct", "--grep=ops 5290", "--", "aws/lambdas/justhodl-bottom/source/lambda_function.py"], cwd=ROOT, capture_output=True, text=True).stdout.strip() or "0")
...
with report("ops_5290_bottom_launch") as R:
    R.heading("ops 5290 -- BOTTOM launch (Wyckoff bottom desk)")
    ...
    arn = cfg["FunctionArn"]
    name, expr, desc = "justhodl-bottom-daily", "cron(45 3 ? * TUE-SAT *)", "BOTTOM daily 03:45 UTC Tue-Sat after polygon-full + fortress, before katlin (ops 5290)"
    ...
    if live and D:
        ...
        with sync_playwright() as p:
            ...
            for width, height in ((1440, 1100), (390, 844)):
                ...
                pg.screenshot(path=str(SHOTS / f"ops5290_bottom_{width}.png"))
