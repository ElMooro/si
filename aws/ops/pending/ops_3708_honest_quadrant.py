"""ops 3708 — Stage 1 of Khalid's reverse-engineered-backlog theory.
...
with report("3708_honest_quadrant") as rep:
    rep.heading("ops 3708 — backlog reverse-engineering, stage 1")
    out = {"gates": {}}
    fails = []
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3708.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        # the whole point of 3708: no row may claim consensus has not moved when
    ...
    Path("aws/ops/reports/3708.json").write_text(json.dumps(out, indent=2, default=str))
