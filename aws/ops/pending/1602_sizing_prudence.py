...
code = zs("aws/lambdas/justhodl-sizing-engine/source")
try:
    lam.get_function(FunctionName="justhodl-sizing-engine")
    rc(lambda: lam.update_function_code(FunctionName="justhodl-sizing-engine", ZipFile=code)); out["fn"]="updated"
except ClientError:
    role = lam.get_function(FunctionName="justhodl-historical-analogs")["Configuration"]["Role"]
    rc(lambda: lam.create_function(FunctionName="justhodl-sizing-engine", Runtime="python3.12", Role=role,
...
ready("justhodl-sizing-engine")
arn=lam.get_function(FunctionName="justhodl-sizing-engine")["Configuration"]["FunctionArn"]
ev.put_rule(Name="justhodl-sizing-engine-daily", ScheduleExpression="cron(10 14 * * ? *)", State="ENABLED")
ev.put_targets(Rule="justhodl-sizing-engine-daily", Targets=[{"Id":"1","Arn":arn}])
try:
    lam.add_permission(FunctionName="justhodl-sizing-engine", StatementId=f"sz{int(time.time())%99999}",
        Action="lambda:InvokeFunction", Principal="events.amazonaws.com",
        SourceArn=f"arn:aws:events:us-east-1:{ACC}:rule/justhodl-sizing-engine-daily")
except ClientError: pass

r = rc(lambda: lam.invoke(FunctionName="justhodl-sizing-engine", InvocationType="RequestResponse", Payload=b"{}"))
out["fn_err"]=r.get("FunctionError","NONE")
if out["fn_err"]!="NONE": out["payload"]=r["Payload"].read().decode()[:600]
time.sleep(2)
d=json.loads(s3.get_object(Bucket=B,Key="data/sizing.json")["Body"].read())
out["verify"]={"duration_s":d.get("duration_s"),
  "scanned_graded":d.get("scanned_graded"),
  "engine_table_head":(d.get("engine_table") or [])[:10],
  "fade_list":d.get("fade_list"),
  "recs_head":(d.get("recommendations") or [])[:8],
  "gross":d.get("gross_recommended_w_pct"),
  "book_status":d.get("book_status")}
...
out["board_row"]=next(({k:e2.get(k) for k in ("signal","signal_label","read")}
          for e2 in (sb.get("engines") or []) if e2.get("engine")=="Sizing Engine"), "MISSING")
...
m.setdefault("key_overrides",{})["data/sizing.json"]=30; m["updated_by"]="ops-1586"
s3.put_object(Bucket=B,Key="data/_freshness-manifest.json",Body=json.dumps(m,indent=1).encode(),ContentType="application/json")
open("aws/ops/reports/1602_sizing_prudence.json","w").write(json.dumps(out,indent=2,default=str))
