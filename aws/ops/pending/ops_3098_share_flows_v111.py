#!/usr/bin/env python3
"""ops 3098 -- SHARE-FLOWS fleet map (Khalid: track issuance,
...
def main():
    fails, warns = [], []
    with report("3098_share_flows_v111") as rep:
        ...
        if not d:
            ...
            sys.exit(1)
        tk = d.get("tickers") or {}
        # insider-trades schema probe (sell tape evidence)
        try:
            _it = json.loads(S3.get_object(
                Bucket=BUCKET, Key="data/insider-trades.json")[
                "Body"].read())
            _tx = _it.get("transactions") or []
            import collections
            _sides = collections.Counter(
                str(x.get("side") or x.get("type")
                    or x.get("transactionType") or "?")[:16].lower()
                for x in _tx[:500])
            rep.kv(it_top_keys=json.dumps(list(_it)[:8]),
                   it_n_tx=len(_tx),
                   it_tx_fields=json.dumps(
                       list(_tx[0])[:12] if _tx else []),
                   it_sides=json.dumps(dict(_sides))[:200])
        except Exception as e:
            warns.append("it probe: %s" % str(e)[:70])
        n_sell = sum(1 for v in tk.values()
                     if v.get("insider_sell_usd_recent"))
        rep.kv(n_sell_names=n_sell)
        if n_sell == 0:
            warns.append("still 0 sell names -- tape may be buy-only "
                         "(see it_sides evidence)")
        uni_n = None
        for w in (d.get("warns") or []):
            if str(w).startswith("universe:"):
                try:
                    uni_n = int(str(w).split()[1])
                except Exception:
                    pass
        rep.kv(universe_claim=uni_n)
        if uni_n is not None and uni_n < 520:
            fails.append("universe %s (<520) -- opp 'all' join "
                         "not landing" % uni_n)
