...
s3=boto3.client("s3",region_name=REGION); R={"ops":4472,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
json.dump(R,open("aws/ops/reports/4472_tickerkey.json","w"),indent=1,default=str)
open("aws/ops/reports/4472_tickerkey.md","w").write(f"# ops 4442 — FIGI — {R['verdict']}\n- ssm: {R.get('ssm')}\n- stats: {json.dumps(fg,default=str)[:500]}\n")
print(R["verdict"])
