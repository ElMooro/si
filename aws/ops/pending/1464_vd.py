...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-vd"
code=r'''
import json,urllib.request
def lambda_handler(e,c):
    out={}
    try:
        h=urllib.request.urlopen(urllib.request.Request("https://justhodl.ai/dossier.html?t=NVDA",headers={"User-Agent":"jh"}),timeout=15).read().decode()
        out["served"]="Research Dossier" in h and "DEMAND" in h and "KILL CRITERIA" in h
        out["bytes"]=len(h)
    ...
    out["board_links_dossier"]="dossier.html?t=" in b
    return out
'''
buf=io.BytesIO()
...
open("aws/ops/reports/1464_vd.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
