...
def main() -> int:
    ...
    with report("ops_5533_own_training_lane_rearm5") as R:
        R.heading("ops 5533 (re-arm of 5528-5532) -- owned training lane: ECR repo, pinned recipe bundle, base-weight staging job (CPU, priced), no flip")
        R.kv(head=head[:10])
        ...
        try:
            ...
            iam.add_user_to_group(GroupName="justhodl-runner-ecr", UserName=user)
            R.ok("pricing:GetProducts granted to %s via group justhodl-runner-ecr; waiting 45 s for propagation" % user)
            time.sleep(45)
