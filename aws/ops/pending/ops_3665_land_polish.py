"""ops 3665 — AIR-CARGO canary + freight page + sidebar: [A] NEW
...
with report("3665_land_polish") as rep:
    rep.heading("ops 3665 — HKIA air canary + freight page + sidebar")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3665.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
    ...
    Path("aws/ops/reports/3665.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
