...
r=lam.invoke(FunctionName="justhodl-ecb-derived",InvocationType="RequestResponse",Payload=b"{}"); out["run"]=r["Payload"].read().decode()[:60]
time.sleep(3)
...
out["divergence"]={k:div.get(k) for k in ['ecb_total_assets_eur_bn','ecb_balance_sheet_6m_chg_eur_bn','fed_net_liquidity_6m_chg_usd_bn','divergence_bn','signal','err']}
out["all_signals"]={k:v.get("signal") for k,v in d["indicators"].items()}
open("aws/ops/reports/1497_r.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
