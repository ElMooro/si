#!/usr/bin/env python3
"""ops 3021 -- Global Stress Matrix as the 10th warroom mechanism
...
def main():
    fails, warns = [], []
    with report("3021_gsm_mechanism") as rep:
        ...
        gsm = [c for c in cans if c.get("mechanism") == "global_stress"]
        gsm_mkt = sum(1 for c in gsm if c["name"].startswith("Tape stress"))
        gsm_tier = sum(1 for c in gsm if c["name"].startswith("Credit tier"))
        mech_keys = [m.get("key") for m in (d.get("mechanisms") or [])]
        rep.kv(gsm_rows=len(gsm), gsm_markets=gsm_mkt, gsm_tiers=gsm_tier,
               mechanisms=mech_keys)
        if "global_stress" not in mech_keys:
            fails.append("global_stress mechanism missing")
        if gsm_mkt < 9:
            fails.append("market tape rows=%d (<9)" % gsm_mkt)
        if gsm_tier < 4:
            fails.append("credit tier rows=%d (<4)" % gsm_tier)
        if (baro.get("n_votes") or 0) <= 230:
            fails.append("n_votes=%s did not grow past 230"
                         % baro.get("n_votes"))
        if any("Escalation" in c["name"] or "Hot signal" in c["name"]
               for c in gsm):
            fails.append("double-count rows leaked in")
...
def _finish(rep, fails, warns, extra):
    payload = {"ops": 3021, "fails": fails, "warns": warns,
               "verdict": "FAIL" if fails else "PASS",
               "ts": datetime.now(timezone.utc).isoformat()}
    payload.update(extra)
    (AWS_DIR / "ops" / "reports" / "3021.json").write_text(
