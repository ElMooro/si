...
DYNAMIC_OK = {"components", "scores", "by_category", "by_sector", "metrics",
              "history", "raw_sample"}
# deliberate non-renders, recorded with a reason (doctrine: surface it OR record why not)
WAIVED = {
    "ai.usage": "LLM token accounting - belongs on llm-cost.html, not a flow desk",
    "ai.claude_elapsed_s": "run telemetry, same reason",
    "rotation.n_failed": "rotation.json is not consumed by this page at all",
    "constituent.as_of_etf_flows_date": "duplicate of the daily.json stamp already in the meta bar",
}
...
def main():
    with report("3860_flows_coverage_recheck") as rep:
        rep.heading("ops 3860 — PROBE: does flows.html render everything etf-flows publishes")
        ...
        for name, d in data.items():
            ...
            for k in sorted(d.keys()):
                ...
                fq = f"{name}.{k}"
                if fq in WAIVED:
                    rep.log(f"  {fq:<34} WAIVED — {WAIVED[fq]}")
                    continue
