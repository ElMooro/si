...
print("MARKET READ:",(d.get("market_read") or "")[:200])
print("\nBRIEF:")
for s in d.get("hot_stocks",[])[:8]:
    a=s.get("analyst") or {}
    print(f"  {s['ticker']:<6} heat={round(s['score'])} ven={s.get('venue_count')} bull%={s.get('bull_pct')} | NET: {a.get('net','—')}")
    if a.get('bull'): print(f"        + {a.get('bull','')[:86]}")
    if a.get('bear'): print(f"        - {a.get('bear','')[:86]}")
    gn=s.get('good_news',[]); bn=s.get('bad_news',[])
    if gn: print(f"        good: {(gn[0].get('title') or '')[:70]}")
    if bn: print(f"        bad:  {(bn[0].get('title') or '')[:70]}")
print("\nwarnings:",[w['ticker'] for w in d.get('warnings',[])[:8]])
import urllib.request
try: code=urllib.request.urlopen(urllib.request.Request("https://justhodl.ai/hot-stocks.html?t="+str(int(time.time())),headers={"User-Agent":"jh"}),timeout=15).getcode()
except Exception as e: code=str(e)[:40]
print("page hot-stocks.html:",code,"| email sent via SES to reports@justhodl.ai")
print("DONE 2139")
