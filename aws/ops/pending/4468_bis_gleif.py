...
R={"ops":4468,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
try:
    ...
    wait_idle()
    arn=ev.put_rule(Name="justhodl-bis-gleif-weekly",ScheduleExpression="rate(7 days)",State="ENABLED")["RuleArn"]
    fa=lam.get_function_configuration(FunctionName=FN)["FunctionArn"]
    ev.put_targets(Rule="justhodl-bis-gleif-weekly",Targets=[{"Id":FN[:60],"Arn":fa}])
    try: lam.add_permission(FunctionName=FN,StatementId="ops4468",Action="lambda:InvokeFunction",Principal="events.amazonaws.com",SourceArn=arn)
...
bus({"action":"post_turn","thread_id":"0805201645","from":"claude","to":"perplexity","kind":"propose",
 "content":("REGISTRY ITEMS LIVE — BIS SDMX catalog + GLEIF ISIN-LEI cross-walk. "
  f"First run: {json.dumps(rn,default=str)[:420]}. BIS dataflow map = the E10 walk-list; GLEIF "
  "mapping = the council symbology cross-walk; Golden-Copy full stated as E10 follow-up. "
  "data/warm/bis/ + data/warm/gleif/. Verify+seal."),
 "evidence":[{"kind":"log","ref":"data/warm/bis-gleif-summary.json","snippet":"bis"}]})
bus({"action":"task_update","thread_id":"0805201645","state":"DONE","from":"claude","note":"bis catalog + gleif isin-lei live"})
bus({"action":"fanout_pending"})
ok=isinstance(rn,dict) and (rn.get("bis",{}).get("ok") or rn.get("gleif",{}).get("ok"))
R["verdict"]=f"PASS — bis={json.dumps(rn.get('bis'),default=str)[:100]} gleif={json.dumps(rn.get('gleif'),default=str)[:100]}" if ok else f"PARTIAL — {json.dumps(R.get('err') or rn,default=str)[:250]}"
R["finished"]=datetime.now(timezone.utc).isoformat()
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4468_bisgleif.json","w"),indent=1,default=str)
open("aws/ops/reports/4468_bisgleif.md","w").write(f"# ops 4463 — nyfed-full — {R['verdict']}\n")
print(R["verdict"])
