"""ops/4924 -- floor-audit v2.0.2 acceptance: whole market, on a clock.
...
Gates:
  ...
  G7 edge: page marker floor-audit-v2.0.2 + feed v2.0.0 (real UA)
"""
import json
import sys
import time
...
MARKER = "floor-audit-v2.0.2"
UA = "JustHodl ops4924 edge-acceptance"
...
def main():
    with report("4924-floor-v202-accept") as r:
        r.heading("ops 4924 -- floor-audit v2.0.2 acceptance")
        t0 = datetime.now(timezone.utc)

        r.section("G1 deploy")
        if ENG.VERSION != "2.0.2":
        ...
        for i in range(90):  # sweep + deep tier: allow 15 min
            time.sleep(10)
            cur = s3_json(OUT_KEY)
            if cur and cur.get("version") == "2.0.2" and (
               ...
               "top_calls": [
                   ...
                   for x in sorted(
                       ok, key=lambda z: -(z["recommendation"]
                                           ["conviction"] or 0))[:15]],
               "edge": "LIVE"}
        (ROOT / "aws" / "ops" / "reports" / "4924.json").write_text(
