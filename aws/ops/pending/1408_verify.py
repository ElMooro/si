...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-v"
code=r'''
...
def lambda_handler(e,c):
    ...
    try:
        ...
        out["owner_default"]="9f48a96b-1a1e-4867-9fc6-e6cc5054c56d" in h
    except Exception as ex: out["page_err"]=str(ex)[:60]
    try:
        t0=time.time()
        r=urllib.request.urlopen(urllib.request.Request("https://api.justhodl.ai/brain?uid=9f48a96b-1a1e-4867-9fc6-e6cc5054c56d&t=1",headers={"User-Agent":"jh","Origin":"https://justhodl.ai"}),timeout=25)
        d=json.loads(r.read().decode())
        out["account_load"]={"notes":len(d.get("notes",[])),"secs":round(time.time()-t0,2),"samples":[(n.get("text") or "")[:45] for n in d.get("notes",[])[:5]]}
    except Exception as ex: out["load_err"]=str(ex)[:80]
    return out
'''
buf=io.BytesIO()
...
open("aws/ops/reports/1408_v.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
