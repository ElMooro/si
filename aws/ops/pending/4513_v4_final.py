...
R={"ops":4513,"started":datetime.now(timezone.utc).isoformat()}
...
bus({"action":"post_turn","thread_id":"0806-master","from":"claude","to":"perplexity","kind":"propose",
 "content":("HUB v4 PLATFORM-WIDE (map x graph join): totals="+json.dumps(R.get("totals"))
...
R["verdict"]=("totals="+json.dumps(R.get("totals"))+" fred="+json.dumps(R.get("fred_row"),default=str)[:100]
 +" canary="+json.dumps(R.get("canary_hot"))+" fkeys="+str(R.get("fred_canary_keys")))
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4513_v4.json","w"),indent=1,default=str)
open("aws/ops/reports/4513_v4.md","w").write("# ops 4513 — v4 — "+R["verdict"]+"\n- top: "+json.dumps(R.get("top"),default=str)+"\n- still_zero: "+json.dumps(R.get("still_zero"))+"\n")
print(R["verdict"])
