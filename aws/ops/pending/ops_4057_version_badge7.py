"""ops_4057 — dynamic version badge from the served zip's own manifest."""
import io
import json
import sys
import time
...
def main():
    with report("4057_version_badge7") as rep:
        ...
        checks = [("zip is v1.6.0", ver == "1.7.4"),
                  ("version.json publicly serves it",
                   got.get("version") == "1.7.4")]
        n, htm = 0, ""
        for i in range(9):
            try:
