"""ops_4196 — convert: vault fire, class-wise LIVE ledger, realistic gates."""
import io
import json
import sys
import time
...
def main():
    with report("4196_collapse_convert") as rep:
        ...
        if pend2 > 600:
            rep.fail(f"collapse incomplete: pending={pend2}")
            sys.exit(1)
        if pmi2 < 60 or fam2 < 80:
            rep.fail(f"wave2 weak: pmi={pmi2} fam={fam2}")
            sys.exit(1)
