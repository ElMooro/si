"""ops 3449 — cannibals v1.0.1: keyed-by-symbol walker; expect real scan
breadth + live plans list."""
...
with report("3449_cannibals_v101") as rep:
    out={"gates":{}}; fails=[]
    def gate(n,ok,d):
    ...
    gate("G2_scan_breadth", (feed.get("generated_at") or "")>t0 and feed.get("n_rows_scanned",0)>=100,
         f"scanned={feed.get('n_rows_scanned')} plans={[(p.get('ticker'),p.get('sh_3y_cagr_pct')) for p in (feed.get('plans') or [])][:8]} logged={feed.get('logged')}")
    out["plans"]=(feed.get("plans") or [])
