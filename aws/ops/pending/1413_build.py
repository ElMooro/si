...
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-build"
code=r'''
...
def lambda_handler(e,c):
    ...
    for i in range(40):
        ...
        off=d.get("next_offset",off+150); out["built"]=d.get("built"); out["total"]=d.get("total")
        if d.get("done"): out["done"]=True; break
    # verify instant cached read
    import time as _t; t0=_t.time(); g=call("/brain?uid="+UID+"&t=9")
    out["cached_load"]={"notes":len(g.get("notes",[])) if isinstance(g.get("notes"),list) else g,"secs":round(_t.time()-t0,2)}
    return out
'''
buf=io.BytesIO()
...
try:
    try: lam.get_function_configuration(FunctionName=FN); lam.update_function_code(FunctionName=FN,ZipFile=buf.getvalue())
        ...
        lam.create_function(FunctionName=FN,Runtime="python3.12",Role=ROLE,Handler="lambda_function.lambda_handler",Code={"ZipFile":buf.getvalue()},Timeout=400,MemorySize=256)
    for _ in range(30):
...
open("aws/ops/reports/1413_b.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
