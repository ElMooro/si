...
UA = {"User-Agent": "justhodl-ops-5308", "Cache-Control": "no-cache", "Pragma": "no-cache"}
...
def write_verdict(status, title, t_start):
    doc = {"ops": 5308, "status": status, "title": title, "finished_at": datetime.now(timezone.utc).isoformat(), "duration_s": round(time.time() - t_start, 1),
           "steps": STEPS, "fails": [f[:300] for f in FAILS], "warns": [w[:300] for w in WARNS],
           "report": "https://github.com/ElMooro/si/blob/main/aws/ops/reports/latest/ops_5308_ai_v2_launch.md"}
    try:
...
T_START = time.time()
with report("ops_5308_ai_v2_launch") as R:
    R.heading("ops 5308 -- AI v2.1.0 (Perplexity governance modules + Claude pipeline/read/lessons) deployed in review mode on the live role; GET /inventory for the page; RoBERTa-SEC on GPU + CPU retrieval cards; learn-from-mistakes loop")
    ...
    for stale in ("AI_EVENT_BUS", "SIGNAL_FEATURE_GROUP", "PREDICTION_FEATURE_GROUP", "PREDICTION_LEDGER_TABLE", "PREDICTION_LEDGER_ARCHIVE_BUCKET", "MODEL_PACKAGE_GROUP",
                  "MLFLOW_TRACKING_SERVER_NAME", "MLFLOW_TRACKING_SERVER_ARN", "MLFLOW_EXPERIMENT_NAME", "TRAINING_INPUT_BUCKET", "AI_BRAIN_SOURCE_BUCKET"):
        env.pop(stale, None)
    ...
    ctl = {"function_url": fn_url, "sagemaker_role_arn": SM_ROLE_ARN, "private_bucket": PRIVATE, "updated_at": datetime.now(timezone.utc).isoformat(), "ops": 5308}
    try:
    ...
    write_verdict("RED" if FAILS else "GREEN", "AI v2.1.0 -- review mode on the live role, pipeline restarted", T_START)
    if FAILS:
        sys.exit(1)
