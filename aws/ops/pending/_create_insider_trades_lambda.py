    ...
    try:
        ...
        if "body" in body:
            try:
                ...
                r.log(f"     atom feed:   {stats.get('atom_feed_count', '?')} filings")
                d = stats.get("diagnostics", {})
                r.log(f"     filings_seen:        {d.get('filings_seen', '?')}")
                r.log(f"     xml_fetched:         {d.get('xml_fetched', '?')}")
                r.log(f"     no_issuer_or_ticker: {d.get('no_issuer_or_ticker', '?')}")
                r.log(f"     no_nonderiv_table:   {d.get('no_nonderivative_table', '?')}")
                r.log(f"     txn_seen:            {d.get('txn_seen', '?')}")
                r.log(f"     skipped_not_buy:     {d.get('skipped_not_buy_or_sell', '?')}")
                r.log(f"     skipped_threshold:   {d.get('skipped_below_threshold', '?')}")
                r.log(f"     buys_kept:           {d.get('buys_kept', '?')}")
                r.log(f"     sells_kept:          {d.get('sells_kept', '?')}")
                r.log(f"     ─────────────────────────────────")
                r.log(f"     buys (window):       {stats.get('total_buys', '?')}")
                r.log(f"     value (window):      ${stats.get('total_value_usd', 0):,.0f}")
