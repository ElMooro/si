"""ops_4043 — SOURCE DELTA: which agencies does TradingView cite that the
...
def main():
    with report("4043_source_delta") as rep:
        ...
        try:
            ...
            rep.log("  data/tv-sources.json NOT BORN — harvest finish-upload "
                    "hasn't landed. Delta runs the moment it exists.")
            rep.fail("SOURCES NOT LANDED YET")
            sys.exit(1)
        m = sr.get("sources") or {}
        ...
        rep.ok(f"DELTA DONE — {len(new_rows)} new sources across "
