"""ops_5137 -- chart-pro: full stacks (user-code frames) for the residual "Value is null" / "Object is disposed" on native renders.
...
def http(url, timeout=60):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5137", "Accept-Encoding": "identity"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def main():
    with report("5137-chartpro-stacks") as r:
        r.heading("ops 5137 -- chart-pro: where do the residual LightweightCharts errors originate")
        fails = []
        ...
        while time.time() - t0 < 900 and not all(got.values()):
            try:
                ...
                got["chart"] = b"static createChart(el, opts) {" in c and b"_patchLWC" not in c
        ...
        if got["chart"]:
            ...
            with sync_playwright() as p:
                ...
                page = ctx.new_page()
                errors = []
                page.on("pageerror", lambda e: errors.append({"msg": str(e)[:120], "sym": page.evaluate("() => State.activeTicker"),
                                                                "frames": [ln.strip() for ln in (getattr(e, "stack", "") or "").split("\n") if "chart-pro.html" in ln or "jhf" in ln or ".js" in ln and "unpkg" not in ln][:6],
                                                                "lwc_frames": [ln.strip()[:90] for ln in (getattr(e, "stack", "") or "").split("\n") if "unpkg" in ln][:6]}))
                bad_resp = []
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5137-chartpro-native.jpg"), type="jpeg", quality=60)
                errs = [e for e in errors if "TradingView" not in json.dumps(e)]
                seen = set()
                for e in errs:
                    key = json.dumps(e.get("frames"))
                    if key in seen:
                        continue
                    seen.add(key)
                    r.log(f"  page error ({e.get('sym')}): {e.get('msg')} | user frames: {json.dumps(e.get('frames'))[:600]} | lwc: {json.dumps(e.get('lwc_frames'))[:400]}")
                if errs:
