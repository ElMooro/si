#!/usr/bin/env python3
"""ops 3795 — verify rev share + dependency now reach the boards (v4.2.2).
...
def main():
    with report("3795_verify_revshare_dep_fix") as rep:
        rep.heading("ops 3795 — rev share + dependency on the boards (v4.2.2)")
        ...
        for i in range(16):
            ...
            if c.get("State") == "Active" and c.get("LastUpdateStatus") == "Successful":
                ...
                if "refresh the copies WHOLESALE" in body:
                    settled = True
        ...
        st = cap.get("stats") or {}
        gate(rep, "LIVE.v422", d.get("version") == "4.2.2", "version=%s" % d.get("version"))
