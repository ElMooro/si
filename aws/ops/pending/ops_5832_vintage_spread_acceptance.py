...
def main():
    expected=subprocess.check_output(['git','log','-1','--format=%H','--','aws/lambdas/justhodl-vintage-fred/source/vintage_source_store.py'],text=True).strip()
    dependency_commit=subprocess.check_output(['git','log','-1','--format=%H','--','aws/shared/fred_vintage_model.py'],text=True).strip()
    ...
    with report('ops_5832_vintage_spread_acceptance') as r:
        ...
        while len(runtimes)!=len(names):
            for fn in names:
                ...
                try:receipt=read('data/ops/releases/'+fn+'.json')
                    ...
                    raise
                intended=expected if fn=='justhodl-vintage-fred' else dependency_commit
                if receipt.get('commit')!=intended:continue
                ...
                runtimes[fn]={'commit':intended,'code_sha256':conf['CodeSha256']}
        ...
        proof={'contract':'vintage-archive-verification.v1','generated_at':datetime.now(timezone.utc).isoformat(),
            ...
            'collection_key':result['collection_key'],'series':accepted,'original_replay_reproduced':True,'segmented_series':['NFCI','STLFSI4','T10Y2Y'],
