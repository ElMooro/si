...
def main():
    with report("3718_est_rev_verify") as rep:
        rep.heading("ops 3718 — verify estimate-revisions v2.1.0 live + directions populate")
        fails = []
        out = {}
        ...
        gate("G1_patch_committed", "_prof_early" in src and '"version": "2.1.0"' in src,
             "source carries the FMP fallback + v2.1.0 (applied by ops 3717)")
        ...
        if not fails:
            ...
            gate("G2_artifact_live", live,
                 f"v2.1.0 zip proven live after {round(time.time()-t0)}s "
                 "(3717 timed out at 424s waiting on the parallel deploy-lambdas run)")
