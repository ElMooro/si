"""ops 5550 -- burst 1 scorecard (MBPP + APPS, all passing samples kept) and the row count against Gear B's floor;
also the latest Gear B job record if the hourly hook has launched generation 1.
...
def main() -> int:
    ...
    with report("ops_5550_burst1_scorecard") as R:
        R.heading("ops 5550 -- burst 1 scorecard: job, verifier summary, kept rows, pass rate at K, cost per pass")
        ...
        fams = {}
        for o in s3.list_objects_v2(Bucket=PRI, Prefix=VERIFIED, MaxKeys=1000).get("Contents", []):
            pass
        jobs = [o["Key"] for o in s3.list_objects_v2(Bucket=PRI, Prefix="factory/gearb/jobs/", MaxKeys=50).get("Contents", [])]
        last = _get_json(s3, sorted(jobs)[-1]) if jobs else None
        R.ok("Gear B jobs: %d; latest: %s" % (len(jobs), json.dumps({k: (last or {}).get(k) for k in ("job_name", "status", "generation", "cap_usd", "launched_at")}) if last else "none yet"))
        floor = 1500
        R.ok("rows vs floor: %d verified rows total (self_trace %d) against min_sft_rows=%d -> %s" % (total, kept, floor, "FLOOR MET: generation 1 may train on the next hourly hook" if total >= floor else "below floor by %d" % (floor - total)))
