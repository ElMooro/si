"""ops_5089 -- justhodl-fortress v2.1.0: the engine acts on its own evidence.

  * location gate becomes ADAPTIVE: the weekly walk-forward now measures a
    'not extended' arm (knife floor < vs EMA250 <= +15%) next to the spec
    'under EMA250' arm; the live gate switches to 'not extended' only if it
    beats the spec by >= 0.5 pt of 63-session beta-adjusted alpha with
    >= 100 observations in both arms (hysteresis), and the payload states
    the mode, the reason and both arms' numbers every run
  * never-worse-than-the-market rule applied to the freshest window that
    has dump episodes (1y, else 2y, else 3y); 3y weighted capture still
    drives the score (5086 saw pct_dump_resilient collapse to 5% because one
    old blow-up disqualified names that held every recent dump)
  * asymmetry downside = the name's capture decile's real median loss in
    the walk-forward's SPY-down windows, scaled to a -10% dump

Order: deploy -> backtest first (new arm measured) -> daily (adaptive gate
resolved from that backtest) -> verify gate resolution + evidence logged.
...
def main():
    with report("5089-fortress-v210") as r:
        r.heading("ops 5089 -- justhodl-fortress v2.1.0: adaptive location gate, windowed worst rule, empirical downside")
        ...
        if not env_vars:
            ...
            sys.exit(1)
        env_vars["FORTRESS_VERSION"] = "2.1.0"
        ...
        json.dump({"ops": 5089, "engine": FN, "session": payload.get("session"),
                   "n_scored": payload.get("n_scored"), "tiers": payload.get("tiers"),
                   "coverage_pct": covs, "as_of": payload.get("as_of")},
                  open(str(ROOT / "aws" / "ops" / "reports" / "5089.json"), "w"), indent=1)
    sys.exit(0)
