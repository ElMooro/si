#!/usr/bin/env python3
"""ops 3065 -- Chart Pro: conviction drawer removed (Alerts button
...
UA = {"User-Agent": "Mozilla/5.0 ops-3065",
...
def main():
    fails, warns = [], []
    with report("3065_chartpro_cleanup") as rep:
        ...
        for i in range(18):                        # worker deploy wait
            try:
                d = json.loads(get(
                    ...
                    "dev/ohlc?ticker=AAPL&mult=1&span=week&days=9999"
                    "&cb=%d" % time.time()))
            ...
            time.sleep(20)
        if not deep:
            fails.append("/ohlc days=9999 never exceeded 300 weekly "
                         "bars (cap not live?)")
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3065.json").write_text(json.dumps(
        {"ops": 3065, "verdict": "FAIL" if fails else "PASS",
