#!/usr/bin/env python3
"""ops 3092 -- FINANCIAL-STATEMENT GRADE per soldier (Khalid):
Piotroski F-Score (45%) + Altman Z zone (25%, bank caveat) + revenue
growth (10%) + margin direction NI-vs-rev (10%) + dilution (10%) ->
A+..F colored FIN chip w/ breakdown tooltip. financial-scores carries
BOTH F and Z (one cached call; credit layer shares FIN_CACHE);
financial-growth adds the confirm legs; 7-day cache via prev output.
Engine v4.0."""
import json
import sys
import time
...
UA = {"User-Agent": "Mozilla/5.0 ops-3092",
...
def main():
    ...
    with report("3092_fin_grade") as rep:
        ...
        for _ in range(45):
            ...
            try:
                ...
                if (datetime.now(timezone.utc) - o["LastModified"]
                    ...
                    if dj.get("version") == "4.0":
                        break
                    dj = None
            except Exception:
                pass
        if not dj:
            fails.append("no fresh v4.0 output")
            _fin(rep, fails, warns)
            sys.exit(1)
        ...
        rep.section("0b. FIN grades (v4.0)")
        sold = [h for l0 in (dj.get("leaders") or [])
                for h in (l0.get("holdings_top") or [])]
        fins = [h for h in sold if h.get("fin")]
        letters = sorted({h["fin"]["grade"] for h in fins})
        nvda_f = next((h for h in sold
                       if h["ticker"] == "NVDA"), None)
        rep.kv(fin_soldiers=len(fins),
               letters=json.dumps(letters),
               fin_sample=json.dumps(
                   [{"t": h["ticker"], **{k: h["fin"].get(k)
                     for k in ("grade", "score", "f_score", "z",
                               "rev_g", "sh_g")}}
                    for h in fins[:4]])[:340],
               nvda_fin=json.dumps(
                   (nvda_f or {}).get("fin"))[:200])
        if len(fins) < 25:
            fails.append("fin on %d soldiers (<25)" % len(fins))
        if len(letters) < 2:
            fails.append("grade distribution degenerate: %s"
                         % letters)
        for h in fins:
            f_ = h["fin"]
            if not (0 <= f_["score"] <= 100) or \
                    f_["f_score"] not in range(0, 10):
                fails.append("%s malformed fin: %s"
                             % (h["ticker"], f_))
                break
        if nvda_f and nvda_f.get("fin") and \
                nvda_f["fin"]["f_score"] < 5:
            warns.append("NVDA F-Score %s (<5) -- check payload"
                         % nvda_f["fin"]["f_score"])
        ...
        if not ok:
            ...
            _fin(rep, fails, warns)
            sys.exit(1)
        for m in ("FIN = statement grade", "Piotroski", 'id="crossboard"', "Cross Board", "fPEG",
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3092.json").write_text(json.dumps(
        {"ops": 3092, "verdict": "FAIL" if fails else "PASS",
