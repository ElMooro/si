"""ops_5305 -- RE-ARM of ops_5304, which finally named the causes: the hub's InferenceEnvironmentVariables carry no DefaultValue for most
...
UA = {"User-Agent": "justhodl-ops-5305", "Cache-Control": "no-cache", "Pragma": "no-cache"}
...
def write_verdict(status, title, t_start):
    doc = {"ops": 5305, "status": status, "title": title, "finished_at": datetime.now(timezone.utc).isoformat(), "duration_s": round(time.time() - t_start, 1),
           "steps": STEPS, "fails": [f[:300] for f in FAILS], "warns": [w[:300] for w in WARNS],
           "report": "https://github.com/ElMooro/si/blob/main/aws/ops/reports/latest/ops_5305_ai_relaunch.md"}
    try:
...
T_START = time.time()
with report("ops_5305_ai_relaunch") as R:
    R.heading("ops 5305 -- AI relaunch v1.2.3: hub env parsed for real (blank overrides killed every container), per-family images, serverless CPU retrieval endpoint, LLM 400 body")
    ...
    ctl = {"function_url": fn_url, "sagemaker_role_arn": SM_ROLE_ARN, "private_bucket": PRIVATE, "updated_at": datetime.now(timezone.utc).isoformat(), "ops": 5305}
    try:
    ...
    if D and not (D.get("inventory_errors") or {}) and (D.get("catalog") or {}).get("n"):
        try:
            ...
            for m in cat.get("article_models_present") or []:
                if "base" in m and "wiki" not in m:
                    ladder.append(m)
            for want in ("huggingface-textembedding-bge-base-en-v1-5", "huggingface-textembedding-all-MiniLM-L6-v2", "huggingface-sentencesimilarity-bge-base-en-v1-5"):
                if any(c["model_id"] == want for c in cards):
                    ladder.append(want)
            if not ladder:
                raise RuntimeError("no embedding card in the catalog: %s" % [c["model_id"] for c in cards[:10]])
            R.log("   embedding card ladder: %s" % ladder)
            sm = boto3.client("sagemaker", region_name=REGION, config=CFG)
            ep, st = None, None
            for pick in ladder:
                try:
                    spec = invoke("model", {"model_id": pick})
                    R.log("   %s: image %s | env %s | default %s | supported %s | variants %s" % (pick, spec.get("hosting_image"), json.dumps(spec.get("inference_env"))[:240], spec.get("default_inference_instance"), spec.get("supported_inference_instances"), json.dumps(spec.get("hosting_variants"))[:400]))
                except Exception as e:
                    WARNS.append("%s describe: %s" % (pick, str(e)[:120]))
                dep = None
                try:
                    dep = invoke("deploy", {"model_id": pick, "serverless": True, "serverless_memory_mb": 6144, "serverless_max_conc": 6})
                    R.log("   deployed serverless: %s" % json.dumps({k: dep.get(k) for k in ("endpoint", "artifact_how", "image", "variant", "action")})[:500])
                    R.log("      env %s" % json.dumps(dep.get("env"))[:400])
                except Exception as e:
                    WARNS.append("serverless deploy of %s failed (%s) -- trying real-time ml.m5.xlarge with a 2h TTL" % (pick, str(e)[:160]))
                    try:
                        dep = invoke("deploy", {"model_id": pick, "serverless": False, "ttl_hours": 2})
                        realtime_ep_to_delete = dep["endpoint"]
                        R.log("   deployed real-time: %s" % json.dumps({k: dep.get(k) for k in ("endpoint", "artifact_how", "image", "variant", "instance_type", "action")})[:500])
                        R.log("      env %s" % json.dumps(dep.get("env"))[:400])
                    except Exception as e2:
                        WARNS.append("real-time deploy of %s failed too: %s" % (pick, str(e2)[:160]))
                        continue
                cand = dep["endpoint"]
                t1 = time.time()
                while time.time() - t1 < 1500:
                    st = sm.describe_endpoint(EndpointName=cand)
                    if st.get("EndpointStatus") in ("InService", "Failed"):
                        break
                    time.sleep(20)
                R.log("   endpoint %s -> %s after %.0fs %s" % (cand, st.get("EndpointStatus"), time.time() - t1, (st.get("FailureReason") or "")[:240]))
                if st.get("EndpointStatus") == "InService":
                    ep = cand
                    break
                for line in endpoint_log_tail(cand):
                    R.log("      log: " + line[:260])
                WARNS.append("%s endpoint %s: %s" % (pick, st.get("EndpointStatus"), (st.get("FailureReason") or "")[:160]))
                try:
                    invoke("endpoint/delete", {"endpoint": cand})
                    R.log("   failed endpoint %s deleted" % cand)
                except Exception as e:
                    WARNS.append("could not delete failed endpoint %s: %s" % (cand, str(e)[:100]))
                if realtime_ep_to_delete == cand:
                    realtime_ep_to_delete = None
            if not ep:
                raise RuntimeError("no embedding card reached InService (ladder %s) -- log tails above" % ladder)
            est = None
            for i in range(14):
        ...
        # ---- persistent retrieval endpoint: a serverless CPU card the daily market read can query at zero idle cost
        try:
            sm = boto3.client("sagemaker", region_name=REGION, config=CFG)
            R.section("6c. serverless retrieval endpoint (playbook lens for the daily read)")
            live_serverless = None
            if ep and not realtime_ep_to_delete:
                live_serverless = ep
                R.log("   %s is already serverless -- it serves retrieval" % ep)
            else:
                for pick in ("huggingface-textembedding-bge-base-en-v1-5", "huggingface-textembedding-all-MiniLM-L6-v2", "huggingface-sentencesimilarity-all-MiniLM-L6-v2", "huggingface-sentencesimilarity-bge-small-en-v1-5"):
                    if not any(c["model_id"] == pick for c in D["catalog"].get("cards") or []):
                        continue
                    try:
                        dep2 = invoke("deploy", {"model_id": pick, "serverless": True, "serverless_memory_mb": 6144, "serverless_max_conc": 6})
                        R.log("   %s serverless: %s" % (pick, json.dumps({k: dep2.get(k) for k in ("endpoint", "artifact_how", "image", "variant", "action")})[:400]))
                    except Exception as e:
                        WARNS.append("retrieval card %s: %s" % (pick, str(e)[:160]))
                        continue
                    cand = dep2["endpoint"]
                    t9 = time.time()
                    while time.time() - t9 < 1200:
                        st2 = sm.describe_endpoint(EndpointName=cand)
                        if st2.get("EndpointStatus") in ("InService", "Failed"):
                            break
                        time.sleep(20)
                    R.log("   %s -> %s after %.0fs %s" % (cand, st2.get("EndpointStatus"), time.time() - t9, (st2.get("FailureReason") or "")[:200]))
                    if st2.get("EndpointStatus") == "InService":
                        live_serverless = cand
                        break
                    for line in endpoint_log_tail(cand):
                        R.log("      log: " + line[:260])
                    try:
                        ...
                        pass
            if live_serverless and live_serverless != ep:
                est2 = None
                for i in range(10):
                    est2 = invoke("embed", {"dataset_id": man["dataset_id"], "endpoint": live_serverless, "auto_train": False, "max_seconds": 600})
                    R.log("   retrieval embed pass %d: %s %s/%s dim %s errors %s" % (i + 1, est2.get("status"), est2.get("cursor"), est2.get("n_rows"), est2.get("dim"), est2.get("errors")))
                    if est2.get("status") == "complete":
                        break
                if not est2 or est2.get("status") != "complete":
                    WARNS.append("retrieval index incomplete on %s -- the hourly inventory resumes it" % live_serverless)
            elif not live_serverless:
                WARNS.append("no serverless retrieval endpoint came up -- the daily read runs without the playbook lens until one does")
        except Exception as e:
            WARNS.append("retrieval endpoint stage: %s" % str(e)[:200])
        finally:
    ...
    try:
        ...
        R.log("   llm_path %s | empty %s | raw %s" % (read.get("llm_path"), read.get("empty"), str(read.get("raw") or "")[:300]))
    ...
    write_verdict("RED" if FAILS else "GREEN", "AI relaunch v1.2.3 -- real env, family images, retrieval endpoint", T_START)
    if FAILS:
        sys.exit(1)
