...
def main():
    ...
    expected=subprocess.check_output(['git','log','-1','--format=%H','--','aws/shared/forward_price_measurement.py'],text=True).strip()
    ...
    def invoke(fn,event):
        ...
        return payload
    with report('ops_5721_forward_measurement_verify') as r:
