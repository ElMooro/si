"""ops 3675 — air LANDING v1.6: x-ray-informed parser. Only value-bearing
...
with report("3675_air_final") as rep:
    rep.heading("ops 3675 — air landing v1.6")
    out = {"gates": {}}
    fails = []
    import traceback
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3675.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
    ...
    Path("aws/ops/reports/3675.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
