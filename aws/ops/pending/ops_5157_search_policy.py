"""ops_5157 -- 5156 follow-up: exact symbols that carry a signal were filtered out of the instant list (SPY/MSFT); probe waits for the answered query.
...
def http(url, timeout=180):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5157", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5157-search-policy") as r:
        r.heading("ops 5157 -- search ranking authority + mobile composition (final)")
        fails = []
        ...
        while time.time() - t0 < 900:
            try:
                ...
                if b"ops 5157: a symbol that also carries a JustHodl signal" in c:
                    live = True
                    break
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                mp.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5157-mobile.jpg"), type="jpeg", quality=55)
