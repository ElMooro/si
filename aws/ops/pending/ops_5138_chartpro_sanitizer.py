"""ops_5138 -- chart-pro: candle sanitizer (finite OHLC, unique ascending times) + pane disposal; deep stacks (Error.stackTraceLimit=80) if anything remains.
...
def http(url, timeout=60):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5138", "Accept-Encoding": "identity"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def main():
    with report("5138-chartpro-sanitizer") as r:
        r.heading("ops 5138 -- chart-pro: sanitized candles, disposed panes, deep stacks")
        fails = []
        ...
        while time.time() - t0 < 900 and not all(got.values()):
            try:
                ...
                got["chart"] = b"function jhCleanBars(bars) {" in c
        ...
        if got["chart"]:
            ...
            with sync_playwright() as p:
                ...
                page.on("pageerror", lambda e: errors.append({"msg": str(e)[:120], "sym": page.evaluate("() => State.activeTicker"),
                                                                "frames": [ln.strip()[:160] for ln in (getattr(e, "stack", "") or "").split("\n") if "unpkg" not in ln and ln.strip().startswith("at ")][:8],
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5138-chartpro-native.jpg"), type="jpeg", quality=60)
