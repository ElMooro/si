"""ops_5130 -- every symbol from the warehouse: closure (5128: US:/X:/I: bank keys had no Yahoo mapping on re-bank; volume histogram null).
...
def http(url, timeout=180):
    req = urllib.request.Request(url, headers={"User-Agent": "justhodl-ops-5130", "Accept-Encoding": "identity"})
    t = time.time()
...
def main():
    with report("5130-every-symbol-from-s3") as r:
        r.heading("ops 5130 -- every symbol from the warehouse: closure (daily bars for every bank key, volume, no page errors)")
        fails = []
        ...
        if h.get("version") != "1.2.1":
            fails.append(f"symdir version {h.get('version')} != 1.2.1")
        http_json(url + "/warm", timeout=180)
        # the bucket denies DeleteObject (doctrine: data is never deleted) -- re-bank the monthly docs IN PLACE:
        # a daily pull's timestamps coincide with the monthly bars' month-start days, so the union overwrites them
        idx, _ = get_json("data/warm/tv-bars/universe/_index.json")
        syms = sorted((idx or {}).get("symbols") or {})
        rebanked = 0
        for i in range(0, len(syms), 6):
            chunk = syms[i:i + 6]
            resp = lam.invoke(FunctionName="justhodl-tv-bars", InvocationType="RequestResponse", Payload=json.dumps({"mode": "pull", "tv_symbols": chunk, "budget": 30}).encode())
            body = json.loads(resp["Payload"].read() or b"{}")
            for sym, res in (body.get("results") or {}).items():
                r.log(f"  re-bank {sym:<18} ok={res.get('ok')} n={res.get('n')} first={res.get('first')} src={res.get('source')} err={res.get('error')}")
                rebanked += 1 if res.get("ok") else 0
        r.kv(step="rebank", symbols=len(syms), rebanked=rebanked)
        ...
        samples = [("TVC:VIX", "1995-01-01"), ("AAPL", "1995-01-01"), ("NASDAQ:AAPL", "1995-01-01"), ("SSE:000001", None), ("FX:EURUSD", None),
                   ...
                   ("BRK.B", None), ("AMEX:SPY", "1995-01-01"), ("OTC:AAAIF", None), ("XETR:DAX", None), ("NVDA", "1999-01-25")]
        banked = 0
        for sid, inception in samples:
            try:
                ...
                if sid in ("AAPL", "AMEX:SPY", "I:SPX", "NVDA", "BRK.B", "X:BTCUSD") and n < 2000:
        ...
        while time.time() - t0 < 900:
            try:
                ...
                if b"AUTO mode (ops 5125)" in body and b"b.volume || 0)), color" in body:
                    page_ok = True
                    break
        ...
        if not page_ok:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5130-chartpro-native.jpg"), type="jpeg", quality=60)
