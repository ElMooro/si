"""
ops_4016 — ship v1.5.0: rebuild the zip Khalid downloads, settle the
...
def main():
    with report("4016_ship_v150") as rep:
        rep.heading("ops 4016 — extension v1.5.0 + sources persistence")
        checks = []
        ...
        checks.append(("zip carries v1.5.0", man.get("version") == "1.5.0"))
        inj = chk.read("inject.js" if rooted
                       else "chrome-extension/inject.js").decode()
        checks.append(("zip inject taps symbol metadata",
                       "symbol_search|scanner" in inj and "domSrc" in inj))
