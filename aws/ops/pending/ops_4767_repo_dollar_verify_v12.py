...
def main():
    with report("4767_repo_dollar_verify_v12") as rep:
        rep.heading("ops 4766 -- dollar fix verify (v1.2 retry-invoke)")
        got = None
        for i in range(9):
        ...
        if not got:
            rep.warn("deploy of v1.2 not observed after ~3 min of retries")
            return
...
# appended check: the self-healed warm bank itself
