...
UA = "JustHodl ops4932 edge-acceptance"
...
def main():
    ...
    fails, notes = [], []
    with report("4932-usd-funding-deploy") as r:
        r.heading("ops 4932 — justhodl-usd-funding deploy (in-ops) + accept")
        ...
        st, txt = edge("https://justhodl.ai/data.html")
        ...
        if fails:
            ...
            r.ok("all gates green" + (" (notes: %s)" % ", ".join(notes)
                                      if notes else ""))
        (ROOT / "aws" / "ops" / "reports" / "4932.json").write_text(
