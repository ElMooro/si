...
both=v.get("sp_table",[])+(v.get("hp_out",[]) or [])
from collections import Counter
tier=sum(1 for r in both if r.get("ev_tier"))
print(f"ev_tier coverage: {tier}/{len(both)} | dist:", dict(Counter(r.get('ev_tier') for r in both if r.get('ev_tier'))))
for t in ["KO","PG","JNJ","UNH","CAT"]:
    r=next((x for x in both if x.get("t")==t),None)
    if r: print(f"  {t}: ev_ebitda={r.get('ev_ebitda')} sec={r.get('sector')} med={r.get('sector_ev_ebitda')} tier={r.get('ev_tier')}")
