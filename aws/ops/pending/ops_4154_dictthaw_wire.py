"""ops_4154 — cot-feed create/settle/invoke + vault v3.21.0 + exact spots
...
def main():
    with report("4154_dictthaw_wire") as rep:
        ...
        checks.append(("resolved >= 235",
                       (fd.get("resolved") or 0) >= 235))
        p = fd.get("prices") or {}
        picks = sorted(p)[:1] + sorted(p)[len(p) // 2:len(p) // 2 + 1]
        for bare in picks:
            rec = p.get(bare) or {}
            rep.log(f"  spot {bare}: v={rec.get('value')} "
                    f"col={rec.get('col')} asof={rec.get('asof')}")
            checks.append((f"spot {bare} plausible",
                           isinstance(rec.get("value"), (int, float))
                           and rec.get("asof", "").startswith("2026")))
        st4, x4 = 0, ""
        try:
            import urllib.request as _u2
            with _u2.urlopen(
                "https://publicreporting.cftc.gov/resource/"
                "yw9f-hn96.json?cftc_contract_market_code=132741"
                "&$limit=1", timeout=12) as r4:
                x4 = r4.read().decode()[:200]
        except Exception as e4:
            x4 = str(e4)[:100]
        rep.log("  whale 132741 probe (yw9f): " + x4[:180])
        ...
        checks += [("feed:cot >= 180", nc >= 180),
                   ("total LIVE >= 3450", live >= 3450)]
