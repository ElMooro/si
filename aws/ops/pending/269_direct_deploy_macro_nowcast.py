#!/usr/bin/env python3
"""Step 269 — Direct deploy of justhodl-macro-nowcast (bypass deploy-lambdas.yml).
...
REPORT_PATH = "aws/ops/reports/269_macro_nowcast_direct_deploy.json"
...
def deploy_lambda():
    ...
    try:
        ...
        print(f"[269] Lambda exists, updating ({len(zip_bytes):,}b)")
    ...
    print(f"[269] creating Lambda {LAMBDA_NAME} ({len(zip_bytes):,}b)…")
    lam.create_function(
        FunctionName=LAMBDA_NAME,
        Runtime="python3.12",
        Handler="lambda_function.lambda_handler",
...
def invoke_and_verify():
    print(f"[269] sync invoking {LAMBDA_NAME}…")
    started = time.time()
