"""Drive the cache-build through all 26k entries. Each Lambda invocation does as
many 150-chunks as it can in ~300s; we invoke repeatedly until done. From AWS."""
import json, os, time, zipfile, io
import boto3
from botocore.config import Config
cfg=Config(read_timeout=350,retries={"max_attempts":1})
lam=boto3.client("lambda",region_name="us-east-1",config=cfg)
ROLE="arn:aws:iam::857687956942:role/lambda-execution-role"; FN="tmp-rb"
code=r'''
...
def lambda_handler(e,c):
    ...
    while time.time()<deadline:
        ...
        off=d.get("scanned_to",off+150); out["kept"]=d.get("kept"); out["total"]=d.get("total_index"); out["scanned_to"]=off
...
try:
    ...
    off=0
    for inv in range(5):  # up to 5 invocations × 300s each
...
open("aws/ops/reports/1417_rb.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
