"""ops_4040 — FULL CHECK v2: everything from 4034 PLUS note-fidelity —
...
MARK = "tv-workbench v1.1.2 ops4039 uncut"
...
def main():
    with report("4040_fidelity_final") as rep:
        ...
        for _i in range(12):
            ...
            if wb.get("marker") == MARK:
                rep.ok(f"  v1.1.2 artifact after ~{_i*15}s")
                break
            _t.sleep(15)
        checks.append(("workbench is v1.1.2 bare-union",
                       wb.get("marker") == MARK))
