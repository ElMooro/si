"""1259 — recheck compounder + cap coverage after fix."""
import json, time
from datetime import datetime, timezone
import boto3
from botocore.config import Config
REPORT="aws/ops/reports/1259_compounder.json"; BUCKET="justhodl-dashboard-live"; REGION="us-east-1"
...
try:
    ...
    print("caps:",capdist,"\nwith_comp:",len(comp))
    for r in comp[:12]: print(f"  {r['ticker']:<6s} comp={r.get('compounder_score')} GO={r.get('growth_opportunity_score')} expG={(r.get('growth_intel') or {}).get('expected_company_growth_pct')}% [{r.get('verdict')}] {r.get('cap_bucket')}")
