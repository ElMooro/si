"""ops 3633 — forensic + hardened retry. Unzipped-marker check on the
...
with report("3633_bucket_cum") as rep:
    rep.heading("ops 3633 — deployed-zip forensic + v1.7.1/v2.5")
    out = {"gates": {}}
    fails = []
    ...
    try:
        err = dep_inv("justhodl-china-liquidity", 300, 768,
                      "china-liquidity v2.7: cumulative-phrasing patterns + phrase_ctx dump")
        ...
        gate("G2_cn", (got or cn.get("phrase_ctx") or cn.get("js_files") is not None
             ...
             f"cum={cn.get('cumulative')} phrase_ctx={str(cn.get('phrase_ctx'))[:220]} refs={[r[-34:] for r in (cn.get('content_refs') or [])[:3]]} probe={str(cn.get('body_probe'))[180:560]} via={cn.get('via')} note={cn.get('error')}")
        out["cn"] = cn
    except Exception as e:
    ...
    Path("aws/ops/reports/3633.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
