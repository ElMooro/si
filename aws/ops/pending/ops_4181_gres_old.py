"""ops_4181 — GRES + COT _OLD micro-wires: settle three, invoke two, gate."""
import io
import json
import sys
import time
...
def main():
    with report("4181_gres_old") as rep:
        rep.heading("ops 4181 — GRES + old-crop")
        ...
        checks.append(("GRES >= 90", (c.get("GRES") or 0) >= 90))
        r2 = lam.invoke(FunctionName="justhodl-cot-feed",
                        InvocationType="RequestResponse", Payload=b"{}")
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok(f"MICRO-WIRED — GRES={c.get('GRES')} "
