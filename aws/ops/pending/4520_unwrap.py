...
out=f"totals={json.dumps(hub.get('totals'))} fred={json.dumps({k:fred.get(k) for k in ('n_keys','hot_feeds','total_mb')})} top={json.dumps(top)[:300]} zero={json.dumps(zero)[:200]}"
os.makedirs("aws/ops/reports",exist_ok=True)
open("aws/ops/reports/4520_unwrapx.md","w").write("# 4518 final join — "+out+"\n")
print(out)
