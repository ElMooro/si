#!/usr/bin/env python3
"""ops 3100 -- SHARE-FLOWS v1.3.0 (Khalid: add P/E + search-bar
...
AAPL external truth bands: mcap 2-6T, shares 13-17B, PE 20-55,
price 100-500."""
import json
import sys
import time
...
UA = {"User-Agent": "Mozilla/5.0 ops-3100",
...
def main():
    fails, warns = [], []
    with report("3100_share_flows_v13") as rep:
        ...
        for _ in range(40):
            ...
            try:
                ...
                if (datetime.now(timezone.utc) - o["LastModified"]
                    ...
                    if cand.get("version") == "1.3.0":
                        d = cand
                        break
            except Exception:
                pass
        if not d:
            fails.append("no fresh v1.3.0 share-flows.json")
            _fin(rep, fails, warns)
            sys.exit(1)
        tk = d.get("tickers") or {}
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3100.json").write_text(json.dumps(
        {"ops": 3100, "verdict": "FAIL" if fails else "PASS",
