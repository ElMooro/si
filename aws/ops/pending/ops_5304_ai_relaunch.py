"""ops_5304 -- RE-ARM of ops_5303: its serverless RoBERTa-SEC endpoint died at model load (\"model process exited\") with an empty log tail
...
UA = {"User-Agent": "justhodl-ops-5304", "Cache-Control": "no-cache", "Pragma": "no-cache"}
...
def write_verdict(status, title, t_start):
    doc = {"ops": 5304, "status": status, "title": title, "finished_at": datetime.now(timezone.utc).isoformat(), "duration_s": round(time.time() - t_start, 1),
           "steps": STEPS, "fails": [f[:300] for f in FAILS], "warns": [w[:300] for w in WARNS],
           "report": "https://github.com/ElMooro/si/blob/main/aws/ops/reports/latest/ops_5304_ai_relaunch.md"}
    try:
...
T_START = time.time()
with report("ops_5304_ai_relaunch") as R:
    R.heading("ops 5304 -- AI relaunch v1.2.2: paged endpoint log tails, embedding-card fallback ladder (RoBERTa-SEC -> BGE -> MiniLM), LLM path diagnostics")
    ...
    ctl = {"function_url": fn_url, "sagemaker_role_arn": SM_ROLE_ARN, "private_bucket": PRIVATE, "updated_at": datetime.now(timezone.utc).isoformat(), "ops": 5304}
    try:
        ...
        try:
            ...
            for pick in ladder:
                try:
                    ...
                    R.log("   %s: image %s | env %s | default instance %s" % (pick, spec.get("hosting_image"), json.dumps(spec.get("inference_env"))[:300], spec.get("default_inference_instance")))
                except Exception as e:
                ...
                try:
                    ...
                    R.log("   deployed serverless: %s" % json.dumps({k: dep.get(k) for k in ("endpoint", "artifact_how", "model_data")})[:400])
                except Exception as e:
                    ...
                    try:
                        ...
                        R.log("   deployed real-time: %s" % json.dumps({k: dep.get(k) for k in ("endpoint", "artifact_how", "model_data", "instance_type")})[:400])
    ...
    write_verdict("RED" if FAILS else "GREEN", "AI relaunch v1.2.2 -- log tails, card ladder, LLM diagnostics", T_START)
    if FAILS:
        sys.exit(1)
