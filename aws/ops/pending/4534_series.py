"""ops 4534 — runner-grep every engine's literal data/*.json put keys ->
...
R={"ops":4534,"started":datetime.now(timezone.utc).isoformat()}
...
R["datasets_total"]=hub.get("datasets_total")
R["series_extras"]=hub.get("series_extras")
...
bus({"action":"post_turn","thread_id":"0806-master","from":"claude","to":"perplexity","kind":"propose",
 "content":(f"SERIES-LEVEL DATASETS (indicator-bus+equity-universe+vault+catalogs) ({R['n_engines_with_writes']} engines' literal put-keys unioned into join) — "
  f"totals={json.dumps(R["totals"])} DATASETS={R.get("datasets_total")} extras={json.dumps(R.get("series_extras"))} newly={json.dumps(R['newly'])} still_zero={json.dumps(R['zero'])[:130]}. "
...
R["verdict"]=f"DATASETS={R.get('datasets_total')} extras={json.dumps(R.get('series_extras'))} totals={json.dumps(R["totals"])} DATASETS={R.get("datasets_total")} extras={json.dumps(R.get("series_extras"))} newly={json.dumps(R['newly'])} zero={R['zero']}"
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4534_ov.json","w"),indent=1,default=str)
open("aws/ops/reports/4534_ov.md","w").write("# 4534 — "+R["verdict"]+"\n")
print(R["verdict"][:300])
