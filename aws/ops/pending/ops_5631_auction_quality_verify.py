...
COMMIT = "45d7a7b0952575ced83cfb4ebf6f02460b24a908"
...
def main():
    ...
    with report("ops_5631_auction_quality_verify") as r:
        ...
        for FN, KEY in TARGETS:
            ...
            assert quality.get("status") == "fresh", "Current auction observations unavailable"
            assert feed.get("call") is None, "Unvalidated directional call"
            ...
            r.kv(function=FN, methodology=feed["methodology_version"], generated_at=feed["generated_at"],
                 latest_auction_date=feed.get("freshness", {}).get("latest_auction_date"), quality_status=quality["status"])
