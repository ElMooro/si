"""ops_5136 -- chart-pro hotfix: 5135 monkeypatched the frozen LightweightCharts namespace and broke every native render (TypeError -> widget). createChart is wrapped instead.
...
def http(url, timeout=60):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5136", "Accept-Encoding": "identity"})
    with urllib.request.urlopen(req, timeout=timeout) as r:
        return r.read()


def main():
    with report("5136-chartpro-hotfix") as r:
        r.heading("ops 5136 -- chart-pro hotfix: native renders restored, instances tracked via a wrapper")
        fails = []
        ...
        if got["chart"]:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5136-chartpro-native.jpg"), type="jpeg", quality=60)
