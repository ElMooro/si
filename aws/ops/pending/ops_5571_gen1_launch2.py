...
def main() -> int:
    ...
    with report("ops_5571_gen1_launch2") as R:
        R.heading("ops 5571 -- generation 1 (floor -> 500 unique tasks): feasible family share for two families, then the Gear B tick (launch=true) via the owner route")
        R.kv(head=head[:10])
        ...
        control["written_by"] = "ops 5571"; control["previous_written_by"] = before and "ops 5542"; control["git_sha"] = head[:12]
        ...
        R.ok("gearb control: %s -> max_family_share 0.85, min_sft_rows 500 (dated copy kept)" % json.dumps(before))
        ...
        event = {"version": "2.0", "rawPath": "/gearb/tick", "rawQueryString": "", "requestContext": {"http": {"method": "POST", "path": "/gearb/tick"}},
                 "headers": {"x-jh-service-token": token, "x-jh-factory-role": "owner", "x-jh-factory-uid": "ops-5571", "content-type": "application/json"},
