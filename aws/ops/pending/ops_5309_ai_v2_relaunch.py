...
UA = {"User-Agent": "justhodl-ops-5309", "Cache-Control": "no-cache", "Pragma": "no-cache"}
...
def write_verdict(status, title, t_start):
    doc = {"ops": 5309, "status": status, "title": title, "finished_at": datetime.now(timezone.utc).isoformat(), "duration_s": round(time.time() - t_start, 1),
           "steps": STEPS, "fails": [f[:300] for f in FAILS], "warns": [w[:300] for w in WARNS],
           "report": "https://github.com/ElMooro/si/blob/main/aws/ops/reports/latest/ops_5309_ai_v2_relaunch.md"}
    try:
...
T_START = time.time()
with report("ops_5309_ai_v2_relaunch") as R:
    ...
    ctl = {"function_url": fn_url, "sagemaker_role_arn": SM_ROLE_ARN, "private_bucket": PRIVATE, "updated_at": datetime.now(timezone.utc).isoformat(), "ops": 5309}
    try:
    ...
    write_verdict("RED" if FAILS else "GREEN", "AI v2.2.1 -- fallback voice, lessons, scoreboard; pipeline restarted", T_START)
    if FAILS:
        sys.exit(1)
