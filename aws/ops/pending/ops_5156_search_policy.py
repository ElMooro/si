"""ops_5156 -- the external Chart Pro audit's acceptance tests, made true.
...
def http(url, timeout=180):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5156", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5156-search-policy") as r:
        r.heading("ops 5156 -- search ranking authority + mobile composition (the audit's acceptance tests)")
        fails = []
        ...
        while time.time() - t0 < 900:
            try:
                ...
                if b"ops 5156: the backend order is authoritative" in c and b"ops 5156: real mobile composition" in c:
                    live = True
                    break
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                for q, want in (("AAPL", "AAPL"), ("SPY", "SPY"), ("BTC", "X:BTCUSD"), ("MSFT", "MSFT"), ("crude", "fred:DCOILWTICO")):
                    page.click("#search-input")
                    page.fill("#search-input", "")
                    page.type("#search-input", q, delay=25)
                    for _ in range(30):
                        page.wait_for_timeout(500)
                        if page.evaluate("() => !!(HeaderSearch._facets)"):
                            break
                ...
                mp.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5156-mobile.jpg"), type="jpeg", quality=55)
