"""
ops/4747 -- family-index fix on 4746: spec paths all start with /api/,
so segment[0] was "api" for every path and each family matched 0
templates. Family = segment AFTER api. Everything else identical.
...
def main():
    with report("4747_nyfed_spec_fam_fix") as rep:
        ...
        fams_all = {}
        for p in paths:
            fams_all.setdefault(fam_of(p), 0)
            fams_all[fam_of(p)] += 1
        ...
        for fam in FAMS:
            fam_paths = [p for p in paths if fam_of(p) == fam]
