"""
ops 3726 — readthrough.html: surface the engine fields the page never showed
  ...
  document.getElementById('picks').innerHTML = picks.length ?
    picks.map(function(p){
      ...
      return '<div class="card" style="margin-bottom:8px">'+
        ...
        ' · gap '+pc(p.gap_pts)+
        (p.materiality!=null?' · materiality '+p.materiality.toFixed(3)+'×':'')+
        '</div>'+
        (p.materiality_note?'<div class="mono" style="font-size:11.5px;color:'+MUT+'">'+esc(p.materiality_note)+'</div>':'')+
        '<div class="mono" style="font-size:12px">'+
        (p.quadrant_note?esc(p.quadrant_note):'')+
        '</div>'+
...
NEW_CELLS = '''      '</td><td class="mono">'+(r.capture_ratio==null?'—':r.capture_ratio.toFixed(2))+
      '</td><td class="mono" title="'+esc(r.materiality_note||'')+'">'+(r.materiality==null?'—':(r.materiality).toFixed(3)+'×')+
...
def main():
    with report("3726_readthrough_page_fix") as rep:
        rep.heading("ops 3726 — readthrough.html: surface the hidden engine fields")
        fails = []
        out = {}
        ...
        have = {
            ...
            "materiality": any(r.get("materiality") is not None for r in rows),
            "materiality_note": any(r.get("materiality_note") for r in rows),
            "quadrant_note": any(r.get("quadrant_note") for r in rows),
        ...
        MUST = ["consensus_coverage", "quadrant_note", "consensus_observed",
                ...
                "rpo_representative", "book_to_bill_spread_pct",
                "materiality", "materiality_note", "top_picks", "capture_share", "edge_source"]
