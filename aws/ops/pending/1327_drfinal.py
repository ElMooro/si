...
out["invoke"]=r.get("Payload").read().decode()[:130]; time.sleep(2)
d=json.loads(s3.get_object(Bucket="justhodl-dashboard-live",Key="data/crypto-cycle-risk.json")["Body"].read())
out["score"]=d.get("dump_risk_score"); out["level"]=d.get("risk_level"); out["nkeys"]=len(d.get("factors",{}))
fac=d.get("factors",{})
for k in ["inflation_print","etf_flows","ai_rotation"]: out[k]=(fac.get(k,{}).get("note") or "MISSING")[:110]
out["drivers"]=[x["note"][:90] for x in d.get("top_drivers",[])]
open("aws/ops/reports/1327_dr.json","w").write(json.dumps(out,indent=2,default=str)); print("done")
