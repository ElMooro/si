"""1291 — deploy justhodl-ask w/ Function URL, redeploy best-setups, verify all."""
...
try:
    ...
    try:
        lam.get_function_configuration(FunctionName="justhodl-ask"); lam.update_function_code(FunctionName="justhodl-ask",ZipFile=zb); act="updated"
    except lam.exceptions.ResourceNotFoundException:
        lam.create_function(FunctionName="justhodl-ask",Runtime="python3.12",Role=ROLE,Handler="lambda_function.lambda_handler",
            Code={"ZipFile":zb},Description="NLQ over JustHodl system",Timeout=60,MemorySize=512,Architectures=["x86_64"],Publish=False); act="created"
    for _ in range(30):
    ...
    # Function URL
    try:
        u=lam.create_function_url_config(FunctionName="justhodl-ask",AuthType="NONE",Cors={"AllowOrigins":["*"],"AllowMethods":["*"],"AllowHeaders":["*"]})
        ask_url=u["FunctionUrl"]
    except lam.exceptions.ResourceConflictException:
        ask_url=lam.get_function_url_config(FunctionName="justhodl-ask")["FunctionUrl"]
    try:
        lam.add_permission(FunctionName="justhodl-ask",StatementId="fnurl",Action="lambda:InvokeFunctionUrl",Principal="*",FunctionUrlAuthType="NONE")
    except lam.exceptions.ResourceConflictException: pass
    out["ask_url"]=ask_url
except Exception as e: out["ask_err"]=str(e)[:300]

# ── Redeploy best-setups (for why) + invoke ──
try:
    zb=zipdir("aws/lambdas/justhodl-best-setups/source")
    lam.update_function_code(FunctionName="justhodl-best-setups",ZipFile=zb)
    for _ in range(20):
    ...
    out["best_setups_why"]=s0.get("why")
except Exception as e: out["bs_err"]=str(e)[:200]

# ── Test the ask Lambda directly ──
try:
    r=lam.invoke(FunctionName="justhodl-ask",InvocationType="RequestResponse",
        Payload=json.dumps({"q":"what are today's highest conviction setups?"}).encode())
    ...
    out["ask_test"]={"answer":(inner.get("answer") or "")[:200],"n_results":len(inner.get("results",[]))}
except Exception as e: out["ask_test"]=str(e)[:200]

# ── Vintage data check (the #1 fix) ──
try:
