"""ops_4218 — WAVE-3: router/tape/activity bus-wrapped, verified, ledger+3."""
import io
import json
import re
import sys
...
def main():
    with report("4218_wave3") as rep:
        ...
        for name, blk, outk in TRIO:
            ...
            if name.endswith("router") and has:
                bl = d[blk]
                rep.log("  router bus_macro: " + json.dumps(bl)[:200])
                wm = bl.get("world_policy_rate_median")
                checks.append(("world rate median 1-12",
                               isinstance(wm, (int, float))
                               and 1 < wm < 12))
            if name.endswith("tape") and has:
                rep.log("  tape added: "
                        + json.dumps(d[blk].get("added")))
                checks.append(("tape added >= 2",
                               len(d[blk].get("added") or []) >= 2))
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok("WAVE-3 LIVE — router sees the world rate, homepage "
               "tape carries bus, activity bridged")
