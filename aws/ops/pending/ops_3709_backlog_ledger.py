"""ops 3709 — Stage 1 of Khalid's reverse-engineered-backlog theory.
...
with report("3709_backlog_ledger") as rep:
    rep.heading("ops 3709 — backlog reverse-engineering, stage 1")
    out = {"gates": {}}
    fails = []
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3709.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        # the whole point of 3709: no row may claim consensus has not moved when
    ...
    Path("aws/ops/reports/3709.json").write_text(json.dumps(out, indent=2, default=str))
