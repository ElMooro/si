#!/usr/bin/env python3
"""ops 3115 -- QUALITY BAR PROPAGATION (Khalid: make master-rank /
...
GROWING / MIXED / SHRINKING verdict). Static-marker verify
...
UA = {"User-Agent": "Mozilla/5.0 ops-3115", "Cache-Control": "no-cache"}
...
def main():
    fails, warns = [], []
    with report("3115_quality_propagation") as rep:
        rep.section("1. Docs joinable")
        try:
            ir = json.loads(S3.get_object(
                Bucket=BUCKET, Key="data/industry-rotation.json"
            )["Body"].read())
            pools = (ir.get("leaders") or []) + (ir.get("regiments")
                                                 or [])
            named = [l for l in pools
                     if l.get("industry") or l.get("name")]
            rep.kv(ir_named_industries=len(named))
            if len(named) < 10:
                fails.append("industry-rotation has too few named "
                             "industries: %d" % len(named))
            scored = sum(1 for l in named
                         if l.get("score") is not None
                         or l.get("total_score") is not None)
            if scored < 10:
                fails.append("industries without scores: only %d "
                             "scored" % scored)
        ...
        checks = {
            ...
            "accumulation.html": ("jh-fund-chips.js",
                                  "data-jhf-read"),
            "why.html": ("Industry health", "SHRINKING / IN TROUBLE",
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3115.json").write_text(json.dumps(
        {"ops": 3115, "verdict": "FAIL" if fails else "PASS",
