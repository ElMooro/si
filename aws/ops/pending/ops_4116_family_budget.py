"""ops_4116 — deterministic: raw vault logs, then self-invoke + poll to
...
MARK = "tradingview-vault v3.15.1 ops4116 family-budget"
...
def main():
    with report("4116_family_budget") as rep:
        rep.heading("ops 4115 — deterministic family run")

        rep.section("A0. settle v3.15.1 (pipeline may also be deploying)")
        import io
        ...
        for _ in range(40):
            ...
            try:
                c = lam.get_function_configuration(
                    FunctionName="justhodl-tradingview")
                if c.get("State") == "Active" and \
                        c.get("LastUpdateStatus") != "InProgress":
                    dep = zf.ZipFile(io.BytesIO(_ur.urlopen(
                        lam.get_function(FunctionName="justhodl-tradingview")
                        ["Code"]["Location"], timeout=60).read())).read(
        ...
        rep.kv(settled=ok)
        if not ok:
            rep.fail("v3.15.1 never settled")
            sys.exit(1)
