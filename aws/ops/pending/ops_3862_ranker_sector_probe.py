...
def main():
    with report("3862_ranker_sector_probe") as rep:
        rep.heading("ops 3862 — PROBE: master-ranker sector resolution, live (no code written)")
        ...
        rows = mr.get("ranked") or mr.get("top_actions") or mr.get("rankings") or []
        if not rows:
            keys = sorted(k for k, v in mr.items() if isinstance(v, list))
            rep.log(f"  list-bearing keys: {keys}")
            rows = mr.get(keys[0]) if keys else []
