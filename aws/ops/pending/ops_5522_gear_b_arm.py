...
def main() -> int:
    ...
    with report("ops_5522_gear_b_arm") as R:
        R.heading("ops 5522 -- arm Gear B: budget, coder card, owner control, holdout freeze dispatch, refusal chain")
        R.kv(head=head[:10], daily_usd=DAILY_USD, season_usd=SEASON_USD)

        # 1. receipt for this push
        ...
        while time.time() < deadline:
            ...
            if str(rc.get("commit", "")) == head:
                ...
                (R.ok if ok else R.fail)("%s receipt commit=%s sha_match=%s run=%s" % (FUNC, head[:7], ok, rc.get("run_id")))
                if not ok:
                    red.append("receipt")
                seen = True
                break
        ...
        if not seen:
            ...
            R.fail("no release receipt for %s within %d min" % (head[:7], RECEIPT_WAIT_S // 60))
        if red:
        ...
        control = {"schema_version": "gearb-control.v1", "enabled": bool(model_id), "model_id": model_id or "unresolved", "model_version": None,
                   ...
                   "approved_by": APPROVAL, "approved_at": now, "season_id": "season-2026-09-14", "written_by": "ops 5522", "git_sha": head[:12],
