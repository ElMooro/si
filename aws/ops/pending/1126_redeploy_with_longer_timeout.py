"""ops 1126 — Redeploy auction-interpreter (now with 90s Claude timeout) + reinvoke + verify."""
...
def main():
    ...
    try:
        ...
        for p in (brief.get("historical_predictions") or []):
            rpt["predictions"].append({
                "asset": p.get("asset"), "ticker": p.get("ticker"),
    ...
    p = os.path.join(REPO_ROOT, "aws/ops/reports/1126.json")
