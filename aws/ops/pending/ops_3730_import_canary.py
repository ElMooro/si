#!/usr/bin/env python3
"""ops 3730 — SHIP justhodl-import-canary v1.0 (canary list item #1).
...
with report("3730_import_canary") as rep:
    rep.heading("ops 3730 — SHIP justhodl-import-canary v1.0 (import flow canary)")
    fails = []
    out = {"gates": {}}
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3730.json").write_text(json.dumps({"verdict": "STARTED"}))
    ...
    try:
        ...
        try:
            ...
            bod = json.loads(payload.get("body", "{}")) if payload.get("body") else {}
            month = bod.get("month")
            gate("G3_invoke", sc == 200 and bool(month),
                 "status=%s month=%s lines=%s signals=%s"
                 % (sc, month, bod.get("lines"), bod.get("signals")))
        except Exception as e:
            gate("G3_invoke", False, "%s %s" % (type(e).__name__, str(e)[:220]))
        ...
        Path("aws/ops/reports/3730.json").write_text(json.dumps(out, indent=2))
        ...
        Path("aws/ops/reports/3730.json").write_text(
