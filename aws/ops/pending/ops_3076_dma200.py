#!/usr/bin/env python3
"""ops 3076 -- 200DMA BREAKS added to the reversal boards (Khalid): fresh break ABOVE the 200DMA = dated bottom trigger (Weinstein Stage 2), fresh break BELOW = dated top trigger (Stage 4); both feed the >=1.5x volume CONFIRMED gate; pct-vs-200DMA on every card. Engine v1.4.1. Base arc
...
UA = {"User-Agent": "Mozilla/5.0 ops-3076",
...
def main():
    fails, warns = [], []
    with report("3076_dma200") as rep:
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3076.json").write_text(json.dumps(
        {"ops": 3076, "verdict": "FAIL" if fails else "PASS",
