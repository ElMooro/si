#!/usr/bin/env python3
"""ops 3046 (diagnose+verify) -- PHASE DETECTOR (Khalid): dated BEGIN/END of accumulation
...
def main():
    fails, warns = [], []
    with report("3046_phase_detector") as rep:
        ...
        try:
            ...
            req = urllib.request.Request(
                ...
                headers={"User-Agent": "Mozilla/5.0 ops-3046"})
...
def _fin(rep, fails, warns, extra):
    payload = {"ops": 3046, "fails": fails, "warns": warns,
               "verdict": "FAIL" if fails else "PASS",
               "ts": datetime.now(timezone.utc).isoformat()}
    payload.update(extra)
    (AWS_DIR / "ops" / "reports" / "3046.json").write_text(
