#!/usr/bin/env python3
"""ops 2976 -- FLEET AUDIT (count-fix rerun of 2974: engine-wiring.json
stores wired/orphaned as ROW ARRAYS in its current format; normalize to
counts so totals stay compact and the page tiles render numbers; also
stamp wiring freshness onto covered gap items so covered-but-STALE is
visible -- e.g. NAAIM exists but its feed is stale): every engine, every page, audited in one
...
def main():
    fails, warns = [], []
    hl = {}
    with report("2976_fleet_audit_fix") as rep:
        import boto3
        ...
        wiring, trust = {}, {}
        wired_n, orph_n = None, None
        try:
            wiring = json.loads(s3.get_object(
                Bucket=BUCKET, Key="data/engine-wiring.json")
                ["Body"].read())
            _w, _o = wiring.get("wired"), wiring.get("orphaned")
            wired_n = len(_w) if isinstance(_w, list) else _w
            orph_n = len(_o) if isinstance(_o, list) else _o
            rep.kv(wired=wired_n, orphaned=orph_n)
        ...
        for item in GAP_MATRIX:
            ...
            if ev:
                row["evidence"] = ev
                stale_hit = [r for r in (wiring.get("wired") or [])
                             if isinstance(r, dict)
                             and ev.lower() in (r.get("feed") or "").lower()
                             and r.get("freshness_at_wiring") == "STALE"]
                if stale_hit:
                    row["caveat"] = "covered but feed STALE -- repair, "                                     "not build"
                covered.append(row)
            else:
                gaps.append(row)
        ...
        # -- ops 2981: stamp ground truth from the build arc (2977-2980) --
        SHIPPED_MAP = {
            "M1": ("data/sloos.json", "lce.html"),
            "M4": ("data/bill-share.json", "treasury-noise.html"),
            "M9": ("data/implied-corr.json", "vol.html"),
            "M11": ("data/revision-breadth.json", "eps-velocity.html"),
            "M13": ("data/stock-bond-corr.json", "correlation.html"),
            "M14": ("data/global-m2.json", "global-macro.html"),
            "M15": ("data/miner-margin.json", "metals-miners.html"),
            "M16": ("data/ofr-fsi.json", "ofr.html"),
            "M19": ("data/em-carry.json", "dollar.html"),
            "M20": ("data/baltic-dry.json", "activity-nowcast.html"),
            "M22": ("data/muni-ratio.json", "credit-desk.html"),
        }
        still_open = []
        for item in gaps:
            gid = item.get("id")
            if gid in SHIPPED_MAP:
                feed, page = SHIPPED_MAP[gid]
                item["status"] = "SHIPPED"
                item["feasibility"] = "SHIPPED"
                item["shipped_to"] = "justhodl-gap-metrics"
                item["feed"] = feed
                item["page"] = page
                item["title"] = "SHIPPED -> " + page + " | " + item.get("title", gid)
                try:
                    live = json.loads(s3.get_object(
                        Bucket=BUCKET, Key=feed)["Body"].read())
                    item["live_status"] = live.get("status")
                except Exception:
                    item["live_status"] = "unverified"
            elif gid in ("M17", "M18"):
                ev = ("justhodl-crypto-etf-flows feed data/crypto-etf-"
                      "flows-history.json wired to crypto-liquidity.html"
                      if gid == "M17" else
                      "justhodl-crypto-funding feed data/crypto-funding"
                      ".json wired to crypto-risk.html")
                item["status"] = "already_covered"
                item["evidence"] = ev
                item["title"] = "COVERED | " + item.get("title", gid)
                covered.append(dict(item))
            else:
                item["status"] = "OPEN"
                still_open.append(gid)
        gaps = [g for g in gaps if g.get("id") not in ("M17", "M18")]
        for c in covered:
            if c.get("id") == "M12":
                c["caveat"] = ("HEALTHY WEEKLY (ops 2979): schedule "
                               "ENABLED, 1043-row history, self-heals "
                               "junk prints; stale flag was cadence "
                               "mismatch")
        rep.kv(shipped=len(SHIPPED_MAP), still_open=still_open,
               demoted_to_covered=["M17", "M18"])
        ...
        out = {
            ...
            "totals": {"engines": len(reg), "pages": len(pages),
                       ...
                       "wired_per_wiring_doc": wired_n,
                       "orphaned_per_wiring_doc": orph_n,
                       "gaps": len(gaps), "gaps_open": len(still_open), "gaps_shipped": len(SHIPPED_MAP),
...
def _write(rep, fails, warns, hl):
    out = {"ops": 2981, "fails": fails, "warns": warns,
           "verdict": "PASS" if not fails else "FAIL",
           "ts": datetime.now(timezone.utc).isoformat()}
    out.update(hl)
    rp = AWS_DIR / "ops" / "reports" / "2981.json"
