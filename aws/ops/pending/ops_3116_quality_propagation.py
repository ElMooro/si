#!/usr/bin/env python3
"""ops 3116 -- QUALITY BAR PROPAGATION (Khalid: make master-rank /
...
GROWING / MIXED / SHRINKING verdict). 3115 lesson: IR per-industry rows live in doc key LADDER ({etf,name,leadership_score,tag}), not leaders/regiments. Static-marker verify
...
UA = {"User-Agent": "Mozilla/5.0 ops-3116", "Cache-Control": "no-cache"}
...
def main():
    fails, warns = [], []
    with report("3116_quality_propagation") as rep:
        ...
        checks = {
            ...
            "why.html": ("Industry health", "SHRINKING / IN TROUBLE",
                         "industry-rotation.json", "IR.ladder",
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3116.json").write_text(json.dumps(
        {"ops": 3116, "verdict": "FAIL" if fails else "PASS",
