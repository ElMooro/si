"""ops_5171 -- Google sign-in forensics (READ-ONLY, from the runner).
...
with report("ops_5171_google_login_forensics") as R:
    R.heading("ops 5171 -- Google sign-in forensics (read-only)")
    ...
    if FINDINGS:
        ...
        R.ok("   every leg up to Google's account chooser works from here; the failure is inside Google's consent step "
             "(Testing-mode app -> only listed test users may sign in; or the account/browser side)")
    (ROOT / "aws" / "ops" / "reports" / "5171_google_login_forensics.json").write_text(
    ...
    if FINDINGS:
        ...
        sys.exit(1)
    R.ok("ops 5171 complete -- read-only")
