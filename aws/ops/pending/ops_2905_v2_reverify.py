"""ops 2905 — v2 fix re-verify: top-level v2 keys on fresh NVDA doc; ka/khalid Sonnet-4-6 healing."""
...
R={"ops":2905,"ts":datetime.now(timezone.utc).isoformat(),"errors":{}}
...
def wait_deploy(fn):
    ...
    return False
def get(url,to=290):
    req=urllib.request.Request(url,headers={"User-Agent":"ops2905"})
    with urllib.request.urlopen(req,timeout=to) as r: return json.loads(r.read())
try:
    R["deploys"]={fn:wait_deploy(fn) for fn in ("justhodl-equity-research","justhodl-khalid-metrics","justhodl-ka-metrics")}
...
os.makedirs("aws/ops/reports",exist_ok=True); json.dump(R,open("aws/ops/reports/2905_v2_reverify.json","w"),ensure_ascii=False,indent=1,default=str)
print("OPS 2905 COMPLETE")
