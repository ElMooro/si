#!/usr/bin/env python3
"""ops 3110 -- forensic desk x3 (Khalid, one by one): (1) sector
...
carry forensic{concern,strength_grade,m_deteriorating}."""
...
UA = {"User-Agent": "Mozilla/5.0 ops-3110", "Cache-Control": "no-cache"}
...
def main():
    fails, warns = [], []
    with report("3110_forensic_trend_gate") as rep:
        ...
        for _ in range(50):
            ...
            try:
                ...
                if (datetime.now(timezone.utc) - o["LastModified"]
                    ...
                    if any(r.get("m_score_delta_q") is not None
                           for r in allr):
                        d = cand
                        break
            except Exception:
        ...
        big = [r["symbol"] for r in allr
               if abs(r.get("m_score_delta_q") or 0) > 5]
        if big:
            fails.append("insane dM values: %s" % big[:5])
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3110.json").write_text(json.dumps(
        {"ops": 3110, "verdict": "FAIL" if fails else "PASS",
