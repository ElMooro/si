#!/usr/bin/env python3
"""ops 3027 -- RISK VISUAL BAROMETER (Khalid, from the ChatGPT mock).
Engine: dual-view barometer -- per-canary (original spec, kept) +
per-mechanism (each mechanism = ONE vote = mean stress of its member
canaries; fixes the row-count artifact where a 59-row feed outvotes a
1-row mechanism 59:1). 5-band display labels CALM/GUARDED/ELEVATED/
HIGH RISK/CRISIS. Page: full visual-barometer layout on LIVE data --
central needle gauge, 12 mechanism segment-bars with markers, legend,
view toggle. ChatGPT numbers rejected as illustrative fiction per its
own disclaimer.

Prior: ops 3026 -- eurodollar CLOSE. 3025 failed on a FALSE-POSITIVE in the
...
def main():
    fails, warns = [], []
    with report("3027_visual_barometer") as rep:
        ...
        views = (baro.get("views") or {})
        pm = views.get("per_mechanism") or {}
        pc = views.get("per_canary") or {}
        bym = pm.get("by_mechanism") or {}
        rep.kv(pm_score=pm.get("score"), pm_band5=pm.get("band5"),
               pc_score=pc.get("score"), pc_band5=pc.get("band5"),
               n_mechanisms=pm.get("n_mechanisms"),
               by_mechanism={k: v.get("score") for k, v in bym.items()})
        if pm.get("score") is None:
            fails.append("per_mechanism score missing")
        if (pm.get("n_mechanisms") or 0) < 12:
            fails.append("n_mechanisms=%s (<12)" % pm.get("n_mechanisms"))
        if not pm.get("band5"):
            fails.append("band5 missing")
        if pc.get("score") is None or pc.get("n_votes") != baro.get(
                "n_votes"):
            fails.append("per_canary view inconsistent with legacy score")
        ...
        try:
            ...
            page = urllib.request.urlopen(req, timeout=25).read().decode(
                "utf-8", "replace")
            ok_g = "RISK VISUAL BAROMETER" in page
...
def _finish(rep, fails, warns, extra):
    payload = {"ops": 3027, "fails": fails, "warns": warns,
               "verdict": "FAIL" if fails else "PASS",
               "ts": datetime.now(timezone.utc).isoformat()}
    payload.update(extra)
    (AWS_DIR / "ops" / "reports" / "3027.json").write_text(
