"""ops_5084 -- justhodl-fortress v1.0.3: overlay / volatility wrappers and
non-operating "stocks" out of the boards.

5083 diagnostics: the equity-only ETF board still surfaced VXZ / VIXM (VIX
futures, typed "US Equities" by Finviz), BTAL (anti-beta market neutral)
and covered-call income overlays (TCAL, IYRI, BEDY) -- resilient by
construction, not by accumulation; RFI (Closed-End Fund) sat on the stock
board. v1.0.3 excludes overlay/volatility products by name, non-operating
industries (Closed-End Fund*, Shell Companies) from the stock universe, and
nulls bogus AUM (VXZ carried 4e13). Same gates, same verification.
...
def main():
    with report("5084-fortress-v103") as r:
        r.heading("ops 5084 -- justhodl-fortress v1.0.3 redeploy + verified re-run")
        ...
        if not env_vars:
            ...
            sys.exit(1)
        env_vars["FORTRESS_VERSION"] = "1.0.3"
        ...
        import re as _re
        _ov = _re.compile(r"\bvix\b|volatility|market neutral|anti-beta|covered call|premium income|high income", _re.I)
        ...
        json.dump({"ops": 5084, "engine": FN, "session": payload.get("session"),
                   "n_scored": payload.get("n_scored"), "tiers": payload.get("tiers"),
                   "coverage_pct": covs, "as_of": payload.get("as_of")},
                  open(str(ROOT / "aws" / "ops" / "reports" / "5084.json"), "w"), indent=1)
    sys.exit(0)
