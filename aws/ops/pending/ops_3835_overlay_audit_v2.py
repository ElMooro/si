"""
ops_3835 — FLEET AUDIT: which overlays are silently multiplying by 1.0?
...
MAX_ARTIFACTS = 1200
# ops 3834 capped at 4MB and thereby EXCLUDED data/master-ranker.json and
# data/best-setups.json — the two engines we had just proven carry rotation_mult
# at 16/25 and 44/50. It then reported "SILENT: none", i.e. "fleet is clean".
# A filter that silently drops the subject of the audit and returns a reassuring
# answer is the exact failure class this ops was written to hunt.
# These MUST be scanned regardless of size, and the run FAILS if they are absent.
MUST_SCAN = ["data/master-ranker.json", "data/best-setups.json"]
KNOWN_OVERLAYS = {"rotation_mult", "nowcast_regime_mult", "risk_regime_mult"}
...
def main():
    with report("3835_overlay_audit_v2") as rep:
        rep.heading("ops 3835 — fleet audit v2: silently-neutral overlays")
        ...
        for m in MUST_SCAN:
            if m not in keys:
                keys.append(m)
                rep.log(f"  force-included {m} (must-scan, size-exempt)")
        rep.ok(f"  {len(keys)} artifacts (cap {MAX_BYTES//1_000_000}MB, "
               f"{len(MUST_SCAN)} size-exempt)")
        ...
        if errors > scanned:
            ...
            sys.exit(1)
        rep.section("0. SELF-VALIDATION — did the audit see what we know is there?")
        seen_arts = {f["artifact"] for f in findings}
        seen_fields = {f["field"] for f in findings}
        for m in MUST_SCAN:
            short = m.replace("data/", "")
            (rep.ok if short in seen_arts else rep.fail)(
                f"  {short}: {'scanned' if short in seen_arts else 'ABSENT — audit blind'}")
        hit = KNOWN_OVERLAYS & seen_fields
        (rep.ok if hit else rep.fail)(
            f"  known overlays rediscovered: {sorted(hit) or 'NONE'}")
        if not hit or any(m.replace("data/", "") not in seen_arts for m in MUST_SCAN):
            rep.fail("SELF-VALIDATION FAILED — the audit cannot see overlays proven "
                     "to exist (ops 3831/3832), so any 'clean' verdict is worthless")
            sys.exit(1)
