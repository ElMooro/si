...
if "CMC_KEY" not in env_:
    ...
    ready("justhodl-altseason")
r = rc(lambda: lam.invoke(FunctionName="justhodl-altseason", InvocationType="RequestResponse", LogType="Tail", Payload=b"{}"))
out["fn_err"]=r.get("FunctionError","NONE")
import base64 as _b64
try:
    out["log_tail"] = _b64.b64decode(r.get("LogResult","")).decode(errors="replace")[-1800:]
except Exception:
    out["log_tail"] = None
out["invoke_payload"] = r["Payload"].read().decode(errors="replace")[:500]
...
open("aws/ops/reports/1605_global_fix.json","w").write(json.dumps(out,indent=2,default=str))
