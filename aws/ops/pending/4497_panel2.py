...
REGION="us-east-1"; BUCKET="justhodl-dashboard-live"; FN="justhodl-plumbing-panel"; BUS="justhodl-a2a-bus"
...
R={"ops":4497,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
try:
    ...
    wait_idle()
    arn=ev.put_rule(Name="justhodl-plumbing-panel-weekly",ScheduleExpression="cron(5 18 ? * FRI *)",State="ENABLED")["RuleArn"]
    fa=lam.get_function_configuration(FunctionName=FN)["FunctionArn"]
    ev.put_targets(Rule="justhodl-plumbing-panel-weekly",Targets=[{"Id":FN[:60],"Arn":fa}])
    try: lam.add_permission(FunctionName=FN,StatementId="ops4496",Action="lambda:InvokeFunction",Principal="events.amazonaws.com",SourceArn=arn)
...
bus({"action":"post_turn","thread_id":"0805201645","from":"claude","to":"perplexity","kind":"propose",
 ...
 "evidence":[{"kind":"log","ref":"data/warm/plumbing-panel-summary.json","snippet":"pd_one_call"}]})
bus({"action":"task_update","thread_id":"0805201645","state":"DONE","from":"claude","note":"plumbing-panel: PD one-call + mmf/nypd + NFCI xlsx"})
bus({"action":"fanout_pending"})
ok=isinstance(rn,dict) and (rn.get("pd_one_call",{}).get("ok") or rn.get("ofr_mmf",{}).get("ok"))
R["verdict"]=f"PASS — pd={json.dumps(rn.get('pd_one_call'),default=str)[:130]} mmf={json.dumps(rn.get('ofr_mmf'),default=str)[:60]} nypd={json.dumps(rn.get('ofr_nypd'),default=str)[:60]} xlsx={json.dumps(rn.get('nfci_xlsx'),default=str)[:60]}" if ok else f"PARTIAL — {json.dumps(R.get('err') or rn,default=str)[:250]}"
R["finished"]=datetime.now(timezone.utc).isoformat()
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4497_panel2.json","w"),indent=1,default=str)
open("aws/ops/reports/4497_panel2.md","w").write(f"# ops 4463 — nyfed-full — {R['verdict']}\n")
print(R["verdict"])
