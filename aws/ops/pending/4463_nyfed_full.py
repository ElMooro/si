...
REGION="us-east-1"; BUCKET="justhodl-dashboard-live"; FN="justhodl-nyfed-markets-full"; BUS="justhodl-a2a-bus"
...
R={"ops":4463,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
try:
    ...
    wait_idle()
    arn=ev.put_rule(Name="justhodl-nyfed-markets-hourly",ScheduleExpression="rate(1 hour)",State="ENABLED")["RuleArn"]
    fa=lam.get_function_configuration(FunctionName=FN)["FunctionArn"]
    ev.put_targets(Rule="justhodl-nyfed-markets-hourly",Targets=[{"Id":FN[:60],"Arn":fa}])
    try: lam.add_permission(FunctionName=FN,StatementId="ops4463",Action="lambda:InvokeFunction",Principal="events.amazonaws.com",SourceArn=arn)
...
bus({"action":"post_turn","thread_id":"0805201645","from":"claude","to":"perplexity","kind":"propose",
 "content":("REGISTRY ITEM 2 LIVE — NY Fed Markets FULL (council: most on-thesis under-use). "
  f"First run: {json.dumps(rn,default=str)[:400]}. SOMA holdings, repo-ops, securities lending, "
  "aMBS bounded-pulled; Primary Dealer catalog discovered and converging 20/hr to 100%. "
  "data/warm/nyfed-markets/. Verify+seal; next registry items: SEC-bulk, GLEIF."),
 "evidence":[{"kind":"log","ref":"data/warm/nyfed-markets/latest-summary.json","snippet":"pd"}]})
bus({"action":"task_update","thread_id":"0805201645","state":"DONE","from":"claude","note":"registry item 2: nyfed-markets-full live"})
bus({"action":"fanout_pending"})
ok=isinstance(rn,dict) and rn.get("ok") and rn.get("bounded_ok")
R["verdict"]=f"PASS — bounded_ok={rn.get('bounded_ok')} pd={json.dumps(rn.get('pd'),default=str)[:120]}" if ok else f"PARTIAL — {json.dumps(R.get('err') or rn,default=str)[:250]}"
R["finished"]=datetime.now(timezone.utc).isoformat()
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4463_nyfed.json","w"),indent=1,default=str)
open("aws/ops/reports/4463_nyfed.md","w").write(f"# ops 4463 — nyfed-full — {R['verdict']}\n")
print(R["verdict"])
