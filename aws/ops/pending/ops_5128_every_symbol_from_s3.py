"""ops_5128 -- every symbol from the warehouse: daily bars since inception (5127 hit the Deny-Delete policy purging the monthly bank; re-bank in place instead).
...
def http(url, timeout=180):
    req = urllib.request.Request(url, headers={"User-Agent": "justhodl-ops-5128", "Accept-Encoding": "identity"})
    t = time.time()
...
def main():
    with report("5128-every-symbol-from-s3") as r:
        r.heading("ops 5128 -- every symbol from the warehouse: daily bars since inception (re-bank in place), aliases, native page")
        fails = []
        ...
        if not page_ok:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5128-chartpro-native.jpg"), type="jpeg", quality=60)
