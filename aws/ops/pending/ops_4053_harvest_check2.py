"""ops_4053 — FULL HARVEST CHECK: sources landed? delta computed, workbench
...
def main():
    with report("4053_harvest_check2") as rep:
        ...
        rep.kv(diag=json.dumps(diag)[:260])
        checks.append(("sources flowing OR diag fingerprint present",
                       len(m) >= 20 or bool(diag)))
        ...
        checks += [("workbench current 491/10k",
                   ...
                   ("attribution joining the page (>=10 sourced, mid-harvest)",
                    (t.get("symbols_with_tv_source") or 0) >= 10 or len(m) < 20)]
