"""ops 3587 — deploy + live-prove the two discovery-queue scrapers: PBoC EN
monthly AFRE (real TSF, self-building cache) into china-liquidity v2.1, and
...
with report("3587_scrapers") as rep:
    rep.heading("ops 3587 — PBoC AFRE + DGBAS export-orders live")
    out = {"gates": {}}
    fails = []
    ...
    try:
        ...
        deploy_lambda(report=rep, function_name="justhodl-asia-leads",
                      ...
                      description="Asia tech-pulse v1.2: KR+TW exports (FRED) + TRUE Taiwan EXPORT ORDERS (DGBAS point page).",
                      create_function_url=False)
    ...
    Path("aws/ops/reports/3587.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
