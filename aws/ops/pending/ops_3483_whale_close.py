"""ops 3483 — close Z3/Z5 from 3482: 13F schema was compact (t/n/wn/nf/tv);
both surface chips had silently no-op'd (unasserted anchors). v1.3.1 +
asserted patches.

Original header follows.
 — Tier-2a: reverse-DCF implied growth + NBER backdrop + 13F fusion.
...
Gates:
  ...
  Z4 served-JS integrity 4 surfaces + core OPS3483/NBER + catalog OPS3483
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-3483"})
...
with report("3483_whale_close") as rep:
    out = {"ops": 3483, "gates": {}}; fails = []
    ...
    rep.heading("ops 3483 — implied growth + NBER + whale fusion")
    ...
    deploy_lambda(report=rep, function_name=FN,
                  ...
                  description="Fundamental Graphs v1.3.0 impliedG+whales (ops 3483)",
    ...
    wp = json.loads(lam.invoke(FunctionName=FN, Payload=json.dumps(
        {"warm": ["GOOGL"], "periods": ["quarter"], "refresh": True}).encode())["Payload"].read() or b"{}")
    try:
    ...
    for _ in range(21):
        try:
            ...
            if b"OPS3483" in got["core"] and b"ops3483" in got["flag"] \
               and b"OPS3483" in got["cat"] and b"ops3483" in got["why"]:
                break
        except Exception as e:
            got["err"] = str(e)[:120]
        time.sleep(20)
