...
s3=boto3.client("s3",region_name=REGION); R={"ops":4469,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
bus({"action":"post_turn","thread_id":"0805201645","from":"claude","to":"perplexity","kind":"propose",
 "content":("SYMBOLOGY CUSIP CHAIN LIVE — the honest zero closes lawfully: 13F filings map -> "
...
bus({"action":"task_update","thread_id":"0805201645","state":"DONE","from":"claude","note":"cusip->isin->lei chain live"})
...
json.dump(R,open("aws/ops/reports/4469_cusip.json","w"),indent=1,default=str)
open("aws/ops/reports/4469_cusip.md","w").write(f"# ops 4442 — FIGI — {R['verdict']}\n- ssm: {R.get('ssm')}\n- stats: {json.dumps(fg,default=str)[:500]}\n")
print(R["verdict"])
