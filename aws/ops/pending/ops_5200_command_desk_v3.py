"""ops_5200 -- Command desk v3 (numbered sections fleet-wide + composable homepage) -- deploy gate + registry crawl.
...
def http(url, timeout=25):
    try:
        with urllib.request.urlopen(urllib.request.Request(url, headers={"User-Agent": "jh-ops-5200", "Cache-Control": "no-cache"}), timeout=timeout) as r:
            return r.status, r.read().decode("utf-8", "replace")
    except Exception as e:  # noqa: BLE001
        return 0, str(e)


with report("ops_5200_command_desk_v3") as R:
    R.heading("ops 5200 -- command desk v3: numbered sections fleet-wide + composable homepage")
    ...
    while time.time() - t0 < 1080:
        ...
        live_home = st == 200 and "home.js" in body and "hd-grid" in body
    ...
    if not FAILS:
        ...
        with sync_playwright() as p:
            ...
            for width, height in ((1440, 1200), (390, 844)):
                ...
                pg.screenshot(path=str(SHOTS / f"ops5200_home_{width}.png"), full_page=width > 700)
                ...
                if facts["blocks"] < 6 or facts["rows"] < 30 or facts["directory"] < 300 or errors or csp or not facts["chrome"]:
            ...
            for key in ("bonds", "auctions", "global-cycle", "sectors"):
                ...
                secs = pg.evaluate("() => { try { window.JustHodlSections.rerun(); } catch (e) {} return (window.JH_SECTIONS || []).map(s => [s.n, s.key, s.title.slice(0, 50), s.sub.map(x => x.n + ' ' + x.title.slice(0, 28))]); }")
                ...
                if key == "bonds":
                    pg.screenshot(path=str(SHOTS / "ops5200_bonds_numbered.png"), full_page=False)
                    ...
                    pe.screenshot(path=str(SHOTS / "ops5200_bonds_embed_s1.png"), full_page=True)
    ...
    if not FAILS:
        ...
        (ROOT / "aws" / "ops" / "reports" / "latest" / "ops5200_crawl.json").write_text(json.dumps({"stats": stats, "failures": bad, "zero": zero}, indent=1), encoding="utf-8")
