"""ops 3486 — Tier-2b: pairs spread/ratio builder (flagship v2.1).

FGChart.ratio (core, OPS3486): pure at-or-before date-aligned division,
...
Gates:
  ...
  T2 served core has FGChart.ratio + OPS3486; 4-surface node-check
  T3 flagship v2.1 live (ops3486, rtbtn, ratioRaw, data-ri) ; prior
...
def fetch(url):
    req = urllib.request.Request(url, headers={"User-Agent": "ops-3486"})
...
with report("3486_pairs_ratio") as rep:
    out = {"ops": 3486, "gates": {}}; fails = []
    ...
    rep.heading("ops 3486 — pairs ratio builder")

    core_local = (REPO / "fg-chart.js").read_text()
    test = core_local.replace("window.FGChart=", "var FGChart=") + """
;(function(){
  ...
  process.exit(t1&&t2?0:1);
    ...
    for _ in range(18):
        try:
            ...
            if b"OPS3486" in got["core"] and b"ops3486" in got["flag"]:
                break
    ...
    gate("T2_served_core", all(checks) and b"OPS3486" in got.get("core", b"")
         and b"ratio:ratio" in got.get("core", b""), {"node_ok": checks})
    f = got.get("flag", b"")
    d3 = {"ops3486": b"ops3486" in f, "rtbtn": b"rtbtn" in f,
    ...
    (REPO / "aws" / "ops" / "reports" / "3486.json").write_text(json.dumps(out, indent=2))
