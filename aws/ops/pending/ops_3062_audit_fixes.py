#!/usr/bin/env python3
"""ops 3062 (retry) -- external-audit fix verification (live, post-CDN).
...
UA = {"User-Agent": "Mozilla/5.0 ops-3062"}
...
def main():
    fails, warns = [], []
    with report("3062_audit_fixes") as rep:
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3062.json").write_text(json.dumps(
        {"ops": 3062, "verdict": "FAIL" if fails else "PASS",
