#!/usr/bin/env python3
"""Ops5242: install confidentiality policies before the full source release.

Extend verified containment to portfolio commentary and both diagnostic reports.
...
def main():
    ...
    result = {"ops": 5242, "checkout_sha": checkout, "ops_target_sha": os.environ.get("OPS_TARGET_SHA"), **contain(ROOT, clients)}
    path = ROOT / "aws/ops/reports/5242_audit_privacy_containment.json"
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(result, indent=2) + "\n")
    print(json.dumps({"ops": 5242, "ok": result["ok"], "policy_verified": result.get("policy_verified", False),
