"""ops 3562 (3561 rerun after deploy-lambdas race) — TV METRIC PARITY, phase 1 (Khalid: every TradingView
...
with report("3562_tv_parity_rerun") as rep:
    fails = []
    def gate(n, ok, d):
    ...
    def deploy_retry(**kw):
        for att in range(4):
            try:
                return deploy_lambda(**kw)
            except Exception as e:  # noqa: BLE001
                if "Conflict" in str(e) or "Pending" in str(e):
                    print("[retry]", kw["function_name"], att,
                          str(e)[:80])
                    time.sleep(25)
                else:
                    raise
        raise RuntimeError("deploy retries exhausted")
    ...
    for fn, mark, tmo, mem in (
        ...
        for _ in range(30):
            ...
            if c0.get("LastUpdateStatus") == "Successful":
                break
            time.sleep(4)
        deploy_retry(report=rep, function_name=fn,
                      source_dir=REPO/"aws"/"lambdas"/fn/"source",
                      env_vars=(cfg.get("Environment") or {})
                      .get("Variables") or {},
                      timeout=cfg["Timeout"], memory=cfg["MemorySize"],
                      description=f"{fn} tv-parity (ops 3562 (3561 rerun after deploy-lambdas race))",
    ...
    (REPO/"aws/ops/reports/3562.json").write_text(
        json.dumps({"ops": 3562, "fails": fails}))
sys.exit(0)
