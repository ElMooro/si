...
e,p2=inv("justhodl-signal-board"); v["sb_err"]=e
sb=json.loads(s3.get_object(Bucket=B,Key="data/signal-board.json")["Body"].read())
eng=sb.get("engines") or []
v["sb"]={"n_engines":sb.get("n_engines"),
  "rows":{n: next(({k:e2.get(k) for k in ("signal","signal_label","read")}
                    for e2 in eng if e2.get("engine")==n), "MISSING")
          for n in ("US Macro Cycle","Market Internals","US Real M2")}}
v["registry_n"]=len(reg["contexts"])
out["verify"]=v
open("aws/ops/reports/1582_brain_wire.json","w").write(json.dumps(out,indent=2,default=str))
