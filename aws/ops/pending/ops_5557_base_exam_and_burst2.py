...
def main() -> int:
    ...
    with report("ops_5557_base_exam_and_burst2") as R:
        R.heading("ops 5557 -- base exam job (frozen HumanEval prompts, greedy) + burst 2 (MBPP+APPS, K=6) on the owned lane")
        R.kv(head=head[:10])
        ...
        # ---- exam tasks: prompts only, in their own prefix (the tests never travel with a job)
        try:
            raw = s3.get_object(Bucket=PRI, Key="factory/exams/code/holdout-prompts.jsonl")["Body"].read().decode("utf-8")
        except Exception as e:  # noqa: BLE001
            red.append("exam-prompts"); R.fail("frozen exam prompts unreadable: %s" % str(e)[:120]); R.fail("RED"); return 1
        prompts = [json.loads(l) for l in raw.splitlines() if l.strip()]
        tasks = "\n".join(json.dumps({"task_id": p["task_id"], "prompt": p["prompt"], "family": "humaneval", "holdout": True}, sort_keys=True) for p in prompts) + "\n"
        stamp = datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")
        exam_prefix = "factory/exams/code/prompts-only/%s/" % stamp
        s3.put_object(Bucket=PRI, Key=exam_prefix + "tasks.jsonl", Body=tasks.encode(), ContentType="application/jsonl", IfNoneMatch="*")
        R.ok("exam tasks: %d frozen HumanEval prompts -> s3://%s/%s (prompts only)" % (len(prompts), PRI, exam_prefix))
        ...
        R.ok("training price %s = $%.4f/h; per-job cap $%.4f" % (INSTANCE, float(hourly), cap))
        ...
        # ---- (a) base exam
        exam_spec = own.burst_spec(s3, PRI, control, mode="exam", tasks_uri="s3://%s/%s" % (PRI, exam_prefix))
        exam_name = re.sub(r"[^a-zA-Z0-9-]", "-", "jh-exam-gen0-%s" % stamp)[:63]
        launch(exam_spec, exam_name, "s3://%s/%s" % (PRI, exam_prefix), "exam", {"task_cap": "1000"}, {"exam_generation": "gen-0", "tasks": len(prompts)})
        R.ok("BASE EXAM launched: %s -- %d prompts, greedy, adapter=base; grade with factory-exam.yml (burst=%s, generation=gen-0)" % (exam_name, len(prompts), exam_name))
        ...
        R.kv(exam_job=exam_name, burst_job=name2, cap_usd_each=cap)
        R.ok("GREEN -- base exam + burst 2 running on the owned lane")
        return 0
