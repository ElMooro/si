"""ops 4448 — E7 v1: Treasury fiscal deep history. 24/34."""
import io,json,os,time,zipfile
from datetime import datetime,timezone
import boto3
from botocore.config import Config
REGION="us-east-1"; BUCKET="justhodl-dashboard-live"; FN="justhodl-treasury-fiscal-full"; BUS="justhodl-a2a-bus"
...
R={"ops":4448,"started":datetime.now(timezone.utc).isoformat()}
buf=io.BytesIO()
...
try:
    ...
    for _ in range(24):
        ...
        time.sleep(5)
    RULE="justhodl-treasury-fiscal-daily"
    arn=ev.put_rule(Name=RULE,ScheduleExpression="cron(55 5 * * ? *)",State="ENABLED")["RuleArn"]
    fa=lam.get_function_configuration(FunctionName=FN)["FunctionArn"]
    ev.put_targets(Rule=RULE,Targets=[{"Id":FN[:60],"Arn":fa}])
    try: lam.add_permission(FunctionName=FN,StatementId="ops4448",Action="lambda:InvokeFunction",Principal="events.amazonaws.com",SourceArn=arn)
...
time.sleep(4)
try:
    d=json.loads(s3.get_object(Bucket=BUCKET,Key="data/warm/treasury/latest-summary.json")["Body"].read())
...
bus({"action":"post_turn","thread_id":"0805201645","from":"claude","to":"perplexity","kind":"propose",
 "content":("E7 v1 SHIPPED — 24/34. justhodl-treasury-fiscal-full (nightly 05:55): official "
  "fiscaldata.treasury.gov — Debt to the Penny + TGA operating cash, newest 2500 obs each -> "
  f"data/warm/treasury/, F4 snapshots, explicit failures. First run: {json.dumps(sm,default=str)[:500]}. "
  "Span honestly labelled single-page; deep backfill is E10's job. Verify+seal."),
 "evidence":[{"kind":"log","ref":"data/warm/treasury/latest-summary.json","snippet":"debt_to_penny"}]})
bus({"action":"task_update","thread_id":"0805201645","state":"DONE","from":"claude","note":"24/34: +E7 treasury fiscal"})
bus({"action":"fanout_pending"})
ok=any(v.get("n_obs") for v in sm.values()) if sm else False
R["verdict"]=f"PASS — {json.dumps({k:v.get('n_obs') for k,v in sm.items()},default=str)}" if ok else f"PARTIAL — {json.dumps(sm,default=str)[:200]}"
R["finished"]=datetime.now(timezone.utc).isoformat()
os.makedirs("aws/ops/reports",exist_ok=True)
json.dump(R,open("aws/ops/reports/4448_e7.json","w"),indent=1,default=str)
open("aws/ops/reports/4448_e7.md","w").write(f"# ops 4448 — E7 — {R['verdict']}\n- datasets: {json.dumps(sm,default=str)[:700]}\n")
print(R["verdict"])
