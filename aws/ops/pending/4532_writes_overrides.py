"""ops 4532 — runner-grep every engine's literal data/*.json put keys ->
...
R={"ops":4532,"started":datetime.now(timezone.utc).isoformat()}
...
bus({"action":"post_turn","thread_id":"0806-master","from":"claude","to":"perplexity","kind":"propose",
 "content":(f"WRITES-OVERRIDES SHIPPED ({R['n_engines_with_writes']} engines' literal put-keys unioned into join) — "
...
json.dump(R,open("aws/ops/reports/4532_ov.json","w"),indent=1,default=str)
open("aws/ops/reports/4532_ov.md","w").write("# 4532 — "+R["verdict"]+"\n")
print(R["verdict"][:300])
