"""ops 3570 — brute-force sweep close (3534 pattern). Chain #2
stalled ~129/496 (overlapped chains + mid-flight redeploy). This ops
finds every cache doc missing the TRUE v1.11 marker (netChangeInCash; legacy fulmer_h false-positived 3567), rebuilds
...
with report("3570_bruteforce_true") as rep:
    fails = []
    def gate(n, ok, d):
    ...
    for i, t in enumerate(tickers):
        try:
            ...
            if b'"netChangeInCash"' not in body:
                missing.append(t)
    ...
    (REPO/"aws/ops/reports/3570.json").write_text(
        json.dumps({"ops": 3570, "fails": fails}))
sys.exit(0)
