...
with report("3405_wires_regate") as rep:
    ...
    while time.time()<dl:
        try:
            sa=LAM.get_function_configuration(FunctionName="justhodl-proven-portfolio")
            sb=LAM.get_function_configuration(FunctionName="justhodl-best-setups")
            if sa.get("LastUpdateStatus")=="Successful" and sb.get("LastUpdateStatus")=="Successful":
    ...
    Path("aws/ops/reports/3405.json").write_text(json.dumps(out,indent=2,default=str)); sys.exit(0)
