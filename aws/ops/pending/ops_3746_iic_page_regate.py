#!/usr/bin/env python3
"""ops 3746 — import-canary.html + FIELD-COVERAGE AUDIT + nav pin.
...
with report("3746_iic_page_regate") as rep:
    rep.heading("ops 3746 — insider-industry-cluster page + field-coverage audit")
    fails = []
    out = {"gates": {}}
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3746.json").write_text(json.dumps({"verdict": "STARTED"}))
    ...
    try:
        ...
        out["verdict"] = verdict
        out["unrendered"] = gaps
        Path("aws/ops/reports/3746.json").write_text(json.dumps(out, indent=2))
        ...
        Path("aws/ops/reports/3746.json").write_text(
