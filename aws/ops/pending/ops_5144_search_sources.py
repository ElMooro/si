"""ops_5144 -- the search bar shows what every source holds; TradingView-only symbols resolve to the warehouse.
...
def http(url, timeout=120):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5144", "Accept-Encoding": "identity"})
...
def main():
    with report("5144-search-sources") as r:
        r.heading("ops 5144 -- search bar shows every source; TradingView-only symbols resolve to the warehouse")
        fails = []
        ...
        if h.get("version") != "1.5.0":
            fails.append(f"version {h.get('version')} != 1.5.0")
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5144-us02my.jpg"), type="jpeg", quality=55)
