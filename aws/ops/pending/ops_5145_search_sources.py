"""ops_5145 -- 5144 re-run: a wrong dictionary source_id raised through /series (500); tolerant HTTP in the op.
...
def http(url, timeout=120):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5145", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5145-search-sources") as r:
        r.heading("ops 5145 -- search bar shows every source; TradingView-only symbols resolve to the warehouse (re-run)")
        fails = []
        ...
        if h.get("version") != "1.5.1":
            fails.append(f"version {h.get('version')} != 1.5.1")

        r.section("S2 resolution + search")
        for sid, want in (("TVC:US02MY", "fred:DGS2MO"), ("ECONOMICS:USINTR", "fred:FEDFUNDS"), ("TVC:DE10Y", "fred:IRLTLT01DEUM156N"), ("TVC:US03MY", "fred:DGS3MO")):
        ...
        while time.time() - t0 < 900:
            try:
                ...
                if b"hs-facets" in c and b"data-alt=" in c and c.count(b"static attachLegend(") == 1:
                    live = True
                    break
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5145-us02my.jpg"), type="jpeg", quality=55)
