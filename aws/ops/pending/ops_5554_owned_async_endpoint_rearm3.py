...
def main() -> int:
    ...
    with report("ops_5554_owned_async_endpoint_rearm3") as R:
        R.heading("ops 5554 (re-arm of 5553) -- owned model async endpoint (scale-to-zero) from the staged weights; proven by one round trip")
        ...
        try:
            ...
            parn = "arn:aws:iam::%s:policy/justhodl-runner-hosting-v2" % ACCOUNT
            try:
                iam.create_policy(PolicyName="justhodl-runner-hosting-v2", Description="Runner: create/describe/delete the owned async endpoint + autoscaling",
        ...
        control = {"schema_version": "factory-inference-control.v1", "enabled": True, "endpoint_name": NAME, "mode": "async-scale-to-zero",
                   ...
                   "endpoint_config": cfg_name, "written_by": "ops 5554", "git_sha": head[:12], "at": datetime.now(timezone.utc).isoformat()}
