...
Either way the desk section runs and v2.3.0's wires get their verdict.
"""
import io
import json
import subprocess
...
fails = []
with report("4299_stress_verdict") as r:
    r.heading("ops 4299 -- deployed-bytes verdict + independent desk "
              "gate")
        ...
        if not fails:
            ...
            if len(cov) >= 11:
                ...
                try:
                    ...
                    pass
                fails.append("coverage %d after verdict path; "
                             "tracebacks=%s" % (len(cov), tb))

    r.section("B. desk v2.3.0 wires -- independent verdict")
