...
print(f"  STRATEGY: tot {s.get('total_return_pct')}% · CAGR {s.get('cagr_pct')}% · Sharpe {s.get('sharpe')} · maxDD {s.get('max_drawdown_pct')}% · inMkt {s.get('time_in_market_pct')}% · {s.get('switches')} switches")
print(f"  BUY&HOLD: tot {h.get('total_return_pct')}% · CAGR {h.get('cagr_pct')}% · Sharpe {h.get('sharpe')} · maxDD {h.get('max_drawdown_pct')}%")
print("  VERDICT:",b.get("verdict"),"| better DD:",b.get("edge_on_drawdown"))
print("  curve pts:",len(b.get("curve") or []),"last:",(b.get("curve") or [{}])[-1])
print("DONE 2625")
