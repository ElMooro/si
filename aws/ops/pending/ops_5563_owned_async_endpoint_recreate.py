"""ops 5563 -- re-create the owned async endpoint after the hourly guard learned to exempt async scale-to-zero endpoints
(it had deleted the 5556 endpoint as idle/past-TTL). Same recipe as 5556 (which was GREEN end to end).

ops 5556 -- re-arm of 5554: InService in 12 min, autoscaling 0..1 registered; the Lambda grant failed because the role
name was assumed (justhodl-lambda-execution-role does not exist) -- now the role is read from justhodl-ai's own
configuration. Endpoint was verified deleted after 5554, so nothing billed.

ops 5554 -- re-arm of 5553: the endpoint reached InService in 12 minutes (double load fixed); autoscaling registration was
refused because the runner lacks sagemaker:UpdateEndpointWeightsAndCapacities (Application Auto Scaling checks the caller).
The hosting policy v2 adds it; the endpoint was verified deleted, so nothing billed. Everything else unchanged.

ops 5553 -- re-arm of 5552 after the third audit (B01/B02): the 5552 log showed model `test` READY and then a SECOND model
loading on the same GPU with 1,810 MB free -- both SERVING_LOAD_MODELS and HF_MODEL_ID registered the model, so it was loaded
twice and the health ping failed. Now: ONE registration (HF_MODEL_ID only), an explicit container startup health-check timeout
(1,800 s) and model download timeout on the variant, fp16 -> bf16 on A10G, cleanup confirmed only on the provider's own
not-found error (a timeout leaves `unresolved_cleanup`), and the same 50-minute wait + CloudWatch tail + chat-path probe.
...
def main() -> int:
    ...
    with report("ops_5563_owned_async_endpoint_recreate") as R:
        R.heading("ops 5563 (re-create after the guard fix) -- owned model async endpoint (scale-to-zero) from the staged weights; proven by one round trip")
        ...
        try:
            ...
            parn = "arn:aws:iam::%s:policy/justhodl-runner-hosting-v3" % ACCOUNT
            try:
                iam.create_policy(PolicyName="justhodl-runner-hosting-v3", Description="Runner: create/describe/delete the owned async endpoint + autoscaling",
                                  PolicyDocument=json.dumps({"Version": "2012-10-17", "Statement": [
                                      {"Effect": "Allow", "Action": ["sagemaker:CreateModel", "sagemaker:DescribeModel", "sagemaker:DeleteModel", "sagemaker:CreateEndpointConfig",
                                                                     ...
                                                                     "sagemaker:DeleteEndpoint", "sagemaker:UpdateEndpoint", "sagemaker:UpdateEndpointWeightsAndCapacities", "sagemaker:InvokeEndpointAsync", "sagemaker:AddTags", "sagemaker:ListTags"],
                                      ...
                                      {"Effect": "Allow", "Action": ["iam:PutRolePolicy", "iam:GetRolePolicy", "iam:GetRole"], "Resource": "arn:aws:iam::%s:role/justhodl-*" % ACCOUNT},
                                      {"Effect": "Allow", "Action": ["lambda:GetFunctionConfiguration"], "Resource": "arn:aws:lambda:%s:%s:function:justhodl-ai" % (REGION, ACCOUNT)}]}))
        ...
        # ONE model registration (B01): HF_MODEL_ID alone; SERVING_LOAD_MODELS is gone (it registered the model a second time)
        env = {"HF_MODEL_ID": "/opt/ml/model", "OPTION_ROLLING_BATCH": "vllm", "OPTION_TENSOR_PARALLEL_DEGREE": "1", "OPTION_MAX_MODEL_LEN": "8192",
               "OPTION_MAX_ROLLING_BATCH_SIZE": "8", "OPTION_DTYPE": "bf16", "OPTION_TRUST_REMOTE_CODE": "false", "OPTION_GPU_MEMORY_UTILIZATION": "0.9"}
        ...
        sm.create_endpoint_config(EndpointConfigName=cfg_name,
                                  ProductionVariants=[{"VariantName": "owned", "ModelName": model_name, "InstanceType": INSTANCE, "InitialInstanceCount": 1,
                                                       "ContainerStartupHealthCheckTimeoutInSeconds": 1800, "ModelDataDownloadTimeoutInSeconds": 1800}],
        ...
        def verified_delete():
            """'deleted' only on the provider's own not-found response; anything else is 'unresolved_cleanup' (B02)."""
            try:
            ...
            for _ in range(30):
                try:
                    ...
                    msg = str(e)
                    if code in ("ValidationException",) and ("Could not find endpoint" in msg or "not found" in msg.lower()):
                        return "deleted"
                    R.log("describe during cleanup: %s %s" % (code, msg[:120]))
                    time.sleep(10)
            return "unresolved_cleanup (endpoint may still bill -- check the console)"
        ...
        # Lambda role may invoke (before the control is enabled) -- the role is READ from the function, never assumed
        try:
            iam = boto3.client("iam")
            lam = boto3.client("lambda", region_name=REGION)
            role_arn = lam.get_function_configuration(FunctionName="justhodl-ai")["Role"]
            role_name = role_arn.split("/")[-1]
            R.ok("justhodl-ai execution role: %s" % role_name)
            iam.put_role_policy(RoleName=role_name, PolicyName="justhodl-owned-inference-invoke", PolicyDocument=json.dumps({"Version": "2012-10-17", "Statement": [
            ...
            R.ok("%s may InvokeEndpointAsync on %s" % (role_name, NAME))
        except Exception as e:  # noqa: BLE001
        ...
        control = {"schema_version": "factory-inference-control.v1", "enabled": True, "endpoint_name": NAME, "mode": "async-scale-to-zero",
                   ...
                   "endpoint_config": cfg_name, "written_by": "ops 5563", "git_sha": head[:12], "at": datetime.now(timezone.utc).isoformat()}
