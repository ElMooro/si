"""ops_5146 -- v1.5.2: FRED gate (429 backoff), catalog leaks banked on demand (DGS2MO), OECD ISO2 ids, local rows carry sources.
...
def http(url, timeout=120):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5146", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5146-search-sources") as r:
        r.heading("ops 5146 -- search bar shows every source; TradingView-only symbols resolve to the warehouse (v1.5.2)")
        fails = []
        ...
        if h.get("version") != "1.5.2":
            fails.append(f"version {h.get('version')} != 1.5.2")
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5146-us02my.jpg"), type="jpeg", quality=55)
