...
def deployed_after_start(fn, tries=70):
    """STRICT: settle + LM >= RUN_START. The unambiguous signature."""
...
LADDER13 = ["SPY","IWM","EFA","EEM","HYG","IEF","GLD","SLV","DBC",
            "VNQ","BTC","ETH","CASH"]
fails = []
with report("4297_wave2_wires") as r:
    r.heading("ops 4297 -- 30 wires + strict-LM law")

    r.section("1. stress ladder, race-proof retry")
    if not deployed_after_start("justhodl-stress-scenarios"):
        fails.append("stress LM never crossed RUN_START")
    else:
    ...
    if fails:
        ...
        fails.append("desk LM never crossed RUN_START")
    else:
    ...
    if fails:
        ...
        r.ok("OPS 4297 PASS -- 30 wires live under the strict-LM law")
if fails:
    sys.exit(1)
