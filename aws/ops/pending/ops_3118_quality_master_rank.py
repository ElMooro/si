#!/usr/bin/env python3
"""ops 3118 -- QUALITY PROPAGATION #1: master-rank to research-desk
...
footer. Zero engine changes, zero API budget."""
...
UA = {"User-Agent": "Mozilla/5.0 ops-3118", "Cache-Control": "no-cache"}
...
def main():
    fails, warns = [], []
    with report("3118_quality_master_rank") as rep:
        ...
        if not ok:
            ...
            for m in ("financials</b>", "concern ", "M\u2191 deteriorating"
                      .encode().decode("unicode_escape"),
                      "forensic \u2197".encode().decode(
                          "unicode_escape"),
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3118.json").write_text(json.dumps(
        {"ops": 3118, "verdict": "FAIL" if fails else "PASS",
