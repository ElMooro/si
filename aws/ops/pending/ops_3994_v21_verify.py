"""
ops_3994 — v2.1 verify: the families that were structurally impossible
...
MARK = "data-census v2.1 ops3993 full-lists"


def main():
    with report("3994_v21_verify") as rep:
        rep.heading("ops 3992 — v2.1 vault-complete verify")
        checks = []
        doc = None
        for i in range(26):
            ...
            if doc.get("marker") == MARK:
                rep.ok(f"  v2.1 artifact after ~{i*30}s")
                break
            rep.log(f"  [{i}] {str(doc.get('marker'))[:40]}")
            time.sleep(30)
        checks.append(("artifact is v2.1", doc.get("marker") == MARK))
        if doc.get("marker") != MARK:
            for l, ok in checks:
                (rep.ok if ok else rep.fail)(f"  {l}")
            rep.fail("v2.1 never wrote in the window")
            sys.exit(1)
        ...
        if failed:
            ...
            sys.exit(1)
        rep.ok(f"PASS_ALL — v2.1 complete: {tot} points / {len(bs)} families; "
