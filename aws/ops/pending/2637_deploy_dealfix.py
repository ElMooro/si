...
print("INVOKE:",r.get("StatusCode"),r.get("FunctionError"),r["Payload"].read().decode()[:140])
time.sleep(2)
j=json.loads(s3.get_object(Bucket="justhodl-dashboard-live",Key="data/deal-scanner.json")["Body"].read())
s=j.get("summary",{})
print("\nsummary:",{k:s.get(k) for k in ['n_prs_scanned','n_deals','n_green','n_yellow','n_ai','n_ai_mega','n_small_cap']})
print("\nDEALS (post-fix):")
for d in (j.get("deals") or []):
    print(f"  {d['symbol']:6s} hl={str(d.get('highlight')):6s} ai={int(bool(d.get('ai_relevant')))} sz={d.get('deal_value_str') or '—':>10s} vsMC={d.get('vs_market_cap_pct')}% mat={d.get('materiality_pct')} | {d['title'][:64]}")
print("DONE 2637")
