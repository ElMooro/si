"""ops_4051 — dynamic version badge from the served zip's own manifest."""
import io
import json
import sys
import time
...
def main():
    with report("4051_version_badge6") as rep:
        ...
        checks = [("zip is v1.6.0", ver == "1.7.3"),
                  ("version.json publicly serves it",
                   got.get("version") == "1.7.3")]
        n, htm = 0, ""
        for i in range(9):
            try:
