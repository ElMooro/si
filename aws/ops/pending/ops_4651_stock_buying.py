...
def main():
    ...
    with report("4651_stock_buying") as r:
        ...
        r.section("column census (293) + concept samples")
        try:
            doc0 = json.loads(s3.get_object(
                Bucket=B, Key="data/fundamental-census-matrix"
                ".json")["Body"].read())
            ...
            for i in range(0, len(names), 10):
                r.log("cols: " + " | ".join(names[i:i + 10]))
            ticks = doc0.get("tickers") or []
            ti = ticks.index("AAPL") if "AAPL" in ticks else 0
            import re as _re
            for concept, pat in (
                    ("peg", r"peg"),
                    ("issuance", r"issu|repurch|buyback|retire"),
                    ("basicshares", r"share"),
                    ("qeps", r"eps.*(_q|quarter|qoq)|"
                     r"(_q|qoq).*eps"),
                    ("qrev", r"rev.*(_q|quarter|qoq)|"
                     r"(_q|qoq).*rev"),
                    ("roic", r"roic")):
                hits = [n for n in names
                        if _re.search(pat, n, _re.I)][:6]
                vals = []
                for h in hits[:3]:
                    col = M.get(h)
                    v = col[ti] if isinstance(col, list)                         and ti < len(col) else None
                    vals.append("%s=%s" % (h, str(v)[:44]))
                r.log("[%s] hits=%s · %s"
                      % (concept, hits, " ; ".join(vals)))
        except Exception as e:
            r.warn("col census: %s" % str(e)[:110])
        try:
            ...
            try:
                ...
                src = zipfile.ZipFile(io.BytesIO(zb)).read(
                    "lambda_function.py").decode("utf-8",
                                                 "replace")
                if "justhodl-stock-buying v1.0.5" in src:
                    settled = True
                    break
            except Exception:
                pass
