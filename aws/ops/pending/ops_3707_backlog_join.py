"""ops 3707 — Stage 1 of Khalid's reverse-engineered-backlog theory.
...
with report("3707_backlog_join") as rep:
    rep.heading("ops 3707 — backlog reverse-engineering, stage 1")
    out = {"gates": {}}
    fails = []
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3707.json").write_text(json.dumps({"verdict": "STARTED"}))
    try:
        def gate(n, ok, d):
        ...
        while time.time() - t0 < 300:
            try:
                ...
                if 'VERSION = "1.1.0"' in src and "rpo_asof" in bsrc:
                    print("  artifacts ready after", round(time.time() - t0), "s")
                    break
            except Exception as e:
                print("  retry:", str(e)[:70])
            time.sleep(20)
        gate("G1_shipped", 'VERSION = "1.1.0"' in src and "load_fundamentals" in src
    ...
    Path("aws/ops/reports/3707.json").write_text(json.dumps(out, indent=2, default=str))
