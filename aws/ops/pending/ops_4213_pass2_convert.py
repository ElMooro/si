"""ops_4213 — convert: vault fire, class-wise LIVE ledger, realistic gates."""
import io
import json
import re
import sys
...
def main():
    with report("4213_pass2_convert") as rep:
        ...
        pmi2 = sum(n for r5, n in reasons.items() if "PMI licensed" in r5)
        fam2 = sum(n for r5, n in reasons.items()
                   if "indicator family" in r5)
        rep.kv(pmi_labeled=pmi2, family_labeled=fam2)
        lotto = sum(1 for r6 in v.get("symbols") or []
                    if "lottery" in str(r6.get("resolution_note")))
        ylive = sum(1 for r6 in v.get("symbols") or []
                    if r6.get("status") == "LIVE" and re.match(
                        r"^[A-Z]{2}\\d{2}Y?$",
                        str(r6.get("symbol"))))
        rep.kv(lottery_pending=lotto, yield_live=ylive)
        pend2 = sum(1 for r6 in v.get("symbols") or []
                    if r6.get("status") == "PENDING_RESOLUTION")
        rep.kv(pending_after=pend2)
        rep.kv(honest_label_rows=lab)
        if lab < 700:
            rep.fail(f"labels regressed: {lab}")
            sys.exit(1)
