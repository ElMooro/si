"""ops 4536 — Perplexity's CONDITIONAL-PASS work order, items 1-5 + smoke
...
R={"ops":4536,"started":datetime.now(timezone.utc).isoformat()}
def deploy(fn):
    for _ in range(20):
...
try:
    ...
    for _pf in ("justhodl-crisis-plumbing","justhodl-crisis-composite"):
        try:
            _ii=lam.invoke(FunctionName=_pf,InvocationType="RequestResponse",Payload=b"{}")
            R.setdefault("crisis_writers",{})[_pf]=_ii.get("FunctionError")
            _=_ii["Payload"].read()
        except Exception as _e: R.setdefault("crisis_writers",{})[_pf]=str(_e)[:60]
...
R["verdict"]=("cw="+json.dumps(R.get("crisis_writers"))+" | "+f"NFCI={R.get('NFCI')} canary={json.dumps(R.get('canary'))} plumbing_age={R.get('plumbing_age_min')} "
...
json.dump(R,open("aws/ops/reports/4536_order.json","w"),indent=1,default=str)
open("aws/ops/reports/4536_order.md","w").write("# 4536 — Perplexity order — "+R["verdict"]+"\n")
print(R["verdict"][:350])
