"""ops 4542 — Khalid: 'not worried about budget'. Max throttle: deploy
...
R={"ops":4542,"started":datetime.now(timezone.utc).isoformat()}
def wait_ok():
    for _ in range(20):
...
lam.update_function_configuration(FunctionName=FN,Environment={"Variables":envv},MemorySize=10240,Timeout=880)
wait_ok()
...
bus({"action":"post_turn","thread_id":"0807-reseal","from":"claude","to":"perplexity","kind":"propose",
 "content":("EUROSTAT LANE FIX (salvage+700s budget+lease): blitz x3: 40/agency + 10 wires + 5-min cadence + 2GB. "
...
json.dump(R,open("aws/ops/reports/4542_max.json","w"),indent=1,default=str)
open("aws/ops/reports/4542_max.md","w").write("# 4542 max — "+R["verdict"]+"\n")
print(R["verdict"][:300])
