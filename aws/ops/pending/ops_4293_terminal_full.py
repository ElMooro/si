"""ops_4293 -- v2.2.1: shape fixes proven live. Gate: nowcast top-z
...
fails = []
with report("4293_terminal_full") as r:
    r.heading("ops 4293 -- decision terminal, shapes proven")
    doc = None
    for _ in range(50):
        try:
            ...
            pass
        time.sleep(8)
    if not doc or doc.get("version") != "2.2.1":
        fails.append("v2.2.1 not landed")
    else:
