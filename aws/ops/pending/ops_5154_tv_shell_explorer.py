"""ops_5154 -- 5153 follow-up: hub-catalog fallback generalized (gdelt lists its 402k files), explorer + probe waits for rows.
...
def http(url, timeout=180):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5154", "Accept-Encoding": "identity"})
    try:
...
def main():
    with report("5154-tv-shell-explorer") as r:
        r.heading("ops 5154 -- TradingView shell + Data Explorer (follow-up)")
        fails = []
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5154-tv-shell.jpg"), type="jpeg", quality=55)
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5154-explorer-chart.jpg"), type="jpeg", quality=55)
