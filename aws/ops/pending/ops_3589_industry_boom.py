"""ops 3589 — justhodl-industry-boom v1 live-proof: deploy via helper, schedule
...
with report("3589_industry_boom") as rep:
    rep.heading("ops 3589 — Industry Boom League v1")
    out = {"gates": {}}
    fails = []
    ...
    try:
        ...
        if isinstance(pl, dict) and pl.get("errorMessage"):
            ...
            top = L[0] if L else {}
            comp_ok = bool(top) and any(top.get("comp", {}).get(f) not in (None, 0)
    ...
    Path("aws/ops/reports/3589.json").write_text(json.dumps(out, indent=2, default=str))
    sys.exit(0)
