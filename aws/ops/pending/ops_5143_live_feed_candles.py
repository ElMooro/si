"""ops_5143 -- 5142 follow-up: FRED 429 backoff, bare-series handoff on the page, evidence for the missing dropdown rows / legend.
...
def http(url, timeout=120):
    req = urllib.request.Request(url + ("&" if "?" in url else "?") + "v=" + str(int(time.time())), headers={"User-Agent": "justhodl-ops-5143", "Accept-Encoding": "identity"})
...
def main():
    with report("5143-live-feed-candles") as r:
        r.heading("ops 5143 -- live feed + candles: follow-up with evidence")
        fails = []
        r.section("S1 deploy + fredupdates schedule")
        for fn, mem, to in (("justhodl-symdir", 6144, 900),):
        ...
        if h.get("version") != "1.4.1":
            fails.append(f"symdir version {h.get('version')} != 1.4.1")
        ...
        while time.time() - t0 < 900:
            try:
                ...
                if b"ops 5143: a bare id the directory resolved" in c:
                    live = True
                    break
        ...
        if not live:
            ...
            with sync_playwright() as p:
                ...
                try:
                    ss = http_json("https://justhodl-data-proxy.raafouis.workers.dev/symsearch?q=HQMCB10YRP&limit=5&nocache=1", timeout=120)
                    r.log(f"  worker /symsearch HQMCB10YRP: rows={[x['id'] for x in ss.get('rows', [])]} total={ss.get('total')} err={ss.get('error')}")
                except Exception as e:  # noqa: BLE001
                    r.log(f"  worker /symsearch failed: {str(e)[:100]}")
                # HQMCB10YRP via the search bar + Enter
                page.click("#search-input")
                page.type("#search-input", "HQMCB10YRP", delay=40)
                page.wait_for_timeout(6000)
                top = page.evaluate("() => { const e = document.querySelector('#hsearch-dropdown .hs-row'); return e ? e.dataset.ticker : null; }")
                r.log("  dropdown html: " + page.evaluate("() => (document.getElementById('hsearch-dropdown').innerHTML || '').slice(0, 300).replace(/\\s+/g, ' ')"))
                ...
                if not (str(pr.get('active')).lower() == "fred:hqmcb10yrp" and "obs" in str(pr.get("meta"))):
                    r.log("  container html: " + page.evaluate("() => (document.getElementById('tv-container-0').innerHTML || '').slice(0, 400).replace(/\\s+/g, ' ')"))
                ...
                if not (lg.get("legend") and " O " in " " + str(lg.get("legend")) + " " and "H " in str(lg.get("legend"))):
                    r.log("  container html: " + page.evaluate("() => (document.getElementById('tv-container-0').innerHTML || '').slice(0, 500).replace(/\\s+/g, ' ')"))
                ...
                page.screenshot(path=str(ROOT / "aws" / "ops" / "reports" / "latest" / "5143-us10y-candles.jpg"), type="jpeg", quality=55)
                # AAPL live tail
