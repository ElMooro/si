"""1244 — Deploy best-setups conviction engine + invoke + verify."""
import json, os, time, zipfile, io
from datetime import datetime, timezone
import boto3
from botocore.config import Config

REPORT = "aws/ops/reports/1244_best_setups_rollout.json"
BUCKET = "justhodl-dashboard-live"
LAMBDA = "justhodl-best-setups"
SOURCE_DIR = "aws/lambdas/justhodl-best-setups/source"
ROLE_ARN = "arn:aws:iam::857687956942:role/lambda-execution-role"
ACCOUNT_ID = "857687956942"
REGION = "us-east-1"
RULE = "justhodl-best-setups-hourly"
SCHEDULE = "cron(10 * * * ? *)"
...
try:
    ...
    try:
        ...
        lam.create_function(FunctionName=LAMBDA, Runtime="python3.12", Role=ROLE_ARN,
            ...
            Description="Unified conviction engine", Timeout=120, MemorySize=512,
...
print("\n[1244] 2. Schedule")
try:
    events.put_rule(Name=RULE, ScheduleExpression=SCHEDULE, State="ENABLED", Description="Hourly best setups")
...
print("\n[1244] 4. Verify best-setups.json")
try:
    doc=json.loads(s3.get_object(Bucket=BUCKET, Key="data/best-setups.json")["Body"].read())
    top=doc.get("top_setups",[])
    out["output"]={"stats":doc.get("stats"),"weight_source":doc.get("weight_source"),
        "top12":[{"ticker":s["ticker"],"conviction":s["conviction"],"verdict":s["verdict"],
                   "n":s["n_signals"],"signals":s["signal_keys"]} for s in top[:12]]}
    print(f"  ✓ stats: {doc.get('stats')} · weights={doc.get('weight_source')}")
    print(f"\n  ━━━ TODAY'S BEST SETUPS ━━━")
    for s in top[:12]:
        print(f"    {s['verdict']:<11s} {s['ticker']:<6s} conv={s['conviction']:>5.1f}  {s['n_signals']}sig  [{', '.join(s['signal_keys'])}]")
except Exception as e:
