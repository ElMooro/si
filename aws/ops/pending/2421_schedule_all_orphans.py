...
lam=boto3.client("lambda","us-east-1"); iam=boto3.client("iam"); s3=boto3.client("s3","us-east-1")
ACCT="857687956942"; role=iam.get_role(RoleName="justhodl-scheduler-role")["Role"]["Arn"]
REPO=os.environ.get("GITHUB_WORKSPACE",".")
# scheduled index (rule targets + scheduler names)
ruled=set(); tok=None
while True:
    kw={"Limit":100}
...
while True:
    ...
    if not tok: break
# live functions (only schedule ones that exist)
fns=set(); tok=None
while True:
    kw={"MaxItems":1000}
...
# enumerate engine configs with a cron
base=os.path.join(REPO,"aws/lambdas")
orphans=[]
for eng in sorted(os.listdir(base)):
    ...
    sc=c.get("schedule") or {}
    cron=sc.get("cron")
    if cron and fn in fns and not is_scheduled(fn):
