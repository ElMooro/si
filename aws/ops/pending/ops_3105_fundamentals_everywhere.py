#!/usr/bin/env python3
"""ops 3105 -- FUNDAMENTALS EVERYWHERE (Khalid: mcap on opportunities
...
Verify: v1.4.0 gate, AAPL bands (fcf 1-6, peg 0.5-12), opp rows
...
UA = {"User-Agent": "Mozilla/5.0 ops-3105", "Cache-Control": "no-cache"}
...
def main():
    fails, warns = [], []
    with report("3105_fundamentals_everywhere") as rep:
        ...
        L.invoke(FunctionName="justhodl-opportunity-engine",
                 InvocationType="Event", Payload=b"{}")
        od = fresh("data/opportunities.json",
                   lambda x: any((r.get("capital_return") or
                                  r.get("market_cap"))
                                 for r in (x.get("opportunities")
                                           or x.get("rows") or [])[:80]),
                   tries=50)
        if not od:
            fails.append("no fresh joined opportunities.json")
        else:
            rws = od.get("opportunities") or od.get("rows") or []
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3105.json").write_text(json.dumps(
        {"ops": 3105, "verdict": "FAIL" if fails else "PASS",
