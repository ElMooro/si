#!/usr/bin/env python3
"""ops 3740 — import-canary.html + FIELD-COVERAGE AUDIT + nav pin.
...
with report("3740_grid_queue_page_regate") as rep:
    rep.heading("ops 3740 — grid-queue page + field-coverage audit")
    fails = []
    out = {"gates": {}}
    Path("aws/ops/reports").mkdir(parents=True, exist_ok=True)
    Path("aws/ops/reports/3740.json").write_text(json.dumps({"verdict": "STARTED"}))
    ...
    try:
        ...
        out["verdict"] = verdict
        out["unrendered"] = gaps
        Path("aws/ops/reports/3740.json").write_text(json.dumps(out, indent=2))
        ...
        Path("aws/ops/reports/3740.json").write_text(
