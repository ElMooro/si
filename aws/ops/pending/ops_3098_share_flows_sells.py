#!/usr/bin/env python3
"""ops 3098 -- SELLS: insider-trades output filtered side==buy at the last line, dropping parsed sells. Now emits sell_transactions; share-flows v1.1.1 joins them. Chain: invoke insider-trades -> wait for sell_transactions in its doc -> invoke share-flows -> verify. Prior: (3096 raced: verify accepted the still-fresh v1.0.0 doc from the prior run; now gate on engine version). ops 3096 was -- SHARE-FLOWS v1.1.0 verify (3095 fails fixed:
universe widened via phase-detector ~683 names + insider docs;
sells joined from insider-trades side=sell tape; split-artifact
data_suspect guard excludes WHLR-class reverse-split mismatches
from boards + sanity gate). Original brief (Khalid: track issuance,
...
UA = {"User-Agent": "Mozilla/5.0 ops-3098",
...
def main():
    fails, warns = [], []
    with report("3098_share_flows_sells") as rep:
        ...
        rep.section("2. Refresh insider-trades (sells at source)")
        # deploy-lambdas updates it on this push; wait for settle
        for _ in range(30):
            st = L.get_function_configuration(
                FunctionName="justhodl-insider-trades"
        ...
        for _ in range(30):
            ...
            try:
                ...
                if "sell_transactions" in doc:
                    n_sell_src = len(doc["sell_transactions"])
                    break
            except Exception:
                pass
        rep.kv(insider_trades_sell_rows=n_sell_src)
        if n_sell_src < 0:
            fails.append("insider-trades never emitted "
                         "sell_transactions")
        elif n_sell_src == 0:
            warns.append("Form-4 window has zero sells right now "
                         "(unusual but possible)")

        rep.section("3. Invoke share-flows + map truth")
        ...
        for _ in range(50):
            ...
            try:
                ...
                # 3096 lesson: freshness alone races the previous
                # run's doc -- require the new engine version too
                if cand.get("version") == "1.1.1" and \
        ...
        bad = [t for t, v in tk.items()
               ...
               and ((v.get("buyback_yield_pct") or 0) > 30
        ...
        n_sell = sum(1 for v in tk.values()
                     if v.get("insider_sell_usd_recent"))
        n_buy = sum(1 for v in tk.values()
                    if v.get("insider_buy_usd_90d"))
        n_susp = sum(1 for v in tk.values()
                     if v.get("data_suspect"))
        rep.kv(n_insider_buy=n_buy, n_insider_sell=n_sell,
               n_data_suspect=n_susp)
        if n_sell == 0 and n_sell_src > 0:
            fails.append("source has %d sell rows but join got 0"
                         % n_sell_src)
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3098.json").write_text(json.dumps(
        {"ops": 3098, "verdict": "FAIL" if fails else "PASS",
