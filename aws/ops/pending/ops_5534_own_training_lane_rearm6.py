...
def main() -> int:
    ...
    with report("ops_5534_own_training_lane_rearm6") as R:
        R.heading("ops 5534 (re-arm of 5528-5533) -- owned training lane: ECR repo, pinned recipe bundle, base-weight staging job (CPU, priced), no flip")
        R.kv(head=head[:10])
        ...
        try:
            ...
            jarn = "arn:aws:iam::%s:policy/justhodl-runner-processing" % ACCOUNT
            try:
                iam.create_policy(PolicyName="justhodl-runner-processing", Description="Runner: create/describe tagged SageMaker processing jobs for weight staging",
                                  PolicyDocument=json.dumps({"Version": "2012-10-17", "Statement": [
                                      {"Effect": "Allow", "Action": ["sagemaker:CreateProcessingJob", "sagemaker:DescribeProcessingJob", "sagemaker:StopProcessingJob",
                                       ...
                                       "Resource": "arn:aws:sagemaker:%s:%s:processing-job/jh-stage-*" % (REGION, ACCOUNT)},
