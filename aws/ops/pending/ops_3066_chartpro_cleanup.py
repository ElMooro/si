#!/usr/bin/env python3
"""ops 3066 (retry) -- Chart Pro: conviction drawer removed (Alerts button
...
UA = {"User-Agent": "Mozilla/5.0 ops-3066",
...
def main():
    fails, warns = [], []
    with report("3066_chartpro_cleanup") as rep:
...
def _fin(rep, fails, warns):
    (AWS_DIR / "ops" / "reports" / "3066.json").write_text(json.dumps(
        {"ops": 3066, "verdict": "FAIL" if fails else "PASS",
