...
with report("3398_etf_phases") as rep:
    ...
    while time.time()<dl:
        try:
            if LAM.get_function_configuration(FunctionName="justhodl-accumulation-radar").get("LastUpdateStatus")=="Successful":
                info=LAM.get_function(FunctionName="justhodl-accumulation-radar")
                with urllib.request.urlopen(urllib.request.Request(info["Code"]["Location"],headers=UA),timeout=60) as r:
                    if 'etf_phases' in zipfile.ZipFile(io.BytesIO(r.read())).read("lambda_function.py").decode("utf-8","replace"):
                        ok1=True; break
        except Exception: pass
        time.sleep(12)
    gate("G1_fix_deployed", ok1, "etf_phases marker")
    t2=datetime.now(timezone.utc).isoformat(); invoke_resilient("justhodl-accumulation-radar"); time.sleep(90); invoke_resilient("justhodl-sector-capital-fusion")
    wy, ok2 = 0, False
    dl=time.time()+300
    while time.time()<dl:
        try:
    ...
    Path("aws/ops/reports/3398.json").write_text(json.dumps(out,indent=2)); sys.exit(0)
