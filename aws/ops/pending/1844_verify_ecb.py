...
back=((d.get("layers") or {}).get("backstops") or {}).get("metrics") or []
print("Backstops layer (%d metrics):"%len(back))
for m in back:
    print("  [%s] %s = %s%s"%(m.get("status"),m.get("label"),m.get("value"),m.get("unit","")))
