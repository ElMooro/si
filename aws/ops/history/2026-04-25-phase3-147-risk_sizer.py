#!/usr/bin/env python3
"""
Step 147 — Phase 3: Risk management layer.

The most important phase of the hedge-fund-grade system. Real hedge
fund risk management has decades of refinement and we can\\'t replicate
that, but we CAN build the 80% that prevents the most common retail
mistakes.

THE FOUR MISTAKES THIS PREVENTS:

  1. CORRELATION BLINDNESS. SPY/QQQ/NVDA/AAPL aren\\'t 4 ideas — they\\'re
     1 idea with 4 expressions. Sizing each at 5% = 20% concentrated
     in US large-cap tech. We cluster correlated ideas and cap exposure
     PER CLUSTER, not per name.

  2. EQUAL-WEIGHTING. Putting same size in 60% conviction trade as 90%
     conviction trade. Conviction-weighted sizing using fractional Kelly
     (1/4 Kelly, conservative — full Kelly tends to overpunch on small
     samples).

  3. DRAWDOWN BLINDNESS. Most retail accounts blow up in regime changes
     because they don\\'t reduce when things go wrong. If hypothetical
     PnL drawdown > 5%, sizes scale down 25%. > 10% = 50%. > 15% = stop
     adding new positions.

  4. REGIME BLINDNESS. Maximum equity exposure shouldn\\'t be the same
     in BEAR regime as BULL regime. Phase 1A regime feeds maximum
     gross exposure:
       RISK_OFF: max 50% equity exposure, prefer defensive setups
       NEUTRAL:  max 75% equity exposure
       RISK_ON:  max 100% equity exposure

WHAT\\'S EXPLICITLY OUT OF SCOPE (we\\'re honest):
  - VaR / CVaR (noise on small portfolios)
  - Stress testing scenarios (premature without longer track record)
  - Multi-asset risk parity (overfit to historical data)
  - Margin/leverage modeling (you\\'re not running a leveraged book)

INPUTS THE LAMBDA READS:
  - opportunities/asymmetric-equity.json (Phase 2B setups)
  - investor-debate/_index.json (Loop 4 watchlist debate)
  - regime/current.json (Phase 1A regime)
  - portfolio/pnl-history.json (Loop 2 hypothetical PnL — for drawdown)
  - portfolio/state.json (current allocation)

  For correlation clustering, we use ETF return correlations as
  proxies for individual stock correlations:
    - data/report.json stocks history (120 daily bars per ticker)

OUTPUT:
  s3://justhodl-dashboard-live/risk/recommendations.json
    {
      \"as_of\": ...,
      \"regime\": ...,
      \"max_gross_exposure_pct\": 75,
      \"drawdown_status\": {\"current_dd\": -2.1, \"size_multiplier\": 1.0},
      \"clusters\": [
        {\"id\": \"us_large_tech\", \"members\": [...], \"cluster_correlation\": 0.78}
      ],
      \"sized_recommendations\": [
        {\"symbol\": \"INCY\", \"raw_conviction\": 7, \"cluster\": \"healthcare\",
         \"recommended_size_pct\": 4.5, \"reasoning\": \"...\"}
      ],
      \"warnings\": [...]
    }

LAMBDA: justhodl-risk-sizer (256MB arm64, 60s)
SCHEDULE: cron(45 13 ? * MON-FRI *) — daily 13:45 UTC, after asymmetric
scorer at 13:30. Reads existing S3 outputs, no external API calls.
"""
import io
import json
import os
...
with report("build_risk_sizer") as r:
    r.heading("Phase 3 — Risk Management Layer (sizing + clustering + circuit breakers)")

    # ─── 1. Verify all input data sources ──────────────────────────────
    r.section("1. Verify input data sources")
    inputs = [
        ("opportunities/asymmetric-equity.json", "Phase 2B setups"),
        ("regime/current.json", "Phase 1A regime"),
        ("portfolio/pnl-history.json", "Loop 2 PnL history"),
        ("portfolio/state.json", "Portfolio state"),
        ("data/report.json", "Stock returns for clustering"),
    ]
    for key, label in inputs:
        try:
            obj = s3.head_object(Bucket=BUCKET, Key=key)
            age_min = (datetime.now(timezone.utc) - obj["LastModified"]).total_seconds() / 60
            r.log(f"  {label:35} {key}  age={age_min:.1f}min  {obj['ContentLength']:,}B")
        except Exception as e:
            r.warn(f"  {label}: {e}")

    # Note: Loop 4 debate hasn't run yet (first scheduled run is tonight 03:00 UTC)
    try:
        obj = s3.head_object(Bucket=BUCKET, Key="investor-debate/_index.json")
        r.log(f"  Loop 4 debate: present, age {(datetime.now(timezone.utc) - obj['LastModified']).total_seconds()/3600:.1f}h")
    except Exception:
        r.log(f"  Loop 4 debate: not yet present (first scheduled run is tonight at 03:00 UTC) — Phase 3 handles missing gracefully")

    # ─── 2. Set up Lambda ───────────────────────────────────────────────
    r.section("2. Set up justhodl-risk-sizer Lambda")
    src_dir = REPO_ROOT / "aws/lambdas/justhodl-risk-sizer/source"
    src_dir.mkdir(parents=True, exist_ok=True)
    (src_dir / "lambda_function.py").write_text(SIZER_SRC)

    import ast
    try:
        ast.parse(SIZER_SRC)
        r.ok(f"  Wrote source: {len(SIZER_SRC):,}B, {SIZER_SRC.count(chr(10))} LOC")
    except SyntaxError as e:
        r.fail(f"  Syntax: {e}")
        if hasattr(e, "lineno"):
            lines = SIZER_SRC.split("\n")
    ...
    try:
        ...
        lam.create_function(
            ...
            Code={"ZipFile": zbytes},
            Description="Phase 3 — Risk management layer (sizing + clustering + circuit breakers)",
            Timeout=60,
            MemorySize=256,
            Architectures=["arm64"],
    ...
    # ─── 4. Read full risk recommendations ──────────────────────────────
    r.section("4. risk/recommendations.json — full report")
    try:
        ...
        for rec in snap.get("sized_recommendations", [])[:10]:
            r.log(f"    {rec['symbol']:6} {rec.get('sector','?')[:15]:15} "
        ...
        r.log(f"\n  Cluster summary:")
        for c in snap.get("clusters", [])[:6]:
            r.log(f"    {c['id'][:25]:25} size={c['size']} avg_corr={c['avg_correlation']}")
    except Exception as e:
        r.warn(f"  read: {e}")

    # ─── 5. Schedule daily 13:45 UTC ────────────────────────────────────
    r.section("5. Schedule cron(45 13 ? * MON-FRI *)")
    ...
    try:
        try:
            ...
            events.put_rule(
                ...
                State="ENABLED",
                Description="Phase 3 — daily risk sizing after asymmetric scorer",
            )
