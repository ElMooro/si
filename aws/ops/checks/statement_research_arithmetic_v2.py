...
def current_pairs(identity_ref, read):
    cap=read_original(identity_ref,read)
    assert cap['url']=='https://www.sec.gov/files/company_tickers.json' and cap['http_status']==200
    raw=read_original(cap['original'],read)
    pairs=defaultdict(set)
    assert isinstance(raw,dict) and 1000<=len(raw)<=50000
    for key,row in raw.items():
        assert key.isdigit() and type(row['cik_str']) is int and 0<row['cik_str']<10**10
        assert isinstance(row['ticker'],str) and row['ticker'] and isinstance(row['title'],str) and row['title']
        pairs[row['ticker']].add(str(row['cik_str']).zfill(10))
    return {k:sorted(v) for k,v in pairs.items()}


def corroboration(row,symbol,pairs):
    ciks=pairs.get(symbol,[]);reported=row.get('cik')
    if not ciks:return 'not_in_current_sec_ticker_index'
    if len(ciks)!=1:return 'ambiguous_current_sec_ticker_index'
    if type(reported) not in (int,str) or not re.fullmatch('[0-9]{1,10}',str(reported)) or int(reported)==0:
        return 'invalid_provider_cik'
    if str(reported).zfill(10)!=ciks[0]:return 'provider_cik_differs_from_current_sec_index'
    return 'current_ticker_cik_pair_corroborated'
...
def metadata(value):
    ...
    if type(value) in (int,Decimal):return 'number',str(value) if type(value) is Decimal else value
    if type(value) is list:return 'array',[metadata(v)[1] for v in value]
    if type(value) is dict:return 'object',{k:metadata(v)[1] for k,v in value.items()}
    raise AssertionError('Invalid original JSON type')


def verify(manifest_ref, identity_ref, compiled, read):
    pairs=current_pairs(identity_ref,read)
    ...
    for url, ref in manifest['captures'].items():
        ...
        for index,row in enumerate(rows):
            ...
            eligible=valid(row,cap['spec'],cap['received_at']) and corroboration(row,cap['spec']['symbol'],pairs)=='current_ticker_cik_pair_corroborated'
    ...
    covered=set(); tested=available=withheld=zeroes=invalid=0; joins=set(); identity_counts=Counter();clock_counts=Counter()
    for symbol,shard in compiled['shards'].items():
        assert shard['requested_symbol']==symbol
        for record in shard['records']:
            assert all(record.get(k) is False for k in ('calls_eligible','sizing_eligible','execution_eligible','forecast_qualified'))
            ...
            assert len(record['identity_evidence'])==len(record['source_rows'])
            assert {(v['capture_id'],v['source_row']) for v in record['identity_evidence']}==coordinates
            matched=[]
            for evidence in record['identity_evidence']:
                cap,row,_=originals[(evidence['capture_id'],evidence['source_row'])]
                assert evidence['source_id']==cap['original']['sha256'] and evidence['endpoint']==cap['spec']['endpoint']
                assert evidence['request_period']==cap['spec']['period']
                assert set(evidence['reported_identity'])==set(FIELDS) and set(evidence['reported_identity_types'])==set(FIELDS)
                for field in FIELDS:
                    kind,value=metadata(row.get(field))
                    assert evidence['reported_identity'][field]==value and evidence['reported_identity_types'][field]==kind
                assert evidence['missing_fields']==[k for k in FIELDS if k not in row]
                status=corroboration(row,symbol,pairs)
                assert evidence['current_identity_status']==status and evidence['current_sec_ciks']==pairs.get(symbol,[])
                assert not evidence['historical_security_continuity_verified']
                identity_counts[status]+=1;matched.append(status=='current_ticker_cik_pair_corroborated')
                issues=[]
                try:
                    end=date.fromisoformat(str(row.get('date')))
                    for field in ('filingDate','acceptedDate'):
                        if date.fromisoformat(str(row.get(field))[:10])<end:issues.append(field+'_precedes_period_end')
                except ValueError:issues.append('invalid_provider_date')
                assert evidence['clock_issues']==issues;clock_counts.update(issues)
            assert record['current_ticker_cik_corroborated']==all(matched)
            assert not record['historical_security_continuity_verified']
            if record['measurements'] is None:
                assert all(not item[2] for values in entries.values() for item in values)
                invalid+=len(coordinates);continue
    ...
    assert all(packet.get(k) is None for k in ('call','score','m_score','grade'))
    assert packet['current_identity_statuses']==dict(identity_counts) and packet['clock_issues']==dict(clock_counts)
    assert packet['current_identity_quarantined_rows']==sum(n for k,n in identity_counts.items() if k!='current_ticker_cik_pair_corroborated')
    raw_universe=read_original(manifest['universe'],read)
    universe_clock=raw_universe.get('generated_at') if isinstance(raw_universe,dict) else None
    universe=next(v for k,v in raw_universe.items() if k in ('rows','stocks','data') and isinstance(v,list)) if isinstance(raw_universe,dict) else raw_universe
    for summary in packet['issuers']:
        origin=summary['universe_record'];row=universe[origin['source_row']]
        assert (row.get('symbol') or row.get('ticker'))==summary['symbol']
        assert origin['source_id']==manifest['universe']['sha256'] and origin['source_generated_at']==universe_clock
        assert origin['reported_classification']=={k:metadata(row.get(k))[1] for k in ('sector','industry')}
        assert origin['classification_fields_present']==[k for k in ('sector','industry') if k in row]
        assert not origin['classification_independently_verified']
    ...
    return {'original_rows_checked':len(covered),'exact_identity_groups_checked':len(joins),
        'invalid_or_uncorroborated_identity_rows_checked':invalid, 'identity_metadata_rows_checked':len(covered),
        'current_sec_pairs_checked':True, 'current_identity_statuses':dict(identity_counts), 'clock_issues':dict(clock_counts),'metric_comparisons':tested,'exact_rational_values_checked':available,
