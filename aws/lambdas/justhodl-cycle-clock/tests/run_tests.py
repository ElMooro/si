...
if __name__=='__main__':
    import subprocess,sys
    from pathlib import Path
    subprocess.run([sys.executable,str(Path(__file__).resolve().parents[4]/'tests/fomc_consumer_test_support.py')],check=True)

if __name__=='__main__':
    import subprocess
    subprocess.run([sys.executable,str(Path(__file__).with_name('test_cycle_native.py'))],check=True)
    subprocess.run([sys.executable,str(ROOT/'tests/cycle_consumer_test_support.py')],check=True)
