"""Canonical-original Eurodollar replay; public inputs, protected retention and CAS.
...
SOURCES=('data/report-measurements.json','data/polygon-fx-regime.json')
...
def compile_output(inputs,read):
    if inputs.get('contract')!='eurodollar-native-inputs.v1':raise ValueError('Unsupported input contract')
    packet=source(inputs['macro'],SOURCES[0],read)
    fx=source(inputs['fx'],SOURCES[1],read) if inputs.get('fx') else {}
    originals=canonical_fred_replay.restore(packet,model.SERIES,read)
    return model.build(packet,originals,fx,inputs['generated_at'])
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'eurodollar-native-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('native Eurodollar original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'eurodollar-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Nine canonical FRED definitions/observations reconstructed without duplicate provider requests. Same-date funding comparison and 60 valid adjacent yield changes. No forecast or historical first-availability claim.'}
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'eurodollar-public-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        try:fx=snapshot(client,bucket,SOURCES[1])
        except Exception as exc:
            if not missing(exc):raise
            fx=None
        inputs={'contract':'eurodollar-native-inputs.v1','macro':macro,'fx':fx,'generated_at':now()}
        ...
        raise RuntimeError('Native Eurodollar publication failed; inspect reviewed request evidence') from None
