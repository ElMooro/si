"""Dated fedwatch input research producer; HTTP reads, scheduled events publish."""
import json
import boto3
from botocore.config import Config
from fedwatch_research_model import CONTRACT
from fedwatch_research_store import CURRENT,reader,run
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod'):
        ...
        if packet.get('contract')!=CONTRACT:
            return {'statusCode':503,'body':json.dumps({'reason':'native_fedwatch_publication_unavailable'})}
