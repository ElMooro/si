...
OUTPUT  data/best-ideas.json        SCHEDULE daily 14:45 UTC
...
SPECS = [
    ...
    ("epsrev", "EPS growth slope (FY2 vs FY1, NOT a revision)", "data/eps-revision-velocity.json",
     ["all_qualifying"], "symbol", "score", "GROWTH",
     "has a steep FY2 vs FY1 consensus EPS slope (projected growth, not a same-fiscal revision)", 80),
    ("estrev", "Same-fiscal estimate revisions", "data/estimate-revisions.json",
     ["upward_revisions"], "ticker", "eps_rev_pct", "EARNINGS",
     "same-fiscal consensus EPS was revised up vs our prior snapshot", None),
    ("flowlt", "ETF look-through pressure", "data/flow-lookthrough.json",
     ["actual_accumulation"], "ticker", "flow_bps_mcap", "FLOW",
     "ETF flow × holdings weight implies accumulation (estimate, not a print)", None),
    ("earnqual", "Earnings cash-flow quality", "data/earnings-quality.json",
     ["top_20_high_quality"], "ticker", None, "QUALITY",
     "cash conversion and accruals screen as high quality (FMP filings)", None),
...
def regime_haircut():
    """Temper ALL conviction when bond-vol / regime says risk-off.
    Missing docs = no haircut (honest), never a fake risk-on."""
    cut, why = 1.0, None
    bv = _load_json("data/bond-vol.json") or {}
    rg = _load_json("data/regime.json") or {}
    state = (str(bv.get("regime") or bv.get("state") or rg.get("regime")
                 or rg.get("state") or "")).lower()
    move = num(bv.get("move") or bv.get("composite") or bv.get("level"))
    if "risk-off" in state or "risk_off" in state or "stress" in state:
        cut, why = 0.82, "regime risk-off haircut"
    elif move is not None and move >= 130:
        cut, why = 0.88, "elevated bond-vol haircut"
    return cut, why, state or None, move


def forensic_flags():
    """Beneish manipulators + critical SEC events: haircut, not a fake family hit."""
    bad = set()
    notes = {}
    bn = _load_json("data/beneish.json") or {}
    ...
    for it in (sec.get("events") or sec.get("critical") or []):
        ...
        if sy:
            bad.add(sy)
            notes[sy] = notes.get(sy, "SEC material event")
    return bad, notes
...
def lambda_handler(event, context):
    ...
    hair, hair_why, regime_state, move_lvl = regime_haircut()
    bad, bad_notes = forensic_flags()
    for s in stack:
        notes = []
        if hair < 1.0:
            s["conviction_score"] = round(s["conviction_score"] * hair, 1)
            notes.append(hair_why)
        sy = s["symbol"]
        if sy in bad:
            s["conviction_score"] = round(s["conviction_score"] * 0.75, 1)
            s["forensic_flag"] = bad_notes.get(sy)
            notes.append(bad_notes.get(sy))
            s["risk"] = (s.get("risk") or "") + " Forensic overlay: " + (bad_notes.get(sy) or "flagged") + "."
        if notes:
            s["overlays"] = notes
    ...
    out = {
        ...
        "how_to_read": (
            ...
            "opportunity engines, in two DIFFERENT factor families. "
