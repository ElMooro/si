"""Original FX quote research; HTTP reads never collect or publish."""
import json
import boto3
from botocore.config import Config
from massive import get_massive_key
from fx_research_model import CONTRACT
from fx_research_store import bounded,run,missing
PUBLISHED_KEY='data/fx-quote-research.json'


def publish_current(client,bucket,key,raw,condition):
    if key!=PUBLISHED_KEY or set(condition) not in ({'IfMatch'},{'IfNoneMatch'}):
        raise ValueError('Reviewed FX target and conditional write required')
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod') or event.get('action')=='current_state':
        ...
        if packet.get('contract')!=CONTRACT:
            return {'statusCode':503,'body':json.dumps({'reason':'native_fx_research_unavailable'})}
    ...
    remaining=context.get_remaining_time_in_millis()/1000 if hasattr(context,'get_remaining_time_in_millis') else 60
