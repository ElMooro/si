...
from sector_research_model import CONTRACT
from sector_research_store import CURRENT,reader,run
PUBLISHED_KEY='data/sector-rotation.json'
if CURRENT!=PUBLISHED_KEY:raise ValueError('Sector publication route drift')
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod') or event.get('action')=='current_state':
        ...
        if packet.get('contract')!=CONTRACT:
            return {'statusCode':503,'body':json.dumps({'reason':'native_sector_publication_unavailable'})}
