...
PRIVATE='audit-private/20260909-originals/dealer-research/'
...
def bounded(body,limit=MAX_BYTES):
    ...
    if len(raw)>limit:raise ValueError('dealer artifact exceeds byte bound')
    return raw
...
LEGACY_KEYS=(model.CURRENT,'data/history/nyfed-pd.json','data/history/nyfed-pd-corp.json','data/config/nyfed-pd-spec.json')
...
def preserve(client,bucket,read):
    ...
    try:marker=native.strict_json(read(key))
        ...
        artifacts=[]
        for target in LEGACY_KEYS:
            try:raw=read(target)
            except Exception as err:
                if not missing(err):raise
                artifacts.append({'source':target,'status':'absent_at_migration'});continue
            old=native.strict_json(raw)
            if target==model.CURRENT and old.get('contract')==model.CONTRACT:raise ValueError('legacy retention marker missing')
            sha=hashlib.sha256(raw).hexdigest();immutable(client,bucket,PRIVATE+sha+'.bin',raw,'application/octet-stream')
            artifacts.append({'source':target,'sha256':sha,'bytes':len(raw),'protected_backup':True,'status':'retained'})
        marker={'contract':'dealer-legacy-preservation.v1','artifacts':artifacts}
        immutable(client,bucket,key,native.encoded(marker))
    if marker.get('contract')!='dealer-legacy-preservation.v1' or {r['source'] for r in marker['artifacts']}!=set(LEGACY_KEYS):
        raise ValueError('legacy retention identity differs')
    return marker


def context(client,bucket,read):
    try:raw=read('data/settlement-fails.json')
    except Exception as exc:
        if not missing(exc):raise
        return None
    sha=hashlib.sha256(raw).hexdigest();key=model.PREFIX+'contexts/'+sha+'.json'
...
def publish(client,bucket,packet,expected_previous):
    ...
    for _ in range(5):
        ...
        try:
            ...
            if not conflict(exc):raise
    raise RuntimeError('dealer publication conflict bound')


def replay(manifest,read):
    if manifest.get('contract')!='dealer-original-replay.v1' or set(manifest.get('compilers',{}))!={m.__name__ for m in COMPILERS}:
        raise ValueError('dealer replay contract differs')
    registry=native.REGISTRY_PATH.read_bytes();registry_sha=hashlib.sha256(registry).hexdigest()
    if manifest.get('registry')!={'key':model.PREFIX+'definitions/'+registry_sha+'.json','sha256':registry_sha} or read(manifest['registry']['key'])!=registry:
        raise ValueError('reviewed registry differs; use matching checkout')
...
def run(client,bucket):
    ...
    stamp=now();inputs={'contract':'dealer-original-inputs.v1','generated_at':stamp,'sources':captures,
                       'legacy_context':legacy,'previous_output':prior,'fails_context':context(client,bucket,read)}
    ...
    registry=native.REGISTRY_PATH.read_bytes();registry_sha=hashlib.sha256(registry).hexdigest();registry_key=model.PREFIX+'definitions/'+registry_sha+'.json'
    immutable(client,bucket,registry_key,registry)
    manifest={'registry':{'key':registry_key,'sha256':registry_sha},'contract':'dealer-original-replay.v1','generated_at':stamp,'compilers':compilers,**refs,'output_sha256':refs['output']['sha256']}
    ...
    return {'published':published,'generated_at':stamp,'replay':ref,'quality':output['quality'],
            'original_series':len(output['native_series']),'retained_observations':sum(len(r['history']) for r in output['native_series'].values()),
