"""Original provider flow research; read-only HTTP and idempotent scheduled publication."""
import json
import os
import boto3
from botocore.config import Config
from provider_flow_model import CONTRACT as CONTRACT
from provider_flow_store import reader, run
PUBLISHED_KEY = 'data/provider-fund-flow-research.json'
KIND = 'flow'
...
def lambda_handler(event=None, context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod') or event.get('action') == 'current_state':
        ...
        if packet.get('contract') != CONTRACT:
            return {'statusCode': 503, 'body': json.dumps({'reason': 'native_provider_flow_evidence_unavailable'})}
    ...
    remaining = context.get_remaining_time_in_millis() / 1000 if hasattr(context, 'get_remaining_time_in_millis') else 300
    credential = (os.environ.get('POLYGON_KEY') or os.environ.get('POLYGON_API_KEY') or '') if KIND == 'flow' else ''
