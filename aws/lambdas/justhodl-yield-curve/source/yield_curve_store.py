...
COMPILERS=(model,arithmetic,catalog,canonical,report_observations,research_brief_model,evidence_store,sys.modules[__name__])
QUALIFIED={'yield_curve_arithmetic': '58b6de8b604213dc428c593826f6e224e13571cd6fe5f8095a2265cd71bf0e9e', 'yield_curve_catalog': 'bcdbc576f803c81973bafe3126624c05f60ab960c79a005e91fcda028a8360ed'}
...
def qualified_arithmetic():
    for module in (arithmetic,catalog):
...
def allowed(key):
    return isinstance(key,str) and (key in (SOURCE,model.CURRENT,'data/term-premium.json') or bool(re.fullmatch(
        r'data/(?:evidence/fred/[a-f0-9]{64}/[a-f0-9]{64}\.bin\.gz|'
        r'(?:report-research|yield-curve-research)/(?:runs|inputs|outputs|compilers|snapshots|series|views)/[a-f0-9]{64}\.(?:json|py))',key)))


def reader(client,bucket):
    def read(key):
        if not allowed(key):raise ValueError('Unapproved curve original path')
...
def compile_output(inputs,read):
    ...
    if inputs.get('contract')!='yield-curve-inputs.v1':raise ValueError('Native input contract required')
    source=checked(inputs['macro'],'snapshots',read)
    originals=canonical.restore(source,catalog.SERIES,read)
    return model.build(source,originals,inputs['generated_at'],inputs['context'],inputs['predecessor'],inputs['previous_watermarks'])
...
def replay(ref,read,verify_series=True):
    ...
    if (key!=model.PREFIX+'runs/'+sha(raw)+'.json' or manifest.get('contract')!='yield-curve-replay.v1'
    ...
    for sid,row in output['series'].items():
        ...
        if not same_json(ref,{'key':model.PREFIX+'series/'+sha(encoded)+'.json','sha256':sha(encoded),'bytes':len(encoded)}):
            raise ValueError('Series artifact identity differs: '+sid)
        if verify_series and not same_json(checked(ref,'series',read),row):raise ValueError('Whole series artifact differs: '+sid)
    view=model.compact(output,manifest['series'])
    if not same_json(view,checked(manifest['view'],'views',read)):raise ValueError('Compact projection differs')
    return output,view
...
def retain(client,bucket,inputs,output):
    ...
    manifest={'contract':'yield-curve-replay.v1','generated_at':inputs['generated_at'],**refs,
    ...
    if not same_json(restored,output) or not same_json(projected,view):raise ValueError('Fresh retained native replay differs')
    return ref,view
...
def context(client,bucket):
    ...
    return {'status':'retained_unqualified_context','source_key':key,'original':private_bytes(client,bucket,raw),
        'captured_at':datetime.now(timezone.utc).isoformat(),'independent_votes':0}
...
def publish(client,bucket,packet):
    ...
    if (packet.get('contract')!=model.CONTRACT or any(packet.get(k) is not False for k in flags)
        ...
        or packet.get('signals')!=[] or packet.get('qualified_term_premium_bps') is not None
        or packet.get('portfolio_consequences',{}).get('status')!='UNAVAILABLE'
        or packet.get('portfolio_consequences',{}).get('target_weights') is not None
        or packet.get('view',{}).get('contract')!='yield-curve-summary.v1'
        or set(packet.get('series',{}))!=set(catalog.SERIES)
        or set(packet.get('derived',{}))!=set(catalog.METRICS)
    ...
    for row in [*packet['series'].values(),*packet['derived'].values(),packet.get('shape',{})]:
        if any(row.get(k) is not False for k in flags):raise ValueError('Measurement acquired unqualified authority')
    # Rebind exactly the verified view, so even an innocuous value edit cannot
    # borrow another run's evidence. The full original replay happened in retain.
    read=reader(client,bucket);key=packet['replay']['manifest_key'];raw=read(key);manifest=strict(raw)
    if (key!=model.PREFIX+'runs/'+sha(raw)+'.json' or manifest.get('contract')!='yield-curve-replay.v1'
        or manifest.get('output_sha256')!=packet['replay']['output_sha256']
        or manifest.get('generated_at')!=packet['generated_at']
        or not same_json(checked(manifest['view'],'views',read),{k:v for k,v in packet.items() if k!='replay'})):
        raise ValueError('Publication differs from retained replay-bound view')
    ...
    for _ in range(4):
        ...
        try:
            ...
            if not same_json(live,packet) and model.clock(live['generated_at'])<=stamp:raise ValueError('Published view readback differs')
            return True
        except Exception as exc:
            if not conflict(exc):raise
...
def run(client,bucket):
    qualified_arithmetic()
    read=reader(client,bucket);raw=read(SOURCE);source=strict(raw)
    ...
    term=context(client,bucket)
    inputs={'contract':'yield-curve-inputs.v1','generated_at':datetime.now(timezone.utc).isoformat(),
        'macro':source_ref,'context':term,'predecessor':predecessor,'previous_watermarks':previous}
