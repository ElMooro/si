...
def get_cftc_crisis_data():
    """Fetch CFTC positioning for Khalid Index"""
    try:
        import ssl as _ssl
        ...
        _req = _ur.Request('https://35t3serkv4gn2hk7utwvp7t2sa0flbum.lambda-url.us-east-1.on.aws/signals', headers={'User-Agent': 'JustHodl-KI/1.0'})
        with _ur.urlopen(_req, timeout=12, context=_ctx) as _r:
            _d = json.loads(_r.read())
        _res = {
            'positioning_score': _d.get('positioning_score', 0),
            'risk_appetite': _d.get('risk_appetite', 50),
            'smart_money': _d.get('smart_money', 'NEUTRAL'),
            'crisis_score': _d.get('crisis_indicators', {}).get('score', 0),
            'crisis_level': _d.get('crisis_indicators', {}).get('level', 'N/A'),
            'extreme_count': len(_d.get('extreme_positions', [])),
            'reversal_count': len(_d.get('reversal_candidates', [])),
            'sector_positioning': _d.get('sector_positioning', {}),
            'summary': _d.get('summary', '')
        }
        print(f"CFTC: crisis={_res['crisis_score']}, sm={_res['smart_money']}, pos={_res['positioning_score']}")
        return _res
    except Exception as _e:
        print(f"CFTC err: {_e}")
        return None

def khalid_cftc_blend(ki_original, cftc):
    """Blend CFTC into Khalid Index: 75% original + 15% crisis + 10% smart money"""
    if not cftc:
        return ki_original
    crisis_comp = max(0, min(100, 100 - cftc.get('crisis_score', 0)))
    sm = cftc.get('smart_money', 'NEUTRAL')
    pos = cftc.get('positioning_score', 0)
    ra = cftc.get('risk_appetite', 50)
    if sm == 'ACCUMULATING':
        smart_comp = 70 + min(pos / 4, 25)
    elif sm == 'DISTRIBUTING':
        smart_comp = 30 - min(abs(pos) / 4, 25)
    else:
        smart_comp = 40 + (ra / 5)
    smart_comp = max(0, min(100, smart_comp))
    blended = round((ki_original * 0.75) + (crisis_comp * 0.15) + (smart_comp * 0.10), 1)
...
def _legacy_lambda_handler_unvalidated(event, context):
    ...
    if payload.get('action') == 'research_measurements':
        ...
        catalog = include_plumbing_series(include_funding_series(include_reversal_series(include_cb_series(include_inflection_series(include_risk_gate_series(include_lce_series(catalog)))))))
