"""Canonical-original valuation replay; public inputs, protected retention and CAS.
...
SOURCES=('data/report-measurements.json','valuations-data.json','data/valuations-data.json')
...
def compile_output(inputs,read):
    if inputs.get('contract')!='valuation-native-inputs.v1':raise ValueError('Unsupported input contract')
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'valuation-native-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('native valuation original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'valuation-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Nineteen declared FRED inputs retained and reviewed available definitions/observations reconstructed without new provider requests. Dated context, exact sample statistics and same-date descriptive differences; no fair-value or historical first-availability claim.'}
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'valuation-public-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        inputs={'contract':'valuation-native-inputs.v1','macro':macro,'legacy':legacy,'generated_at':now()}
        ...
        raise RuntimeError('Native valuation publication failed; inspect reviewed request evidence') from None
