"""Canonical-original nowcast replay; public inputs, protected retention and CAS.
...
import activity_research_catalog as catalog
...
SOURCES=('data/report-measurements.json','data/activity-nowcast.json','data/indicator-bus.json','data/macro-nowcast.json','data/fed-nowcast-join.json','data/baltic-dry.json')
...
def compile_output(inputs,read):
    if inputs.get('contract')!='activity-native-inputs.v1':raise ValueError('Unsupported input contract')
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'activity-native-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('native nowcast original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'activity-native-replay.v1', 'generated_at': output['generated_at'], **refs,
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'activity-public-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        inputs={'contract':'activity-native-inputs.v1','macro':macro,'legacy':legacy,'generated_at':now()}
        ...
        raise RuntimeError('Native activity publication failed; inspect reviewed request evidence') from None
