"""Canonical-original VRP replay; public inputs, protected retention and CAS.
...
SOURCES=('data/report-measurements.json','data/vix-curve.json','data/vix-curve-history.json','data/vol-surface.json')
...
def compile_output(inputs,read):
    if inputs.get('contract')!='vrp-native-inputs.v1':raise ValueError('Unsupported input contract')
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'vrp-native-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('native VRP original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'vrp-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Three canonical FRED definitions/observations reconstructed without new provider requests. SPX trailing realized volatility, same-date descriptive implied gaps and exact 30-calendar-day retrospective close-sampled outcomes. No option P&L or historical first-availability claim.'}
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'vrp-public-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        inputs={'contract':'vrp-native-inputs.v1','macro':macro,'legacy':legacy,'generated_at':now()}
        ...
        raise RuntimeError('Native VRP publication failed; inspect reviewed request evidence') from None
