...
Public artifacts contain derived observations and source identities. Full licensed
provider responses stay in the existing protected AWS archive, replayable with IAM.
...
CURRENT = 'data/market-internals.json'
...
def source_url(day):
    if not re.fullmatch(r'\d{4}-\d{2}-\d{2}', day): raise ValueError('canonical date required')
    return 'https://api.massive.com/v2/aggs/grouped/locale/us/market/stocks/'+day+'?adjusted=true&include_otc=false'


def acquire(day, token, deadline, opener=None):
    """One bounded request; no error body/credential is copied into a public error."""
    if not token: return {'error': 'managed_source_credential_unavailable'}
    if time.monotonic() >= deadline: return {'error': 'collection_budget_exhausted'}
    request = urllib.request.Request(source_url(day), headers={'Authorization': 'Bearer '+token,
        'User-Agent': 'JustHodl/native-breadth-research-v1', 'Accept': 'application/json'})
    opener = opener or urllib.request.build_opener(NoRedirect())
    try:
        response = opener.open(request, timeout=min(25, max(1, deadline-time.monotonic())))
        if response.status != 200:
            response.close(); return {'error': 'provider_non_200'}
        raw = bounded(response)
        # A reflected credential cannot enter either the archive or the packet.
        if token.encode() in raw: return {'error': 'unsafe_provider_reflection'}
...
def collect(client, bucket, days, token, deadline, fetch=acquire):
    """Stream four concurrent acquisitions into protected content-addressed storage."""
    def one(day):
        item = fetch(day, token, deadline)
        if item.get('error'): return day, item
        raw = item.pop('raw'); digest = model.sha(raw); key = PRIVATE+digest+'.bin'
        immutable(client, bucket, key, raw, 'application/octet-stream')
        return day, {**item, 'evidence': {'sha256': digest, 'bytes': len(raw), 'key': key,
            'provider': 'massive', 'source_url': source_url(day), 'access': 'protected_AWS_IAM_source_archive'}}
    with ThreadPoolExecutor(max_workers=4) as pool:
        return dict(pool.map(one, days))


def compile_output(inputs, read):
    if inputs.get('contract') != 'breadth-native-inputs.v1': raise ValueError('unsupported input contract')
    if set(inputs.get('sources', {})) != set(inputs['expected_days']): raise ValueError('source inventory differs')
    def source(day):
        ...
        raw = read(ref['key'])
        if model.sha(raw) != digest or len(raw) != ref.get('bytes'): raise ValueError('original bytes differ')
        return {**item, 'raw': raw}
    output = model.compute(inputs['expected_days'], source, inputs['generated_at'])
    output['collection'] = {'started_at': inputs['started_at'], 'completed_at': inputs['generated_at'],
        'session_count': len(inputs['expected_days']), 'method': 'all sessions reacquired; no undated legacy rolling-cache reuse',
        'original_replay_access': 'Existing authorized AWS IAM; complete licensed originals are not anonymously redistributed.'}
    if not 0 <= (model.stamp(inputs['generated_at'])-model.stamp(inputs['started_at'])).total_seconds() <= 720:
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'breadth-native-replay.v1':
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'breadth-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Retained complete provider responses, native daily breadth arithmetic, populations and calendar. No forecast or historical point-in-time availability claim.'}
...
def publish(client, bucket, packet):
    ...
    for _ in range(4):
        try:
            ...
            if model.stamp(old['generated_at']) > at or old.get('as_of', '') > packet['as_of']: return False
...
def run(client, bucket, request_id, execution_id, token_getter, remaining_seconds=900):
    started = now(); key = request_key(request_id)
    status = {'contract': 'breadth-public-request.v1', 'request_id': request_id, 'execution_id': execution_id,
    ...
    try:
        days = model.sessions(started)
        deadline = time.monotonic()+max(0, min(600, remaining_seconds-240))
        sources = collect(client, bucket, days, token_getter(), deadline)
        inputs = {'contract': 'breadth-native-inputs.v1', 'started_at': started, 'generated_at': now(),
            'expected_days': days, 'sources': sources}
        ...
        raise RuntimeError('native breadth publication failed; inspect reviewed request evidence') from None
