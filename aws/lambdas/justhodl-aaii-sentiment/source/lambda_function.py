"""Public AAII research producer; HTTP requests read, scheduled events publish."""
import json
import boto3
from botocore.config import Config
from aaii_research_model import CONTRACT
from aaii_research_store import CURRENT, reader, run
...
def lambda_handler(event=None, context=None):
    ...
    client = boto3.client('s3', region_name='us-east-1', config=Config(connect_timeout=5, read_timeout=20,
        retries={'max_attempts': 2}, max_pool_connections=4, tcp_keepalive=True))
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod'):
        ...
        if packet.get('contract') != CONTRACT:
            return {'statusCode': 503, 'body': json.dumps({'reason': 'native_survey_publication_unavailable'})}
    ...
    result = run(client, bucket, event.get('request_id', execution_id), execution_id, context.get_remaining_time_in_millis()/1000)
