...
import offexchange_context
BUCKET='justhodl-dashboard-live'
PUBLISHED_KEY='data/dark-pool.json'
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod') or event.get('action')=='current_state':
        try:packet=model.strict(store.bounded(client.get_object(Bucket=BUCKET,Key=PUBLISHED_KEY)['Body']))
            ...
            packet={}
        if not offexchange_context.context(packet)['native_reference_available']:return {'statusCode':503,'headers':{'Cache-Control':'no-store'},'body':json.dumps({'reason':'recorded_offexchange_research_unavailable'})}
    ...
    result=producer.run(client,BUCKET,event.get('request_id') or event.get('id') or execution,execution,remaining_seconds=remaining)
