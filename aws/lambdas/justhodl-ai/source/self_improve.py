...
def lesson_from_grade(call: Mapping[str, Any], window: int, outcome: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    if hit:
        ...
        text = "On %s, %s at %dd was wrong (net %+.2f%%). Do not repeat that sleeve stance without a new regime fact." % (asset, stance or "the call", window, 100 * net)
    return {
...
def inject_lessons_into_read(read: Mapping[str, Any], lessons: Iterable[Mapping[str, Any]]) -> Dict[str, Any]:
    ...
    out["lessons_prompt"] = ("LESSONS CARRIED FROM GRADED CALLS — treat these as binding prior mistakes:\n" + block if block else "")
    return out
...
def category_floor(counts: Mapping[str, int], floor: int = 20) -> Dict[str, Any]:
    ...
    return {"categories_learned": learned, "categories_excluded": excluded, "counts": dict(counts), "floor": floor, "ready": "lesson" not in excluded and "macro" not in excluded}
