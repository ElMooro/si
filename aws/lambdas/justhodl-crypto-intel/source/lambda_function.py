...
def fetch_tvl():
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as r:
            ...
            chains = json.loads(r.read().decode('utf-8'))



        total = sum(c.get('tvl', 0) for c in chains if isinstance(c, dict))
        ...
        top = sorted(
            ...
            [{'name': c.get('name',''), 'tvl': round(c.get('tvl',0))}



             for c in chains if isinstance(c, dict) and c.get('tvl',0) > 1e8],



            key=lambda x: x['tvl'], reverse=True)[:8]
        ...
        return {'status':'success','total':total,
                ...
                'formatted':'${:.1f}B'.format(total/1e9),'top_chains':top}
        ...
        return {'status':'error','error':str(e),'total':0,'top_chains':[]}
...
def fetch_funding():
    # Primary: OKX public API — no auth, Lambda-friendly
    instrs = ['BTC-USDT-SWAP','ETH-USDT-SWAP','SOL-USDT-SWAP',
              'XRP-USDT-SWAP','DOGE-USDT-SWAP','ADA-USDT-SWAP',
              'AVAX-USDT-SWAP','LINK-USDT-SWAP','DOT-USDT-SWAP','BNB-USDT-SWAP']
    out = []
    try:
        ids = ','.join(instrs)
        req = urllib.request.Request(
            'https://www.okx.com/api/v5/public/funding-rate?instId=' + instrs[0],
            headers={'User-Agent': 'JustHodl/2.0', 'Accept': 'application/json'}
        )
    ...
    avg = sum(r['funding_rate'] for r in out) / len(out)
    bias = ('HEAVILY_LONG' if avg > 0.0003 else 'LONG' if avg > 0.0001
            else 'HEAVILY_SHORT' if avg < -0.0002 else 'SHORT' if avg < -0.0001
            else 'NEUTRAL')
    return {
        'status': 'success',
        'rates': out,
        'avg_rate_pct': round(avg * 100, 4),
        'leverage_sentiment': bias,
        'long_count': sum(1 for r in out if r['funding_rate'] > 0),
        'short_count': sum(1 for r in out if r['funding_rate'] < 0)
    }
...
def fetch_fg():
    ...
    try:
        ...
        print(f"          Synthetic sentiment failed: {e}")
...
def fetch_gas():
    ...
    # Primary: Etherscan gas oracle (no key needed for basic)
    ...
    try:
        ...
        if d.get('result') and d['result'] != 'Max rate limit reached':

            res = d['result']

            fast = float(res.get('FastGasPrice', 0))

            std  = float(res.get('ProposeGasPrice', 0))

            slow = float(res.get('SafeGasPrice', 0))

            if fast > 0:

                return {'rapid': fast, 'fast': fast, 'standard': std, 'slow': slow}

    except Exception as e:

        print('Etherscan gas err: ' + str(e))
    ...
    try:
        ...
        if d2.get('status')=='OK' and d2.get('data'):

            gd = d2['data']

            gw = lambda v: round(v/1e9, 1) if v else 0

            return {'rapid': gw(gd.get('rapid',0)), 'fast': gw(gd.get('fast',0)),

                    'standard': gw(gd.get('standard',0)), 'slow': gw(gd.get('slow',0))}

    except Exception as e:

        print('beaconchain gas err: ' + str(e))
    ...
    try:
        ...
        if d3.get('speeds'):
            ...
            get_fee = lambda s: round(s.get('maxFeePerGas', s.get('gasPrice', 0)), 1)

            return {

                'rapid':    get_fee(spds[3]) if len(spds)>3 else 0,

                'fast':     get_fee(spds[2]) if len(spds)>2 else 0,

                'standard': get_fee(spds[1]) if len(spds)>1 else 0,

                'slow':     get_fee(spds[0]) if len(spds)>0 else 0,

            }

    except Exception as e:

        print('owlracle err: ' + str(e))

    return {'rapid': 0, 'fast': 0, 'standard': 0, 'slow': 0}
...
def lambda_handler(event, context):
    ...
    with ThreadPoolExecutor(max_workers=8) as ex:
        ...
        for f in as_completed(fs):
            ...
            try:R[k]=f.result();print(f"    {'   ' if R[k].get('status')=='ok' else '     '} {k}")
