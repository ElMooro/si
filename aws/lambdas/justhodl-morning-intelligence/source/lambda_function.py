...
def load_all():
    ...
    return {k:__import__("nowcast_research").guard(v,__import__("valuation_research").guard(v,__import__("retail_research").guard(v,__import__("extremes_research").guard(v,__import__("crisis_authority").guard(v,fs3(v)))))) for k,v in keys.items()}
...
def extract_metrics(data,weights):
    ...
    return {
        ...
        **(lambda xf=crypto.get("exchange_flows",{}) or {}, ct=crypto.get("cot",{}) or {}, cb=crypto.get("coinbase_premium",{}) or {}, sp=crypto.get("stablecoin_peg",{}) or {}, ocr=crypto.get("onchain_ratios",{}) or {}, ef=crypto.get("etf_flows",{}) or {}, hl=crypto.get("hyperliquid",{}) or {}, gx=crypto.get("gex",{}) or {}: {
            ...
            "crypto_cot_divergence": ct.get("divergence"),
            "crypto_coinbase_premium_pct": cb.get("btc_pct"),
            "crypto_stablecoin_status": sp.get("status"),
            "crypto_stablecoin_worst": sp.get("worst_coin"),
