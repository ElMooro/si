"""Bounded insider originals, protected retention, deterministic replay and CAS."""
from datetime import datetime,timezone
from pathlib import Path
import json,re,sys,time,urllib.request,urllib.error,urllib.parse
import insider_research_model as model
PREFIX=model.PREFIX;PRIVATE=model.PRIVATE;CURRENT=model.CURRENT;MAX=32*1024*1024
...
def collect(client,bucket,credential,deadline,opener=None,max_pages=model.MAX_PAGES):
    if not isinstance(credential,str) or not credential:raise ValueError('Configured provider credential unavailable')
    if type(max_pages) is not int or not 1<=max_pages<=model.MAX_PAGES:raise ValueError('Reviewed page bound required')
    opener=opener or urllib.request.build_opener(NoRedirect());pages=[];seen=set();total=0;stop='page_limit'
    for page in range(max_pages):
        if time.monotonic()>=deadline:stop='time_budget';break
        url=model.source_url(page);req=urllib.request.Request(url+'&'+urllib.parse.urlencode({'apikey':credential}),headers={'User-Agent':'JustHodl-insider-research/2.0','Accept':'application/json','Accept-Encoding':'identity'})
        try:
            response=opener.open(req,timeout=min(20,max(1,deadline-time.monotonic())))
            if response.status!=200:response.close();raise ValueError('Unexpected provider status')
            raw=bounded(response,model.MAX_BYTES)
            if credential.encode() in raw:raise ValueError('Credential echo refused')
            rows=model.decode(raw)
        except urllib.error.HTTPError as exc:
            stop='provider_http_'+str(int(exc.code));exc.close();break
        except (ValueError,OSError,TimeoutError):stop='provider_transport_or_body_failure';break
        if total+len(raw)>MAX:stop='total_byte_limit';break
        digest=model.sha(raw);key=PRIVATE+digest+'.bin';collected=now()
        immutable(client,bucket,key,raw,'application/octet-stream');total+=len(raw)
        pages.append({'page':page,'acquired_at':collected,'evidence':{'key':key,'sha256':digest,'bytes':len(raw),'provider':'Financial Modeling Prep','request_url':url,'access':'protected_AWS_IAM_source_archive'}})
        representations={model.row_digest(r) for r in rows if isinstance(r,dict)}
        if not rows:stop='empty_page';break
        if representations and representations<=seen:stop='repeating_page';break
        seen.update(representations)
        if page+1<max_pages:time.sleep(.3)
    if not pages:raise ValueError('No original source page acquired')
    return {'pages':pages,'stop_reason':stop,'max_pages':max_pages,'source_bytes':total}

def compile_output(inputs,read):
    if inputs.get('contract')!='insider-native-inputs.v1':raise ValueError('Unsupported inputs')
    started=model.clock(inputs['started_at']);completed=model.clock(inputs['generated_at'])
    if not 0<=(completed-started).total_seconds()<=210 or started.date()!=completed.date():raise ValueError('Collection clock exceeds reviewed bound')
    collection=inputs['collection'];items=collection['pages'];limit=collection['max_pages'];stop=collection['stop_reason']
    if type(limit) is not int or not 1<=limit<=model.MAX_PAGES or not 1<=len(items)<=limit:raise ValueError('Page bound differs')
    if not re.fullmatch(r'(?:page_limit|time_budget|total_byte_limit|empty_page|repeating_page|provider_http_[0-9]{3}|provider_transport_or_body_failure)',stop):raise ValueError('Unsafe stop reason')
    pages=[];total=0
    for page,item in enumerate(items):
        ref=item['evidence'];digest=ref.get('sha256','')
        if item['page']!=page or not started<=model.clock(item['acquired_at'])<=completed:raise ValueError('Page identity or clock differs')
        if (not re.fullmatch('[a-f0-9]{64}',digest) or ref.get('key')!=PRIVATE+digest+'.bin' or ref.get('request_url')!=model.source_url(page) or ref.get('provider')!='Financial Modeling Prep'):raise ValueError('Original source identity differs')
        raw=read(ref['key'])
        if type(ref.get('bytes')) is not int or len(raw)!=ref['bytes'] or model.sha(raw)!=digest:raise ValueError('Original source bytes differ')
        total+=len(raw)
        if total>MAX:raise ValueError('Total source byte bound exceeded')
        pages.append({**item,'raw':raw})
    if collection['source_bytes']!=total:raise ValueError('Original byte count differs')
    output=model.compute(pages,inputs['generated_at'],stop)
    output['elapsed_s']=round((completed-started).total_seconds(),3)
    output['collection']={'started_at':inputs['started_at'],'completed_at':inputs['generated_at'],'pages_retained':len(pages),'maximum_pages':limit,'source_bytes':total,'stop_reason':stop,'unqualified_filing_snapshot':True}
    return output
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'insider-native-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('native Insider original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'insider-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Exact bounded FMP page originals; vendor representation counts and separate filing/transaction date windows. No SEC population, plan, currency or timing qualification.'}
...
def run(client,bucket,request_id,execution_id,credential,remaining_seconds=300,max_pages=model.MAX_PAGES):
    started=now();key=request_key(request_id)
    status={'contract':'insider-public-request.v1','request_id':request_id,'execution_id':execution_id,'started_at':started,'status':'running','phase':'collect'}
    ...
    try:
        collection=collect(client,bucket,credential,time.monotonic()+max(0,min(180,remaining_seconds-110)),max_pages=max_pages)
        inputs={'contract':'insider-native-inputs.v1','started_at':started,'generated_at':now(),'collection':collection}
        ...
        raise RuntimeError('Native insider publication failed; inspect reviewed request evidence') from None
