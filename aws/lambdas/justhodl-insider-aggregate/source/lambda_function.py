"""Native insider research producer; HTTP reads, scheduled events publish."""
import json
import boto3
from botocore.config import Config
from insider_research_model import CONTRACT
from insider_research_store import CURRENT,reader,run
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod'):
        ...
        if packet.get('contract')!=CONTRACT:
            return {'statusCode':503,'body':json.dumps({'reason':'native_insider_publication_unavailable'})}
    ...
    import os
    credential=os.environ.get('FMP_KEY','')
    result=run(client,bucket,event.get('request_id',execution_id),execution_id,credential,context.get_remaining_time_in_millis()/1000,40)
