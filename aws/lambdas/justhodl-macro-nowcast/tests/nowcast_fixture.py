...
def fixture(missing=('RRSFS',)):
    objects={};entries={};measurements={};originals={}
    for sid,(label,unit,freq,adj) in model.SPECS.items():
        if sid in missing:continue
        if freq=='M':days=[model.month('2000-01-01',i) for i in range(319 if sid=='UMCSENT' else 320)]
        else:
            start=date(2011,5,23) if sid=='T10Y2Y' else date(2016,9,19)
            days=[str(start+timedelta(days=i)) for i in range((date(2026,9,18)-start).days+1) if (start+timedelta(days=i)).weekday()<5]
        ...
        base=Decimal('4') if sid=='UNRATE' else Decimal('0.25') if sid=='T10Y2Y' else Decimal('100')
        rows=[{'date':d,'value':str(base+Decimal(i%12)/100 if sid=='UNRATE' else base+Decimal(i)/100+Decimal(i%7)/100)} for i,d in enumerate(days)]
        if sid=='UNRATE':next(r for r in rows if r['date']=='2025-10-01')['value']='.'
    ...
    objects['data/macro-nowcast.json']=b'{"normalized_score":99,"regime":"SLOWING","original":"whole"}'
    client=Storage(objects);inputs={'contract':'nowcast-native-inputs.v1','generated_at':STAMP,'macro':store.snapshot(client,'b',store.SOURCES[0]),
        'legacy':{key:store.snapshot(client,'b',key) for key in store.SOURCES[1:]}}
    return client,inputs,macro,originals

def packet(missing=('RRSFS',)):
