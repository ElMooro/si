...
from nowcast_research_model import CONTRACT
from nowcast_research_store import CURRENT,reader,run
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod') or event.get('action')=='current_state':
        ...
        if packet.get('contract')!=CONTRACT:
            return {'statusCode':503,'body':json.dumps({'reason':'native_nowcast_publication_unavailable'})}
