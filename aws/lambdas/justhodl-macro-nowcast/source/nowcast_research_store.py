...
import nowcast_research_catalog as catalog
...
SOURCES=('data/report-measurements.json','data/macro-nowcast.json')
...
def compile_output(inputs,read):
    if inputs.get('contract')!='nowcast-native-inputs.v1':raise ValueError('Unsupported input contract')
    ...
    return model.build(packet,originals,inputs['generated_at'])
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'nowcast-native-replay.v1':
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'nowcast-native-replay.v1', 'generated_at': output['generated_at'], **refs,
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'nowcast-public-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        inputs={'contract':'nowcast-native-inputs.v1','macro':macro,'legacy':legacy,'generated_at':now()}
        ...
        raise RuntimeError('Native nowcast publication failed; inspect reviewed request evidence') from None
