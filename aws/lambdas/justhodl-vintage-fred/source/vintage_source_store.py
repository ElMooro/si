...
def publish_current(client,bucket,key,output):
    for _ in range(4):
        ...
        if previous:
            ...
            if previous.get('contract') not in (model.CONTRACT,'fred-vintage-index.v1'):
...
def collect_series(client,bucket,sid,api_key,deadline,collection_id,archive_end,compiler,started):
    ...
    stamp=now();output=model.compile_series(sid,definition,pages,stamp,collection_id,archive_end,started)
    ...
    rebuilt=model.compile_series(sid,load_original(client,bucket,manifest['definition']),
        [load_original(client,bucket,p) for p in manifest['pages']],stamp,collection_id,archive_end,started)
    ...
    return {'key':output_key,'sha256':model.digest(output),'replay':output['replay'],
            'coverage':output['coverage'],'generated_at':stamp,'acquired_at':output['acquired_at']}
...
def run(client,bucket,api_key):
    ...
    deadline=time.monotonic()+510;results={};errors={}
    with ThreadPoolExecutor(max_workers=3) as executor:
        futures={executor.submit(collect_series,client,bucket,sid,api_key,deadline,collection_id,archive_end,compiler,started):sid for sid in model.SERIES}
    ...
    output={'contract':'fred-vintage-index.v1','version':'2.0.0','generated_at':now(),'updated':now(),
