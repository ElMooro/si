"""Public native FRED capture, original-byte replay and compare-and-swap publication."""
from datetime import datetime, timezone
import hashlib,json,re,time,urllib.request,urllib.error,urllib.parse
from pathlib import Path
import stress_model as model
MAX_BYTES=16*1024*1024
PRIVATE='audit-private/20260909-originals/stress-index/'
COMPILERS=(model,)
CONTEXT_KEYS=('global-stress','tail-risk','bank-stress','crisis-composite','eurodollar-stress',
    'eurodollar-plumbing','ciss-stress','crisis-canaries','risk-regime','polygon-fx-regime',
    'vvix-vov-regime','tail-hedge','euro-fragmentation','carry-surface','global-tide',
    'risk-ratios','sovereign-stress','global-sovereign')
...
class Collector:
    ...
    def fetch(self, name, url, provider):
        try:
            if self.calls >= 26 or time.monotonic() >= self.deadline: raise TimeoutError('collection budget exhausted')
            self.calls += 1
            ...
            headers = {'User-Agent': 'JustHodl-stress-research/2.0', 'Accept': 'application/json'}
...
def replay(ref, read):
    ...
    if manifest.get('contract') != 'stress-replay.v1' or manifest.get('output_sha256') != ref.get('output_sha256'):
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'stress-replay.v1', 'generated_at': inputs['generated_at'], **refs,
...
def publish(client, bucket, packet, key=model.CURRENT):
    if key not in (model.CURRENT,'data/jsi-history.json'):raise ValueError('unsupported mutable publication')
...
def collect(client,bucket,fred_key,transport=None):
    collector=Collector(client,bucket,fred_key,None,transport)
    evaluation=datetime.now(timezone.utc).date()
    for sid in model.SERIES:
        ...
        q.update(observation_start=model.HISTORY_START,observation_end=str(evaluation),units='lin',sort_order='asc',limit=20000,offset=0,output_type=1)
    ...
    return {'contract':'stress-inputs.v1','generated_at':now(),'evaluation_date':str(evaluation),'sources':collector.sources,
        'context':context,'provider_request_count':collector.calls},collector.bodies


def compile_output(inputs,bodies,read):return model.build(inputs,bodies)


def run(client,bucket,fred_key,validation_only=False):
    inputs,bodies=collect(client,bucket,fred_key)
    ...
    if published:
        publish(client,bucket,{'contract':'stress-history-pointer.v1','generated_at':output['generated_at'],
            'series':[],'native_history_run':ref,'source_packet':'data/jsi.json',
            ...
            **model.PERMISSIONS},'data/jsi-history.json')
