"""Recorded cross-asset composition; no provider, AI, account or notification calls."""
import json,re
import boto3
...
PUBLISHED_KEY='data/cross-asset-flow-state.json'

def publish_current(client,request):
    if request.get('Bucket')!=BUCKET or request.get('Key')!=PUBLISHED_KEY:raise ValueError('Reviewed composition target required')
...
class EvidenceStorage:
    ...
    def get_object(self,**request):
        key=request.get('Key')
        if request.get('Bucket')!=BUCKET or not(key in model.CAPTURE_KEYS or store.artifact_key(key) or request_path(key)):raise ValueError('Reviewed research read required')
    ...
    def put_object(self,**request):
        ...
        if not (request_path(key) or (store.artifact_key(key) and key.startswith((model.PREFIX,model.PRIVATE)))):raise ValueError('Composition evidence write required')
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod') or event.get('action')=='current_state':
        try:packet=model.strict(store.bounded(client.get_object(Bucket=BUCKET,Key=PUBLISHED_KEY)['Body']))
            ...
            packet={}
        if not native(packet):return {'statusCode':503,'headers':{'Cache-Control':'no-store'},'body':json.dumps({'reason':'recorded_composition_unavailable'})}
    ...
    result=store.run(client,BUCKET,event.get('request_id') or event.get('id') or execution,execution)
