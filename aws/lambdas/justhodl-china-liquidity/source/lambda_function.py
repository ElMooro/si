    ...
    try:
        ...
        if not m:
            ...
            if not mc:
                ...
                _cum = True
        if out.get("flow_trn_cny") is None:
            mh = _re.search(r"(上半年|前三季度|一季度|全年)[，,]?社会融资规模增量"
                            r"(?:累计)?为([\d.]+)(万亿|亿)元", body)
            if mh:
                _yr = None
                _ty = _re.search(r"(20\d\d)", str(out.get("att_title") or ""))
                if _ty:
                    _yr = _ty.group(1)
                ...
                v = float(mh.group(2))
                out["flow_trn_cny"] = round(
                    v if mh.group(3) == "万亿" else v / 10000.0, 3)
                _cum = True
        if _cum:
            out["cumulative"] = True
        if m:
