...
import canonical_macro_sources, report_observations, research_brief_model

PRIVATE='audit-private/20260909-originals/official-research/'
COMPILERS=(native,model,evidence_store,canonical_macro_sources,report_observations,research_brief_model)
...
def bounded(body,limit=MAX_BYTES):
    ...
    if len(raw)>limit:raise ValueError('official artifact bound exceeded')
    return raw
...
def immutable(client,bucket,key,raw,kind='application/json'):
    if not(key.startswith(model.PREFIX) or key.startswith(PRIVATE)):raise ValueError('official retention prefix differs')
...
def preserve(client,bucket,read):
    ...
    try:marker=json.loads(read(key))
        ...
        marker={'contract':'official-legacy-preservation.v1','source':model.CURRENT,'sha256':sha,'bytes':len(raw),
    ...
    if marker.get('contract')!='official-legacy-preservation.v1' or not re.fullmatch('[a-f0-9]{64}',marker.get('sha256','')):raise ValueError('legacy marker differs')
    return marker
...
def acquire(client,bucket,url,key=None,deadline=None):
    ...
    if not raw or len(raw)>limit:raise ValueError('official-balance response bound')
    if key and len(key)>=12 and key.encode() in raw:raise ValueError('credential echoed')
    stamp=now();receipt=evidence_store.capture(client,bucket,'h41',url,raw,native.clock(stamp))
    return {'url':url,'acquired_at':stamp,'evidence':receipt}


def collect(client,bucket,fred_key,read,deadline):
    stamp=now();results={};errors={}
    urls={'native_archive':native.XML_URL,'release_html':native.HTML_URL,'scope_definition':native.FAQ_URL,**native.calendar_urls(stamp)}
    for name,url in urls.items():
        try:
            key=fred_key if name in ('series_release','release_dates') else None
            if name in ('series_release','release_dates') and not key:raise ValueError('configured calendar credential unavailable')
            results[name]=acquire(client,bucket,url,key,deadline)
    ...
    for _ in range(5):
        try:
            ...
            if old.get('contract')==model.CONTRACT:
                if native.clock(old['generated_at'])>native.clock(packet['generated_at']) or native.clock(old['source_generated_at'])>native.clock(packet['source_generated_at']):return False
                if old['generated_at']==packet['generated_at'] and old!=packet:raise ValueError('same-clock conflicting official output')
        ...
        try:
            ...
            if not conflict(exc):raise
    raise RuntimeError('official publication contention bound')

def replay(manifest,read):
    if manifest.get('contract')!='official-original-replay.v1' or set(manifest.get('compilers',{}))!={m.__name__ for m in COMPILERS}:raise ValueError('official replay contract differs')
    ...
    if output!=reference or model.digest(output)!=manifest['output_sha256']:raise ValueError('official original replay differs')
    load('output')
    for key,body in histories.items():
        if read(key)!=body:raise ValueError('official history shard differs')
    return output
...
def run(client,bucket,fred_key,context=None):
    ...
    originals,errors=collect(client,bucket,fred_key,read,deadline)
    try:canonical=json.loads(read('data/report-measurements.json'))
    except Exception as exc:
        canonical={};errors['canonical_source']=type(exc).__name__
    try:tic=json.loads(read('data/foreign-flows.json'))
    except Exception:tic=None
    stamp=now()
    inputs={'contract':'official-original-inputs.v1','originals':originals,'acquisition_errors':errors,'canonical':canonical,'tic':tic,'legacy':legacy}
    ...
    try:output,histories=model.build(inputs,read,stamp)
        ...
        failed={'contract':'official-failed-attempt.v1','generated_at':stamp,'input':refs['input'],'compilers':compilers,
        ...
        print('[official-research] '+json.dumps({'failed_attempt':key,'source_status_codes':errors,'failure_class':type(exc).__name__}))
        raise
    ...
    manifest={'contract':'official-original-replay.v1','generated_at':stamp,'compilers':compilers,**refs,'output_sha256':sha}
