"""Native complete-source FI/FX research; HTTP reads never execute the engine."""
import json, os
from datetime import datetime, timezone
import boto3
from botocore.config import Config
from fifx_store import run, qualified_arithmetic
from fifx_model import CONTRACT, CURRENT
...
def lambda_handler(event=None,context=None):
    ...
    if event.get('httpMethod') or (event.get('requestContext') or {}).get('http'):
        return {'statusCode':307,'headers':{'Location':'https://justhodl-data-proxy.raafouis.workers.dev/'+CURRENT,'Cache-Control':'no-store'},'body':''}
    ...
    # Scheduler's empty payload receives one durable daily identity, including retries.
    request=str(event.get('id') or 'scheduled:'+datetime.now(timezone.utc).date().isoformat())
    result=run(client,BUCKET,request)
