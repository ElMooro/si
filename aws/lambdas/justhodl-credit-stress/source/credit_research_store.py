...
Public artifacts contain typed credit measurements and source identities. Whole
...
import urllib.parse
import threading
import dealer_research_context

import credit_research_model as model

PREFIX = model.PREFIX
CURRENT = 'data/credit-stress.json'
PRIVATE = model.PRIVATE
MAX = 24*1024*1024
COMPILERS = (model, sys.modules[__name__], dealer_research_context)
...
class Collector:
    """56 fixed FRED calls, bounded concurrency, rate spacing and private originals."""
    ...
    def acquire(self,sid,kind):
        if not self.credential: return {'error':'configured_credential_unavailable'}
        ...
        request=urllib.request.Request(url+'&api_key='+urllib.parse.quote(self.credential,safe=''),headers={
            'User-Agent':'JustHodl-credit-research/2.0','Accept':'application/json','Accept-Encoding':'identity'})
        try:
            ...
            if len(raw)>model.MAX_BYTES or self.credential.encode() in raw: return {'error':'provider_body_rejected'}
            model.json_object(raw)
            ...
            return {'acquired_at':collected,'evidence':{'sha256':digest,'bytes':len(raw),'key':key,
                'provider':'fred','request_url':url,'access':'protected_AWS_IAM_source_archive'}}
...
def collect(client,bucket,credential,evaluation,deadline):
    ...
    def one(sid):
        return sid,{kind:collector.acquire(sid,kind) for kind in ('definition','observations')}
    with ThreadPoolExecutor(max_workers=4) as pool:
        return dict(pool.map(one,model.SERIES))


def collect_dealer(client,bucket):
    try:
        raw=bounded(client.get_object(Bucket=bucket,Key='data/nyfed-primary-dealer.json')['Body'])
        model.json_object(raw);digest=model.sha(raw);key=PRIVATE+digest+'.bin'
        immutable(client,bucket,key,raw,'application/octet-stream')
        return {'sha256':digest,'bytes':len(raw),'key':key,'source':'data/nyfed-primary-dealer.json'}
    except Exception: return None


def compile_output(inputs,read):
    if inputs.get('contract')!='credit-native-inputs.v1': raise ValueError('Unsupported input contract')
    if set(inputs.get('sources',{}))!=set(model.SERIES): raise ValueError('Source inventory differs')
    ...
    for sid,items in inputs['sources'].items():
        if set(items)!= {'definition','observations'}: raise ValueError('Original pair inventory differs')
        result={}
        for kind,item in items.items():
            ...
            if (not re.fullmatch('[a-f0-9]{64}',digest) or ref.get('key')!=PRIVATE+digest+'.bin'
                or ref.get('request_url')!=model.source_url(sid,kind,inputs['evaluation_date']) or ref.get('provider')!='fred'):
    ...
    dealer={};ref=inputs.get('dealer')
    if ref:
        digest=ref.get('sha256','')
        if (not re.fullmatch('[a-f0-9]{64}',digest) or ref.get('key')!=PRIVATE+digest+'.bin'
            or ref.get('source')!='data/nyfed-primary-dealer.json'): raise ValueError('Dealer source identity differs')
        raw=read(ref['key'])
        if model.sha(raw)!=digest or type(ref.get('bytes')) is not int or len(raw)!=ref['bytes']: raise ValueError('Dealer context bytes differ')
        dealer=model.json_object(raw)
    dc=dealer_research_context.project(dealer,completed)
    corp=dc.get('corporate')
    output['dealer_positioning']={**corp,'source_replay':dc.get('source_replay')} if corp else None
    output['dealer_context']=dc
    output['europe_sovereign']={'status':'unverified_source_identity','measurements':None,
        'source_symbols':['TVC:BTPBUND','TVC:FR10Y-TVC:IT10Y','TVC:ES10Y-TVC:IT10Y','ICEEUR:EON2!'],
        'research_url':'/sovereign-stress.html','role':'linked_context_only',**model.PERMISSIONS}
    output['wl_research']={'status':'not_collected','role':'linked_context_only',
        'note':'No private watchlist/Brain input is acquired by this public-only producer.',**model.PERMISSIONS}
    output['collection']={'started_at':inputs['started_at'],'completed_at':inputs['generated_at'],
        'source_count':len(model.SERIES),'request_count':2*len(model.SERIES),
        'method':'Full available native FRED responses, same current vintage; original null rows retained.',
        'original_replay_access':'Existing authorized AWS IAM; licensed whole source history is not anonymously redistributed.'}
    output['generated_at_unix']=int(completed.timestamp())
    output['elapsed_seconds']=round((completed-started).total_seconds(),3)
    output['history_reference']={'legacy_key':'data/credit-stress-history.json','role':'legacy_unverified_history',
        'used_in_native_calculations':False,'native_original_history_access':'Protected AWS IAM source archive',
        'note':'Previous public history remains dated and unchanged. Native calculations replay newly retained complete originals.'}
    return output
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'credit-native-replay.v1':
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'credit-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Retained complete FRED definitions/observations, strict units and vintage, same-date comparisons and explicit statistical windows. No forecast or historical availability claim.'}
...
def run(client, bucket, request_id, execution_id, credential, remaining_seconds=300):
    ...
    status = {'contract': 'credit-public-request.v1', 'request_id': request_id, 'execution_id': execution_id,
    ...
    try:
        ...
        dealer = collect_dealer(client,bucket)
        inputs = {'contract': 'credit-native-inputs.v1', 'started_at': started, 'generated_at': now(),
            'sources': sources, 'dealer': dealer, 'evaluation_date': model.stamp(started).date().isoformat()}
        ...
        raise RuntimeError('native credit publication failed; inspect reviewed request evidence') from None
