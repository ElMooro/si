...
def lambda_handler(event=None, context=None):
    ...
    for s in (bs.get("top_setups") or [])[:30]:
        ...
        sized.append({
            ...
            "rationale": f"conv {conv} × {posture.split('—')[0].strip() or posture} posture ({posture_mult}×) × regime {bvr}/{fpr} ({round(regime_mult,2)}×)",
        })
    ...
    out = {"engine": "position-sizer", "version": "1.0",
           ...
           "regime": {"bond_vol": bvr, "plumbing": fpr, "combined_mult": round(regime_mult, 2)},
    ...
    print(f"[position-sizer] {len(sized)} sized, posture={posture_mult}x regime={round(regime_mult,2)}x")
