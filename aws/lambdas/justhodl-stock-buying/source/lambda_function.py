"""justhodl-stock-buying v1.5.2 (restored from blob eaadb467 = v1.5.1; FMP /stable/)
v1.5.2 (2026-09-11): FMP moved to /stable/ (this was the last engine on the old v3 base):
  earnings-surprises/{sym} -> earnings?symbol= (epsActual/epsEstimated; future quarters have
  null actuals and are skipped); income-statement/{sym} -> income-statement?symbol= (epsDiluted).
  Key via managed_secret (FMP_API_KEY | FMP_KEY | SSM /justhodl/fmp/api-key), audit 2026-09-08 INST-06.
v1.5.1 (ops 4658)
...
def eps_beats(beats, n=4):
    """(beat_count, [surprise_pct, ...]) over the n most recent REPORTED quarters.
    /stable/earnings rows: epsActual / epsEstimated (v3 earnings-surprises used
    actualEarningResult / estimatedEarning -- both read, so a cached v3 body still
    parses). Future-dated quarters carry null actuals and are skipped. None when
    ...
    for b in beats:
        if not isinstance(b, dict):
            continue
        a = fnum(b.get("epsActual", b.get("actualEarningResult")))
        e = fnum(b.get("epsEstimated", b.get("estimatedEarning")))
        if a is not None and e is not None:
            rows.append((a, e))
    rows = rows[:n]
    if not rows:
        return None
    n_beat = sum(1 for a, e in rows if a > e)
    mag = [(a / abs(e) - 1) * 100 for a, e in rows if e != 0]
    return n_beat, mag
...
def lambda_handler(event=None, context=None):
    ...
    for row in crows:
        ...
        pillars = {}
        beats = warm_fmp(sym, "earn", "earnings", "symbol=" + sym + "&limit=8")\
            if gates["below_sma"] else None
        eps_b = eps_beats(beats)
        ...
        if FMP_KEY and gates.get("below_sma") \
            ...
            inc = fmp("income-statement",
                      "symbol=%s&period=quarter&limit=6" % sym)
