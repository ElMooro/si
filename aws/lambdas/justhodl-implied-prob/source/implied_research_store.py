"""Canonical-original implied replay; public inputs, protected retention and CAS.
...
import implied_research_catalog
import tail_research

PREFIX=model.PREFIX;CURRENT=model.CURRENT;PRIVATE=model.PRIVATE;MAX=32*1024*1024
SOURCES=('data/report-measurements.json','data/implied-prob.json','data/tail-risk.json')
COMPILERS=(model,canonical_fred_replay,report_observations,research_brief_model,evidence_store,implied_research_catalog,tail_research,sys.modules[__name__])
...
def compile_output(inputs,read):
    if inputs.get('contract')!='implied-native-inputs.v1':raise ValueError('Unsupported input contract')
    ...
    return model.build(packet,originals,legacy,inputs['generated_at'])
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'implied-native-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('native implied original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'implied-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Eighteen source-defined rate, coincident recession-model and volatility-index series reconstructed from canonical originals. Exact common dates, descriptive statistics and retained option-context identity; no futures-derived meeting or physical-return probabilities.'}
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'implied-public-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        inputs={'contract':'implied-native-inputs.v1','macro':macro,'legacy':legacy,'generated_at':now()}
        ...
        raise RuntimeError('Native implied publication failed; inspect reviewed request evidence') from None
