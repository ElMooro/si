...
PRIVATE='audit-private/20260909-originals/tic-research/'
...
def bounded(body,limit=MAX_BYTES):
    ...
    if len(raw)>limit:raise ValueError('tic artifact bound exceeded')
    return raw
...
def immutable(client,bucket,key,raw,kind='application/json'):
    if not(key.startswith(model.PREFIX) or key.startswith(PRIVATE)):raise ValueError('tic retention prefix differs')
...
def preserve(client,bucket,read):
    ...
    try:marker=json.loads(read(key))
        ...
        marker={'contract':'tic-legacy-preservation.v1','source':model.CURRENT,'sha256':sha,'bytes':len(raw),
    ...
    if marker.get('contract')!='tic-legacy-preservation.v1' or not re.fullmatch('[a-f0-9]{64}',marker.get('sha256','')):raise ValueError('legacy marker differs')
    return marker

def acquire(client,bucket,url,key=None,deadline=None):
    global _last
    if deadline is not None and time.monotonic()>=deadline:raise TimeoutError('TIC acquisition budget')
    actual=url+('&'+urlencode({'api_key':key}) if key else '')
    request=urllib.request.Request(actual,headers={'User-Agent':'JustHodl original TIC research','Accept':'application/json,application/zip'})
    ...
    if not raw or len(raw)>limit:raise ValueError('TIC response bound')
...
def collect(client,bucket,fred_key,read,deadline):
    ...
    if not fred_key:
        errors['FRED']='configured_credential_unavailable'
        return results,errors,vintage
    for sid in native.SERIES:
        get(sid+':definition',native.definition_url(sid,vintage),fred_key)
        ref=results.get(sid+':definition')
        if not ref:continue
        try:
            doc=native.strict_json(native.original(ref,read,ref['url'],now()));meta=doc['seriess']
            if len(meta)!=1 or meta[0]['id']!=sid:raise ValueError('definition identity differs')
            url=native.observations_url(sid,meta[0],vintage)
        except Exception as exc:
            errors[sid+':definition']='validation_'+type(exc).__name__
            continue
        get(sid+':observations',url,fred_key)
    ...
    for _ in range(5):
        try:
            ...
            if old.get('contract')==model.CONTRACT:
                ...
                if old['generated_at']==packet['generated_at'] and old!=packet:raise ValueError('same-clock conflicting tic output')
        ...
        try:
            ...
            if not conflict(exc):raise
    raise RuntimeError('tic publication contention bound')
...
def run(client,bucket,fred_key,context=None):
    ...
    deadline=time.monotonic()+max(10,min(180,remaining-90))
    ...
    inputs={'contract':'tic-original-inputs.v1','originals':originals,'acquisition_errors':errors,'requested_fred_vintage':vintage,'legacy':legacy}
    ...
    try:output,histories=model.build(inputs,read,stamp)
        ...
        failed={'contract':'tic-failed-attempt.v1','generated_at':stamp,'input':refs['input'],'compilers':compilers,
        ...
        print('[tic-research] '+json.dumps({'failed_attempt':key,'source_status_codes':errors,'failure_class':type(exc).__name__}))
        raise
    ...
    manifest={'contract':'tic-original-replay.v1','generated_at':stamp,'compilers':compilers,**refs,'output_sha256':sha}
