"""Native volatility research producer; HTTP reads, scheduled events publish."""
import json
import boto3
from botocore.config import Config
from volatility_research_model import CONTRACT
from volatility_research_store import CURRENT,reader,run
...
def lambda_handler(event=None,context=None):
    ...
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod'):
        ...
        if packet.get('contract')!=CONTRACT:
            return {'statusCode':503,'body':json.dumps({'reason':'native_volatility_publication_unavailable'})}
    ...
    credential=managed_secret(('FRED_KEY','FRED_API_KEY'),('/justhodl/fred/api-key',))
    result=run(client,bucket,event.get('request_id',execution_id),execution_id,credential,context.get_remaining_time_in_millis()/1000)
