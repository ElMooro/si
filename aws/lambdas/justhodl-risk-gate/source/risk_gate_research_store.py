"""Retained input, reviewed compiler and conditional Risk Gate publication."""
...
def originals(client,bucket,source):
    ...
    result={}
    for sid in risk_gate_research_catalog.SERIES:
...
FLEET_KEYS=('data/air-cargo.json', 'data/asia-leads.json', 'data/auction-grades.json', 'data/bis-crossborder.json', 'data/bond-vol.json', 'data/cftc-deep-view.json', 'data/china-liquidity.json', 'data/credit-composite.json', 'data/credit-stress.json', 'data/crisis-composite.json', 'data/crisis-plumbing.json', 'data/etf-true-flows.json', 'data/euro-fragmentation.json', 'data/eurodollar-plumbing.json', 'data/fifx-vol-history.json', 'data/freight-pulse.json', 'data/nyfed-primary-dealer.json', 'data/official-pulse.json', 'data/plumbing-composite.json', 'data/portwatch.json', 'data/settlement-fails.json', 'data/term-premium.json', 'data/tradingview.json', 'data/treasury-rehypo.json', 'data/yen-carry.json')
...
def run(client,bucket,validation_only=False):
    ...
    ciss=optional(client,bucket,'data/ciss-stress.json')
    ...
    for _ in range(4):
        ...
        try:
            ...
            if code(exc) not in ('412','PreconditionFailed','409','ConditionalRequestConflict'):raise
    raise RuntimeError('Risk Gate publication race retry limit exceeded')
