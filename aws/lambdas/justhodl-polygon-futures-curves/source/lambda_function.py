"""Original dated futures research; HTTP reads never collect or publish."""
import json
import boto3
from botocore.config import Config
from massive import get_massive_key
from futures_research_model import CONTRACT
from futures_research_store import bounded,run,missing
PUBLISHED_KEY='data/futures-research.json'


def publish_current(client,bucket,key,raw,condition):
    if key!=PUBLISHED_KEY or set(condition) not in ({'IfMatch'},{'IfNoneMatch'}):
        raise ValueError('Reviewed futures target and conditional write required')
...
def lambda_handler(event=None,context=None):
    ...
    client=boto3.client('s3',region_name='us-east-1',config=Config(connect_timeout=5,read_timeout=10,
        retries={'max_attempts':2},max_pool_connections=8,tcp_keepalive=True));bucket='justhodl-dashboard-live'
    if (event.get('requestContext') or {}).get('http') or event.get('httpMethod') or event.get('action')=='current_state':
        try:packet=json.loads(bounded(client.get_object(Bucket=bucket,Key=PUBLISHED_KEY)['Body']))
        except Exception as exc:
            if not missing(exc):raise
            packet={}
        if packet.get('contract')!=CONTRACT:
            return {'statusCode':503,'body':json.dumps({'reason':'native_futures_research_unavailable'})}
    ...
    credential='' if recovery is not None else get_massive_key()
    remaining=context.get_remaining_time_in_millis()/1000 if hasattr(context,'get_remaining_time_in_millis') else 90
