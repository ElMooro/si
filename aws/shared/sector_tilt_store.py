...
import sector_tilt_model as model
...
SOURCES=('data/sector-rotation.json','data/macro-nowcast.json','data/sector-tilt.json')
COMPILERS=(model,*sector_store.COMPILERS,sys.modules[__name__])
...
def allowed(key):
    return isinstance(key,str) and (key in SOURCES or artifact(key) or sector_store.allowed(key) or bool(re.fullmatch(
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'sector-tilt-replay.v1':
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'sector-tilt-replay.v1', 'generated_at': output['generated_at'], **refs,
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'sector-tilt-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        inputs={'contract':'sector-tilt-inputs.v1','market':market,'legacy':legacy,'generated_at':now()}
