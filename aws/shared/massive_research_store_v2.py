...
import massive_research_store as legacy_store

COMPILERS = (model.legacy, legacy_store, model, sys.modules[__name__])
...
def capture(client, bucket, key, read):
    if key not in (model.CURRENT, *model.CAPTURE_KEYS, *model.PREDECESSORS): raise ValueError('Reviewed public research key required')
    stamp = now()
...
def capture_inputs(client, bucket, read, checkpoint):
    snapshots = {}; size = 0
    for key in (model.CURRENT, *model.CAPTURE_KEYS, *model.PREDECESSORS):
...
def verified_run(identity, read):
    ...
    if (model.ref(raw, 'runs')['key'] != identity['manifest_key'] or run.get('contract') != 'massive-composite-replay.v2'
...
def retain(client, bucket, inputs, output, read, checkpoint):
    ...
    run = {'contract': 'massive-composite-replay.v2', 'generated_at': output['generated_at'], **refs,
...
def compatibility(packet, key):
    ...
    out = {'contract': 'massive-research-compatibility.v2', 'generated_at': packet['generated_at'],
...
def native_clocks(packet):
    clocks = {}
    for kind, row in packet.get('sources', {}).items():
        if not isinstance(row, dict): continue
        if row.get('source_generated_at'): clocks[kind] = model.clock(row['source_generated_at'])
        for name, stamp in row.get('measurement_capture_clocks', {}).items():
            clocks[kind + ':measurement:' + name] = model.clock(stamp)
        if row.get('source_definition_date'):
            clocks[kind + ':definition_date'] = model.clock(row['source_definition_date'] + 'T00:00:00Z')
    return clocks
...
def conditional(client, bucket, key, packet, read, publish=None):
    ...
    expected = model.CONTRACT if key == model.CURRENT else 'massive-research-compatibility.v2'
    ...
    for _ in range(4):
        try:
            ...
            elif key == model.CURRENT and old.get('contract') == model.legacy.CONTRACT:
                # A version transition must bind the exact retained v1 head and
                # replay its original compiler; no implicit schema reinterpretation.
                previous = packet['prior_composite']
                if previous['status'] != 'retained' or model.original(previous['original'], read) != raw:
                    raise ValueError('Canonical v1 predecessor changed after capture')
                model.permissions(old)
                if legacy_store.replay(old['replay'], read) != {k: v for k, v in old.items() if k != 'replay'}:
                    raise ValueError('Prior v1 composition no longer reconstructs')
                if model.clock(old['generated_at']) >= stamp:
                    raise ValueError('Version migration requires a later composition clock')
                prior, current = native_clocks(old), native_clocks(packet)
                if any(kind not in current or current[kind] < value for kind, value in prior.items()):
                    raise ValueError('Version migration cannot drop or roll back prior sources')
...
def run(client, bucket, request_id, execution_id, recover_run=None, publish=None, publish_current=True):
    ...
    status = {'contract': 'massive-composite-request.v2', 'request_id': request_id, 'execution_id': execution_id,
