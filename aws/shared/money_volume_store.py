"""Original-source price-volume replay; protected retention and conditional publication.
...
import money_volume_source as market
import money_volume_sessions as sessions
import money_volume_pins as pins
PREFIX=model.PREFIX;CURRENT=model.CURRENT;PRIVATE=model.PRIVATE;MAX=32*1024*1024
SOURCES=('data/market-internals.json','data/universe.json','data/money-flow-state.json','data/13f-positions.json')
COMPILERS=(model,market,sessions,pins,sys.modules[__name__])
...
def allowed(key):
    return isinstance(key,str) and (key in SOURCES or artifact(key) or market.artifact(key))
...
def compile_output(inputs,read):
    if inputs.get('contract')!='money-volume-inputs.v1':raise ValueError('Unsupported input contract')
    ...
    universe=source(inputs['legacy'][SOURCES[1]],SOURCES[1],read)
    contexts={key:source(ref,key,read,metadata_only=True) if ref else None for key,ref in inputs['legacy'].items() if key!=SOURCES[1]}
    return model.build(packet,universe,inputs['generated_at'],read,contexts,inputs['legacy'])
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'money-volume-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('Native price-volume original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'money-volume-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Six retained grouped-session originals, pinned upstream source compilers and current universe. Matched price and turnover proxy arithmetic with full constituent coverage; not investor net capital flow, a forecast or portfolio authority.'}
...
def publish(client, bucket, packet):
    ...
    for _ in range(4):
        try:
            ...
            if old.get('contract') == model.CONTRACT and (old['as_of']>packet['as_of'] or model.clock(old['universe']['generated_at'])>model.clock(packet['universe']['generated_at'])):return False
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'money-volume-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        market=snapshot(client,bucket,SOURCES[0])
        legacy={}
        ...
        inputs={'contract':'money-volume-inputs.v1','market':market,'legacy':legacy,'generated_at':now()}
        ...
        raise RuntimeError('Native price-volume publication failed; inspect reviewed request evidence') from None
