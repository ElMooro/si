...
class DealerGexAdapter(SignalAdapter):
    """Population evidence has no qualified directional, sizing or veto policy."""
    signal_type, category = "gamma_regime", "options_positioning"

    def parse_existing_output(self, doc, meta):
        result = AdapterResult(self.engine_id)
        result.source_status = "UNQUALIFIED"
        result.diagnostics = [__import__("option_population_context").NOTE]
        return result
    ...
    def rows(self, doc):
        yield {"skip": "dealer ownership and predictive authority are unqualified"}
