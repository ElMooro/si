"""Immutable FINRA daily research retention and matching-compiler source replay.
...
COMPILERS = (source, index, measurements, model, sys.modules[__name__])
MAX = model.MAX
...
def retain(client, bucket, inputs, compiled):
    ...
    run = {'contract': 'short-volume-original-replay.v1', 'generated_at': packet['generated_at'],
...
def verified_run(ref, read):
    if (not isinstance(ref, dict) or set(ref) != {'manifest_key', 'output_sha256'}
        ...
        raise ValueError('Immutable short-volume run required')
    raw = read(ref['manifest_key'])
    run = model.strict(raw)
    if (identity(raw, 'runs')['key'] != ref['manifest_key'] or run.get('contract') != 'short-volume-original-replay.v1'
