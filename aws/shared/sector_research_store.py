"""Original-source sector replay; public inputs, protected retention and CAS.
...
SOURCES=('data/report.json','data/sector-rotation.json','data/sector-tilt.json','data/macro-nowcast.json','data/etf-true-flows.json','data/etf-desk.json','etf-flows/daily.json','data/finviz-groups.json','signals/anomalies.json')
COMPILERS=(model,catalog,sector_market_replay,daily_market_model,daily_macro_model,research_brief_model,evidence_store,sys.modules[__name__])
...
def allowed(key):
    return isinstance(key,str) and (key in SOURCES or artifact(key) or bool(re.fullmatch(
        r'data/(?:daily-research/(?:runs|inputs|compilers)/[a-f0-9]{64}\.(?:json|py)|evidence/polygon/[a-f0-9]{64}/[a-f0-9]{64}\.bin\.gz)',key)))
...
def replay(ref, read):
    ...
    if key != PREFIX+'runs/'+model.sha(raw)+'.json' or manifest.get('contract') != 'sector-native-replay.v1':
    ...
    if (result != checked(manifest['output'], 'outputs', read) or model.sha(model.encoded(result)) != ref.get('output_sha256')
        ...
        raise ValueError('native sector original-source replay differs')
    return result
...
def retain(client, bucket, inputs, output):
    ...
    manifest = {'contract': 'sector-native-replay.v1', 'generated_at': output['generated_at'], **refs,
        'compilers': compilers, 'output_sha256': refs['output']['sha256'],
        'scope': 'Exact retained daily aggregate originals, matched benchmark dates, price-only relative performance and descriptive covariance; whole predecessor contexts retained, no calibrated forecast or portfolio authority.'}
...
def run(client,bucket,request_id,execution_id):
    ...
    status={'contract':'sector-public-request.v1','request_id':request_id,'execution_id':execution_id,
    ...
    try:
        ...
        inputs={'contract':'sector-native-inputs.v1','market':market,'legacy':legacy,'generated_at':now()}
        ...
        raise RuntimeError('Native sector publication failed; inspect reviewed request evidence') from None
