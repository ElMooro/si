...
class Collector:
    ...
    def response(self, url):
        ...
        for attempt in range(2):
            with self.lock:
                ...
                if self.requests > len(DESK) * 2 * (native.MAX_PAGES + 1) * 2:
                    return None, 'response_rejected', None
            request = urllib.request.Request(url, headers={'User-Agent': 'JustHodl-OriginalETFProfiles/1.0',
            ...
            try:
                ...
                if not 0 < len(raw) <= native.MAX_SOURCE_BYTES or self.credential.encode() in raw:
                    return None, 'response_rejected', None
                with self.lock:
                    if self.source_bytes + len(raw) > 64 * 1024 * 1024: return None, 'response_rejected', None
    ...
    def snapshot(self, ticker, cutoff):
        ...
        for _ in range(native.MAX_PAGES):
            ...
            if not body.get('next_url'):
                result['status'] = 'complete_returned_profile_snapshot';break
            url = body['next_url']
    ...
    def collect(self):
        ...
        with ThreadPoolExecutor(max_workers=6) as pool:
            futures = {pool.submit(self.fund, ticker): ticker for ticker in sorted(DESK)}
