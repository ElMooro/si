...
def replay(ref, read):
    ...
    for module in COMPILERS:
        ...
        if run['compilers'][module.__name__] != expected or read(expected['key']) != body: raise ValueError('Matching reviewed compiler required')
...
def run(client, bucket, kind, request_id, execution_id, credential='', remaining_seconds=840, publish=None):
    ...
    try:
        if kind == 'holdings':
            ...
            try: previous = snapshot(client, bucket, model.CURRENT)
                ...
                if not missing(exc): raise
                previous = None
            acquisition = collector.Collector(credential, lambda raw: protect(client, bucket, raw), budget_end - 180)
            ...
            try: previous = snapshot(client, bucket, model.LOOK_CURRENT)
            except Exception as exc:
                if not missing(exc): raise
                previous = None
        ...
        output = compile_output(inputs, reader(client, bucket), lambda k, b: immutable(client, bucket, k, b))
