...
def reconcile(rows,product,venue,start,end,complete):
    ...
    for index,row in enumerate(rows):
        try:
            ...
            if not day-timedelta(days=2)<=stamp.date()<=day+timedelta(days=1):raise ValueError('Event outside bounded session window')
    ...
    for session,events in sorted(groups.items()):
        ...
        eligible=bool(complete and not invalid and len(closes)==1 and opens and all(instant(x)<instant(closes[0]) for x in opens))
