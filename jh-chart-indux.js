...
(function () {
  ...
  var favs = (function () {
    ...
    return ["sma20", "sma50", "sma200", "ema9", "bb", "avgdev", "vwap", "rsi", "macd", "adx", "hv", "bbp", "vol", "voltape", "vsa", "livermore", "wyckoff", "gdx", "bbw", "bbsqz", "keylv", "pvwap", "ddown", "alpha", "rngpos", "earn", "pats", "sdmd", "sr"];
  })();
  function saveFav() {
  ...
  window.jhInduxOpen = function () {
    ...
    function draw() {
      ...
      var rows = allStudies().filter(function (s) {
        ...
        if (s.id === "ib") keys += " initial balance ibh ibl first hour market profile";
        if (s.id === "onhl") keys += " overnight high low onh onl globex premarket";
        if (s.id === "adr") keys += " average daily range expected high low stretch adr20";
        if (s.id === "lrch") keys += " linear regression channel 2 sigma linreg";
        if (s.id === "eavwap") keys += " anchored vwap earnings eps event avwap";
        if (s.id === "gsess") keys += " asia london new york session overlay forex crypto";
        if (s.id === "sep") keys += " session separator midnight day change";
        if (s.id === "ins") keys += " insider cluster form 4 buy";
        if (s.id === "buyb") keys += " buyback repurchase authorization";
        if (s.id === "ratio") keys += " price ratio vs spy relative";
        if (s.id === "adrpct") keys += " adr used percent range consumed";
        ...
        rows.filter(function (s) { return s.cat === c; }).forEach(function (s) {
          html += "<button type=button class='irow" + (s.item.on ? " on" : "") + "' data-add='" + s.id + "' data-osc='" + (s.osc ? "1" : "0") + "'>" +
            "<div><b>" + s.n + "</b><span>" + (
              ...
              s.id === "ib" ? "First-hour IBH / IBL · " :
              s.id === "onhl" ? "ONH / ONL · " :
              s.id === "adr" ? "Open ± ADR 20 · " :
              s.id === "lrch" ? "Fit 100-bar ±2σ · " :
              s.id === "eavwap" ? "VWAP from last EPS · " :
              s.id === "gsess" ? "Asia · London · NY · " :
              s.id === "sep" ? "NY day lines · " :
  ...
  window.jhInduxHelp = function (id) {
    ...
    var STUDY = {
      ...
      ib: ["Initial Balance", "Session", "First 60 minutes of NY RTH (09:30–10:30 ET) high/low — Market Profile's IBH/IBL. Same 5m source as Opening Range.", "IB extension (price leaving the first hour) is the trend day tell. A day that never leaves IB is a balance day. Pair with ADR used %.", "Not TPO letters. Time-at-price from 5m bars, not tick."],
      onhl: ["Overnight H/L", "Session", "High/low from prior 16:00 ET through 09:30 open. Premarket + Globex equivalent for US names, from 5m bars.", "ONH is the first magnet after the open. A drive through ONH that fails back inside often is the morning fade. Empty when 5m is missing.", "Not a 24-hour high. Equity overnight only."],
      adr: ["ADR 20 range", "Levels", "Average of the last 20 completed daily high–low ranges. Expected stretch = today's open ± ADR. Quote and HUD show ADR used % (today's range / ADR).", "80%+ of ADR used by lunch with IB still holding is exhaustion risk. A trend day often spends >100% of ADR. Open ± ADR is stretch, not a target.", "Daily ranges from this series. Intraday is rolled up to NY days."],
      lrch: ["LinReg Channel 2σ", "Channel", "One regression on the last 100 closes (not a rolling 20), ±2 residual σ, extended 8 bars. Different from Std Error Bands, which re-fit every bar.", "Slope is the local drift. A close outside +2σ that cannot hold is mean-reversion; a close that holds is a regime shift. The right-hand extension is a forecast, not a print.", "OLS on close. Not a high/low channel."],
      eavwap: ["AVWAP from earnings", "Volume", "Volume-weighted average price from the last warehouse earnings date (catalyst calendar / tracker) to now.", "The Street's cost basis since the print. Holding above it after a BEAT is repair; losing it after a BEAT that sold off is the post-earn fade. Empty when the warehouse has no dated EPS for the name.", "Real calendar only. Manual anchored VWAP is the drawing tool."],
      gsess: ["Asia / London / NY", "Session", "Intraday background: Asia (outside London/NY), London 03:00–09:30 ET, NY RTH 09:30–16:00 ET, plus NY day separators.", "Useful on 24h names (futures, crypto, FX). On US cash 5m the London window is premarket. Daily interval has nothing to shade.", "Clock boxes, not volume. NY calendar."],
